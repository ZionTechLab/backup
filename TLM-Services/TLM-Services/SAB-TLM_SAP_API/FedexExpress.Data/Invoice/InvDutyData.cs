    ...
    {       
                    ...
                    SqlParameter[] paraList = new SqlParameter[]
                      {
                        ...
                        new SqlParameter("@DirPayY" , (typePara.DirectPayY  ==null  ) ? "" : typePara.DirectPayY),
						new SqlParameter("@PayDocType" , (typePara.PaymentType  ==null  ) ? "" : typePara.PaymentType),
                        new SqlParameter("@ConsoleID" , (typePara.ConsID  ==null  ) ? "" : typePara.ConsoleID),
                ...
                using (IExpressUnitOfWork<InvDutyResult> uof = new ExpressUnitOfWork<InvDutyResult>())
                {
                    ...
                    var customerHead = (from SR in uof.Reposotery.GetDataBySp("[Express].[TLM_GetInvDutyTAX]", paraList)
                                        ...
                                        }).ToList().Select(SR => new InvDutyDomainView
                                        {
                                            ...
                                            PayDate=SR.PayDate,
                                            PayNo=SR.PayNo,
                                            DirectPayY =SR.DirectPayY,
											PayDoctype = SR.PayDoctype,
                                            RevsDocNo = SR.RevsDocNo
        ...
        public IList<InvDutyDoctypeDomainView> GetDutyDoctypes(int companyID, int agencyID, string shiptype, string billto, decimal shipV, string dutyEx)
        {
            try
            {
                ...
                using (IExpressUnitOfWork<InvDutyDoctypeDomainView> uof = new ExpressUnitOfWork<InvDutyDoctypeDomainView>())
                {
        ...
        public IList<InvDutyPaymentTypeDomainView> GetDutyPaymentDoctypes()
        {
            try
            {
                using (IExpressUnitOfWork<InvDutyPaymentTypeDomainView> uof = new ExpressUnitOfWork<InvDutyPaymentTypeDomainView>())
                {
                    ...
                    var dutDoctypes = (from SR in uof.Reposotery.GetDataBySp("[Express].[TLM_GetDutyCalTypes]", paraList)
                                       select new InvDutyPaymentTypeDomainView
                                       {
                                         ShipValueType =SR.ShipValueType ,
                                         ShipValueTypeN =SR.ShipValueTypeN 
                         ...
                         new SqlParameter ("@expressID" ,((expressID==null )?"": expressID )) ,
                          new SqlParameter ("@invoiceNo" ,((invno ==null || invno =="")? "0" : invno)),
                           new SqlParameter("@invType"  ,((invtype==null )? "" :invtype)),
                            new SqlParameter("@payNo"  ,((payno==null || payno=="")? "0" :payno)) 
        ...
        public ResponseMessage PaymentReverse(InvDutyDomainView _param)
        {
            ...
            try
            {
                using (IExpressUnitOfWork<ResponseProcessResult> uof = new ExpressUnitOfWork<ResponseProcessResult>())
                {
                    SqlParameter[] paraList = new SqlParameter[]
                      {
                        new SqlParameter("@PaymentNo", _param.PayNo),
                        new SqlParameter("@ExpressID", _param.ExpressID),
                        new SqlParameter("@Agancy", _param.AgncyCode),
                        new SqlParameter("@CMPY", _param.CompanyID),
                        //// new SqlParameter("@AgnID", typePara.AgncyRptID),                      
                        new SqlParameter("@UserID", _param.UserID),

                      };

                    var responce = (from SR in uof.Reposotery.GetDataBySp("[Express].[TLM_DutyReversal]", paraList)
                                    select new ResponseMessage
                                    {
                                        StrMessage = SR.ResponseMessage,
                                        ReturnValue = SR.ReturnValue
