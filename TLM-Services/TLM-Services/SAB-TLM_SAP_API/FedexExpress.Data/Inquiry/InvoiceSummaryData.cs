...
{
    public class InvoiceSummaryData : IInvoiceSummary<InvoiceSummaryDomainView>
    {
        ...
        public IList<InvoiceSummaryDomainView> GetInvoiceList(string fDate, string frominvNo, string ToInvNo, string todate, int CMPY, int agency, int groupID, string Gate, string Station, string InvoiceType, bool isInvoiceRange)
        {
            try
            {
                using (IExpressUnitOfWork<NewInvoiceSummaryResult> uof = new ExpressUnitOfWork<NewInvoiceSummaryResult>())
                {
                    SqlParameter[] paraList = new SqlParameter[]
                    ...
                    var OrgRegistryList = (from Ag in uof.Reposotery.GetDataBySp("[Express].[TLM_GetInvoiceSummaryData]", paraList)
                                           select new InvoiceSummaryDomainView
                                           {
                                               CMPY = Ag.CMPY.Value,
