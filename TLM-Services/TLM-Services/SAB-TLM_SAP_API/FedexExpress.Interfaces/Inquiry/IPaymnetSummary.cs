...
namespace Express.Interfaces.Inquiry
{
    public interface IPaymnetSummary<T> : IDataAccess<PaymetSummaryDomainView> where T : class
    {
        ...
        IList<PaymetSummaryDomainView> GetInvoiceList(string fDate, string frominvNo, string ToInvNo, string todate, int CMPY, int agency, int groupID, string Gate, string Station, string InvoiceType, bool isInvoiceRange , int payAcc ,int IsPayRev);
