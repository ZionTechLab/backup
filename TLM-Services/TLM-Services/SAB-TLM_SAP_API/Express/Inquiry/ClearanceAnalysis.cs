...
{
    public partial class ClearanceAnalysis : Form
    {
        private readonly IClearanceAnalysis<ClearanceAnalysisDomainView> dataProvider;
        ...
        List<ClearanceAnalysisDomainView> ClearanceAnalysisList = new List<ClearanceAnalysisDomainView>();
        public ClearanceAnalysis()
        {
            InitializeComponent();

            if (dataProvider == null)
            {
                dataProvider = InquryUIFacotry.GetService<IClearanceAnalysis<ClearanceAnalysisDomainView>>();
            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            ClearanceAnalysisList.Clear();
            ClearanceAnalysisList = dataProvider.GetInvoiceList(paraFdeate.ToString("MM-dd-yyyy"), ParaFromInvoice, ParaToInvoice, paraTodate.ToString("MM-dd-yyyy"), SelectedAgency.CompID, SelectedAgency.AgncyCode, SelectedAgency.GroupID,ParaGate,ParaStation,paraDoctype, paraInvoiceRange).ToList();
            grdinvoice.AutoGenerateColumns = false;
            grdinvoice.DataSource = null;
            grdinvoice.DataSource = ClearanceAnalysisList;
        }
        ...
        {
            var SelectedAgency1 = (AgencyDomainViewcs)cmb_agency.SelectedItem;
            ...
            bool paraInvoiceRange = false;
            ClearanceAnalysisList.Clear();
            ClearanceAnalysisList = dataProvider.GetInvoiceList(paraFdeate.ToString("MM-dd-yyyy"), ParaFromInvoice, ParaToInvoice, paraTodate.ToString("MM-dd-yyyy"), SelectedAgency1.CompID, SelectedAgency1.AgncyCode, SelectedAgency1.GroupID, ParaGate, ParaStation, paraDoctype, paraInvoiceRange).ToList();
            if (ClearanceAnalysisList.Count > 0)
            {
                var SelectedAgency = (AgencyDomainViewcs)cmb_agency.SelectedItem;
                IInquiryReportProvider _report = RptInquiryUIFactory.GetService<IInquiryReportProvider>();
                ClearanceAnalysisList.ForEach(cc => cc.CompanyName = SelectedAgency == null ? "All Company" : SelectedAgency.CompName);
                ClearanceAnalysisList.ForEach(cc => cc.FilterValue = FilterByString);

                _report.ClearanceAnalysisPrint(ClearanceAnalysisList);
            }
            else
            {
