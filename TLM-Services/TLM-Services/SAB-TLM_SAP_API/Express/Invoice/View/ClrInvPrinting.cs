...
namespace Express.UI.Invoice.View
{
    public partial class ClrInvPrinting : Form
    {
        ...
        private void btn_DirectPrint_Click(object sender, EventArgs e)
        {
            if (grdInvoiceDTAX.RowCount == 0)
            {
                MessageNotification.MessageBoxError("Please retrieve invoice details", LoginInfoView.COMPANYNAME, MessagHeaderInfo.ValidationError);
            ...
            if (chkInvSummery.Checked == false)
            {
                ...
                if (_invDuty != null && _company != null)
                {
                    List<string> stringList = _invDuty.ToList().Select(z => z.InvNo).Distinct().ToList();

                    foreach (var item in stringList)
                    {
                        List<TaxInvoiceReportDomainView> List = new List<TaxInvoiceReportDomainView>();
                        List = _invDuty.Where(z => z.InvNo == item).ToList();
                        _report.ClearenceDutyPrintDirect(List, _company);
                    }
                }
            }
