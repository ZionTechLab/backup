...
{
    public class ManifestUploadTNTData : IManifestUploadTNT<ManifestUploadTNTDomainView>
    {
        ...
        public ConsMasterDomainView CheckConsExist(int CompanyId, int GroupId, int AgencyId, string ConsId)
        {
            ...
            try
            {
                ...
                {
                    return (from OC in uof.Reposotery.GetDetails()
                            where OC.CMPY == CompanyId && OC.GroupID == GroupId && OC.AgncyCode == AgencyId && OC.Deleted.Value == false
                            && OC.ConsId == ConsId
                            select new ConsMasterDomainView
                            {
                                CMPY = OC.CMPY,
        ...
        public ResponseMessage SaveConsList(ManifestUploadWrappingDomain typePara)
        {
            ...
            try
            {
                using (IExpressUnitOfWork<ResponseProcessResult> uof = new ExpressUnitOfWork<ResponseProcessResult>())
                {
                    ...
                    foreach (var item in typePara.ConsList)
                    {
                        string hValue = item.HighValueY == true ? "Y" : "";
                        xmlString = xmlString + "<ROW>"
                        + "<Deleted>" + item.Deleted + "</Deleted>"
                        + "<GroupID>" + item.GroupID + "</GroupID>"
                        + "<CMPY>" + item.CMPY + "</CMPY>"
                        + "<AgncyCode>" + item.AgncyCode + "</AgncyCode>"
                        + "<AgncyID>" + item.AgncyID + "</AgncyID>"
                        + "<ConsId>" + item.ConsId + "</ConsId>"
                        + "<TransDate>" + item.TransDate + "</TransDate>"
                        + "<VisaRootID>" + item.VisaRootID + "</VisaRootID>"
                        + "<OrgHubID>" + item.OrgHubID + "</OrgHubID>"
                        + "<DesHubID>" + item.DesHubID + "</DesHubID>"
                        + "<ShpmentType>" + item.ShipType + "</ShpmentType>"
                        + "<AlNumCode>" + item.AlNumCode + "</AlNumCode>"
                        + "<FlightNo>" + item.FlightNo + "</FlightNo>"
                        + "<AriDate>" + item.AriDate + "</AriDate>"
                        + "<DepDate>" + item.DepDate + "</DepDate>"
                        + "<AriTime>" + item.AriTime + "</AriTime>"
                        + "<DepTime>" + item.DepTime + "</DepTime>"
                        + "<MAWBNo>" + item.MAWBNo + "</MAWBNo>"
                        + "<ALActWgt>" + 0 + "</ALActWgt>"
                        + "<ALChgWgt>" + 0 + "</ALChgWgt>"
                        + "<AlFreightChg>" + 0 + "</AlFreightChg>"
                        + "<Currency>" + item.Currency + "</Currency>"
                        + "<Remarks>" + item.Remarks + "</Remarks>"
                        + "<HighValue>" + hValue + "</HighValue>"
                        + "<TransMode>" + item.TransMode + "</TransMode>"
                        + "</ROW>";
                    }
                    xmlString = xmlString + "</ROOT>";
                    ...
                    var responce = (from SR in uof.Reposotery.GetDataBySp("[Express].[TLM_ManufestUploadTntConsDetail]", paraList)
                                    select new ResponseMessage
                                    {
                                        StrMessage = SR.ResponseMessage,
