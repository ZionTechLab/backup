...
namespace CredValidityAlert
{
	class Program
	{
		static void Main(string[] args)
		{
			try
			{
				Console.WriteLine("Running Credit Validity Alert Report Sending for : " + DateTime.Today.ToString("yyyy-MM-dd") + " ..........................................");
				...
				using (List<ReportDocRecepDomain>.Enumerator enumerator = genEmail.getRecep().GetEnumerator())
				{
					while (enumerator.MoveNext())
					{
						...
						XLWorkbook xLWorkbook = genEmail.GenerateAttachment();
						string emailBody = genEmail.GetEmailBody();
						if (xLWorkbook != null)
						{
							if (genEmail.SendEmail(from, mailboxAddress, cc, bcc, subject, header + emailBody + footer, "CUSTOMER CREDIT VALIDITY", xLWorkbook, smtp, port, username, password, ssl))
							{
