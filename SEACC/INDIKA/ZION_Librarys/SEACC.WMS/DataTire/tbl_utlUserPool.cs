...
namespace DataTire {
	public sealed class tbl_utlUserPool {
		...
		/// Initializes a new instance of the tbl_utlUserPool class.
		/// </summary>
		public tbl_utlUserPool(string user_ID, string terminal_ID, int activeForm_ID, string loginStatus_ID, DateTime logedTime, bool isForceShoutdown, bool isForceLogout, bool isNewLogin) {
		...
		/// </summary>
		public static tbl_utlUserPool Select(string user_ID_Incoming, string terminal_ID_Incoming){
		...
		private static tbl_utlUserPool Maketbl_utlUserPool(SqlDataReader dataReader) {
			...
			if (dataReader.IsDBNull(0) == false) {
				tbl_utlUserPool.User_ID = dataReader.GetString(0);
			}
			if (dataReader.IsDBNull(1) == false) {
				tbl_utlUserPool.Terminal_ID = dataReader.GetString(1);
			}
			if (dataReader.IsDBNull(2) == false) {
				tbl_utlUserPool.ActiveForm_ID = dataReader.GetInt32(2);
			}
			if (dataReader.IsDBNull(3) == false) {
				tbl_utlUserPool.LoginStatus_ID = dataReader.GetString(3);
			}
			if (dataReader.IsDBNull(4) == false) {
				tbl_utlUserPool.LogedTime = dataReader.GetDateTime(4);
			}
			if (dataReader.IsDBNull(5) == false) {
				tbl_utlUserPool.IsForceShoutdown = dataReader.GetBoolean(5);
			}
			if (dataReader.IsDBNull(6) == false) {
				tbl_utlUserPool.IsForceLogout = dataReader.GetBoolean(6);
			}
			if (dataReader.IsDBNull(7) == false) {
				tbl_utlUserPool.IsNewLogin = dataReader.GetBoolean(7);
			}
		...
		dt.Columns.AddRange(new DataColumn[] { col_user_ID,col_terminal_ID,col_activeForm_ID,col_loginStatus_ID,col_logedTime,col_isForceShoutdown,col_isForceLogout,col_isNewLogin,});		return dt;
		}
		/// <summary>
