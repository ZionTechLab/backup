...
namespace SEACC_PRODUCTION_APPAREL.Transactions
{
    ...
    public partial class UC_Production_BatchCreation : UserControl
    {
        ...
        public UC_Production_BatchCreation()
        {
            ...
            InitializeComponent();
            SEACC_Form.enmFormName = FormName.Prod_BatchCreation;
            ...
            dgr_Main.Add_DatagridColoumn(ColoumnType.Numaric, "Job Qty", "BatchQty", 75, true, true);
            ...
            try
            {
                if (SEACC_Form.CheckPermission_ToCancel())
                {
                    if (CheckValidity())
                    {
                        ...
                        {
                            tbl_prodTxBatch oBatch = tbl_prodTxBatch.Select(txtProdBatchID.Text);
                            if (oBatch != null)
                            {
        ...
        private void btn_Save_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (CheckValidity())
            {
                try
                {
                    tbl_prodTxJobCard oProdJobBoM = tbl_prodTxJobCard.Select(txtBoMID.Tag.ToString());
                    if (oProdJobBoM != null)
                    {
                        #region Update
                        if (SEACC_Form.IsUpdateMode)
                        {
                            ...
                            {
                                tbl_prodTxBatch oOldBatch = tbl_prodTxBatch.Select(txtProdBatchID.Text);
                                if (oOldBatch != null)
                                {
                                    if (!oOldBatch.IsApproved && !oOldBatch.IsCanceled)
                                    {

                                        tbl_prodTxBatch_Material.DeleteAllByProdBatch_ID(txtProdBatchID.Text);

                                        tbl_prodTxBatch oUpdateBatch = new tbl_prodTxBatch(
                                            txtProdBatchID.Text,
                                            ...
                                            clsHelpMethods_Prod.Get_TotalQtyofBatches_FromBoM(txtBoMID.Tag.ToString()),
                        ...
                        else
                        {
                            ...
                            {
                                tbl_prodTxBatch oBatch = new tbl_prodTxBatch(
                                     txtProdBatchID.Text,
                                     txtBoMID.Tag.ToString(),
                                     dtpbatchDate.GetDateTime(),
                                     (int)prod_Batch_Status.Open,
        ...
        private void RefreshGrid()
        {
            try
            {
                dgr_Main.dt.Clear();
                //int iCount = 0;
                //foreach (tbl_prodTxBatch oBatch in tbl_prodTxBatch.SelectAll().Where(p => p.ProdBatch_ID != "default").OrderByDescending(o => o.DateCreate))
                //{
                //    dgr_Main.dt.Rows.Add(++iCount, oBatch.ProdBatch_ID, oBatch.ProdJob_ID, 
                //        clsGenaralName.getName_Item(oBatch.Item_ID), clsGenaralName.getName_Uom(oBatch.Uom_ID), cls_Formater.FormatDecimal(oBatch.BatchQty, clsConfig.sDecimalPlaces_Quantity), 
                //        clsGenaralName.getName_User(oBatch.CreateUser_ID), clsHelpMethods_Prod.Format_DateTime(oBatch.DateCreate),
                //        clsGenaralName.getName_User(oBatch.ModifiedUser_ID), clsHelpMethods_Prod.Format_DateTime(oBatch.DateModified),
                //        clsGenaralName.getName_User(oBatch.ApprovedUser_ID), clsHelpMethods_Prod.Format_DateTime(oBatch.DateApproved),
                //        oBatch.IsCanceled);
                //}

                dgr_Main.dt.Merge(DBHandling.ExecQuery("Exec sp_BatchDetails").Tables[0]);
        ...
        private bool CheckValidity_DuplicateFiled()
        {
            ...
            if (!SEACC_Form.IsUpdateMode)
            {
                ...
                tbl_prodTxBatch oJob = tbl_prodTxBatch.Select(txtProdBatchID.Text);
                if (oJob != null)
                {
                    bStatus = false;
        ...
        private void FillDetails(string sBatchId)
        {
            try
            {
                ...
                txtCustomer.Tag = clsHelpMethods_Prod.IsProdJobBoM_MakeToSupply(oBoM.ProdJob_ID) ? "CUS/00000" : clsGenaralName.getCustomerID_FromCO(oBatch.CustomerOrder_ID);
                ...
                txtPreviousBatchQty.Text = cls_Formater.FormatDecimal((clsHelpMethods_Prod.Get_TotalQtyofBatches_FromBoM(oBatch.ProdJob_ID) - (!oBatch.IsCanceled ? oBatch.BatchQty : 0)), clsConfig.sDecimalPlaces_Quantity);
        ...
        private void FillRawMaterialGrid_byBoM(string sBoM_ID)
        {
            ...
            string sIsSelect = "\uE003";
            foreach (tbl_prodTxJobCard_Material oJob_Meterial in tbl_prodTxJobCard_Material.SelectAllByProdJob_ID(sBoM_ID).Where(r1 => r1.Line_No_Sub1 == 0).OrderBy(r2 => r2.Line_No).ThenBy(r3 => r3.Line_No_Sub2))
            {
                ...
                foreach (tbl_prodTxJobCard_Material oJob_Meterial_ForSemi in tbl_prodTxJobCard_Material.SelectAllByProdJob_ID(sBoM_ID).Where(r => r.Line_No == oJob_Meterial.Line_No && r.Line_No_Sub1 != 0).OrderBy(r2 => r2.Line_No_Sub1).ThenBy(r3 => r3.Line_No_Sub2))
                {
                    string sSemiFG_ItemName = clsGenaralName.getName_Item(oJob_Meterial_ForSemi.Item_ID);
                    if (oJob_Meterial_ForSemi.Line_No_Sub2 == 0)
                    {
                        sSemiFG_ItemGroup_Name = sSemiFG_ItemName;
                        if (clsHelpMethods_Prod.Get_SubstituteMaterials(oJob_Meterial_ForSemi.Line_No, oJob_Meterial_ForSemi.Line_No_Sub1, oJob_Meterial_ForSemi.ProdJob_ID).Count == 1)
                            sSemiFG_IsSelect = "\uE0A2";
                        else
                            sSemiFG_IsSelect = "\uE003";
                    }
                ...
                if (oJob_Meterial.Line_No_Sub2 == 0)
                {
                    ...
                    if (clsHelpMethods_Prod.Get_SubstituteMaterials(oJob_Meterial.Line_No, oJob_Meterial.Line_No_Sub1, oJob_Meterial.ProdJob_ID).Count == 1)
                        sIsSelect = "\uE0A2";
                    else
                        sIsSelect = "\uE003";
                }
                ...
                dtMeterialReq.Rows.Add("0", oJob_Meterial.Line_No, oJob_Meterial.Line_No_Sub1, oJob_Meterial.Line_No_Sub2, sIsSelect,
                                    ...
                                    sInputQty, sWastage_Pct, sTotalInputQty, oJob_Meterial.Section_ID, sSectionName,
                                    oJob_Meterial.IsSemiFinishItem, oFrmMaterialsForSF,
                                    sItemGroup_Name,
        ...
        private void FillRawMaterialGrid_byBatch(string sBatch_ID)
        {
            ...
            string sItemGroup_Name = "";
            string sIsSelect = "\uE003";
            foreach (tbl_prodTxBatch_Material oBatch_Meterial in tbl_prodTxBatch_Material.SelectAllByProdBatch_ID(sBatch_ID).Where(r1 => r1.Line_No_Sub1 == 0).OrderBy(r2 => r2.Line_No).ThenBy(r3 => r3.Line_No_Sub2))
            {
                ...
                string sSemiFG_ItemGroup_Name = "";
                string sSemiFG_IsSelect = "\uE003";
                foreach (tbl_prodTxBatch_Material oBatch_Meterial_ForSemi in tbl_prodTxBatch_Material.SelectAllByProdBatch_ID(sBatch_ID).Where(r => r.Line_No == oBatch_Meterial.Line_No && r.Line_No_Sub1 != 0).OrderBy(r2 => r2.Line_No_Sub1).ThenBy(r3 => r3.Line_No_Sub2))
                {
                    ...
                    oFrmMaterialsForSF.dtMeterialReq.Rows.Add("",
                        ...
                        oBatch_Meterial_ForSemi.Section_ID, clsGenaralName.getName_Section(oBatch_Meterial_ForSemi.Section_ID),
                        sSemiFG_ItemGroup_Name,
                ...
                if (oBatch_Meterial.Line_No_Sub2 == 0)
                    sItemGroup_Name = sItemName + " - (" + oBatch_Meterial.Line_No + ")";
                ...
                dtMeterialReq.Rows.Add("0", oBatch_Meterial.Line_No, oBatch_Meterial.Line_No_Sub1, oBatch_Meterial.Line_No_Sub2, sIsSelect,
                                   ...
                                   sInputQty, sWastage_Pct, sTotalInputQty, oBatch_Meterial.Section_ID, sSectionName,
        ...
        private void dgr_Main_LoadingRow(object sender, DataGridRowEventArgs e)
        {
            try
            {
                if (Convert.ToBoolean(((DataRowView)(e.Row.DataContext)).Row["Is_Canceled"].ToString()))
                {
        ...
        private void dgr_MererialReq_MouseLeftButtonUp(object sender, MouseButtonEventArgs e)
        {
            try
            {
                ...
                if (oSelectedItem != null)
                {
                    string Grid_LineNo = (dgr_MererialReq.SelectedCells[0].Column.GetCellContent(oSelectedItem) as TextBlock)?.Text;
                    DataRow drRow = dtMeterialReq.Select("LineNo = '" + Grid_LineNo + "'").FirstOrDefault();
                    if (drRow != null && (vDG_Cell.Column.SortMemberPath == "IsSelect"))
                    {
                    ...
                    else if (drRow != null && (vDG_Cell.Column.SortMemberPath == "Item_ID" || vDG_Cell.Column.SortMemberPath == "ItemName"))
                    {
                        string sItem_ID = drRow["Item_ID"].ToString();
                        frm_ItemStoreStock oFrmItemStoreStock = new frm_ItemStoreStock(sItem_ID);
                        oFrmItemStoreStock.ShowDialog();
                    }
        ...
        private void txtBoMID_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            ...
            if (rowDataSearch.DialogResult == true)
            {
                ...
                txtPreviousBatchQty.Text = cls_Formater.FormatDecimal(clsHelpMethods_Prod.Get_TotalQtyofBatches_FromBoM(oBoM.ProdJob_ID), clsConfig.sDecimalPlaces_Quantity);

                tbl_prod_apparelTxFinishedGoodSpecsSheet oProdSepec = tbl_prod_apparelTxFinishedGoodSpecsSheet.Select(oBoM.Item_ID_FG);
        ...
        private void InsertBatchMaterials()
        {
            foreach (DataRow row in dtMeterialReq.Rows)
            {
                ...
                tbl_prodTxJobCard_Material oBoM_Material = tbl_prodTxJobCard_Material.Select(iLineNoMain, iLineNoSub1, iLineNoSub2, txtBoMID.Tag.ToString());
                if (oBoM_Material != null)
                {
                    tbl_prodTxBatch_Material oBatch_Materials = new tbl_prodTxBatch_Material(
                        iLineNoMain, iLineNoSub1, iLineNoSub2, txtBoMID.Tag.ToString(), txtProdBatchID.Text,
                        sItem_ID, sUoM_ID, bIsSemiFinished, dQty, oBoM_Material.IsWastagePercent, dWastagePct, 0, dTotalQty, sSectionID, oBoM_Material.Smv_TimeMinutes, oBoM_Material.TotalLabour,
                        oBoM_Material.LowestCost, oBoM_Material.HighestCost, oBoM_Material.WeightedAvgCost, oBoM_Material.BomCost, oBoM_Material.CostTypeSelection, oBoM_Material.Cost, oBoM_Material.AllowCostEdit,
                ...
                if (frm_Semi != null)
                {
                    ...
                    {
                        int iLineNoMain_Semi = Convert.ToInt32(clsValidate.ValidateRowValue(row_Semi, "LineNoMain", 0));
                        int iLineNoSub1_Semi = Convert.ToInt32(clsValidate.ValidateRowValue(row_Semi, "LineNoSub1", 0));
                        int iLineNoSub2_Semi = Convert.ToInt32(clsValidate.ValidateRowValue(row_Semi, "LineNoSub2", 0));
                        ...
                        tbl_prodTxJobCard_Material oBoM_Material_Semi = tbl_prodTxJobCard_Material.Select(iLineNoMain_Semi, iLineNoSub1_Semi, iLineNoSub2_Semi, txtBoMID.Tag.ToString());
                        if (oBoM_Material != null)
                        {
                            tbl_prodTxBatch_Material oBatch_Material_Semi = new tbl_prodTxBatch_Material(
                                iLineNoMain_Semi, iLineNoSub1_Semi, iLineNoSub2_Semi, txtBoMID.Tag.ToString(), txtProdBatchID.Text,
                                sItem_ID_Semi, sUoM_ID_Semi, oBoM_Material_Semi.IsSemiFinishItem, dQty_Semi, oBoM_Material_Semi.IsWastagePercent, dWastagePct_Semi, 0, dTotalQty_Semi, sSectionID_Semi, oBoM_Material_Semi.Smv_TimeMinutes, oBoM_Material_Semi.TotalLabour,
                                oBoM_Material_Semi.LowestCost, oBoM_Material_Semi.HighestCost, oBoM_Material_Semi.WeightedAvgCost, oBoM_Material_Semi.BomCost, oBoM_Material_Semi.CostTypeSelection, oBoM_Material_Semi.Cost, oBoM_Material_Semi.AllowCostEdit,
