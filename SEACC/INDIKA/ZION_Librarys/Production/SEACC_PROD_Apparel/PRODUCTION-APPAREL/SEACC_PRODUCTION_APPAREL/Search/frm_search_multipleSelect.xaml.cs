...
namespace SEACC_PRODUCTION_APPAREL.Search
{
    /// <summary>
    /// Interaction logic for frm_search.xaml
    /// </summary>
    public partial class frm_search_multipleSelect : Window
    {
        ...
        public List<string> lstPara = new List<string>();
        ...
        public delegate void delegate_SearchClosed(DataRow[] drSelected);
        public event delegate_SearchClosed SearchClosed;
        #endregion

        public frm_search_multipleSelect()
        {
            InitializeComponent();
        }

        public frm_search_multipleSelect(DataRow[] drSelectedRows)
        {
            InitializeComponent();
            this.drSelectedRows = drSelectedRows;
        }

        public frm_search_multipleSelect(List<string> lstParameeters)
        {
            InitializeComponent();
        ...
        private void RefrechAll()
        {
            try
            {
                ...
                string strScript = "SELECT CONVERT(bit, 0) [IsSelected], " + strFields.Substring(0, strFields.Length - 2) + " FROM " + strTable + sWhere + sOrderBy;
                ...
                {
                    //drSelectedRows.ToList<DataRow>().ForEach(r => {   r["IsSelected"] = false;});
                    //var vPrvSelectedRows = dtResult.AsEnumerable().Intersect(drSelectedRows, DataRowComparer.Default);
                    //vPrvSelectedRows.ToList<DataRow>().ForEach(r => { r["IsSelected"] = true; });
                    drSelectedRows.ToList<DataRow>().ForEach(r =>
                    {
                        foreach (var vResultRow in dtResult.Select("mr_No = '" + r["mr_No"].ToString() + "'"))
                        {
                            vResultRow["IsSelected"] = true;
                        }
                    });

                    if (dtResult.Select("IsSelected = false").Count() == 0)
        ...
        private void chk_SelectAll_Checked(object sender, RoutedEventArgs e)
        {
            ...
            if (dtResult != null && dtResult.Rows.Count > 0)
            {
                dtResult.Select().ToList<DataRow>().ForEach(r => { r["IsSelected"] = true; });
            }
        }
        ...
        private void chk_SelectAll_Unchecked(object sender, RoutedEventArgs e)
        {
            ...
            if (dtResult != null && dtResult.Rows.Count > 0)
            {
                dtResult.Select().ToList<DataRow>().ForEach(r => { r["IsSelected"] = false; });
            }
        }
        ...
        private void frmSearch1_Closed(object sender, EventArgs e)
        {
            if (dtResult != null && dtResult.Rows.Count > 0)
            {
                var vResult = dtResult.Select("IsSelected = true");
                if (vResult.Length > 0)
                    SearchClosed(vResult);
            }
        }
