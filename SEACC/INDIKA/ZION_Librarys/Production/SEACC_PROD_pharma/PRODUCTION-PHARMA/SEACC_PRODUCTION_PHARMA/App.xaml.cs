...
namespace SEACC_PRODUCTION_PHARMA
{
    ...
    public partial class App : Application
    {
        ...
        private void Application_Startup(object sender, StartupEventArgs e)
        {
            try
            {
                #region Check the previous process
                ...
                if (processes.Length > 1)
                {
                    ...
                    int iLineNo = 0;
                    foreach (var process in processes.OrderByDescending(r => r.StartTime))
                    {
                        if (iLineNo != 0)
                        {
                            process.Kill();
                ...
                SEACC.PROD.DATA.DBHandling.ConnectionString = clsSecurity.decryptPassword(e.Args[0]);
            ...
            #region Load modules
            foreach (tbl_cfgModule oModule in tbl_cfgModule.SelectAll().Where(p => p.Module_ID.Trim() == "PROD/018" || p.Module_ID.Trim() == "ADM/000" || 
