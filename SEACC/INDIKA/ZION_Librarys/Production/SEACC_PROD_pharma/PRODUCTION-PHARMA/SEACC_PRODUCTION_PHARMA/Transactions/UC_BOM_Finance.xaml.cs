...
namespace SEACC_PRODUCTION_PHARMA.Transactions
{
    ...
    public partial class UC_BOM_Finance : UserControl
    {
        ...
        private void AppDomainInitializer(FormName enmForm)
        {
            ...
            if (SEACC_Form.enmFormName == FormName.ProdPharma_BOMCosting_SpecialPermission)
            {
            ...
            dgr_Main.Add_DatagridColoumn("Is Locked", "IS_LOCKED", 100, false);
                            ...
                            if (oJob != null)
                            {
                                if ((!oJob.IsApproved3 && !oJob.IsLocked) || bIsSpecialPermission_EditBoM)
                                {
                                    ...
                                    tbl_prod_pharmaTxJobCard oOldJob = new tbl_prod_pharmaTxJobCard(
                                                                ...
                                                                oJob.CanceledUserTerminal_ID, oJob.LockedUserTerminal_ID, oJob.CompanyID, oJob.CompanyBranchID, oJob.CustomerOrder_Qty, oJob.IsTemporaryBoM);
                                    oOldJob.Update();
                                    #endregion
                                    ...
                                    foreach (DataRow row in dtRawMeterialCost.Rows)
                                    {
                                        ...
                                        tbl_prod_pharmaTxJobCard_Material oMeterial = tbl_prod_pharmaTxJobCard_Material.Select(iLineNoMain, iLineNoSub1, iLineNoSub2, oOldJob.ProdJob_ID);
                                        if (oMeterial != null)
                                        {
                                    ...
                                    foreach (DataRow row in dtSemiFinishedCost.Rows)
                                    {
                                        ...
                                        if (oMeterial != null)
                                        {
                                            ...
                                            tbl_prod_pharmaTxJobCard_Material_Outsource oOld_SF_Outsource = tbl_prod_pharmaTxJobCard_Material_Outsource.Select(oMeterial.Line_No, oMeterial.Line_No_Sub1, oMeterial.Line_No_Sub2, oMeterial.ProdJob_ID);
                                            if (oOld_SF_Outsource != null)
                                            {
                                                tbl_prod_pharmaTxJobCard_Material_Outsource oSF_Outsource = new tbl_prod_pharmaTxJobCard_Material_Outsource(oMeterial.Line_No, oMeterial.Line_No_Sub1, oMeterial.Line_No_Sub2, oMeterial.ProdJob_ID, oMeterial.Item_ID, oMeterial.Uom_ID, oMeterial.Consumption, dOutsourceRate, dOutsourceCost);
                                    ...
                                    tbl_prod_pharmaTxJobCard_Labour.DeleteAllByProdJob_ID(oOldJob.ProdJob_ID);
                                    ...
                                    {
                                        int iLineNo = Convert.ToInt32(clsValidate.ValidateRowValue(row, "LineNo", 0m));
                                        ...
                                        tbl_prod_pharmaTxJobCard_Labour oProdJob_Labour = new tbl_prod_pharmaTxJobCard_Labour(
                                    ...
                                    foreach (DataRow row in dtOH_Labour_CostSMV.Rows)
                                    {
                                        ...
                                        tbl_prod_pharmaTxJobCard_CostCenter oCostCenter = tbl_prod_pharmaTxJobCard_CostCenter.Select(iLineNo, txtProdJobID.Text);
                                        if (oCostCenter != null)
                                        {
                                    ...
                                    #region Cost Footer
                                    tbl_prod_pharmaTxJobCard_CostFooter.DeleteAllByProdJob_ID(oJob.ProdJob_ID);
                                    foreach (DataRow row in dtCostFooter.Rows)
                                    {
                                        ...
                                        tbl_prod_pharmaTxJobCard_CostFooter oProd_Footer = new tbl_prod_pharmaTxJobCard_CostFooter(iLineNo, oJob.ProdJob_ID, sFooterID, clsValidation.Validate_DecimalNumber(sPercentage), dCost);
                                        oProd_Footer.Insert();
                                    }
                                    #endregion
        ...
        private void btnAddActivity_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (RowDataSearch.DialogResult == true)
            {
                ...
                if (bAddItem)
                {
                    tbl_prod_pharmaMasSectionActivity oSecActivity = tbl_prod_pharmaMasSectionActivity.Select(lstResult[2]);
        ...
        private void RefreshGrid()
        {
            try
            {
                ...
                List<tbl_prod_pharmaTxJobCard> oProdJobList;
                if (SEACC_Form.enmFormName == FormName.ProdPharma_BOMCosting_SpecialPermission)
                    oProdJobList = tbl_prod_pharmaTxJobCard.SelectAll().Where(p => p.ProdJobStatus != (int)prod_BoM_Status.Obsolete && p.ProdJob_ID != "default" && p.IsApproved3).OrderByDescending(o => o.DateCreate).ToList();
                else
                    oProdJobList = tbl_prod_pharmaTxJobCard.SelectAll().Where(p => p.ProdJobStatus != (int)prod_BoM_Status.Obsolete && p.ProdJob_ID != "default" && p.IsApproved2).OrderByDescending(o => o.DateCreate).ToList();

                int iCount = 0;
                foreach (tbl_prod_pharmaTxJobCard oJob in oProdJobList)
                {
                    ...
                    if (oItem != null)
                    {
                        ...
                        dgr_Main.dt.Rows.Add(++iCount, oJob.ProdJob_ID,
                            ...
                            clsFormatter.FormatDecimalPlaces_Quantity(oJob.FGoodQty),
                            ...
                            clsGenaralName.getName_User(oJob.Approved3User_ID),
                            clsGenaralName.getName_User(oJob.LockedUser_ID), oJob.IsCanceled, oJob.IsLocked);
                    }
                }
                dgr_Main.RefreshGrid();
            }
        ...
        private bool CheckValidity()
        {
            ...
            if (CheckValidity_EmptyField())
            {
                if (CheckValidity_DuplicateFiled())
                {
                    ...
                    {
                        if (CheckSpecialPermision_EditBoM())
                        {
                            bStatus = true;
                        }
                    }
        ...
        private bool CheckSpecialPermision_EditBoM()
        {
            bool bStatus = true;
            if (SEACC_Form.enmFormName == FormName.ProdPharma_BOMCosting_SpecialPermission)
            {
        ...
        private void FillRawMaterialGrid(string sProdJob_ID)
        {
            ...
            foreach (tbl_prod_pharmaTxJobCard_Material oJob_Meterial in tbl_prod_pharmaTxJobCard_Material.SelectAllByProdJob_ID(sProdJob_ID).Where(r => !r.IsSemiFinishItem))
            {
                ...
                if (oJob_Meterial.Line_No_Sub2 == 0)
                    sItemGroup_Name = clsGenaralName.getName_Section(oJob_Meterial.Section_ID) + ", " + clsGenaralName.getName_PharmaSectionActivity(oJob_Meterial.Activity_ID) + ", " + sItemName; // sItemName;
                ...
                dtRawMeterialCost.Rows.Add("0",
                                    ...
                                    sTotalInputQty, sConsumption, sdItemWAvgCost, sLowestCost, sHighestCost, iCostSeclection, "0", sCost_Edited, "0", oJob_Meterial.IsSemiFinishItem, sItemGroup_Name);
        ...
        private void FillSemiFinishedGrid(string sProdJob_ID)
        {
            ...
            foreach (tbl_prod_pharmaTxJobCard_Material oJob_Meterial in tbl_prod_pharmaTxJobCard_Material.SelectAllByProdJob_ID(sProdJob_ID).Where(r => r.IsSemiFinishItem))
            {
                ...
                string sLowestCost = cls_Formater.FormatDecimal(oJob_Meterial.LowestCost, clsConfig.sCurrencyDecimalPlaces_UnitPrice);
                string sHighestCost = cls_Formater.FormatDecimal(oJob_Meterial.HighestCost, clsConfig.sCurrencyDecimalPlaces_UnitPrice);
                string sCost = cls_Formater.FormatDecimal(0, clsConfig.sCurrencyDecimalPlaces_UnitPrice); //cls_Formater.FormatDecimal(oJob_Meterial.Cost, clsConfig.sCurrencyDecimalPlaces_UnitPrice);
                string sCost_Edited = cls_Formater.FormatDecimal(oJob_Meterial.EditedCost, clsConfig.sCurrencyDecimalPlaces_UnitPrice);

                int iCostSeclection = oJob_Meterial.CostTypeSelection;

                tbl_prod_pharmaTxJobCard_Material_Outsource oSF_Outsource = tbl_prod_pharmaTxJobCard_Material_Outsource.Select(oJob_Meterial.Line_No, oJob_Meterial.Line_No_Sub1, oJob_Meterial.Line_No_Sub2, oJob_Meterial.ProdJob_ID);
                ...
                dtSemiFinishedCost.Rows.Add("0",
                                    oJob_Meterial.Line_No, oJob_Meterial.Line_No_Sub1, oJob_Meterial.Line_No_Sub2,
                                    oJob_Meterial.Item_ID, sItemName, sUoMName,
                                    sTotalInputQty, sConsumption, sdItemWAvgCost, sLowestCost, sHighestCost, iCostSeclection, "0", sCost_Edited, "0", oJob_Meterial.IsSemiFinishItem, sSF_OutsourceRate, sSF_OutsourceAmount);
        ...
        {
            dtOH_LabourCost_SectionActivities.Clear();
            foreach (tbl_prod_pharmaTxJobCard_Labour oJob_labour in tbl_prod_pharmaTxJobCard_Labour.SelectAllByProdJob_ID(sProdJob_ID))
            {
                dtOH_LabourCost_SectionActivities.Rows.Add("",
                    oJob_labour.ProdSection_ID,
                     clsGenaralName.getName_Section(oJob_labour.ProdSection_ID),
                     oJob_labour.ProdActivity_ID,
                     clsGenaralName.getName_PharmaSectionActivity(oJob_labour.ProdActivity_ID),
        ...
        {
            dtOH_Labour_CostSMV.Clear();
            foreach (tbl_prod_pharmaTxJobCard_CostCenter oJob_CostCenter in tbl_prod_pharmaTxJobCard_CostCenter.SelectAllByProdJob_ID(sProdJob_ID))
            {
                tbl_prod_pharmaMasCostCenter oCostCenter = tbl_prod_pharmaMasCostCenter.Select(oJob_CostCenter.Cost_Center_ID);
                if (oCostCenter != null)
        ...
        private void Fill_GridFooter_fromProdJob(string sProdJob_ID)
        {
            ...
            foreach (tbl_prod_pharmaTxJobCard_CostFooter oCostFooter in tbl_prod_pharmaTxJobCard_CostFooter.SelectAllByProdJob_ID(sProdJob_ID).OrderBy(o => o.Line_No))
            {
                tbl_prod_pharmaMasCostFooter oFooter = tbl_prod_pharmaMasCostFooter.Select(oCostFooter.Footer_ID);
        ...
        private void Fill_GridFooter()
        {
            ...
            foreach (tbl_prod_pharmaMasCostFooter oCFooter in tbl_prod_pharmaMasCostFooter.SelectAll().Where(r => r.IsEnable).OrderBy(r => r.Line_No))
            {
        ...
        private void dgr_Main_LoadingRow(object sender, DataGridRowEventArgs e)
        {
            ...
            {
                //Locked BoM
                if (Convert.ToBoolean(((DataRowView)(e.Row.DataContext)).Row.ItemArray[12].ToString()))
                {
                    e.Row.Foreground = (Brush)bc.ConvertFrom("#a0ffa0");
                }
                //Canceled BoM
                else if (Convert.ToBoolean(((System.Data.DataRowView)(e.Row.DataContext)).Row.ItemArray[11].ToString()))
                {
        ...
        private void ComboBox_DirectMeterialCost_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            foreach (DataRow row in dtRawMeterialCost.Rows)
            {
                ...
                int iSelectedIndex_ComboBox = Convert.ToInt32(clsValidate.ValidateRowValue(row, "CostSelection", 0m));
                decimal dCost = clsValidate.ValidateRowValue(row, "Cost", 0m);
        ...
        private void CalculateCostingTotals()
        {
            try
            {
                ...
                int iCurrencyDecimals = (clsConfig.sCurrencyDecimalPlaces_UnitPrice);
