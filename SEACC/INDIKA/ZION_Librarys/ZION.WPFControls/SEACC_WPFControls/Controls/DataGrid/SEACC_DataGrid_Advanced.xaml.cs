...
namespace SEACC_WPFControls
{
    ...
    public partial class SEACC_DataGrid_Advanced : UserControl
    {
        ...
        public void Add_DatagridColoumn(ColoumnType DatagridColoumnType, string Header, string Binding, int width, bool isVisible, bool isReadOnly)
        {
            Add_DatagridColoumn(DatagridColoumnType, "Segoe UI", Header, Binding, width, isVisible, isReadOnly);
        }

        public void Add_DatagridColoumn(ColoumnType DatagridColoumnType, string sFontFamily, string Header, string Binding, int width, bool isVisible, bool isReadOnly)
        {
            // dt.Columns.Add(Binding);
            Style sNumerics = new Style();
            sNumerics.Setters.Add(new Setter(TextBox.TextAlignmentProperty, TextAlignment.Right));

            Style sSymbols = new Style();
            sSymbols.Setters.Add(new Setter(TextBox.TextAlignmentProperty, TextAlignment.Center));
            ...
            switch (DatagridColoumnType)
            {
                ...
                case ColoumnType.Numaric:
                    {
                        ...
                        textColumn1.FontFamily = new FontFamily(sFontFamily);
                        ...
                        if (sFontFamily == "Segoe MDL2 Assets")
                        {
                            textColumn1.CellStyle = sSymbols;
                            textColumn1.FontSize = 16;
                        }
                    }
                    break;
                case ColoumnType.CheckBox:
                    ...
                    break;
                //Try to implement Button Column
                //By Gayan on 2017-08-05
                //case ColoumnType.Button:
                //    {
                ...
                //        bind.Mode = BindingMode.TwoWay;
                //        bind.UpdateSourceTrigger = UpdateSourceTrigger.PropertyChanged;
                //        buttonColumn.SortMemberPath = Binding;

                //        FrameworkElementFactory buttonFactory = new FrameworkElementFactory(typeof(Button));
                //        buttonFactory.SetBinding(Button.ContentProperty, bind);
                //        DataTemplate dataTemplate = new DataTemplate();
                ...
                //        grdMain.Columns.Add(buttonColumn);
                //    }
                //    break;
                default:
                    break;
        ...
        public void SetFilterValue(string columnName_of_DataGrid, string columnName_of_DataTable, string value, bool bIsLikeOperator)
        {
            foreach (object child in dds.Children)
            {
                if (child is SEACC_Filter)
                {
                    ...
                    if (columnName_of_DataGrid == element.Caption)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                if (bIsLikeOperator)
                                    dt.DefaultView.RowFilter = columnName_of_DataTable + " like '%" + ValidateSearchText(value) + "%'";
                                else
                                    dt.DefaultView.RowFilter = columnName_of_DataTable + "='" + value + "'";
                            }
                            else
        ...
        public void SetFilterValue(string columnName_of_DataGrid, string columnName_of_DataTable, string value)
        {
            SetFilterValue(columnName_of_DataGrid, columnName_of_DataTable, value, false);
        }
        ...
        private string ValidateSearchText(string value)
        {
