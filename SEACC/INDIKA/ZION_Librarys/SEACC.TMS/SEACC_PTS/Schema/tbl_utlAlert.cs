...
namespace Schema {
	public sealed class tbl_utlAlert {
		...
		public static tbl_utlAlert Select(string alert_ID_Incoming){
            ...
            //string sScript = "Select [alert_ID] , [alertName] , [formCategory_ID] , [isActive] From [dbo].[tbl_utlAlert] Where [alert_ID] = '" + alert_ID_Incoming + "'";
            //bool bQuaryStatus = DBConnection.SelectToDataTable(sScript);
            //tbl_utlAlert oTable = null;
            //if (bQuaryStatus && DBConnection.ResultTable.Rows.Count > 0)

            //{
            //    oTable = new tbl_utlAlert();

            //    oTable.alert_ID = DBConnection.ResultTable.Rows[0]["alert_ID"].ToString();
            //    oTable.alertName = DBConnection.ResultTable.Rows[0]["alertName"].ToString();
            //    oTable.formCategory_ID = DBConnection.ResultTable.Rows[0]["formCategory_ID"].ToString();
