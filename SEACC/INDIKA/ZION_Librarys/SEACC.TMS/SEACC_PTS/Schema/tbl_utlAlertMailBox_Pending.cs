...
namespace Schema {
	public sealed class tbl_utlAlertMailBox_Pending {
		...
		/// Initializes a new instance of the tbl_utlAlertMailBox_Pending class.
		/// </summary>
		public tbl_utlAlertMailBox_Pending(int eMail_ID, string alert_ID, string subject, string body, int status) {
        ...
        //    string sScript = "INSERT INTO [dbo].[tbl_utlAlertMailBox_Pending] ([eMail_ID] , [alert_ID] , [subject] , [body] , [status] ) VALUES (" + eMail_ID + " , '" + alert_ID + "' ,' " + subject + "' , '" + body + "' , " + status + " ) select scope_identity()";
        //    DBConnection.Execute_Quary(sScript);
        //}
        ...
        public void Insert()
        {
            ...
            //scom.Parameters.Add("@body", SqlDbType.VarChar,-1);
        ...
        public static List<tbl_utlAlertMailBox_Pending> SelectAll_UnsentMails()
        {
            SqlConnection scon = DBHandling.GetConnection();
            SqlCommand scom = new SqlCommand("tbl_utlAlertEmailSelectAll_UnsentMails", scon);
            scom.CommandType = CommandType.StoredProcedure;
            scon.Open();

            List<tbl_utlAlertMailBox_Pending> tbl_utlAlertEmailList = new List<tbl_utlAlertMailBox_Pending>();
            using (SqlDataReader dataReader = scom.ExecuteReader())
            {
                while (dataReader.Read())
                {
                    tbl_utlAlertMailBox_Pending tbl_utlAlertEmail = Maketbl_utlAlertMailBox_Pending(dataReader);
		...
		/// Selects all records from the tbl_utlAlertMailBox_Pending table by a foreign key.
		/// </summary>
		public static List<tbl_utlAlertMailBox_Pending> SelectAllByAlert_ID(string alert_ID) {
		...
		private static tbl_utlAlertMailBox_Pending Maketbl_utlAlertMailBox_Pending(SqlDataReader dataReader) {
			...
			if (dataReader.IsDBNull(1) == false) {
				tbl_utlAlertMailBox_Pending.Alert_ID = dataReader.GetString(1);
			}
