	...
	public sealed class tbl_genItemMaster {
		...
		private static tbl_genItemMaster Maketbl_genItemMaster(SqlDataReader dataReader) {
			...
			if (dataReader.IsDBNull(36) == false) {
				tbl_genItemMaster.Gusset = dataReader.GetDecimal(36);
			}
			if (dataReader.IsDBNull(37) == false) {
				tbl_genItemMaster.Qty = dataReader.GetDecimal(37);
			}
			if (dataReader.IsDBNull(38) == false) {
				tbl_genItemMaster.CalculationRate_Weight = dataReader.GetDecimal(38);
			}
			if (dataReader.IsDBNull(39) == false) {
				tbl_genItemMaster.CalculationRate_LFeet = dataReader.GetDecimal(39);
			}
			if (dataReader.IsDBNull(40) == false) {
				tbl_genItemMaster.MeasureType_ID = dataReader.GetString(40);
			}
			if (dataReader.IsDBNull(41) == false) {
				tbl_genItemMaster.IsWeightCalculation_Sales = dataReader.GetBoolean(41);
			}
			if (dataReader.IsDBNull(42) == false) {
				tbl_genItemMaster.IsWeightCalculation_Purchase = dataReader.GetBoolean(42);
			}
			if (dataReader.IsDBNull(43) == false) {
				tbl_genItemMaster.IsDeleted = dataReader.GetBoolean(43);
			}
			if (dataReader.IsDBNull(44) == false) {
				tbl_genItemMaster.IsVATinclusive = dataReader.GetBoolean(44);
			}
			if (dataReader.IsDBNull(45) == false) {
				tbl_genItemMaster.IsNBTinclusive = dataReader.GetBoolean(45);
			}
			if (dataReader.IsDBNull(46) == false) {
				tbl_genItemMaster.ImagePath = dataReader.GetString(46);
			}
