...
namespace DataTire {
	public sealed class tbl_utlAlert {
		...
		/// Initializes a new instance of the tbl_utlAlert class.
		/// </summary>
		public tbl_utlAlert(int alert_ID, string alertName, string alertSender_ID, bool isActive) {
		...
		/// <summary>
		/// Selects all records from the tbl_utlAlert table by a foreign key.
		/// </summary>
		public static void DeleteAllByAlertSender_ID(string alertSender_ID) {
 
			SqlConnection scon = DBHandling.GetConnection();
			SqlCommand scom = new SqlCommand("tbl_utlAlertDeleteAllByAlertSender_ID", scon);
			scom.CommandType = CommandType.StoredProcedure;
			scon.Open();
		...
		/// </summary>
		public static tbl_utlAlert Select(int alert_ID_Incoming){
		...
		/// Selects all records from the tbl_utlAlert table by a foreign key.
		/// </summary>
		public static List<tbl_utlAlert> SelectAllByAlertSender_ID(string alertSender_ID) {
 
			SqlConnection scon = DBHandling.GetConnection();
			SqlCommand scom = new SqlCommand("tbl_utlAlertSelectAllByAlertSender_ID", scon);
			scom.CommandType = CommandType.StoredProcedure;
			scon.Open();
		...
		private static tbl_utlAlert Maketbl_utlAlert(SqlDataReader dataReader) {
			...
			if (dataReader.IsDBNull(0) == false) {
				tbl_utlAlert.Alert_ID = dataReader.GetInt32(0);
			}
			if (dataReader.IsDBNull(1) == false) {
				tbl_utlAlert.AlertName = dataReader.GetString(1);
			}
			if (dataReader.IsDBNull(2) == false) {
				tbl_utlAlert.AlertSender_ID = dataReader.GetString(2);
			}
		...
		dt.Columns.AddRange(new DataColumn[] { col_alert_ID,col_alertName,col_AlertSender_ID,col_isActive,});		return dt;
		}
		/// <summary>
