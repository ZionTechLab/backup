        ...
        private void frmFormList_Load(object sender, EventArgs e)
        {
            ...
            if (glbOrderRefNo.Length > 0 && glbOrderRefNo != "default")
            {
                ...
                if (order != null && pn != null)
                    clsHelpMethods.FillProcessNotes(order.OrderRefNo_ID, dgvDetail, pn, true);
            }
            else if (glbNotes.Count > 0)
            {
                clsHelpMethods.FillProcessNotes(glbNotes, dgvDetail, pn, true);
            }
                            ...
                            if (bApprovalDone && bCheckingDone)
                            {
                                ...
                                if (bOkToPrint && bApprovalDone)
                                {
                                    ...
                                    if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.ePackWithSubCategory.ToString() || clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.idealWheels.ToString())
                                    {
                                        ...
                                        RD.DataDefinition.FormulaFields["Outstanding"].Text = clsCommon.fncsetstring(clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.GetCustomerTotalDues_All(order.Customer_ID)));
                                        RD.DataDefinition.FormulaFields["Cheques-In-Hand"].Text = clsCommon.fncsetstring(clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.GetCustomerChequesInHand(order.Customer_ID)));
