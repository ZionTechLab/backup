    ...
    public partial class frmDocumentApproval : Form
    {
                            ...
                            if (con is StatusStrip)
                            {
                                ...
                                foreach (ToolStripItem item in stp.Items)
                                {
                                  //  if (item.Name == "tslPendingApproval")
                                      //  item.Text = clsHelpMethods.GetPendingApprovalCount().ToString();
                                }
                            }
                        }
                    }
        ...
        private void FillCategory(tbl_securityProcessNoteCatogory Category, Button btnCategory, int width)
        {
            try
            {
                ...
                btnCategory.Text = Category.ProcessNoteCategoryName + " (" + clsHelpMethods.GetPendingApprovalCount_ProcessNoteCategory(Category.ProcessNoteCategory_ID).ToString() + ")";
        ...
        private void FillNotes(tbl_securityProcessNoteMaster Category, Button btnCategory, int width)
        {
            try
            {
                int iNoOfProcessNote = clsHelpMethods.GetPendingApprovalCount_ProcessNote(Category.ProcessNote_ID);
        ...
        private void NoteClick(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    //clsHelpMethods.FormatGrid_DocumentCheckingOrApproval(dgvDetail, ProcessNote.CustomerOrder);
                ...
                {
                    //clsHelpMethods.FormatGrid_DocumentCheckingOrApproval(dgvDetail, ProcessNote.ProductionJob);
                    RefreshGridProductionJob();
                }
                else if (sNoteID == clsAutocode.GetProcessNoteID(ProcessNote.iSR_Dept).ToString())
                {
                    //clsHelpMethods.FormatGrid_DocumentCheckingOrApproval(dgvDetail, ProcessNote.iSR_Dept);
                    RefreshGridDepartment_iSR();
                }

                else if (sNoteID == clsAutocode.GetProcessNoteID(ProcessNote.PurchaseOrder).ToString())
                {
                    //clsHelpMethods.FormatGrid_DocumentCheckingOrApproval(dgvDetail, ProcessNote.PurchaseOrder);
                    RefreshGridPurchaseOrder();
                }
                else if (sNoteID == clsAutocode.GetProcessNoteID(ProcessNote.ExternalGoodReceivedNote).ToString())
                {
                    //clsHelpMethods.FormatGrid_DocumentCheckingOrApproval(dgvDetail, ProcessNote.ExternalGoodReceivedNote);
                    RefreshGridGRN();
                }
                else if (sNoteID == clsAutocode.GetProcessNoteID(ProcessNote.ExternalGoodIssuedNote).ToString())
                {
                    //clsHelpMethods.FormatGrid_DocumentCheckingOrApproval(dgvDetail, ProcessNote.ExternalGoodIssuedNote);
                    RefreshGridGIN();
                }
                ...
                {
                    //clsHelpMethods.FormatGrid_DocumentCheckingOrApproval(dgvDetail, ProcessNote.PurchaseReturned);
                    RefreshGridPurchaseReturn();
                }
                else if (sNoteID == clsAutocode.GetProcessNoteID(ProcessNote.PurchaseRequisition).ToString())
                {
                    //clsHelpMethods.FormatGrid_DocumentCheckingOrApproval(dgvDetail, ProcessNote.PurchaseRequisition);
                    RefreshGridPurchaseRequisition();
                }
                else if (sNoteID == clsAutocode.GetProcessNoteID(ProcessNote.iGRN_Store).ToString())
                {
                    //clsHelpMethods.FormatGrid_DocumentCheckingOrApproval(dgvDetail, ProcessNote.iGRN_Store);
                    RefreshGridIGRN();
                }
                else if (sNoteID == clsAutocode.GetProcessNoteID(ProcessNote.iGIN_Store).ToString())
                {
                    //clsHelpMethods.FormatGrid_DocumentCheckingOrApproval(dgvDetail, ProcessNote.iGIN_Store);
                    RefreshGridIGIN();
                }
                else if (sNoteID == clsAutocode.GetProcessNoteID(ProcessNote.iSR_Store).ToString())
                {
                    //clsHelpMethods.FormatGrid_DocumentCheckingOrApproval(dgvDetail, ProcessNote.iSR_Store);
                    RefreshGridISRN();
                }
                else if (sNoteID == clsAutocode.GetProcessNoteID(ProcessNote.GoodsTransferNote).ToString())
                {
                    //clsHelpMethods.FormatGrid_DocumentCheckingOrApproval(dgvDetail, ProcessNote.GoodsTransferNote);
                    RefreshGridGTN();
                }
                else if (sNoteID == clsAutocode.GetProcessNoteID(ProcessNote.FinishedGoodsTransferNote).ToString())
                {
                    //clsHelpMethods.FormatGrid_DocumentCheckingOrApproval(dgvDetail, ProcessNote.FinishedGoodsTransferNote);
                    RefreshGridFGTN();
                }
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                foreach (tbl_sasCustomerOrder detail in details)
                {
                    if (!detail.IsApproved && !detail.IsDeleted && !detail.IsFinished && detail.CustomerOrder_ID != "default")
                    {
                        ...
                        //dAge30to60 = clsHelpMethods.GetCustomerTotalDues_Invoice30to60(sCustomerID);
                        //dAge60to90 = clsHelpMethods.GetCustomerTotalDues_Invoice60to90(sCustomerID);
                        //dAge90plus = clsHelpMethods.GetCustomerTotalDues_Invoice90plus(sCustomerID);
                        //dChequesInHand = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.GetCustomerChequesInHand(sCustomerID));
                        //dReturnedOutstanding = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.GetCustomerTotalDues_ReturnedCheque(sCustomerID));
            ...
            try
            {
                ...
                foreach (tbl_pmsProductionJobRegister detail in details)
                {
                    if (!detail.IsApproved && !detail.IsDeleted && !detail.IsFinished && detail.ProductionJob_ID != "default")
                    {
                        ...
                        //dReturnedOutstanding = clsHelpMethods.GetItemSizeByItemID(detail.Item_ID);                      
