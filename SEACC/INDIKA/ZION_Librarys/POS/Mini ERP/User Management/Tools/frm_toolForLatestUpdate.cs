    ...
    public partial class frm_ToolUpdateOldestInvoiceDate : MettroForm
    {
        ...
        private void btnExecute2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                tbl_genCompanyInfo oInfo = tbl_genCompanyInfo.Select(clsSecurity.CompanyID);
        ...
        private void btn_CRNPosting_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                {
                    Cursor = Cursors.WaitCursor;
                    foreach (tbl_bpsCreditNote OCRN in tbl_bpsCreditNote.SelectAll().Where(p => !p.IsDeleted && p.CreditNoteType_ID != "TP/003" && p.CreditNote_ID != "default"))
                    {
        ...
        {
            DialogResult result = MessageBox.Show("Do You Want to Update Cash Amounts?", clsFormatter.GetMessageCaption(), MessageBoxButtons.YesNo, MessageBoxIcon.Information);
            if (result == DialogResult.Yes)
            {
                try
                {
                    ...
                    foreach (tbl_bpsReceipt oReceipt in tbl_bpsReceipt.SelectAll().Where(p => p.CashAmount > 0 && p.Receipt_ID != "default"))
                    {
                        string sChequeRegisterCode = clsAutocode.getAutoGeneratedCode(sFormConfigCashRegisterCode);

                        tbl_bpsChequeRegister RegisterDetails = new tbl_bpsChequeRegister(sChequeRegisterCode, "", oReceipt.ReceiptDate, (int)PaymentMethod.Cash,
                            (-1), "", (-1), 0, "", "", (-1), (-1), oReceipt.ReceiptDate, oReceipt.Customer_ID, "default", "", -1, "default", "default", "default", "default",
                            "default", "default", "default", "default", oReceipt.Receipt_ID, "default", "default", "default", "", oReceipt.GlPosting_ID, oReceipt.PostingStatus_ID,
                            oReceipt.PostingStatus_ID2, oReceipt.FinancialYear_ID, oReceipt.CashAmount,
                            oReceipt.IsSeattled, false, oReceipt.IsCashDeposited, false, false, false, false,
                            oReceipt.CreateUser_ID, oReceipt.ModifiedUser_ID, oReceipt.DateCreate, oReceipt.DateModified,
                            oReceipt.IsDeleted, oReceipt.IsLocked,
                            0, 0, oReceipt.SeattleAmount, oReceipt.DepositedCashAmount, oReceipt.DateDeposited, clsSecurity.getServerDateTime(), clsSecurity.getServerDateTime(), clsSecurity.getServerDateTime(), oReceipt.CompanyID, oReceipt.CompanyBranch_ID , (-1), (-1),(-1));
                        RegisterDetails.Insert();

                        oReceipt.CashAmount = 0;
                        oReceipt.SeattleAmount = 0;
                        oReceipt.Update();
                    }
        ...
        private void btnCreditorSettlement_Click(object sender, EventArgs e)
        {
            DialogResult result = MessageBox.Show("Do You Want to Update Creditor Settlement ID?", clsFormatter.GetMessageCaption(), MessageBoxButtons.YesNo, MessageBoxIcon.Information);
            if (result == DialogResult.Yes)
            {
                try
                {
                    string sFormConfigCode = clsAutocode.getFormConfigCode(FormName.accCreditorSettlement);
                    Cursor = Cursors.WaitCursor;
                    foreach (tbl_accPaymentVoucher_Detail oSettlement in tbl_accPaymentVoucher_Detail.SelectAll())
                    {
                        int sSettlement_ID = int.Parse(clsAutocode.getAutoGeneratedCode(sFormConfigCode));
                        oSettlement.Line_No = sSettlement_ID;
                        oSettlement.Update();
                    }
        ...
        public static byte[] CopyImageToByteArray(Image theImage)
        {
            ...
            {
                theImage.Save(memoryStream, System.Drawing.Imaging.ImageFormat.Bmp);
                return memoryStream.ToArray();
            }
        }
        private void btnUpdateFormImages_Click(object sender, EventArgs e)
        {
            DialogResult result = MessageBox.Show("Do You Want to Update Form Images?", clsFormatter.GetMessageCaption(), MessageBoxButtons.YesNo, MessageBoxIcon.Information);
            if (result == DialogResult.Yes)
            {
                try
                {
                    foreach (tbl_securityFormMaster detail in tbl_securityFormMaster.SelectAll())
                    {
                        bool bImageOK = false;
                        ...
                        if (!bImageOK)
                        {
                            detail.Image = CopyImageToByteArray(pbxImage.Image);
                            detail.Update();
                        }

                    }

                    // DBHandling.ExecQuery("exec [sp_UpdateFormImages]");
        ...
        private void btnUpdateIGINTable_Click(object sender, EventArgs e)
        {
            DialogResult result = MessageBox.Show("Do You Want to Update IGIN Table?", clsFormatter.GetMessageCaption(), MessageBoxButtons.YesNo, MessageBoxIcon.Information);
            if (result == DialogResult.Yes)
            {
                try
                {
                    //decimal dQtySettled = 0;
                    //bool bSettled = false;

                    foreach (tbl_scsStoreGoodIssueNote IGIN in tbl_scsStoreGoodIssueNote.SelectAll().Where(p => !p.IsDeleted && !p.IsSeattled))
                    {
                        bool bSettled = false;

                        foreach (tbl_scsStoreGoodIssueNote_Detail IGINDetail in tbl_scsStoreGoodIssueNote_Detail.SelectAllByStoreGoodIssueNote_ID(IGIN.StoreGoodIssueNote_ID))
                        {
                            decimal dQtySettled = 0;

                            foreach (tbl_scsStoreGoodReceiveNote_Detail IGRNDetail in tbl_scsStoreGoodReceiveNote_Detail.SelectAll().Where(p => p.StoreGoodIssueNote_ID == IGINDetail.StoreGoodIssueNote_ID && p.Item_ID == IGINDetail.Item_ID))
                            {
                                foreach (tbl_scsStoreGoodReceiveNote IGRN in tbl_scsStoreGoodReceiveNote.SelectAll().Where(p => p.StoreGoodReceiveNote_ID == IGRNDetail.StoreGoodReceiveNote_ID && !p.IsDeleted))
                                {
                                    dQtySettled += IGRNDetail.Qty;
                                }
                            }

                            IGINDetail.QtySettle = dQtySettled;
                            IGINDetail.Update();

                            if (IGINDetail.QtySettle >= IGINDetail.Qty)
                                bSettled = true;
                        }

                        IGIN.IsSeattled = bSettled;
                        IGIN.Update();
                    }
        ...
        private void btnUpdateCRN_Click(object sender, EventArgs e)
        {
            DialogResult result = MessageBox.Show("Do You Want to Update CRN Settlement", clsFormatter.GetMessageCaption(), MessageBoxButtons.YesNo, MessageBoxIcon.Information);
            if (result == DialogResult.Yes)
            {
                try
                {
                    foreach (tbl_bpsCreditNote detail in tbl_bpsCreditNote.SelectAll().Where(p => !p.IsDeleted && (p.CreditNoteType_ID == "TP/002" || p.CreditNoteType_ID == "TP/004")))
                    {
                        clsMethods_GL.GLPosting_Delete(detail.GlPosting_ID);
                   //     clsMethods_GL.PostTransaction_CustomerCRN(detail.CreditNote_ID);
                    }

                    foreach (tbl_sasInvoice_Sattled oSettled in tbl_sasInvoice_Sattled.SelectAll().Where(p => p.CreditNote_ID != "default"))
                    {
                        tbl_bpsCreditNote oCrn = tbl_bpsCreditNote.Select(oSettled.CreditNote_ID);

                        if (oCrn != null && !oCrn.IsDeleted && (oCrn.CreditNoteType_ID == "TP/002" || oCrn.CreditNoteType_ID == "TP/004"))
                        {
                            tbl_sasInvoice oInv = tbl_sasInvoice.Select(oSettled.Invoice_ID);
                            if (oInv != null & !oInv.IsDeleted && oInv.IsReturnedCheque)
                                continue;
                            clsMethods_GL.GLPosting_Delete(oSettled.GlPosting_ID);
                          //  clsMethods_GL.PostTransaction_Allocation(oSettled.Settled_ID);
                        }
