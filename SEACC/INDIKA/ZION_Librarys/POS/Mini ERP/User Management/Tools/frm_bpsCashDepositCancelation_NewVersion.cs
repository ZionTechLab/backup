                        ...
                        if (oldRecord != null && oCashDeposit != null)
                        {
                            if (!oCashDeposit.IsReconciled)
                            {
                                if (clsMethods_GL.CheckValidity_FinancialYear(oCashDeposit.DateDeposit))
                                {
                                    ...
                                    #region POS Receipts
                                    tbl_bpsChequeRegister oPosPayment = tbl_bpsChequeRegister.SelectAllByPosReceipt_ID(oldRecord.Receipt_ID).Where(p => p.Amount >= 0 && p.PaymentMethod_ID == (int)PaymentMethod.Cash).FirstOrDefault();
                                    if (oPosPayment != null && oPosPayment.ChequeRegister_ID != "default")
                                    {
                                        clsMethods_GL.GLPosting_Delete(oldRecord.GlPosting_ID);

                                        oPosPayment.DepositedCashAmount -= oldRecord.DepositedAmount;
                                        oPosPayment.IsDepositted = false;
                                        oPosPayment.Update();
