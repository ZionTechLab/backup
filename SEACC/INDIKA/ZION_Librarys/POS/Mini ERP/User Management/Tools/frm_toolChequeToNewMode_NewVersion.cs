...
{
    public partial class frm_toolChequeToNewMode_NewVersion : SEACC_Form
    {
        #region Form Load
        public frm_toolChequeToNewMode_NewVersion(FormName _enmForm)
        {
            enmForm = _enmForm;
            InitializeComponent();
            Initialize();
        ...
        private void frm_toolChequeToNewMode_NewVersion_Load(object sender, EventArgs e)
        {
                        ...
                        try
                        {
                            ...
                            if (oldRecord != null && oldRecord.ChequeRegister_ID != "default" && !oldRecord.IsDeleted)
                            {
                                if (!oldRecord.IsReconcilied)
                                {
                                    ...
                                    MessageBox.Show("This Cheque is already Realized , It is Not Allow To Set To New Mode .", clsFormatter.GetMessageCaption(), MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                            else
                            {
