            ...
            try
            {
                if (txtDriverID.TextLength > 0)
                {
                    if (clsSecurity.PermissionToDelete(clsSecurity.UserIDLoged, iFormID))
                    {
                        ...
                        DialogResult msgResult = MessageBox.Show(clsFormatter.GetMessageFrom(MessageType.AskForDelete, " Driver Master " + txtDriverName.Text), clsFormatter.GetMessageCaption(), MessageBoxButtons.YesNo, MessageBoxIcon.Stop);
                        if (msgResult == DialogResult.Yes)
                        {
                            tbl_zDriver detail = tbl_zDriver.Select(txtDriverID.Text.Trim());
                            tbl_genEmployeeMaster EmpMasterdetail = tbl_genEmployeeMaster.Select(txtDriverID.Text.Trim());
                        ...
                        try
                        {
                            ...
                            if (txtDriverID.TextLength > 0)
                            {
                                ...
                                else  //insert records
                                {
                                    ...
                                    tbl_genEmployeeMaster detailEmp = new tbl_genEmployeeMaster(txtDriverID.Text.Trim(), txtDriverName.Text.Trim(), "", txtNIC.Text.Trim(), "", "", "", "", "", "default", clsSecurity.getServerDateTime(), true, false, false, false, false, false, false, 0, false, 0, 0, 0, 0);
                                    detailEmp.Insert();
                                    tbl_zDriver detail = new tbl_zDriver(txtDriverID.Text.Trim(), txtDriverName.Text.Trim(), txtNIC.Text.Trim());
                                    detail.Insert();
        ...
        private void RefreshGrid()
        {
            try
            {
                ...
                foreach (tbl_zDriver detail in details)
                {
                    ...
                    {
                        tbl_genEmployeeMaster oldRecordEmp = tbl_genEmployeeMaster.Select(detail.Driver_ID);
                        if (oldRecordEmp != null)
                        {
                            dgvDetail.Rows.Add();
