    ...
    public partial class frm_mtrEmpAreaManager : MettroForm
    {
                                ...
                                if (IsUpdate)  //update records
                                {
                                    ...
                                    if (oldRecord != null)
                                    {
                                        if (!oldRecord.IsDelete)
                                        {
                                            ...
                                            tbl_ZEmpAreaManager detail = new tbl_ZEmpAreaManager(txtAreaManagerID.Text.Trim(), txtAreaManagerName.Text.Trim(), txtSalesManagerName.Tag.ToString(), false);
                                            detail.Update();
        ...
        private void Search_AreaManagerID()
        {
            try
            {
                ...
                if (txtAreaManagerID.Tag != null && txtAreaManagerID.Tag.ToString().Trim().Length > 0)
                    FillDetails(txtAreaManagerID.Tag.ToString());
            }
            catch (Exception ex)
            {
                clsValidate.WriteErrorLog("", iFormID,ex);
        ...
        {
            clsSearch.Search_SalesManager(ref txtSalesManagerName);
        }
        #endregion
