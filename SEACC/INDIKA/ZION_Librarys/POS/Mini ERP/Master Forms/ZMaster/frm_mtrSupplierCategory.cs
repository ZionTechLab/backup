...
namespace Digiteq
{
    public partial class frm_mtrSupplierCategory : MettroForm
    {
        #region Variables
        ...
        #region Form Load
        public frm_mtrSupplierCategory()
        {
            sFormConfigCode = clsAutocode.getFormConfigCode(FormName.ZSupplierCategory);
            iFormID = clsSecurity.getFormID(FormName.ZSupplierCategory);
        ...
        private void frm_mtrSupplierCategory_Load(object sender, EventArgs e)
        {
                ...
                if (txtCategoryID.TextLength > 0)
                {
                    if (CheckValidity())
                    {
                        if (clsSecurity.PermissionToDelete(clsSecurity.UserIDLoged, iFormID))
                        {
                            ...
                            tbl_zSupplierCategory detail = tbl_zSupplierCategory.Select(txtCategoryID.Text.Trim());
                            if (detail != null)
                            {
                                detail.Delete();
                   ...
                   if (clsSecurity.PermissionToSave(clsSecurity.UserIDLoged, iFormID, IsUpdate))
                    {
                        try
                        {
                            ...
                            if (txtCategoryID.TextLength > 0)
                            {
                                ...
                                {
                                    tbl_zSupplierCategory oldRecord = tbl_zSupplierCategory.Select(txtCategoryID.Text.Trim());
                                    if (oldRecord != null)
                                    {
                                        //Country Header
                                        tbl_zSupplierCategory detail = new tbl_zSupplierCategory(txtCategoryID.Text.Trim(), txtCategoryName.Text.Trim());
                                        detail.Update();
        ...
        private void RefreshGrid()
        {
            try
            {
                ...
                List<tbl_zSupplierCategory> details = tbl_zSupplierCategory.SelectAll();
                foreach (tbl_zSupplierCategory detail in details)
                {
                    if (detail.SupplierCategory_ID.Trim() != "default")
                    {
                        dgvDetail.Rows.Add();
                        iRow = dgvDetail.Rows.Count - 1;
                        dgvDetail["CategoryID", iRow].Value = detail.SupplierCategory_ID;
        ...
        private void FillDetails(string sID)
        {
            try
            {
                if (sID.Length > 0)
                {
                    tbl_zSupplierCategory detail = tbl_zSupplierCategory.Select(sID);
                    if (detail != null)
                    {
                        ...
                        //asign values
                        txtCategoryID.Text = detail.SupplierCategory_ID;
        ...
        private void frm_mtrSupplierCategory_KeyDown(object sender, KeyEventArgs e)
        {
