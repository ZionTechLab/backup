    ...
    public partial class frm_mtrEmpSalesManager : MettroForm
    {
                                ...
                                if (IsUpdate)  //update records
                                {
                                    ...
                                    if (oldRecord != null)
                                    {
                                        if (!oldRecord.IsDelete)
                                        {
                                            ...
                                            tbl_ZEmpSalesManager detail = new tbl_ZEmpSalesManager(txtSalesManagerID.Text.Trim(), txtManagerName.Text.Trim(), false);
                                            detail.Update();
        ...
        private void Search_SalesManagerID()
        {
            try
            {
                ...
                if (txtSalesManagerID.Tag != null && txtSalesManagerID.Tag.ToString().Trim().Length > 0)
                    FillDetails(txtSalesManagerID.Tag.ToString());
            }
            catch (Exception ex)
            {
                clsValidate.WriteErrorLog("", iFormID, ex);
