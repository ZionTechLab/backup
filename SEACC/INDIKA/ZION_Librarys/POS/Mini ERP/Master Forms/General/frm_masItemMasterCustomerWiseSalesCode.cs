    ...
    public partial class frm_masItemMasterCustomerWiseSalesCode : MettroForm
    {
        ...
        private void RefreshGrid(string sCustomer_ID, string sBranch_ID)
        {
            try
            {
                ...
                dgvDetail.DataSource = dtItemRecodes;
            }
            catch (Exception ex)
            {
                clsValidate.WriteErrorLog("", iFormID, ex);
            ...
            try
            {
                ...
                if (chkItemID.Checked)
                {
                    if (sFinalQuary.Trim().Length > 0)
                        sFinalQuary += " AND item_ID LIKE '%" + clsHelpMethods.CheckValue(txtItemID.Text.Trim()) + "%'";
                    else
                        sFinalQuary = " item_ID LIKE '%" + clsHelpMethods.CheckValue(txtItemID.Text.Trim()) + "%'";
                }
                if (chkItemName.Checked)
                {
                    if (sFinalQuary.Trim().Length > 0)
                        sFinalQuary += " AND ItemName LIKE '%" + clsHelpMethods.CheckValue(txtItemName.Text.Trim()) + "%'";
                    else
                        sFinalQuary = " ItemName LIKE '%" + clsHelpMethods.CheckValue(txtItemName.Text.Trim()) + "%'";
                }
                if (chkItemClass.Checked)
                {
                    if (sFinalQuary.Trim().Length > 0)
                        sFinalQuary += " AND itemClass LIKE '%" + clsHelpMethods.CheckValue(txtItemClass.Text.Trim()) + "%'";
                    else
                        sFinalQuary = " itemClass LIKE '%" + clsHelpMethods.CheckValue(txtItemClass.Text.Trim()) + "%'";
                }
                if (chkItemType.Checked)
                {
                    if (sFinalQuary.Trim().Length > 0)
                        sFinalQuary += " AND itemType LIKE '%" + clsHelpMethods.CheckValue(txtItemType.Text.Trim()) + "%'";
                    else
                        sFinalQuary = " itemType LIKE '%" + clsHelpMethods.CheckValue(txtItemType.Text.Trim()) + "%'";
                }
                if (chkItemCatagory.Checked)
                {
                    if (sFinalQuary.Trim().Length > 0)
                        sFinalQuary += " AND itemCategory LIKE '%" + clsHelpMethods.CheckValue(txtItemCategory.Text.Trim()) + "%'";
                    else
                        sFinalQuary = " itemCategory LIKE '%" + clsHelpMethods.CheckValue(txtItemCategory.Text.Trim()) + "%'";
                }
                ...
                if (sFinalQuary.Trim().Length > 0)
                    dtItemRecodes.DefaultView.RowFilter = sFinalQuary;
