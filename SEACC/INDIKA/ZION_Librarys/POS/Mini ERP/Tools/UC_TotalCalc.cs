...
namespace Digiteq
{
    public partial class UC_TotalCalc : UserControl
    {
        ...
        public void FillDetail(decimal _SubTotal, decimal _DiscountTotal, decimal _NbtTotal, decimal _VatTotal, decimal _OtherTaxTotal, decimal _GrandTotal, decimal _DiscountPrecentage, decimal _NbtPercentage, decimal _VatPercentage, decimal _OtherTaxPercentage, string _SupplierID, string _Customer_ID, decimal _ExRate)
        {
            ...
            txtSubTotal.Text = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.getDisplayPrice(_SubTotal, _ExRate));
            txtDesc.Text = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.getDisplayPrice(_DiscountTotal, _ExRate));
            txtNBT.Text = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.getDisplayPrice(_NbtTotal, _ExRate));
            txtOtherTax.Text = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.getDisplayPrice(_OtherTaxTotal, _ExRate));
            txtVat.Text = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.getDisplayPrice(_VatTotal, _ExRate));
            txtGrandTotal.Text = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.getDisplayPrice(_GrandTotal, _ExRate));
        }
        ...
        public void SetGL(int LineNo, int TransactionCategoryID, string AccountCode, decimal Amount, bool isCredit, string SubAcct1_ID, string SubAcct2_ID, string remarks)
        {
            ...
            {
                txtSubTotal.Open(LineNo, TransactionCategoryID, AccountCode, (isCredit ? 0 : Amount)/ExRate, (isCredit ? Amount : 0)/ExRate, SubAcct1_ID, SubAcct2_ID, remarks);
            }
