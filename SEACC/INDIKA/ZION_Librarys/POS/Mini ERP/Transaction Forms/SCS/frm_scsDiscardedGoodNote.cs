                                        ...
                                        if (msgResult == DialogResult.Yes)
                                        {
                                            ...
                                            foreach (tbl_scsDiscardedGoodNote_Detail Olddetail in tbl_scsDiscardedGoodNote_Detail.SelectAllByDiscardedGoodNote_ID(txtDINID.Text.Trim()))
                                            {
                                                if (Olddetail.Item_ID != null)
                                                {
                                                    ...
                                                    clsHelpMethods.UpdateStoreStock(iFormID, Olddetail.DiscardedGoodNote_ID, detail.DiscardedGoodNoteDate, Olddetail.Item_ID, "0", txtStoreID.Tag.ToString(), Olddetail.DiscardingQty, Olddetail.DiscardingWeight, Olddetail.SalvageValue, true, false, true, ref dWeightedAverageCostPrice);
                                                                     ...
                                                                     clsSecurity.UserIDLoged, iFormID)))
                                                            {
                                                                if (clsValidate.CheckValidity_TransactionCodeLength(txtDINID.Text))
                                                                {
                                                                                ...
                                                                                txtDINID.Text.Trim()))
                                                                    {
                                                                        ...
                                                                        //  clsHelpMethods.UpdateFifo_Stock(iFormID, DGN.DiscardedGoodNote_ID, DGN.DiscardedGoodNoteDate, DGN.Store_ID, oUpdatedRecord.Item_ID, oUpdatedRecord.ItemSerialNo, oUpdatedRecord.DiscardingQty, 0, false, dCostFifo);
                                                ...
                                                else
                                                {
                                                    ...
                                                    if (clsValidate.CheckValidity_TransactionCodeLength(txtDINID.Text))// if (txtDINID.Text.Trim().Length > 0)
                                                    {
                                                        ...
                                                        foreach (DataGridViewRow row in dgvDetail.Rows)
                                                        {
                                                            ...
                                                            // decimal dCostFifo = clsHelpMethods.GetFifoCost(sItemCode , dDamagedQty);

                                                            tbl_scsDiscardedGoodNote_Detail DINdetail = new tbl_scsDiscardedGoodNote_Detail(clsHelpMethods.GetMaxzimumLineNoDiscardedGoodNote(txtDINID.Text.Trim()), txtDINID.Text.Trim(), sItemCode, sItemSubCategoryID1,
                                                            ...
                                                            #region Update Store Stock
                                                            //     clsHelpMethods.UpdateStoreStock(sItemCode, txtStoreID.Tag.ToString(), dDiscardingQty, dDiscardingWeight, false, false);
                                                            //  clsHelpMethods.UpdateFifo_Stock(iFormID, DIN.DiscardedGoodNote_ID, DIN.DiscardedGoodNoteDate, DIN.Store_ID, DINdetail.Item_ID, DINdetail.ItemSerialNo, DINdetail.DiscardingQty, 0, false, dCostFifo);

                                                            decimal dWeightedAverageCostPrice = 0;
                                                            decimal dCostFifo = clsHelpMethods.UpdateStoreStock(iFormID, DIN.DiscardedGoodNote_ID, DIN.DiscardedGoodNoteDate, DINdetail.Item_ID, "0", txtStoreID.Tag.ToString(), DINdetail.DiscardingQty, DINdetail.DiscardingWeight, DINdetail.SalvageValue, false, false, true, ref dWeightedAverageCostPrice);
                    ...
                    if (bPermissinOkToPrint)
                    {
                        ...
                        if (oDisGN != null)
                        {
                            ...
                            if (bApprovalDone && bCheckingDone)
                            {
                                ...
                                //glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("IsDel", isDel, true,false);
                                ...
                                string sGetRptPath = clsHelpMethods.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_DiscardedItemNote));
