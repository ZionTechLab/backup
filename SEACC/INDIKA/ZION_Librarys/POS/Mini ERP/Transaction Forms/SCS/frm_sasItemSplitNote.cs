    ...
    public partial class frm_sasItemSpradeNote : SEACC_Form
    {
                            ...
                            if (!oldRecord.IsLocked && !oldRecord.IsApproved && !oldRecord.IsFinished && !oldRecord.IsDeleted && clsValidate.CheckPrintingValidity(oldRecord.PrintCount))
                            {
                                if (clsValidate.CheckValidity_TransactionCodeLength(txtSpradeCode.Text)) //if (txtSpradeCode.TextLength > 0)
                                {
                                    ...
                                    foreach (tbl_scsItemSpred_Detail_From oUpdatedRecore in tbl_scsItemSpred_Detail_From.SelectAllByItemSpred_ID(txtSpradeCode.Text.Trim()))
                                    {
                                        ...
                                        clsHelpMethods.UpdateStoreStock(iFormID, oUpdatedRecore.ItemSpred_ID, oldRecord.ItemSpredDate, oUpdatedRecore.Item_ID, "0", txtFromStore.Tag.ToString(), oUpdatedRecore.Qty, oUpdatedRecore.Weight, 0, true, false, false, ref dWeightedAverageCostPrice);
                                    ...
                                    {
                                        decimal dWeightedAverageCostPrice = 0;
                                        clsHelpMethods.UpdateStoreStock(iFormID, oUpdatedRecore.ItemSpred_ID, oldRecord.ItemSpredDate, oUpdatedRecore.Item_ID, "0", txtFromStore.Tag.ToString(), oUpdatedRecore.Qty, oUpdatedRecore.Weight, 0, true, true, false, ref dWeightedAverageCostPrice);
                                        oUpdatedRecore.WeightedAvgCost = clsProcessMethods.GetItemWeightedAvarageCostPrice(oUpdatedRecore.Item_ID);
                                        oUpdatedRecore.Update();
                                    }
                                    #endregion
                                    ...
                                    {
                                        decimal dWeightedAverageCostPrice = 0;
                                        decimal dCostFifo = clsHelpMethods.UpdateStoreStock(iFormID, ItemSdetail.ItemSpred_ID, ItemSdetail.ItemSpredDate, oUpdatedRecord.Item_ID, "0", txtFromStore.Tag.ToString(), oUpdatedRecord.Qty, oUpdatedRecord.Weight, 0, false, false, false, ref dWeightedAverageCostPrice);
                                    ...
                                    {
                                        decimal dWeightedAverageCostPrice = 0;
                                        clsHelpMethods.UpdateStoreStock(iFormID, ItemSdetail.ItemSpred_ID, ItemSdetail.ItemSpredDate, oUpdatedRecord.Item_ID, "0", clsConfig.bitemSplitNote_ToStoreActive ? txtToStore.Tag.ToString() : txtFromStore.Tag.ToString(), oUpdatedRecord.Qty, oUpdatedRecord.Weight, 0, false, true, false, ref dWeightedAverageCostPrice);                                       
                                    }
                                    #endregion
                                    ...
                                    clsAlerts_Email.createEmail_IS(txtSpradeCode.Text.Trim(), enum_Alerts.ItemSpliteCreate);
                            ...
                            foreach (DataGridViewRow Row in dgvNewItem.Rows)
                            {
                                ...
                                if (sItemID.Length > 0)
                                {
                                    if (bIsInput)
                                    {
                                        ...
                                        #region Update Store Stock
                                        decimal dWeightedAverageCostPrice = 0;
                                        decimal dCostFifo = clsHelpMethods.UpdateStoreStock(iFormID, ItemSdetail.ItemSpred_ID, ItemSdetail.ItemSpredDate, sItemID, "0", txtFromStore.Tag.ToString(), dQtyInput, dWeightInput, 0, false, false, false, ref dWeightedAverageCostPrice);
                                    ...
                                    else
                                    {
                                        ...
                                        #region Update Store Stock
                                        decimal dWeightedAverageCostPrice = 0;
                                        clsHelpMethods.UpdateStoreStock(iFormID, ItemSdetail.ItemSpred_ID, ItemSdetail.ItemSpredDate, sItemID, "0", clsConfig.bitemSplitNote_ToStoreActive ? txtToStore.Tag.ToString() : txtFromStore.Tag.ToString(), dQtyOutput, dWeightInput, 0, false, true, false, ref dWeightedAverageCostPrice);
                                            ...
                                            if (Olddetail.ItemSpred_ID != null)
                                            {
                                                ...
                                                foreach (tbl_scsItemSpred_Detail_From oItemFrom in tbl_scsItemSpred_Detail_From.SelectAllByItemSpred_ID(Olddetail.ItemSpred_ID))
                                                {
                                                    if (oItemFrom != null)
                                                    {
                                                        #region Update Store Stock
                                                        decimal dWeightedAverageCostPrice = 0;
                                                        clsHelpMethods.UpdateStoreStock(iFormID, oItemFrom.ItemSpred_ID, Olddetail.ItemSpredDate, oItemFrom.Item_ID, "0", txtFromStore.Tag.ToString(), oItemFrom.Qty, oItemFrom.Weight, 0, true, false, false, ref dWeightedAverageCostPrice);
                                                ...
                                                foreach (tbl_scsItemSpred_Detail_To oItemTo in tbl_scsItemSpred_Detail_To.SelectAllByItemSpred_ID(Olddetail.ItemSpred_ID))
                                                {
                                                    if (oItemTo != null)
                                                    {
                                                        #region Update Store Stock
                                                        decimal dWeightedAverageCostPrice = 0;
                                                        clsHelpMethods.UpdateStoreStock(iFormID, oItemTo.ItemSpred_ID, Olddetail.ItemSpredDate, oItemTo.Item_ID, "0", clsConfig.bitemSplitNote_ToStoreActive ? txtToStore.Tag.ToString() : txtFromStore.Tag.ToString(), oItemTo.Qty, oItemTo.Weight, 0, true, true, false, ref dWeightedAverageCostPrice);
        ...
        private void txtItemName_DoubleClick(object sender, EventArgs e)
        {
            if (CheckValidity_ForignKey())
            {
                ...
                clsHelpMethods.SearchItemAdvanceStock(ref txtItemName, ref txtCategoryBox, ref txtSerialBox, txtFromStore.Tag.ToString(), "", "");
        ...
        private void txtNewItemName_DoubleClick(object sender, EventArgs e)
        {
            if (CheckValidity_ForignKey())
            {
                TextBox txtCategoryBox = new TextBox();
                TextBox txtSerialBox = new TextBox();
                clsHelpMethods.SearchItemAdvance(ref txtNewItemName, ref txtCategoryBox, ref txtSerialBox);
                ...
                if (txtSpradeCode.TextLength > 0 && txtSpradeCode.Text != "<Auto Generate>")
                {
                    ...
                    if (bPermissinOkToPrint)
                    {
                        ...
                        if (clsConfig.bDataSetActive_SplitNote)
                        {
                            ...
                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("FromStore", clsGenaralName.getName_Store(sFromStore), true,false);
                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("ToStore", clsGenaralName.getName_Store(sToStore), true,false);
                            ...
                            string sGetRptPath = clsHelpMethods.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_ItemSplitNote));
                ...
                foreach (DataRow row in dt_ItemGrouped.Rows)
                {
                    ...
                    if (dQty > 0 || dWeight > 0)
                    {
                        if (!clsHelpMethods.IsNonInventoryItem(sItemCode))
                        {
                            ...
                            {
                                if ((clsConfig.bStockValidateQty_SplitNote || clsConfig.bStockValidateWeight_SplitNote) && !clsHelpMethods.IsNonInventoryItem(sItemCode) && (clsConfig.sSoftwareModel.Trim() != SoftwareModel_Sales.akt.ToString()))
                                {
