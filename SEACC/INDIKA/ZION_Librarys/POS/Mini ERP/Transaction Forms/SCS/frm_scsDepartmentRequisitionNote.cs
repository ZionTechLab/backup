...
namespace Digiteq
{
    public partial class frm_scsDepartmentRequisitionNote : SEACC_Form
    {
        ...
        private void frmCustomerOrder_Load(object sender, EventArgs e)
        {
            ...
            ///clsFormatter.setFormatForm(this, clsHelpMethods.getFormName(iFormID), 7, iFormID);
            ClearFields();
            ...
            try
            {
                ...
                foreach (tbl_pmsPrePlan PrePlan in PrePlans)
                {
                    ...
                    foreach (tbl_pmsPrePlan_SectionPath_InputItem input in inputs)
                    {
                        ...
                        if (item != null)
                        {
                            clsHelpMethods.Fill_StockDatagrid(dgvDetail, iRow, input.Line_No, item.Item_ID, item.Uom_ID, sJob_ID, getSelectAriaID(), getToDepartment(),
        ...
        private void RefreshGridByItem_ID(string sItem_ID)
        {
            try
            {
                ...
                if (detail != null)
                {
                    ...
                    clsHelpMethods.Fill_StockDatagrid(dgvDetail, iRow, dgvDetail.Rows.Count, detail.Item_ID, detail.Uom_ID, sJobID, getSelectAriaID(), getToDepartment(),
