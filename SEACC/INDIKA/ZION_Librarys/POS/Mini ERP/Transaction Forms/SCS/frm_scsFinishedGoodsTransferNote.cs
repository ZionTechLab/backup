...
namespace Digiteq
{
    public partial class frm_scsStoreProduction : SEACC_Form
    {
        ...
        {
            //sFormConfigCode = clsAutocode.getFormConfigCode(FormName.scsStoreProduction);
            //iFormID = clsSecurity.getFormID(FormName.scsStoreProduction);
                                    ...
                                    if (!detail.IsDeleted)
                                    {
                                        ...
                                        if (msgResult == DialogResult.Yes)
                                        {
                                            foreach (tbl_scsStoreProduction_Detail oItem in tbl_scsStoreProduction_Detail.SelectAllByStoreProduction_ID(detail.StoreProduction_ID).Where(p => p.Item_ID != "default"))
                                            {
                                                ...
                                                clsHelpMethods.UpdateStoreStock(iFormID, oItem.StoreProduction_ID, detail.StoreProductionDate, oItem.Item_ID, "0", txtStoreName.Tag.ToString(), oItem.Qty, oItem.Weight, oItem.TotalAmount, true, true, true, ref dWeightedAverageCostPrice);
                                                oItem.WeightedAvgCost = dWeightedAverageCostPrice;
                                                oItem.Update();
                                                //    clsHelpMethods_Local.RollBackFifo_Stock(iFormID, oItem.StoreProduction_ID, oItem.Qty);
                                                #endregion
                                            }
                                            ...
                                            clsHelpMethods.Delete_Inventory(iFormID, 0, txtProductID.Text.Trim());
                        ...
                        if (clsSecurity.permissionToSave_Store(clsSecurity.UserIDLoged, txtStoreName.Tag.ToString(), IsUpdate))
                        {
                            try
                            {
                                        ...
                                        if (oldRecord != null)
                                        {
                                                ...
                                                clsValidate.CheckPrintingValidity(oldRecord.PrintCount))
                                            {
                                                if (clsValidate.CheckValidity_TransactionCodeLength(txtProductID.Text))
                                                {
                                                    ...
                                                    {
                                                        // clsHelpMethods.UpdateStoreStock(oUpdatedRecord.Item_ID, txtStoreName.Tag.ToString(), oUpdatedRecord.Qty, oUpdatedRecord.Weight, true, true);
                                                        // clsHelpMethods_Local.RollBackFifo_Stock(iFormID, oUpdatedRecord.StoreProduction_ID, oUpdatedRecord.Qty);
                                                        decimal dWeightedAverageCostPrice = 0;
                                                    ...
                                                    foreach (tbl_scsStoreProduction_Detail oldCoDetail in oldCoDetails)
                                                    {
                                                        ...
                                                        if (bHasItemInDB)
                                                        {
                                                            ...
                                                            if (bConfigurationOK)
                                                            {
                                                                //    clsHelpMethods.UpdateOrInsertStoreStock(true, true, itemCode, ItemCategory, ItemSubCategory2_ID, "0", "0", "default", txtStoreName.Tag.ToString(), Qty, Weight, oldCoDetail.Qty, oldCoDetail.Weight, true, true, true);
                                                            }
                                                        ...
                                                        else
                                                        {
                                                            ...
                                                            if (bConfigurationOK)
                                                            {
                                                                //      clsHelpMethods.UpdateOrInsertStoreStock(true, true, oldCoDetail.Item_ID, oldCoDetail.ItemSubCategory_ID, oldCoDetail.ItemSubCategory2_ID, oldCoDetail.ItemSerialNo, oldCoDetail.ItemSerialNo2, "default", txtStoreName.Tag.ToString(), oldCoDetail.Qty, oldCoDetail.Weight, 0, 0, false, false, true);
                                                            }
                                                    ...
                                                    foreach (DataGridViewRow row in dgvDetail.Rows)
                                                    {
                                                        ...
                                                        if (itemCode.Length > 0)
                                                        {
                                                            ...
                                                            if (bConfigurationOK)
                                                            {
                                                                //     clsHelpMethods.UpdateOrInsertStoreStock(true, true, itemCode, ItemCategory, ItemSubCategory2_ID, "0", "0", "default", txtStoreName.Tag.ToString(), Qty, Weight, 0, 0, false, true, true);
                                                            }
                                                            ...
                                                            txtProductID.Tag.ToString().Trim()))
                                                    {
                                                        ...
                                                        //   clsHelpMethods.UpdateFifo_Stock(iFormID, SProduct.StoreProduction_ID, SProduct.StoreProductionDate, SProduct.Store_ID, oUpdatedRecord.Item_ID, oUpdatedRecord.ItemSerialNo, oUpdatedRecord.Qty, oUpdatedRecord.UnitPrice, true, dCostFifo);
                                                        oUpdatedRecord.WeightedAvgCost = dWeightedAverageCostPrice;
                                                        oUpdatedRecord.Update();
                                                    ...
                                                    //Attachments.Insert(iFormID, oldRecord.StoreProduction_ID);
                                                    //Attachments.Remove(iFormID, oldRecord.StoreProduction_ID);
                                ...
                                else
                                {
                                        ...
                                        if (oSP == null)
                                        {
                                            ...
                                            foreach (DataGridViewRow row in dgvDetail.Rows)
                                            {
                                                ...
                                                if (itemCode.Length > 0)
                                                {
                                                    ...
                                                    // decimal dCostFifo = clsHelpMethods.GetFifoCost(itemCode , Qty);
                                                    //  clsHelpMethods.UpdateStoreStock(itemCode, txtStoreName.Tag.ToString(), Qty, Weight, false, true);
                                                    // clsHelpMethods.UpdateFifo_Stock(iFormID, SPdetail.StoreProduction_ID, SPdetail.StoreProductionDate, SPdetail.Store_ID, detail.Item_ID, detail.ItemSerialNo, detail.Qty, detail.UnitPrice, true, dCostFifo);
                                                    decimal dWeightedAverageCostPrice = 0;
                                                    clsHelpMethods.UpdateStoreStock(iFormID, SPdetail.StoreProduction_ID, SPdetail.StoreProductionDate, detail.Item_ID, "0", txtStoreName.Tag.ToString(), detail.Qty, detail.Weight, detail.TotalAmount, false, true, true, ref dWeightedAverageCostPrice);
                                    ...
                                    //else
                                    // MessageBox.Show("FGTN " + clsFormatter.GetMessageFrom(MessageType.IDIsEmpty), clsFormatter.GetMessageCaption(), MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ...
                    if (bPermissinOkToPrint)
                    {
                        ...
                        if (oOrder != null)
                        {
                            ...
                            if (!bIsDraft)
                            {
                                ...
                                //    sDuplicateCopy = "Duplicate Copy " + oOrder.PrintCount;
                            ...
                            string sGetRptPath = clsHelpMethods.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_FinishedGoodsTransferNote));
        ...
        private void RefreshGridByItem(string sItemID, string sItemSubCategoryID, string sItemSubCategoryID2, string sSerialNo, string sSerialNo2)
        {
            try
            {
                ...
                if (detail != null)
                {
                    ...
                    decimal dUnitPrice = clsProcessMethods.GetRecommendedUnitPrice_Basic(detail.Item_ID, sItemPriceCategory);
                ...
                else if (e.KeyCode == Keys.F5)
                {
                    ...
                    if (frm.lstclsTmpMultipleSelectedItems.Count > 0)
                    {
                        foreach (clsTmpMultipleSelectedItems oItem in frm.lstclsTmpMultipleSelectedItems)
                        {
                            ...
                            Fill_Datagrid(iRow, oItem.sItemID, clsGenaralName.getName_Item(oItem.sItemID), oItem.sItemSubCategoryID, oItem.sItemSubCategoryID2, oItem.sItemSerialNo, oItem.sItemSerialNo2, oItem.sUOMID, oItem.dQty, oItem.dWeight, 0, 0, "", oItem.dUnitPrice, oItem.dTotalAmount);
                        }
                    }
                }
            }
