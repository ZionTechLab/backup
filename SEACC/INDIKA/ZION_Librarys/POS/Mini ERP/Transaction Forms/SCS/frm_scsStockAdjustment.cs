...
namespace Digiteq
{
    public partial class frm_scsStockAdjustment : SEACC_Form
    {
        ...
        //public bool bHasApproved = false;
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (clsValidate.CheckValidity_TransactionCodeLength(txtSANID.Text)) //if (txtSANID.TextLength > 0)
                        {
                            ...
                            foreach (DataGridViewRow row in dgvDetail.Rows)
                            {
                                try
                                {
                                    ...
                                    if (sItemCode.Length > 0)
                                    {
                                        ...
                                        //bool bIsStockIn = (dQuantity < 0) ? false : true;
                                        decimal dWeightedAverageCostPrice = 0;
                                        clsHelpMethods.UpdateStoreStock(iFormID, detail.StockAdjustment_ID, detail.StockAdjustmentDate, sItemCode, "0", txtStoreID.Tag.ToString(), dQuantity, dWeight, dQuantity * dUnitPrice, false, true, true, ref dWeightedAverageCostPrice);
                                        STAdetail.WeightedAvgCost = dWeightedAverageCostPrice;
                                        STAdetail.Update();
                                        #endregion
                            ...
                            clsAlerts_Email.createEmail_SAN(txtSANID.Text.Trim(), enum_Alerts.StockAdjustmentCreate);
                                    ...
                                    if (CheckCancelValidity_WATollarance())
                                    {
                                        ...
                                        foreach (tbl_scsStockAdjustment_Detail oUpdatedRecord in tbl_scsStockAdjustment_Detail.SelectAllByStockAdjustment_ID(txtSANID.Text.Trim()))
                                        {
                                            if (oUpdatedRecord.Item_ID != null)
                                            {
                                                ...
                                                decimal dWeightedAverageCostPrice = 0;
                                                clsHelpMethods.UpdateStoreStock(iFormID, oUpdatedRecord.StockAdjustment_ID, detail.StockAdjustmentDate, oUpdatedRecord.Item_ID, "0", txtStoreID.Tag.ToString(), oUpdatedRecord.Qty, oUpdatedRecord.Weight, oUpdatedRecord.Qty * oUpdatedRecord.UnitPrice, true, true, true, ref dWeightedAverageCostPrice);
                                                oUpdatedRecord.WeightedAvgCost = dWeightedAverageCostPrice;
                                                oUpdatedRecord.Update();
                                                //   clsHelpMethods.UpdateStoreStock(iFormID, oUpdatedRecord.StockAdjustment_ID, detail.StockAdjustmentDate, oUpdatedRecord.Item_ID, "0", txtStoreID.Tag.ToString(), dNewQty, dNewWeight, dNewQty * oUpdatedRecord.UnitPrice, (oUpdatedRecord.Qty < 0) ? false : true, true, true);
                                            }
                                        }
                                        #endregion
        ...
        {
            tbl_scsStockAdjustment detail = tbl_scsStockAdjustment.Select(txtSANID.Text.Trim());
        ...
        private void txtInputMaterialID_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F1)
                ...
                clsHelpMethods.SearchItemAdvanceByKeyPress(ref txtInputMaterialID, ref txtItemSubCategory, ref txtItemSerialNo, e);
        }
                    ...
                    else if (e.KeyCode == Keys.F5)
                    {
                        ...
                        if (frm.lstclsTmpMultipleSelectedItems.Count > 0)
                        {
                            foreach (clsTmpMultipleSelectedItems oItem in frm.lstclsTmpMultipleSelectedItems)
                            {
                                dgvDetail.Rows.Add();
                                int iRow = dgvDetail.Rows.Count - 1;
                                string sToLocation = clsHelpMethods.getToLocationName(txtStoreID, txtStoreID, txtStoreID);
                                string sNoteID = "N/A";
            ...
            try
            {
                ...
                if (txtSANID.TextLength > 0 && txtSANID.Text != "<Auto Generate>")
                {
                    ...
                    if (bPermissinOkToPrint)
                    {
                        ...
                        string sGetRptPath = clsHelpMethods.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_StockAdjustment));
                        string s_Path = "";
