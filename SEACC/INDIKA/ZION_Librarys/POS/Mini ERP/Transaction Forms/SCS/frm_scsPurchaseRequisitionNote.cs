    ...
    public partial class frm_scsPurchaseRequisitionNote : SEACC_Form
    {
            ...
            try
            {
                if (txtPurchaseRequisitionNoteID.TextLength > 0 && txtPurchaseRequisitionNoteID.Text.Trim() != "default")
                {
                    ...
                    if (detail != null && detail.PurchaseRequisitionNote_ID != "default" && !detail.IsDeleted)
                    {
                        ...
                        string message = "";
                        if (true)//clsConfig.bApprovalEnabledCustomerOrder
                        {
           ...
           string sSerial1, string sSerial2, string sItemStatus, string sRemarks)
        {
            try
            {
                ...
                clsHelpMethods.AddMultipleItems_Grid(dgvDetail, sItemID, ref iRow, ref iLineNo, ref dQuantity, ref UnitPrice, ref dWeight, ref WeightAvg);
                ...
                dgvDetail["Note_ID", iRow].Value = clsFormatter.FormatDecimalPlaces_Quantity(clsHelpMethods.GetQty_MinimumPR(sItemID, sItemSubCategory1, sItemSubCategory2, sSerial1, sSerial2));  //clsCommon.GetForeignKeyValue(sNoteID);
        ...
        private void RefreshGrid(string sSRNID)
        {
            try
            {
                ...
                foreach (tbl_scsPurchaseRequisition_Detail detail in details)
                {
                    ...
                    string sToLocation = clsHelpMethods.GetToLocationName(detail.FromSelectArea_ID, detail.FromDepartment_ID, detail.FromSection_ID, detail.FromStore_ID);
                    string sNoteID = clsHelpMethods.GetSelectAreaNoteID(detail.FromSelectArea_ID, detail.DepartmentReqositionNote_ID, detail.SectionReqositionNote_ID, detail.StoreRecositionNote_ID);
            ...
            try
            {
                ...
                foreach (tbl_pmsPrePlan PrePlan in PrePlans)
                {
                    ...
                    foreach (tbl_pmsPrePlan_SectionPath_InputItem input in inputs)
                    {
                        ...
                        iRow = dgvDetail.Rows.Count - 1;
                        ValidateEmptyForeignKey();
                        string sFromLocation = clsHelpMethods.getToLocationName(txtDepartmentID, txtSectionID, txtStoreID);
                        string sNoteID = "N/A";
            ...
            try
            {
                if (txtPurchaseRequisitionNoteID.Text.Trim().Length > 0 && txtPurchaseRequisitionNoteID.Text.Trim() != "<Auto Generate>")
                {
                    try
                    {
                        ...
                        if (clsHelpMethods.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_PurchaseRequisitionNote), ref sReportTitle_Main, ref sReportTitle_Sub, ref sReportPath))
                        {
                            ...
                            if (bPermissinOkToPrint)
                            {
                                ...
                                if (oPurchaseReq != null)
                                {
                                    ...
                                    glb_dtsScsPurchaseRequisitionNote.dt_PurchaseRequisitionNote.Adddt_PurchaseRequisitionNoteRow(oPurchaseReq.PurchaseRequisitionNote_ID,
                                          ...
                                          oPurchaseReq.RequestedBy, oPurchaseReq.DateCreate, oPurchaseReq.IsDeleted, 0, oPurchaseReq.StockNoteType_ID, clsGenaralName.getName_StockNoteType(oPurchaseReq.StockNoteType_ID));
                                    ...
                                    {
                                        tbl_genItemMaster oItem = tbl_genItemMaster.Select(oDetails_PR.Item_ID);
                                        ...
                                        glb_dtsScsPurchaseRequisitionNote.dt_PurchaseRequisitionNoteDetail.Adddt_PurchaseRequisitionNoteDetailRow(oDetails_PR.PurchaseRequisitionNote_ID, oDetails_PR.Item_ID,
                                            ...
                                            clsGenaralName.getName_Uom(oDetails_PR.Uom_ID), oDetails_PR.Remark, oItem.ReOrderQty);
                                    }
                                }

                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CreatedUser", sCreateUser, true, false);
                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CheckedUser", sCheckedUser, true, false);
                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("ApprovedUser", sApprovedUser, true, false);
