                                                ...
                                                if (CheckStockValidity(detail.DamagedGoodNote_ID))
                                                {
                                                    ...
                                                    foreach (tbl_scsDamagedGoodNote_Detail Olddetail in tbl_scsDamagedGoodNote_Detail.SelectAllByDamagedGoodNote_ID(txtDGNID.Text.Trim()))
                                                    {
                                                        if (Olddetail.Item_ID != null)
                                                        {
                                                            ...
                                                            clsHelpMethods.UpdateStoreStock(iFormID, Olddetail.DamagedGoodNote_ID, detail.DamagedGoodNoteDate, Olddetail.Item_ID, "0", txtStoreID.Tag.ToString(), Olddetail.Qty, Olddetail.Weight, Olddetail.UnitPrice, true, false, false, ref dWeightedAverageCostPrice);
                                                            clsHelpMethods.UpdateStoreStock(iFormID, Olddetail.DamagedGoodNote_ID, detail.DamagedGoodNoteDate, Olddetail.Item_ID, "0", txtDamageGoodStore.Tag.ToString(), Olddetail.Qty, Olddetail.Weight, Olddetail.UnitPrice, true, true, false, ref dWeightedAverageCostPrice);
                                                                ...
                                                                if (clsValidate.CheckValidity_TransactionCodeLength(txtDGNID.Text))
                                                                {
                                                                        ...
                                                                        oldDetails)
                                                                    {
                                                                        ...
                                                                        else
                                                                        {
                                                                            ...
                                                                            //if (clsHelpMethods.isStore_StockAvailabel(oldDetail.Store_ID, oldDetail.Item_ID, sJobCode, oldDetail.ItemSubCategory_ID, oldDetail.ItemSubCategory2_ID, oldDetail.ItemSerialNo, oldDetail.ItemSerialNo2))
                                                                            //{
                                                                    ...
                                                                    foreach (DataGridViewRow row in dgvDetail.Rows)
                                                                    {
                                                                        ...
                                                                        //if (clsHelpMethods_Local.Store_StockQuantityDecrease(sStoreID, sItemCode, sJobCode, sItemSubCategoryID1, sItemSubCategoryID2, sItemSerialNo1, sItemSerialNo2, dQty))
                                                                        //{
                                                                        //    if (clsHelpMethods.isStore_StockAvailabel(txtDamageGoodStore.Tag.ToString(), sItemCode, sJobCode, sItemSubCategoryID1, sItemSubCategoryID2, sItemSerialNo1, sItemSerialNo2))
                                                                        //        clsHelpMethods_Local.Store_StockQuantityIncrease(txtDamageGoodStore.Tag.ToString(), sItemCode, sJobCode, sItemSubCategoryID1, sItemSubCategoryID2, sItemSerialNo1, sItemSerialNo2, dQty);
                                                                        //    else
                                                ...
                                                else
                                                {
                                                    ...
                                                    if (clsValidate.CheckValidity_TransactionCodeLength(txtDGNID.Text))// if (txtDGNID.Text.Trim().Length > 0)
                                                    {
                                                        ...
                                                        foreach (DataGridViewRow row in dgvDetail.Rows)
                                                        {
                                                            ...
                                                            tbl_scsDamagedGoodNote_Detail DGNDetail = new tbl_scsDamagedGoodNote_Detail(clsHelpMethods.GetMaxzimumLineNoDameagedGoodsNote(txtDGNID.Text.Trim()), txtDGNID.Text.Trim(),
                                                            ...
                                                            decimal dWeightedAverageCostPrice = 0;
                                                            clsHelpMethods.UpdateStoreStock(iFormID, DGN.DamagedGoodNote_ID, DGN.DamagedGoodNoteDate, sItemCode, "0", txtStoreID.Tag.ToString(), dQty, dWeight, dUnitPrice, false, false, false, ref dWeightedAverageCostPrice);
                                                            clsHelpMethods.UpdateStoreStock(iFormID, DGN.DamagedGoodNote_ID, DGN.DamagedGoodNoteDate, sItemCode, "0", txtDamageGoodStore.Tag.ToString(), dQty, dWeight, dUnitPrice, false, true, false, ref dWeightedAverageCostPrice);
            ...
            try
            {
                ...
                foreach (DataRow row in dt_ItemGrouped.Rows)
                {
                    ...
                    if (!clsHelpMethods.IsNonInventoryItem(sItemCode))
                    {
                        ...
                        {
                            if ((clsConfig.bStockValidateQty_DamageGood || clsConfig.bStockValidateWeight_DamageGood) && !clsHelpMethods.IsNonInventoryItem(sItemCode) && (clsConfig.sSoftwareModel.Trim() != SoftwareModel_Sales.akt.ToString()))
                            {
                ...
                if (txtDGNID.TextLength > 0 && txtDGNID.Text != "<Auto Generate>")
                {
                    ...
                    if (!clsConfig.bDataSetActive_DamageGood)
                    {
                        ...
                        try
                        {
                            ...
                            if (txtDGNID.TextLength > 0 && txtDGNID.Text != "<Auto Generate>")
                            {
                                ...
                                string sGetRptPath = clsHelpMethods.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_DamagedGoodsNote));
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            if (clsHelpMethods.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_DamagedGoodsNote), ref sReportTitle_Main, ref sReportTitle_Sub, ref sReportPath))
                            {
                                ...
                                if (bPermissinOkToPrint)
                                {
                                    ...
                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("IsDraft", sDraft, true,false);
                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("DupicateCopy", sDuplicate, true,false);
                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("isDel", sDeleted, true,false);

                                    //if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.ePackWithSubCategory.ToString())// && !chkPrintWithAmounts.Checked && !chkPrintWithBreakdown.Checked)
                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("SubCategory", clsConfig.sItemSubCategory, true,false);
                                    //else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.idealWheels.ToString())// && !chkPrintWithAmounts.Checked && !chkPrintWithBreakdown.Checked)
                                    //    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("SubCategory", sSubCat, true,false);

                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("Damagedstore", clsGenaralName.getName_Store(txtDamageGoodStore.Tag.ToString()), true,false);
