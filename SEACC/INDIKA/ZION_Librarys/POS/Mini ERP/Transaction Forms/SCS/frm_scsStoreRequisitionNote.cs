    ...
    public partial class frm_scsStoreRequisitionNote : SEACC_Form
    {
        ...
        private void RefreshGridByJob_ID(string sJob_ID)
        {
            ...
            foreach (tbl_pmsPrePlan PrePlan in PrePlans)
            {
                ...
                foreach (tbl_pmsPrePlan_SectionPath_InputItem input in inputs)
                {
                    ...
                    if (item != null)
                    {
                        clsHelpMethods.Fill_StockDatagrid(dgvDetail, iRow, input.Line_No, item.Item_ID, item.Uom_ID, sJob_ID, getSelectAriaID(), txtDepartmentID.Tag.ToString(),
        ...
        private void RefreshGridByItem_ID(string sItem_ID)
        {
            ...
            if (detail != null)
            {
                ...
                clsHelpMethods.Fill_StockDatagrid(dgvDetail, iRow, maxLineNo + 1, detail.Item_ID, detail.Uom_ID, sJobID, getSelectAriaID(), txtDepartmentID.Tag.ToString(),
                ...
                if (txtStoreRequisitionNoteID.Text.Trim().Length > 0 && txtStoreRequisitionNoteID.Text.Trim() != "<Auto Generate>")
                {
                    ...
                    if (bPermissinOkToPrint)
                    {
                        ...
                        if (oStore != null)
                        {
                            ...
                            {
                                string sGetRptPath = clsHelpMethods.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_iSRN));
                                if (sGetRptPath != null)
                                {
                ...
                else if (e.KeyCode == Keys.F5)
                {
                    ...
                    if (frm.lstclsTmpMultipleSelectedItems.Count > 0)
                    {
                        foreach (clsTmpMultipleSelectedItems oItem in frm.lstclsTmpMultipleSelectedItems)
                        {
                            ...
                            clsHelpMethods.Fill_StockDatagrid(dgvDetail, iRow, dgvDetail.Rows.Count, oItem.sItemID, oItem.sUOMID, txtjobID.Text.Trim(), getSelectAriaID(), txtDepartmentID.Tag.ToString(), 
