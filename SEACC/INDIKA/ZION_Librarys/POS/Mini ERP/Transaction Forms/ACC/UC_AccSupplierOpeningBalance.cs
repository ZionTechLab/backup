...
namespace Digiteq
{
    public partial class UC_AccSupplierOpeningBalance : SEACC_Form
    {
        #region Variables
        ...
        #region Form Load
        public UC_AccSupplierOpeningBalance(FormName _enmForm)
        {
            enmForm = _enmForm;
            InitializeComponent();
            Initialize();
        ...
        private void UC_AccSupplierOpeningBalance_Load(object sender, EventArgs e)
        {
            ClearFields();
                ...
                if (clsSecurity.PermissionToSave(clsSecurity.UserIDLoged, iFormID, IsUpdate))
                {
                    try
                    {
                        ...
                        foreach (DataGridViewRow row in dgvDetail.Rows)
                        {
                            ...
                            string sAcctCode = "", sSupplierID = "";

                            sAcctCode = clsValidate.ValidateGridValue(dgvDetail, "AccCode", row.Index, "");
                            sSupplierID = clsValidate.ValidateGridValue(dgvDetail, "SupplierID", row.Index, "");
        ...
        private void RefreshGrid()
        {
            try
            {
                ...
                foreach (tbl_accGLMaster_Supplier oSupplier in tbl_accGLMaster_Supplier.SelectAll().OrderBy(p => p.Supplier_ID))
                {
                    dt.Rows.Add(oSupplier.Gl_ID, clsGenaralName.getName_AccountName(oSupplier.Gl_ID), oSupplier.Supplier_ID, clsGenaralName.getName_Supplier(oSupplier.Supplier_ID), 0);
                }
