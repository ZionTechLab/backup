                ...
                if (txtAccountReceiptID.Text.Trim().Length > 0 && txtAccountReceiptID.Text.Trim() != "<Auto Generate>")
                {
                    ...
                    if (bPermissinOkToPrint)
                    {
                        ...
                        if (bOkToPrint && bApprovalDone)
                        {
                            ...
                            string s_Path = "";
                            string sGetRptPath = clsHelpMethods.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_ReceiptVoucher));//NP_ReceiptVoucher
                            if (sGetRptPath != null && sGetRptPath.Length > 0)
                                s_Path = sGetRptPath;
                            else
                                s_Path = "\\Reports\\ACC\\NotePrinting\\rpt_accAccountReceipt.rpt";

                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("ReportTitle", "ACCOUNT RECEIPT", true,false);
                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CreateUserName", sCreateUser, true,false);
                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CreateUserID", sCreateUserID, true,false);
                                                ...
                                                if (msgResult == DialogResult.Yes)
                                                {
                                                    ...
                                                    foreach (tbl_bpsChequeRegister oCheque in oCheques)
                                                    {
                                                        if (!oCheque.IsLocked)
                                                        {
                                                            ...
                                                            clsHelpMethods.RemoveSattlementsFrom_ChequeID(oCheque.ChequeRegister_ID);
