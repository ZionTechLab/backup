                                    ...
                                    if (clsValidate.CheckValidity_TransactionCodeLength(txtAPNID.Text))
                                    {
                                        ...
                                        if (txtGRN.Text.Trim() != "" && txtGRN.Text.Trim() != "default")
                                            clsProcessMethods.SetSettle_GRN_From_APN(txtGRN.Tag.ToString().Trim(),
                                                clsHelpMethods.getDisplayPrice(
                        ...
                        if (bApprovalDone && bCheckingDone)
                        {
                            ...
                            if (bOkToPrint && bApprovalDone)
                            {
                                ...
                                if (bisDataset)
                                {
                                    ...
                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CreateUserName", clsCommon.fncsetstring(sCreateUser), true,false);
                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CheckUserName", clsCommon.fncsetstring(sCheckedUser), true,false);
                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("ApproveUserName", clsCommon.fncsetstring(sApprovedUser), true,false);

                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("IDTitle", "APN No", true,false);
                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("IDDateTitle", "APN Date", true,false);
                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("NumberID", "GRN No", true,false);
                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("NumberDate", "GRN Date", true,false);
        ...
        private void FillTaxDetailByPO(string sPurchaseOrderID)
        {
            try
            {
                ...
                if (detail != null)
                {
                    ...
                    //txtSubTotal.Text = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.getDisplayPrice(detail.SubTotal-detail.DiscountTotal, detail.ForexRate));
                    //txtSubTotal.Tag = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.getDisplayPrice(detail.SubTotal - detail.DiscountTotal, detail.ForexRate));
        ...
        private void FillTaxDetailByGRN(string sGRNID)
        {
            try
            {
                ...
                if (detail != null)
                {
                    ...
                    //txtSubTotal.Text = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.getDisplayPrice((detail.SubTotal - detail.DiscountTotal), detail.CurrencyRate));
                    //txtSubTotal.Tag = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.getDisplayPrice(detail.SubTotal - detail.DiscountTotal, detail.CurrencyRate));
                                ...
                                if (CheckValiditySettleAmmount())
                                {
                                    if (clsMethods_GL.CheckValidity_FinancialYear(dtpAPNDate.Value.Date))
                                    {
                                        ...
                                        if (clsFormatter.FormatDecimalPlaces_Price(decimal.Parse(txtDebitAmount.Text)) == clsFormatter.FormatDecimalPlaces_Price(
                                                                                                                                         clsHelpMethods.getSavePrice(decimal.Parse(txtGrandTotal.Text.Trim()), txtCurrencyRate)))
                                        {
                                            bIsOk = true;
                                        }
                                        else
        ...
        private bool CheckValiditySettleAmmount()
        {
            ...
            try
            {
                ...
                if (txtGRN.Tag != null && txtGRN.Tag.ToString().Trim().Length > 0 && txtGRN.Tag.ToString().Trim() != "default")
                    bSettoffOk = clsProcessMethods.Check_ARNTotal_With_GRNTotal(txtGRN.Text.ToString().Trim(), clsHelpMethods.getDisplayPrice(dSettleAmount - dOldGrandTotal, dExRate));//dSettleAmount - dOldGrandTotal is it OK
                else if (txtPONo.Tag != null && txtPONo.Tag.ToString().Trim().Length > 0 && txtPONo.Tag.ToString().Trim() != "default")
                    bSettoffOk = clsProcessMethods.Check_ARNTotal_With_POTotal(txtPONo.Text.ToString().Trim(), clsHelpMethods.getDisplayPrice(dSettleAmount - dOldGrandTotal, dExRate));
                else
                    bSettoffOk = true;
