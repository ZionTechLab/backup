                                ...
                                if (!bIsReconciled)
                                {
                                    if (!detail.IsLocked)
                                    {
                                        if (!detail.IsDeleted)
                                        {
                                            ...
                                            {
                                                if (clsHelpMethods.RemoveSattlementsFrom_JournalEntryID(detail.JournalEntry_ID))
                                                {
        ...
        private void Print(bool bIsDraft)
        {
            try
            {
                if (txtJournalID.Text.Trim().Length > 0 && txtJournalID.Text.Trim() != "<Auto Generate>")
                {
                    ...
                    if (clsHelpMethods.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_JournalVoucher), ref sReportTitle_Main, ref sReportTitle_Sub, ref sReportPath))
                    {
                        ...
                        if (bPermissinOkToPrint)
                        {
                            ...
                            if (oJV != null)
                            {
                                ...
                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CreateUserName", clsGenaralName.getName_User(oJV.CreateUser_ID), true,false);
                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CheckUserName", clsGenaralName.getName_User(oJV.CheckedUser_ID), true,false);
                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("ApproveUserName", clsGenaralName.getName_User(oJV.ApprovedUser_ID), true,false);
                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("isDel", oJV.IsDeleted ? "CANCELLED" : "", true,false);
