    ...
    public partial class frm_sasInquiry : SEACC_Form
    {
                                            ...
                                            if (clsValidate.CheckValidity_TransactionCodeLength(txtInquiryID.Text))
                                            {
                                                ...
                                                foreach (tbl_sasInquiry_Detail oldInq in oldInqDetails)
                                                {
                                                    ...
                                                    if (bHasInqInDB)
                                                    {
                                                        ...
                                                        dUnitPrice =
                                                            clsHelpMethods.getSavePrice(dUnitPrice,
                                                                txtCurrencyRate);
                                                        dWeightPrice =
                                                ...
                                                foreach (DataGridViewRow row in dgvDetail.Rows)
                                                {
                                                    ...
                                                    if (sItemCode.Length > 0)
                                                    {
                                                        tbl_sasInquiry_Detail items = new tbl_sasInquiry_Detail(
                                                            clsHelpMethods.GetMaxzimumLineNo_Inquiry(
                    ...
                    else  //insert records
                    {
                        ...
                        if (clsValidate.CheckValidity_TransactionCodeLength(txtInquiryID.Text)) //if (txtInquiryID.TextLength > 0)
                        {
                            tbl_sasInquiry detail = new tbl_sasInquiry(txtInquiryID.Text.Trim(), dtpInquiryDate.Value, txtRemark.Text.Trim(),
                               ...
                               clsHelpMethods.getSavePrice(decimal.Parse(txtGrandTotal.Text.Trim()), txtCurrencyRate), clsHelpMethods.getSavePrice(decimal.Parse(txtAdvanceAmount.Text.Trim()), txtCurrencyRate), clsHelpMethods.getSavePrice(decimal.Parse(txtSubTotal_Rec.Text.Trim()), txtCurrencyRate),
                               clsHelpMethods.getSavePrice(decimal.Parse(txtGrandTotal_Rec.Text.Trim()), txtCurrencyRate), clsSecurity.UserIDLoged, clsSecurity.UserIDLoged, "default", "default",
                            ...
                            foreach (DataGridViewRow row in dgvDetail.Rows)
                            {
                                try
                                {
                                    ...
                                    if (sItemCode.Length > 0)
                                    {
                                        tbl_sasInquiry_Detail items = new tbl_sasInquiry_Detail(clsHelpMethods.GetMaxzimumLineNo_Inquiry(txtInquiryID.Text.Trim()), txtInquiryID.Text.Trim(), sItemCode,
        ...
        decimal Width, decimal Height, decimal Gauge, decimal Gusset, decimal Weight, decimal Qty, string ItemSubCategoryID, string ItemSubCategoryID2, string SerialNo, string SerialNo2, string Remark, decimal dExRate)
        {
            try
            {
                ...
                TatalAmount = clsHelpMethods.getDisplayPrice(TatalAmount, dExRate);
            ...
            try
            {
                if (txtInquiryID.TextLength > 0 && txtInquiryID.Text != "<Auto Generate>")
                {
                    ...
                    if (true)
                    {
                        ...
                        if (clsHelpMethods.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_SalesInquiry), ref sReportTitle_Main, ref sReportTitle_Sub, ref sReportPath))
                        {
                            ...
                            if (oInquiry != null)
                            {
                                ...
                                if (bApprovalDone && bCheckingDone)
                                {
                                    ...
                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("isDel", oInquiry.IsDeleted ? "CANCELLED" : "", true,false);
                    ...
                    else
                    {
                        ...
                        if (clsConfig.bDirectPrint_NP_Inquiry) //Direct Print
                        {
                            ...
                            clsHelpMethods.SetPrinterSetting(clsAutocode.getReportID(enum_ReportName.NP_SalesInquiry), ref RD);
