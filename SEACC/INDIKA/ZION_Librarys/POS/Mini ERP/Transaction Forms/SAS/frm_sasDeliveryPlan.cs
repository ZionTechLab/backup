...
namespace Digiteq
{
    public partial class frm_sasDeliveryPlan : Form
    {
                                                    ...
                                                    if (detail.IsChecked)
                                                    {
                                                        ...
                                                        foreach (tbl_sasDeliveryPlan_CustomerOrder_Items Dodetail in Dodetails)
                                                        {
                                                            if (Dodetail.Item_ID != null)
                                                            {
                                                                ...
                                                                {
                                                                    if (!clsHelpMethods.IsItemRawMaterial(sOldItemCode))
                                                                    {
                                                                ...
                                                                if (clsHelpMethods.isStore_StockAvailabel(txtSoteID.Tag.ToString(), sOldItemCode, sTmpJobCode, sOldItemSubCategoryID, sOldItemSubCategoryID2, sOldItemSerialNo, sOldItemSerialNo2))
                                                                {
                    ...
                    if (IsValid)
                    {
                        ...
                        for (int x = 0; x < 2; x++)
                        {
                            ...
                            if (clsConfig.bDirectPrint_NP_Invoice) //Direct Print
                            {
                                ...
                                clsHelpMethods.SetPrinterSetting(clsAutocode.getReportID(enum_ReportName.NP_SalesInvoice), ref RD);
                ...
                if (clsSecurity.PermissionToChecked(clsSecurity.UserIDLoged, iFormID))
                {
                    ...
                    {   
                        ...
                        if (bUpdateOk)
                        {
                            ...
                            {
                                if (clsHelpMethods.isStore_StockAvailabel(txtSoteID.Tag.ToString(), Detail.Item_ID, "default", Detail.ItemSubCategory_ID, Detail.ItemSubCategory2_ID, Detail.ItemSerialNo, Detail.ItemSerialNo2))
                                {
        ...
        private void UpdateGridItemStockBalance(string ItemID, string ItemSubCategoryID, string ItemSubCategoryID2, string SerialNo, string SerialNo2)
        {
            ...
            if (txtSoteID.Tag != null)
            {
                dBalanceQty = clsHelpMethods.Get_StoreStockBalance_Qty(txtSoteID.Tag.ToString(), ItemID, "default", ItemSubCategoryID, ItemSubCategoryID2, SerialNo, SerialNo2);
                foreach (DataGridViewRow row in dgvDetail.Rows)
                {
