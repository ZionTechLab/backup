    ...
    public partial class frm_sasDeliveryOrder : SEACC_Form
    {
                                ...
                                if (!oldRecord.IsLocked && !oldRecord.IsApproved && !oldRecord.IsFinished && !oldRecord.IsDeleted)
                                {
                                    if (!oldRecord.IsChecked || (oldRecord.IsChecked && clsSecurity.PermissionToApproved(clsSecurity.UserIDLoged, iFormID)))
                                    {
                                        if (clsValidate.CheckValidity_TransactionCodeLength(txtDeliveryOrderID.Text))
                                        {
                                            ...
                                            {
                                                decimal dWeightedAverageCostPrice = 0;
                                                decimal dCostFifo = clsHelpMethods.UpdateStoreStock(iFormID,
                    ...
                    else
                    {
                        ...
                        if (clsValidate.CheckValidity_TransactionCodeLength(txtDeliveryOrderID.Text)) //if (txtDeliveryOrderID.Text.Trim().Length > 0 && txtDeliveryOrderID.Text != "<Auto Generate>")
                        {
                            ...
                            if (oDO == null)
                            {
                                ...
                                tbl_sasDeliveryOrder detail = new tbl_sasDeliveryOrder(txtDeliveryOrderID.Text.Trim(), dtpDODate.Value, txtRemark.Text.Trim(),
                                    txtAddress.Text.Trim(), txtVehicalNo.Text, clsSecurity.getServerDateTime(), dtpTimeOut.Value, dtpReceivedDate.Value, txtReceiptBy.Text.Trim(), txtCustomerID.Tag.ToString(),
                                    txtCustomerOrderID.Tag.ToString(), txtQuotationID.Tag.ToString(), txtJobCode.Tag.ToString(), txtDriverID.Tag.ToString(), txtVehicleID.Tag.ToString(),
                                    txtAssistantID.Tag.ToString(), txtStoreID.Tag.ToString(), txtSalesExecutiveID.Tag.ToString(), glbOrderRefNo, "default",
                                    txtCurrencyID.Tag.ToString(), "default", clsAutocode.getGLPostingStatusID(GLPostingStatus.NewTransaction), clsSecurity.FinancialYearID, txtSalesNoteType.Tag.ToString(),
                                    ...
                                    clsHelpMethods.getSavePrice(decimal.Parse(txtGrandTotal.Text.Trim()), txtCurrencyRate), decimal.Parse(txtSubTotal_Rec.Text.Trim()), decimal.Parse(txtGrandTotal_Rec.Text.Trim()), clsSecurity.UserIDLoged, clsSecurity.UserIDLoged, "default", "default",
                                    ...
                                    bHasChecked, bHasApproved, false, false, false, false, !chkUnitPricing.Checked, 0, false, chkReverseCalculation.Checked, chkFreeOrder.Checked, clsHelpMethods.isTaxActiveNote(txtVat), clsHelpMethods.isTaxActiveNote(txtOtherTax), txtBatchNo.Text.Trim(), txtCustomerBranchID.Tag.ToString(), chkIsReplasement.Checked, ((ComboBoxItem)cmbItemPrice.SelectedItem).Value, clsSecurity.CompanyID, clsSecurity.BranchID, int.Parse(lblRoute.Tag.ToString()));
                                detail.Insert();
                                #endregion
                                ...
                                foreach (DataGridViewRow row in dgvDetail.Rows)
                                {
                                    try
                                    {
                                        ...
                                        if (sItemCode.Trim().Length > 0)
                                        {
                                            ...
                                            decimal dWeightedAverageCostPrice = 0;
                                            decimal dCostFifo = clsHelpMethods.UpdateStoreStock(iFormID, detail.DeliveryOrder_ID, detail.DeliveryOrderDate, sItemCode, "0", txtStoreID.Tag.ToString(), dQuantity, dWeight, items.TatalAmount, false, false, false, ref dWeightedAverageCostPrice);
                                ...
                                clsAlerts_Email.Email_DO(txtDeliveryOrderID.Text.Trim(), enum_Alerts.DeliveryOrderCreate);
                                        ...
                                        if (!detail.IsLocked)
                                        {
                                            if (!detail.IsDeleted)
                                            {
                                                ...
                                                if (frm.glbValied)
                                                {
                                                    ...
                                                    foreach (tbl_sasDeliveryOrder_Detail Dodetail in tbl_sasDeliveryOrder_Detail.SelectAllByDeliveryOrder_ID(txtDeliveryOrderID.Text.Trim()))
                                                    {
                                                        if (Dodetail.Item_ID != null)
                                                        {
                                                            ...
                                                            #region Update Store Stock
                                                            decimal dWeightedAverageCostPrice = 0;
                                                            clsHelpMethods.UpdateStoreStock(iFormID, Dodetail.DeliveryOrder_ID, detail.DeliveryOrderDate, Dodetail.Item_ID, "0", txtStoreID.Tag.ToString(), Dodetail.Qty, Dodetail.Weight, Dodetail.TatalAmount, true, false, false, ref dWeightedAverageCostPrice);
                                                    ...
                                                    clsAlerts_Email.Email_DO(txtDeliveryOrderID.Text.Trim(), enum_Alerts.DeliveryOrderCancel);
        ...
        private void btnAddCustomerOrder_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    tbl_sasCustomerOrder oCO = tbl_sasCustomerOrder.Select(txtCustomerOrderID.Tag.ToString());
                    if (oCO != null)
                    {
                        tbl_genCustomerMaster oCustomer = tbl_genCustomerMaster.Select(oCO.Customer_ID);
                        if (oCustomer != null && !oCustomer.IsDeleted)
                        {
                            chkUnitPricing.Checked = !oCO.IsWeightCalculation;
                            chkFreeOrder.Checked = oCO.IsFreeOrder;

                            FillDetailsCustomer(oCO.Customer_ID);

                            #region Order Ref
                            tbl_zOrderRefNo orf = tbl_zOrderRefNo.Select(oCO.OrderRefNo_ID);
                            if (orf != null)
                            {
                            ...
                            if (oCO.Branch_ID != "default")
                            {
                                tbl_genCustomerMaster_Branches oBranch = tbl_genCustomerMaster_Branches.Select(oCO.Customer_ID, int.Parse(oCO.Branch_ID));
                                if (oBranch != null)
                                {
                                    txtCustomerBranchID.Text = oBranch.BranchName;
                                    txtCustomerBranchID.Tag = oCO.Branch_ID;

                                    lblRoute.Text = "Route Code - " + clsGenaralName.getCode_Route(oBranch.Route_ID);
                                    lblRoute.Tag = oBranch.Route_ID;
                                }
                            }
                            txtAddress.Text = oCO.DeliveryAddress;
                            clsCommon.SetEnableDisable_ForeignKeyTextboxMust(txtCustomerOrderID, false);

                            glbCustomerOrderID = oCO.CustomerOrder_ID;
                            glbOrderRefNo = oCO.OrderRefNo_ID;
                            txtOrderRefNo.Text = clsCommon.GetForeignKeyValue(clsGenaralName.getName_OrderRefNo(oCO.OrderRefNo_ID));
                            txtOrderRefNo.Tag = oCO.OrderRefNo_ID;
                            clsCommon.SetEnableDisable_NormalTextbox(txtOrderRefNo, false);
                            clsCommon.SetEnableDisable_NormalLabel(lblOrderRefNo, false);

                            txtJobCode.Tag = clsHelpMethods.GetProductionJobIDByCustomerOrderID(oCO.CustomerOrder_ID);
                            txtJobCode.Text = clsCommon.GetForeignKeyValue(clsHelpMethods.GetProductionJobIDByCustomerOrderID(oCO.CustomerOrder_ID));

                            txtStoreID.Tag = oCO.Store_ID;
                            txtStoreID.Text = clsGenaralName.getName_Store(oCO.Store_ID);

                            dtpReceivedDate.Value = oCO.DeliveryDate;

                            if (clsConfig.sSoftwareModel != SoftwareModel_Sales.ePackWithSubCategory.ToString())
                            {
                                txtSalesNoteType.Tag = oCO.SalesNoteType_ID;
                                txtSalesNoteType.Text = clsCommon.GetForeignKeyValue(clsGenaralName.getName_SalesNoteType(oCO.SalesNoteType_ID));
                            }

                            if (oCO.ItemPriceCategory.Length > 0 && oCO.ItemPriceCategory != "default")
                            {
                                foreach (ComboBoxItem d in cmbItemPrice.Items)
                                {
                                    if (d.Value == oCO.ItemPriceCategory)
                                    {
                            ...
                            //add currency detail
                            FillDetailsCurrency(oCO.Currency_ID);
                            txtCurrencyRate.Text = oCO.CurrencyRate.ToString();
                            FillTaxDetailByCustomerOrderID(oCO.CustomerOrder_ID);

                            RefreshGridByCustomerOrderID(oCO.CustomerOrder_ID);
        ...
        private void CusDataGridViewFormat()
        {
            ...
            {
                dgvBreakdown.Columns["BrkItemName"].Visible = true;

                dgvBreakdown.Columns["BrkSerialNo"].HeaderText = "Packs";
                dgvBreakdown.Columns["BrkQuantity"].HeaderText = "Quantity";
                dgvBreakdown.Columns["BrkWeight"].HeaderText = "Weight";
                dgvBreakdown.Columns["BrkRemarks"].HeaderText = "Running No";
                dgvBreakdown.Columns["BrkItemName"].HeaderText = "Total Weight";

                dgvBreakdown.Columns["BrkItemName"].Width = 90;
                dgvBreakdown.Columns["BrkQuantity"].Width = 70;
                dgvBreakdown.Columns["BrkWeight"].Width = 70;
                dgvBreakdown.Columns["BrkSerialNo"].Width = 33;
                dgvBreakdown.Columns["BrkRemarks"].Width = 113;
        ...
        private void RefreshGrid(string sDeliveryOrderID)
        {
            try
            {
                ...
                List<tbl_sasDeliveryOrder_Detail> details = tbl_sasDeliveryOrder_Detail.SelectAllByDeliveryOrder_ID(sDeliveryOrderID).OrderBy(p => p.Line_No).ToList();
                foreach (tbl_sasDeliveryOrder_Detail detail in details)
                {
                    ...
                    if (item != null)
                    {
                        ...
                        Fill_Datagrid(true, iRow, detail.Line_No, detail.Item_ID, detail.CustomerOrder_ID, detail.Quotation_ID, detail.Job_ID, item.Uom_ID, detail.UnitPrice, detail.WeightPrice, detail.BIsFreeItem, detail.DiscountPresentage, detail.DiscountAmount, detail.TatalAmount,
                            item.Width, item.Height, item.Thickness, item.Gusset, detail.Weight, dCOQty, detail.Qty, "O", detail.ItemSubCategory_ID, detail.ItemSubCategory2_ID, detail.ItemSerialNo, detail.ItemSerialNo2, detail.Remark, detail.Carton_No, bHasSettledBefore, dExRate);
                        if (detail.IsLocked)
        ...
        private void RefreshGridByItemID(string sItemID, string sItemSubCategoryID, string sItemSubCategoryID2, string sSerialNo, string sSerialNo2)
        {
            try
            {
                ...
                if (detail != null && oItemF != null)
                {
                    ...
                    Fill_Datagrid(false, iRow, MaxID + 1, detail.Item_ID, "default", "default", "default", detail.Uom_ID, dUnitPrice, dWeightPrice, false, 0, 0, dAmount, detail.Width, detail.Height, detail.Thickness, detail.Gusset, dWeight, 0, dQty, "N", sItemSubCategoryID, sItemSubCategoryID2, sSerialNo, sSerialNo2, detail.Description, "", bHasSettledBefore, dExRate);
                }
                CalcualteSubTotal();
                CalculateTaxesAndGrandTotal();
            }
        ...
        private void Fill_Datagrid(bool IsUpdateMode, int iRow, int lineNo, string ItemID, string CusOrderID, string QuotationID, string JobID, string Uom_ID, decimal UnitPrice, decimal WeightPrice, bool isFreeItem, decimal DiscountPresentage, decimal DiscountAmount, decimal GrossTotal,
                            decimal Width, decimal Height, decimal Gauge, decimal Gusset, decimal Weight, decimal COQty, decimal Qty, string sItemStatus, string ItemSubCategoryID, string ItemSubCategoryID2, string SerialNo, string SerialNo2, string Remark, string cartonNo, bool bHasSettled, decimal dExRate)
        {
            try
            {
                if (!IsUpdateMode)
            ...
            try
            {
                ...
                foreach (DataRow row in dt_ItemGrouped.Rows)
                {
                    ...
                    if (!clsHelpMethods.IsNonInventoryItem(sItemCode))
                    {
                        ...
                        {
                            if ((clsConfig.bStockValidateQty_DeliveryOrder || clsConfig.bStockValidateWeight_DeliveryOrder) && !clsHelpMethods.IsNonInventoryItem(sItemCode) && (clsConfig.sSoftwareModel.Trim() != SoftwareModel_Sales.akt.ToString()))
                            {
        ...
        {
            clsSearch.Search_TransactionDeliveryOrder_Direct(ref txtDeliveryOrderID, chkShowSettle.Checked);
            if (txtDeliveryOrderID.Tag != null && txtDeliveryOrderID.Tag.ToString().Trim().Length > 0)
        ...
        private void print(bool bIsDraft)
        {
            try
            {
                if (txtDeliveryOrderID.TextLength > 0 && txtDeliveryOrderID.Text != "<Auto Generate>")
                {
                    ...
                    if (!clsConfig.bisDataSetActive_DONotePrinting)
                    {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            string sReportTitle_Main = "", sReportTitle_Sub = "", sReportPath = "", sDeliveryAddress = "", sVatRegi = "", sDeliveryTel = "", sBranchId = "";
                            bool bApprovalDone = true, bCheckingDone = true;
                            DateTime dtOrderDate = DateTime.MinValue; decimal dCustomerOrderQty = 0;
                            if (clsHelpMethods.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_DeliveryOrder), ref sReportTitle_Main, ref sReportTitle_Sub, ref sReportPath))
                            {
                                ...
                                if (bPermissinOkToPrint)
                                {
                                    ...
                                    if (oDelOrder != null)
                                    {
                                        ...
                                        if (bApprovalDone && bCheckingDone)
                                        {
                                            ...
                                            {
                                                tbl_genCustomerMaster oCustomer1 = tbl_genCustomerMaster.Select(oDelOrder.Customer_ID);
                                                //sDeliveryAddress = clsGenaralName.getName_CustomerDeliveryAddress(oDelOrder.Customer_ID);
                                                #region Get Customer Branch
                                                if (oDelOrder.Branch_ID != null && oDelOrder.Branch_ID != "default")
                                                {
                                                    
                                                    var oCusBranches = tbl_genCustomerMaster_Branches.SelectAllByCustomer_ID(oDelOrder.Customer_ID);
                                                    if (oCusBranches != null && oCusBranches.Count > 1)
                                                    {
                                                        tbl_genCustomerMaster_Branches oBranchCUs = tbl_genCustomerMaster_Branches.Select(oDelOrder.Customer_ID, Convert.ToInt16(oDelOrder.Branch_ID));

                                                        if (oBranchCUs != null)
                                                        {
                                                            if (oBranchCUs.BranchName != null && oBranchCUs.BranchName.Trim() != "")
                                                            {
                                                                //Branch Exist
                                                                sBranchId = oBranchCUs.BranchName;
                                                                sDeliveryTel = oBranchCUs.Telephone;

                                                                if (oBranchCUs.Address != null && oBranchCUs.Address.Trim() != "")
                                                                {
                                                                    //Branch Address Exist
                                                                    sDeliveryAddress = oBranchCUs.Address;
                                                                }
                                                                else
                                                                {
                                                                    //No Address Exist
                                                            ...
                                                            {
                                                                //No Branch
                                                                sBranchId = oCustomer1.CustomerName;
                                                                sDeliveryAddress = oCustomer1.AddressRegister;
                                                                sDeliveryTel = oCustomer1.Telephone;
                                                            }
                                                        }
                                                        else
                                                        {
                                            ...
                                            //clsGenaralName.getName_BranchCustomer(oDelOrder.Customer_ID, oDelOrder.Branch_ID == "default" ? 29949 : int.Parse(oDelOrder.Branch_ID))
                                            ...
                                            glb_dts_DeliveryOrders.dt_deliveryOrderHeader.Adddt_deliveryOrderHeaderRow(oDelOrder.DeliveryOrder_ID, oDelOrder.DeliveryOrderDate, oDelOrder.Remark,
                                                oDelOrder.Customer_ID, clsGenaralName.getName_Customer(oDelOrder.Customer_ID), clsGenaralName.getName_CustomerRegisterAddress(oDelOrder.Customer_ID), sPoNO, sBranchId,
                                                ...
                                                oDelOrder.Employee_ID, oDelOrder.IsWeightCalculation, clsGenaralName.getName_Employee(oDelOrder.Employee_ID), oDelOrder.IsDeleted, 0, dtOrderDate, sDeliveryAddress, clsGenaralName.getName_Assistant(oDelOrder.Assitant_ID), clsGenaralName.getName_Driver(oDelOrder.Driver_ID), clsGenaralName.getName_DriverNIC(oDelOrder.Driver_ID), oDelOrder.CustomerOrder_ID, sVatRegi, sDeliveryTel, clsGenaralName.getName_User(oDelOrder.CreateUser_ID), oDelOrder.DateCreate);

                                            foreach (tbl_sasDeliveryOrder_Detail oDetails_DO in tbl_sasDeliveryOrder_Detail.SelectAllByDeliveryOrder_ID(oDelOrder.DeliveryOrder_ID))
                                            {
                                                dCustomerOrderQty = 0;
                                                ...
                                                #endregion
                                                glb_dts_DeliveryOrders.dt_deliveryOrderDetail.Adddt_deliveryOrderDetailRow(oDetails_DO.DeliveryOrder_ID, oDetails_DO.ItemSerialNo, oDetails_DO.Item_ID, clsGenaralName.getName_Item(oDetails_DO.Item_ID), oDetails_DO.Remark, oDetails_DO.Carton_No, oDetails_DO.Qty, oDetails_DO.Weight, clsGenaralName.getName_ItemUOMName(oDetails_DO.Item_ID), oDetails_DO.UnitPrice, oDetails_DO.BIsFreeItem, oDetails_DO.DiscountPresentage, oDetails_DO.DiscountAmount, oDetails_DO.TatalAmount, dCustomerOrderQty, clsHelpMethods.GetPLU(oCustomer.Customer_ID, oItmaster.Item_ID));
                                            }

                                            #region Fill Company details
                                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("ReportTitle", sReportTitle_Main, true,false);
                                            ...
                                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CompanyBusinessRegNo", clsCommon.getCompanyBusinessRegisterNo(), true,false);
                                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("SVAT", clsCommon.getCompanySVAT(), true,false);
                                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CustomerAddress", clsCommon.getCompanyBusinessRegisterNo(), true,false);
                                            ...
                                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("isDel", oDelOrder.IsDeleted ? "CANCELLED" : "", true,false);
                                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CreateUser", sCreateUser, true,false);
                                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CheckUser", sCheckedUser, true,false);
                                            ...
                                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CreateUserCel", sCreateUserCel, true,false);
                                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CheckUserCel", sCheckedUserCel, true,false);
                                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CreateDateCel", sCreatedate, true,false);
                                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CheckedDateCel", sChequeDate, true,false);
                                            #endregion
                                            ...
                                            if (bIsDraft)
                                            {
                                                if (!clsConfig.isVisibleCompanyInfoInDraftPrint)
                                                {
                                                    ...
                                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CompanyBusinessRegNo", "", true,false);
        ...
        private void doPrintWithBrackDown(string sDuplicateCopy)
        {
            ...
            if (oOrder != null && oOrder.DeliveryOrder_ID != "default")
            {
                ...
                #region Print The Doc
                {
                    ...
                    {
                        string sGetRptPath = clsHelpMethods.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_DeliveryOrder_BrackDown));
                    ...
                    {
                        string sGetRptPath = clsHelpMethods.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_DeliveryOrder));
                        if (sGetRptPath != null && sGetRptPath.Length > 0)
                            s_Path += sGetRptPath;
                        else
                        {
                    ...
                    if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.akt.ToString() && !chkPrintWithBreakdown.Checked)
                    {
                        ...
                        foreach (tbl_sasDeliveryOrder_Detail oOrderDetail in oOrderDetails)
                        {
                            if (oOrderDetail.Item_ID != "default")
                            {
                                ...
                                RD.DataDefinition.FormulaFields["size"].Text = clsCommon.fncsetstring(clsHelpMethods.GetItemSizeByItemID(oOrderDetail.Item_ID));
                        ...
                        string sPONo = oOrder.Job_ID != "default" ? clsHelpMethods.GetPONoByProductionJobID(oOrder.Job_ID) : clsHelpMethods.GetPONoByCustomerOrderID(oOrder.CustomerOrder_ID);
                        ...
                        try
                        {
                            ...
                            RD.DataDefinition.FormulaFields["Combination"].Text = clsCommon.fncsetstring(clsHelpMethods.getCombinationMaterialByProductionJobID(oOrder.Job_ID.ToString()));
                            RD.DataDefinition.FormulaFields["Gauge"].Text = clsCommon.fncsetstring(clsHelpMethods.getCombinationMaterialThicknessByProductionJobID(oOrder.Job_ID.ToString()));
                    ...
                    if (clsConfig.bDirectPrint_NP_DeliveryOrder) //Direct Print
                    {
                        ...
                        clsHelpMethods.SetPrinterSetting(clsAutocode.getReportID(enum_ReportName.NP_DeliveryOrder), ref RD);
