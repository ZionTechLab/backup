...
namespace Digiteq
{
    public partial class UC_bpsReceiptSales : SEACC_Form
    {
        ...
        public UC_bpsReceiptSales(FormName _enmForm)
        {
            ...
            if (_enmForm == FormName.UCReceipt)
                IsSalesReceipt = true;
        }
                            ...
                            if (CheckValidity_Dependancies(oldRecord.Receipt_ID))
                            {
                                if (!oldRecord.IsLocked && !oldRecord.IsApproved && !oldRecord.IsChecked && !oldRecord.IsFinished && !oldRecord.IsDeleted)
                                {
                                    if (clsValidate.CheckValidity_TransactionCodeLength(txtReceiptID.Text))
                                    {
                                        ...
                                        clsHelpMethods.RemoveSattlementsFrom_ReceiptID(txtReceiptID.Text.Trim());
                                        ...
                                        tbl_bpsReceipt detail = new tbl_bpsReceipt(txtReceiptID.Text.Trim(), dtpReceiptDate.Value, txtRemark.Text.Trim(), txtTmpReceiptNo.Text, txtCustomerID.Tag.ToString(), txtInvoiceID.Tag.ToString(), "default", "default", "default",
                                            txtOrderRefNo.Tag.ToString(), oldRecord.GlPosting_ID, oldRecord.PostingStatus_ID, oldRecord.PostingStatus_ID2, oldRecord.FinancialYear_ID, txtSalesNoteType.Tag != null ? txtSalesNoteType.Tag.ToString() : "default", txtCollector.Tag.ToString(), txtCurrencyID.Tag.ToString(),
                                            decimal.Parse(txtCurrencyRate.Text.Trim()), 0, 0, 0, clsHelpMethods.getSavePrice(decimal.Parse(lblTotalAmount.Text.Trim()), txtCurrencyRate), txtAmountInWord.Text.Trim(),
                                            ...
                                            clsHelpMethods.getOldedInvoiceDate(sInvoiceList), clsHelpMethods.getInvoiceList(sInvoiceList), oldRecord.IsCashDeposited, oldRecord.DateDeposited, oldRecord.CompanyID, oldRecord.CompanyBranch_ID);
                                        detail.Update();
                                        #endregion
                                        ...
                                        Insert_Cash(detail.CurrencyRate);
                                        Update_Cheque(detail.CurrencyRate);
                                        Update_Card(detail.CurrencyRate);
                                        Update_BankTransfer(detail.CurrencyRate);
                    ...
                    else
                    {
                        ...
                        if (clsValidate.CheckValidity_TransactionCodeLength(txtReceiptID.Text))
                        {
                            ...
                            tbl_bpsReceipt detail = new tbl_bpsReceipt(txtReceiptID.Text.Trim(), dtpReceiptDate.Value, txtRemark.Text.Trim(), txtTmpReceiptNo.Text.Trim(), txtCustomerID.Tag.ToString(), txtInvoiceID.Tag.ToString(), "default", "default", "default", txtOrderRefNo.Tag.ToString(),
                              ...
                              txtCollector.Tag.ToString(), txtCurrencyID.Tag.ToString(), decimal.Parse(txtCurrencyRate.Text.Trim()), 0, 0, 0, clsHelpMethods.getSavePrice(decimal.Parse(lblTotalAmount.Text.Trim()), txtCurrencyRate), txtAmountInWord.Text.Trim(),
                              ...
                              false, false, false, false, false, 0, rdoAdvancePayment.Checked, false, 0, false, IsSalesReceipt, clsHelpMethods.getOldedInvoiceDate(sInvoiceList), clsHelpMethods.getInvoiceList(sInvoiceList), false, clsSecurity.getServerDateTime(), clsSecurity.CompanyID, clsSecurity.BranchID);
                            detail.Insert();
                            #endregion
                            ...
                            if (dgvCheq.Rows.Count > 0)
                                Insert_Cheques(detail.CurrencyRate);
                            if (dgvCard.Rows.Count > 0)
                                Insert_Cards(detail.CurrencyRate);
                            if (dgvBankTransfer.Rows.Count > 0)
                                Insert_BankTransfer(detail.CurrencyRate);
                            #endregion

                            Attachments.Insert(txtReceiptID.Text);
                            clsMethods_GL.PostTransaction_SalesReciept(txtReceiptID.Text);
                            setPaymentAllocation(txtReceiptID.Text.Trim(), true);
                            clsAlerts_Email.createEmail_Receipt(txtReceiptID.Text.Trim(), enum_Alerts.ReceiptCreated);
        ...
        private void Update_Cheque(decimal CurrencyRate)
        {
            try
            {
                    ...
                    if (oCheque != null)
                    {
                        ...
                        foreach (DataGridViewRow row in dgvCheq.Rows)
                        {
                            ...
                            if (oCheque.ChequeRegister_ID == sChequeRegisterCode)
                            {
                                ...
                                dAmount = clsValidate.ValidateGridValue(dgvCheq, "Amount", row.Index, decimal.Parse("0.00"))* CurrencyRate;
        ...
        private void Update_BankTransfer(decimal CurrencyRate)
        {
            try
            {
                    ...
                    if (oCheque != null)
                    {
                        ...
                        foreach (DataGridViewRow row in dgvBankTransfer.Rows)
                        {
                            ...
                            if (oCheque.ChequeRegister_ID == sRegisterCode)
                            {
                                ...
                                dAmount = clsValidate.ValidateGridValue(dgvBankTransfer, "BTAmount", row.Index, decimal.Parse("0.00"))* CurrencyRate;
        ...
        private void Update_Card(decimal CurrencyRate)
        {
            try
            {
                #region Update old details
                foreach (tbl_bpsChequeRegister oCheque in tbl_bpsChequeRegister.SelectAllByReceipt_ID(txtReceiptID.Text.Trim()).Where(p => p.PaymentMethod_ID == (int)PaymentMethod.Card))
                {
                    if (oCheque != null)
                    {
                        ...
                        int sTypeID = 0;
                        decimal dAmount = 0;
                        DateTime dChequeDate = dtpReceiptDate.Value;
                        bool bHasChequeInGrid = false;
                        ...
                        foreach (DataGridViewRow row in dgvCard.Rows)
                        {
                            ...
                            if (oCheque.ChequeRegister_ID == sRegisterCode)
                            {
                                ...
                                dAmount = clsValidate.ValidateGridValue(dgvCard, "crdAmount", row.Index, decimal.Parse("0.00"))*  CurrencyRate;
        ...
        private void Insert_Cash(decimal CurrencyRate)
        {
            try
            {
                string sChequeRegisterCode = "";

                #region Init Cheque register
                ...
                {
                   tbl_bpsChequeRegister RegisterDetails = new tbl_bpsChequeRegister(
                       sChequeRegisterCode, "", dtpReceiptDate.Value.Date, (int)PaymentMethod.Cash,
                   ...
                   clsAutocode.getGLPostingStatusID(GLPostingStatus.NewTransaction), clsSecurity.FinancialYearID, 
                   decimal.Parse(txtCashAmount.Text.Trim())* CurrencyRate,
        ...
        private void Insert_Cheques(decimal CurrencyRate)
        {
            try
            {
                ...
                foreach (DataGridViewRow row in dgvCheq.Rows)
                {
                    ...
                    decimal dAmount = clsValidate.ValidateGridValue(dgvCheq, "Amount", row.Index, decimal.Parse("0.00"))* CurrencyRate;
        ...
        private void Insert_BankTransfer(decimal CurrencyRate)
        {
            try
            {
                #region insert Grid - Bank Transfer Details
                foreach (DataGridViewRow row in dgvBankTransfer.Rows)
                {
                    ...
                    decimal dAmount = clsValidate.ValidateGridValue(dgvBankTransfer, "BTAmount", row.Index, decimal.Parse("0.00"))*  CurrencyRate;
        ...
        private void Insert_Cards(decimal CurrencyRate)
        {
            try
            {
                #region Insert Grid - Card Details
                foreach (DataGridViewRow row in dgvCard.Rows)
                {
                    ...
                    decimal dAmount = clsValidate.ValidateGridValue(dgvCard, "crdAmount", row.Index, decimal.Parse("0.00"))* CurrencyRate;
                                ...
                                if (!detail.IsLocked)
                                {
                                    if (!detail.IsDeleted)
                                    {
                                        ...
                                        if (msgResult == DialogResult.Yes)
                                        {
                                            ...
                                            clsHelpMethods.RemoveSattlementsFrom_ReceiptID_CashAndCheque(txtReceiptID.Text.Trim());//need to rewrite
                                            ...
                                            Cursor = Cursors.Default;
                                            clsAlerts_Email.createEmail_Receipt(txtReceiptID.Text.Trim(), enum_Alerts.ReceiptCanceled);
        ...
        private void RefreshChequeGrid_ByReceiptID(string sReceiptID)
        {
            try
            {
                ...
                foreach (tbl_bpsChequeRegister detail in oListCheque)
                {
                    ...
                    dgvCheq["Amount", iRow].Value = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.getDisplayPrice(detail.Amount, txtCurrencyRate)); //clsFormatter.FormatToCurrecyWithThousendSep(detail.Amount);
                ...
                if (oListCheque.Count > 0)
                    expanderCheque.DisplayAmount = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.getDisplayPrice(dAmount, txtCurrencyRate)); //clsFormatter.FormatToCurrecyWithThousendSep(dAmount);
            }
            catch (Exception ex)
            {
                clsValidate.WriteErrorLog("", iFormID, ex);
        ...
        private void RefreshCardGrid_ByReceiptID(string sReceiptID)
        {
            try
            {
                ...
                foreach (tbl_bpsChequeRegister detail in oListCard)
                {
                    ...
                    dgvCard["crdAmount", iRow].Value = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.getDisplayPrice(detail.Amount, txtCurrencyRate));// clsFormatter.FormatToCurrecyWithThousendSep(detail.Amount);
                ...
                if (oListCard.Count > 0)
                    expanderCard.DisplayAmount = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.getDisplayPrice(dAmount, txtCurrencyRate));// clsFormatter.FormatToCurrecyWithThousendSep(dAmount);
        ...
        private void RefreshBankTransferGrid_ByReceiptID(string sReceiptID)
        {
            try
            {
                ...
                foreach (tbl_bpsChequeRegister detail in oListBankTransfer)
                {
                    ...
                    dgvBankTransfer["BTAmount", iRow].Value = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.getDisplayPrice(detail.Amount, txtCurrencyRate));// clsFormatter.FormatToCurrecyWithThousendSep(detail.Amount);
                ...
                if (oListBankTransfer.Count > 0)
                    expanderBankTransfer.DisplayAmount = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.getDisplayPrice(dAmount, txtCurrencyRate));// clsFormatter.FormatToCurrecyWithThousendSep(dAmount);
            }
            catch (Exception ex)
            {
                clsValidate.WriteErrorLog("", iFormID, ex);
            ...
            try
            {
                if (sID.Length > 0)
                {
                    ...
                    if (detail != null)
                    {
                        ...
                        txtCollector.Tag = detail.Collector_ID;

                        txtCustomerID.Text = clsCommon.GetForeignKeyValue(clsGenaralName.getName_Customer(detail.Customer_ID));
                        txtSalesNoteType.Text = clsGenaralName.getName_SalesNoteType(detail.SalesNoteType_ID);
                        txtCollector.Text = clsGenaralName.getName_Employee(detail.Collector_ID);
                        ...
                        lblTotalAmount.Text = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.getDisplayPrice(detail.TotalAmount, detail.CurrencyRate));
                        ...
                        #region fill cash amount
                        foreach (tbl_bpsChequeRegister detailCash in tbl_bpsChequeRegister.SelectAllByReceipt_ID(detail.Receipt_ID).Where(p => p.PaymentMethod_ID == (int)PaymentMethod.Cash))
                        {
                            txtCashAmount.Text = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods.getDisplayPrice( detailCash.Amount,txtCurrencyRate));
                            expanderCash.DisplayAmount = txtCashAmount.Text;
            ...
            try
            {
                if (sInvID != null && sInvID.Length > 0)
                {
                    if (CheckInvoiceValidity(sInvID))
                    {
                        ...
                        if (detail != null)
                        {
                            if (detail.CurrencyRate != 1)
                            {
                                MessageBox.Show("Foreign currency invoices are not allowed to settle in this form", clsFormatter.GetMessageCaption(), MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                            else
                            {
                                int iRow;
                                ...
                                #region set the orderdetail/salesrep     
                                tbl_zOrderRefNo order = tbl_zOrderRefNo.Select(detail.OrderRefNo_ID);
                                if (order != null)
                                {
                                    if (order.Employee_ID != "default")
                                    {
                                        ...
                                        txtCollector.Tag = order.Employee_ID;
                                        txtCollector.Text = clsCommon.GetForeignKeyValue(clsGenaralName.getName_Employee(order.Employee_ID));
                                    }
            ...
            try
            {
                ...
                if (customer != null)
                {
                    ...
                    if (customer.SalesRep_ID != null && customer.SalesRep_ID != "default")
                    {
                        ...
                        {
                            txtCollector.Tag = customer.SalesRep_ID;
                            txtCollector.Text = sRepName;
                        }
                        else
                        {
                            txtCollector.Tag = "default";
                            txtCollector.Text = "-";
                        }
                    }
                }
        ...
        private void Search_Invoice()
        {
            try
            {
                if (txtCurrencyID.Tag.ToString() != clsConfig.sLocalCurrencyCode)
                {
              MessageBox.Show("Foreign currency transactions are not allow to settle in this form", clsFormatter.GetMessageCaption(), MessageBoxButtons.OK, MessageBoxIcon.Stop);
                }
                else
                {
        ...
        private void Search_Currency()
        {
            try
            {
                ...
                if (txtCurrencyID.Tag == null)
                    txtCurrencyID.Tag = clsConfig.sLocalCurrencyCode;

                if (txtCurrencyID.Tag.ToString() != clsConfig.sLocalCurrencyCode)
                {
                    if (dgvInvoice.Rows.Count > 0)
                    {
                        DialogResult msgResult = MessageBox.Show("Foreign currency transactions are not allow to settle in this form\nDo you need to remove invoices to preseed?", clsFormatter.GetMessageCaption(), MessageBoxButtons.YesNo, MessageBoxIcon.Stop);
                        if (msgResult == DialogResult.Yes)
                        {
                            dgvInvoice.Rows.Clear();
                        }
                            ...
                            if (CheckValidity_InvoiceCurencyRate())
                            {
                                if (CheckValidity_ChequeNo())
                                {
                                    if (CheckValidity_Posting())// need to rewrite
                                    {
                                        if (clsMethods_GL.CheckValidity_FinancialYear(dtpReceiptDate.Value.Date))
                                        {
                                            if (CheckValidity_ForignCurrencyTXN())
                                                bStatus = true;
                                        }
                                    }
                                }
        ...
        private bool CheckValidity_ForignCurrencyTXN()
        {
            bool bStatus = true;
        ...
        private bool CheckValidity_EmptyField()
        {
            ...
            if (clsValidate.ValidateTextBox_EmptyValue(txtCustomerID, "Customer Name"))
            {
                ...
                {
                    if (clsValidate.ValidateTextBox_EmptyValue(txtCollector, "Collector"))
                        bStatus = true;
                }
                else
                    bStatus = true;
        ...
        private void ValidateEmptyForeignKey()
        {
            ...
            clsCommon.ValidateForeignKey(ref txtCollector);
        }
            ...
            if (txtReceiptID.TextLength > 0 && txtReceiptID.Text != "<Auto Generate>")
            {
                ...
                try
                {
                    ...
                    if (clsHelpMethods.GetReportPath(sReportID, ref sReportTitle_Main, ref sReportTitle_Sub, ref sReportPath))
                    {
                                ...
                                if (bOkToPrint)
                                {
                                    ...
                                    foreach (tbl_bpsChequeRegister oChq in tbl_bpsChequeRegister.SelectAllByReceipt_ID(receipt.Receipt_ID))
                                    {
                                        ...
                                        glb_dts_sasReceiptAllocation.dt_sasSalesReceiptDetails.Adddt_sasSalesReceiptDetailsRow(receipt.Receipt_ID, oChq.AccountNumber,
                                            ...
                                            oChq.ChequeNumber, oChq.DateCheque,clsHelpMethods.getDisplayPrice( oChq.Amount, receipt.CurrencyRate), clsGenaralName.getName_ChequeType(oChq.ChequeType_ID), oChq.ChequeStatus_ID);
                                    }
                                    #endregion
                                    ...
                                    glb_dts_sasReceiptAllocation.dt_sasSalesReceiptHeader.Adddt_sasSalesReceiptHeaderRow(receipt.Receipt_ID, receipt.ReceiptDate, receipt.Remark,
                                        sCustomerName, sAddressRegister, sSalesRep, clsHelpMethods.getDisplayPrice(dCash, receipt.CurrencyRate), clsHelpMethods.getDisplayPrice(dCheque, receipt.CurrencyRate), clsHelpMethods.getDisplayPrice(dCard, receipt.CurrencyRate), clsHelpMethods.getDisplayPrice(dBankTransfer, receipt.CurrencyRate), clsHelpMethods.getDisplayPrice(receipt.TotalAmount, receipt.CurrencyRate), sTelephone, sFax,
                                    ...
                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("BusinessRegNo ", clsCommon.fncsetstring(clsCommon.getCompanyBusinessRegisterNo()), true,false);
                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CompanyVatNo", clsCommon.fncsetstring(clsCommon.getCompanyVAT()), true,false);
                                    ...
                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("isDel", receipt.IsDeleted ? "CANCELLED" : "", true,false);

                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("IsDraft", bIsDraft ? "DRAFT" : "", true,false);

                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("ChequeNo", clsCommon.fncsetstring(sChequNo), true,false);
                                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("ChequeDate", clsCommon.fncsetstring(sDate), true,false);
                        ...
                        clsAlerts_Email.createEmail_Receipt(txtReceiptID.Text.Trim(), enum_Alerts.ReceiptPrinted);
                    }
                }
                catch (Exception ex)
                {
                ...
                if (dgvInvoice.Rows.Count > 0)
                {
                    ...
                    if (oReceipt != null && oReceipt.Receipt_ID != "default")
                    {
                        ...
                        foreach (DataGridViewRow row in dgvInvoice.Rows)
                        {
                            ...
                            decimal dAmount = clsHelpMethods.getSavePrice(clsValidate.ValidateGridValue(dgvInvoice, "InvoiceAmount", row.Index, decimal.Parse("0.0")), txtCurrencyRate);
                            decimal AllocationAmount = clsHelpMethods.getSavePrice(clsValidate.ValidateGridValue(dgvInvoice, "AllocatedAmount", row.Index, decimal.Parse("0.0")), txtCurrencyRate);
                            ...
                            {
                                dAmountToBeSettle -= clsHelpMethods.AutoSettledInvoiceWithCheque(sInvoiceID, oPaymentRegister.ChequeRegister_ID, dAmountToBeSettle, sAllocationID, frm_toolPaymentAllocate.bAdvancePayment, frm_toolPaymentAllocate.bOverPayment);
