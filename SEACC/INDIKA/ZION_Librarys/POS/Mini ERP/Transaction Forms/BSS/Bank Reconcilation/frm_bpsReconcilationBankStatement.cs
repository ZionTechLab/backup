    ...
    public partial class frm_bpsReconcilationBankStatement : SEACC_Form
    {
        ...
        public frm_bpsReconcilationBankStatement(FormName _enmForm, int iCompanyAccountID, string sStatementNo, DateTime dtFromDate, DateTime dtStatementDate, decimal dStatementBalance, decimal glbdLastBalance)
        {
            ...
            if (oComAccount != null)
            {
                ...
                if (oBank != null)
                {
                    string sQuary = "select opbl from dbo.func_AccountOPBL('1988-8-23','" + dtStatementDate.ToString("yyyy-MM-dd") + "','" + clsMethods_GL.getFinancialYear_ID(dtStatementDate.Date) + "','%') where gl_id='" + oBank.Gl_ID + "'";
        ...
        public frm_bpsReconcilationBankStatement(FormName _enmForm, int iCompanyAccountID, int iRecSerial_NO)
        {
            ...
            if (oBankRec != null && oBankRec.RecSerialNo != -1)
            {
                ...
                if (oComAccount != null)
                {
                    ...
                    if (oBank != null)
                    {
                        string sQuary1 = "select opbl from dbo.func_AccountOPBL('1988-8-23','" + oBankRec.DateTo.ToString("yyyy-MM-dd") + "','" + clsMethods_GL.getFinancialYear_ID(oBankRec.DateTo.Date) + "','%') where gl_id='" + oBank.Gl_ID + "'";
                        ...
                        try
                        {
                            ...
                            if (CheckValidityGridSelection())
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        if (oRec.ChequeStatus_ID == clsAutocode.getChequeStatusID(ChequeStatus.Deposited))
                                            oRec.Delete();
                                    }
                                    #endregion
        ...
        private void Calculate()
        {
            try
            {
                ...
                {
                    iCountTot++;
                    dAmountReceipt += clsValidate.ValidateRowValue(dtRow, "Receipt", d);
                    dAmountPayment += clsValidate.ValidateRowValue(dtRow, "Payment", d);

                    if (clsValidate.ValidateRowValue(dtRow, "IsSelected", false))
                    {
                        iCountSelected++;
                        dAmountSelected_Receipt += clsValidate.ValidateRowValue(dtRow, "Receipt", d);
                        dAmountSelected_Payment += clsValidate.ValidateRowValue(dtRow, "Payment", d); ;
                    }
                }
                ...
                txtDifference.Text = clsFormatter.FormatToCurrecyWithThousendSep(dLedgerBalance - dClosingBalance);
        ...
        #region Btn Approve
        private void frm_bpsReconcilationBankStatement_SF_approveButton_Click(object sender, EventArgs e)
        {
            Search_ApprovedBy();
        }
        #endregion
        ...
        #region Btn Check
        private void frm_bpsReconcilationBankStatement_SF_checkButton_Click(object sender, EventArgs e)
        {
            Search_CheckedBy();
        }
        #endregion
