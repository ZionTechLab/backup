                    ...
                    if (txtCreditNoteID.Text.Trim().Length > 0 && txtCreditNoteID.Text.Trim() != "<Auto Generate>")
                    {
                        ...
                        if (CreditNote != null)
                        {
                            ...
                            if (bApprovalDone)
                            {
                                ...
                                sReportPath = clsHelpMethods.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_CreditNote));
                                #endregion
        ...
        private DataSet AdvancedPrint_CreditNote(tbl_bpsCreditNote oCreditNote, bool IsVATSelected, bool IsNBTSelected, bool IsSVATSelected)
        {
            try
            {
                ...
                if (oCreditNote != null)
                {
                    ...
                    decimal dSubTotal = clsHelpMethods.getDisplayPrice(oCreditNote.SubTotal, oCreditNote.CurrencyRate);
                    dDiscountTotal = clsHelpMethods.getDisplayPrice(dDiscountTotal, oCreditNote.CurrencyRate);
                    decimal dNbtAmout = clsHelpMethods.getDisplayPrice(oCreditNote.NbtTotal, oCreditNote.CurrencyRate);
                    decimal dvatAmount = clsHelpMethods.getDisplayPrice(oCreditNote.VatTotal, oCreditNote.CurrencyRate);
                    decimal dSvatAmount = clsHelpMethods.getDisplayPrice(oCreditNote.OtherTaxTotal, oCreditNote.CurrencyRate);
                    decimal dGrandToatal = clsHelpMethods.getDisplayPrice(oCreditNote.TotalAmount, oCreditNote.CurrencyRate);
