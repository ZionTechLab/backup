                                ...
                                if (!oDBN.IsLocked)
                                {
                                    if (!oDBN.IsDeleted)
                                    {
                                        ...
                                        if (msgResult == DialogResult.Yes)
                                        {
                                            ...
                                            clsAlerts_Email.createEmail_DebitNote(txtDebitNoteID.Text.Trim(), enum_Alerts.DebitNoteCancel);
        ...
        private void Print(bool bIsDraft)
        {
            try
            {
                if (txtDebitNoteID.Text.Trim().Length > 0 && txtDebitNoteID.Text.Trim() != "<Auto Generate>")
                {
                    ...
                    {
                        tbl_bpsDebitNote oDeditNote = tbl_bpsDebitNote.Select(txtDebitNoteID.Text.Trim());
                        if (oDeditNote != null)
                        {
                            if (!bIsDraft)
                            {
                            ...
                            if (bApprovalDone && bCheckingDone)
                            {
                                ...
                                if (bOkToPrint && bApprovalDone)
                                {
                                  ...
                                  //  if (clsConfig.bisDatasetActive_DebitNotePrinting)
                                    {
                                        #region Datasets
                                        tbl_genCustomerMaster oCustomer = tbl_genCustomerMaster.Select(oDeditNote.Customer_ID);
                                        if (oCustomer != null)
                                        {
                                            glb_dtsSales.dt_sasTaxDetails_DebitNote.Adddt_sasTaxDetails_DebitNoteRow(oDeditNote.DebitNote_ID, oDeditNote.DebitNoteDate,
                                                clsGenaralName.getName_DebitNoteType(oDeditNote.DebitNoteType_ID), oDeditNote.SalesReturnedNote_ID,
                                                oDeditNote.Invoice_ID, oDeditNote.Currency_ID, oCustomer.CustomerName,
                                                clsGenaralName.getName_CustomerRegisterAddress(oDeditNote.Customer_ID), oDeditNote.DeliveryOrder_ID,
                                                oDeditNote.OrderRefNo_ID, oDeditNote.Remark, oDeditNote.SubTotal, oDeditNote.DiscountTotal, oDeditNote.NbtTotal,
                                                oDeditNote.VatTotal, oDeditNote.OtherTaxTotal, oDeditNote.TotalAmount, oDeditNote.ChequeRegister_ID, oCustomer.SalesRep_ID,clsGenaralName.getName_SalesRep(oCustomer.SalesRep_ID));
