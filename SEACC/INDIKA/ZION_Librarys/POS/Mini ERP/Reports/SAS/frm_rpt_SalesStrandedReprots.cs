...
namespace Digiteq
{
    public partial class frm_rpt_SalesStrandedReprots : MettroForm
    {
        ...
        bool bRouteSelected = false, bSelesRepSelected = false, bCustomerSelected = false, bCustomerClassSelected = false, bCustomerTypeSelected = false, bCustomerCategorySelected = false, bJobSelected = false, bJobTypeSelected = false, bSalesNoteTypeSelected = false,
            bItemClassSelected = false, bItemTypeSelected = false, bItemcatagorySelected = false, bItemSelected = false, bSalesManagerSelected = false, bAreaManagerSelected = false, bBranchSelected = false;// bIsItemSelected = false;
        //string sReportNo = "";
        int iReport;
        ...
        dts_sasSalesReports glb_dtsSasSalesReports = new dts_sasSalesReports();
        #endregion
                    ...
                    try
                    {
                        ...
                        if (clsSecurity.PermissionToPrint_WithMessage(clsAutocode.getReportID(Report)))
                        {
                            ...
                            if (clsHelpMethods.GetReportPath(clsAutocode.getReportID(Report), ref sReportTitle_Main, ref sReportTitle_Sub, ref sReportPath))
                            {
                                ...
                                bJobSelected = false;
                                bJobTypeSelected = false;
                                bSalesNoteTypeSelected = false;
                                bItemClassSelected = false;
                                bItemTypeSelected = false;
                                bItemcatagorySelected = false;
                                bItemSelected = false;
                                bSalesManagerSelected = false;
                                bAreaManagerSelected = false;
                                ...
                                #region Total Sales
                                if (Report == enum_ReportName.ST_Monthly_Sales_Customer_Wise_Rupees)
                                {
                                    try
                                    {
                                        Cursor = Cursors.WaitCursor;
                                        ...
                                        foreach (tbl_sasInvoice oInvoice in oInvoiceList)
                                        {
                                            ...
                                            glb_dtsSales.dt_sasMonthlySalesReport_Summary.Adddt_sasMonthlySalesReport_SummaryRow(oInvoice.Invoice_ID, oInvoice.InvoiceDate,
                                                   CusDetail.CustomerName, clsGenaralName.getName_BranchCustomer(oInvoice.Customer_ID, int.Parse(oInvoice.Branch_ID)), oInvoice.GrandTotal, dTotalWeight, dTotalQty, oInvoice.InvoiceDate.Month, oInvoice.InvoiceDate.Year.ToString());
                                        }
                                        #endregion
                                        ...
                                        rpt.print("\\Reports\\SAS\\Standard\\rpt_sas_SalesMonthWise_New.rpt", glb_dtsSales, glb_dtsReportExport.dt_rptParameter, clsAutocode.getReportID(Report));
                                    }
                                    catch (Exception ex)
                                    {
                                ...
                                else if (Report == enum_ReportName.ST_DONotInvoiced)
                                {
                                    try
                                    {
                                        ...
                                        foreach (tbl_sasDeliveryOrder detail in tbl_sasDeliveryOrder.SelectAll().Where(p => p.CompanyBranch_ID == txtBranch.Tag.ToString() && p.DeliveryOrder_ID != "default" && !p.IsSeattled && !p.IsDeleted && p.DeliveryOrderDate.Date >= dtpFrom.Value.Date && p.DeliveryOrderDate.Date <= dtpTo.Value.Date))
                                        {
                                            ...
                                            glb_dtsDeliveryOrders.dt_deliveryOrderHeader.Adddt_deliveryOrderHeaderRow(detail.DeliveryOrder_ID, detail.DeliveryOrderDate, detail.Remark, detail.Customer_ID, clsGenaralName.getName_Customer(detail.Customer_ID), clsGenaralName.getName_CustomerDeliveryAddress(detail.Customer_ID), "", "", "", detail.Store_ID, clsGenaralName.getName_Store(detail.Store_ID),
                                                "", detail.OrderRefNo_ID, detail.Vehicle_No, detail.SubTotal, detail.DiscountTotal, detail.DiscountPercentage, detail.NbtTotal, detail.NbtPercentage, detail.VatTotal, detail.VatPercentage, 0, 0, detail.GrandTotal, detail.Employee_ID, detail.IsWeightCalculation, sSalesman, detail.IsDeleted, 0, DateTime.MinValue, "", "", "", "", "", "", "", clsGenaralName.getName_User(detail.CreateUser_ID), detail.DateCreate);
                                        }
                                ...
                                else if (Report == enum_ReportName.ST_SalesPriceList_MRP)
                                {
                                    ...
                                    string sItemClass = bItemClassSelected ? txtItemClass.Tag.ToString() : "%";
                                    string sItemType = bItemTypeSelected ? TxtItemType.Tag.ToString() : "%";

                                    string sQuary = "exec [srh_sasSalesPriceReport] '" + sItem + "', '" + sItemCat + "', '" + sItemClass + "', '" + sItemType + "'";
                                ...
                                else if (Report == enum_ReportName.ST_MounthlySalesSummaryReport)
                                {
                                    try
                                    {
                                        ...
                                        foreach (tbl_sasInvoice oInvoice in oInvoiceList)
                                        {
                                            ...
                                            //clsHelpMethods.startProgressBar(0, oInvoiceList.Count + 2, 1, ProgressBar);
                                        }
                                    ...
                                    if (cmbTaxType.Text.Trim() == "Local NBT/VAT")
                                    {
                                        ...
                                        foreach (tbl_sasInvoice oInvoice in Query)
                                        {
                                            ...
                                            glb_dtsSales.dtTaxSummary.AdddtTaxSummaryRow(oInvoice.InvoiceDate, dSubTotal, dNBTAmount, dWithNBTAmount, dVatAmount, "INV", oInvoice.GrandTotal,
                                                clsGenaralName.getName_CurrencyCode(oInvoice.Currency_ID), oInvoice.CurrencyRate, clsHelpMethods.getDisplayPrice(dWithNBTAmount, oInvoice.CurrencyRate),
                                    ...
                                    else if (cmbTaxType.Text.Trim() == "Export VAT")//"Local SVAT"
                                    {
                                                               ...
                                                               && !p.IsOpeningBalance && !p.IsDebitNote && !p.IsReturnedCheque && p.IsSVatInvoice && p.Currency_ID == clsConfig.sLocalCurrencyCode))
                                        {
                                            ...
                                            glb_dtsSales.dtTaxSummary.AdddtTaxSummaryRow(oInvoice.InvoiceDate, dSubTotal, dNBTAmount, dWithNBTAmount, dVatAmount, "INV",
                                                ...
                                                clsHelpMethods.getDisplayPrice(dWithNBTAmount, oInvoice.CurrencyRate), clsHelpMethods.getDisplayPrice(dVatAmount, oInvoice.CurrencyRate));
                                        }
                                    ...
                                    foreach (tbl_bpsCreditNote oCRN in tbl_bpsCreditNote.SelectAll().Where(p => p.CompanyBranch_ID == txtBranch.Tag.ToString() && p.CreditNoteType_ID != clsAutocode.getCreditNoteTypeID(CreditNoteType.ReturnedChequeDeposit)))//
                                    {
                                        if (oCRN.IsDeleted == false && oCRN.CreditNoteDate >= dtpFrom.Value && oCRN.CreditNoteDate <= dtpTo.Value && oCRN.Currency_ID == clsConfig.sLocalCurrencyCode)
                                        {
                                            ...
                                            glb_dtsSales.dtTaxSummary.AdddtTaxSummaryRow(oCRN.CreditNoteDate, dSubTotal, dNBTAmount, dWithNBTAmount, dVatAmount, "CRN", oCRN.TotalAmount,
                                                clsGenaralName.getName_CurrencyCode(oCRN.Currency_ID), oCRN.CurrencyRate, clsHelpMethods.getDisplayPrice(dWithNBTAmount, oCRN.CurrencyRate),
                                                        ...
                                                        if (oInvStl != null)
                                                        {
                                                            ...
                                                            if (oInvoice != null)
                                                            {
                                                                ...
                                                                glb_dtsSales.dt_sasTaxDetails_CreditNote.Adddt_sasTaxDetails_CreditNoteRow(oCreditNote.CreditNote_ID, oCreditNote.CreditNoteDate, oCustomer.VatRegistrationNo != "" ? clsGenaralName.getName_Customer(oCreditNote.Customer_ID) + "\nVAT Reg : " + oCustomer.VatRegistrationNo : clsGenaralName.getName_Customer(oCreditNote.Customer_ID), oInvStl.SattledAmount,
                                                                    ...
                                                                    oCreditNote.CurrencyRate, clsHelpMethods.getDisplayPrice(dSubTotal, oCreditNote.CurrencyRate), clsHelpMethods.getDisplayPrice(dVatAmount, oCreditNote.CurrencyRate),
                                        ...
                                        foreach (tbl_sasInvoice oInv in oInvoices.Where(p => p.CompanyBranch_ID == txtBranch.Tag.ToString() && !p.IsDeleted && p.Invoice_ID != "default" && p.InvoiceDate.Date >= dtpFrom.Value.Date && p.InvoiceDate.Date <= dtpTo.Value.Date && !p.IsOpeningBalance && !p.IsDebitNote && !p.IsReturnedCheque).ToList())
                                        {
                                            ...
                                            if (oCustomer != null && oCustomer.CustomerCode != "default")
                                            {
                                                ...
                                                if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.akt.ToString() && (clsFormatter.DigiteqTitle == "SEACC ePack" || clsFormatter.DigiteqTitle == "SEACC ePack Test"))
                                                {
                                                    ...
                                                    {
                                                        string sInvType = clsHelpMethods.getProductionJobType_Simple(oInv.Job_ID);
                                            ...
                                            clsHelpMethods.startProgressBar(0, oInvoices.Count + 2, 1, ProgressBar);
                                        }
                                ...
                                else if (Report == enum_ReportName.ST_Dilivery_Listing_Report)
                                {
                                    ...
                                    foreach (tbl_sasDeliveryOrder oDO in Query)
                                    {
                                        ...
                                        if (oCustomer != null)
                                        {
                                            ...
                                            sPONo = clsHelpMethods.GetPONoByDeliveryOrderID(oDO.DeliveryOrder_ID);
                                            ...
                                            foreach (tbl_sasDeliveryOrder_Detail detail in tbl_sasDeliveryOrder_Detail.SelectAllByDeliveryOrder_ID(oDO.DeliveryOrder_ID))
                                            {
                                                ...
                                                if (oItem != null)
                                                {
                                                    ...
                                                    {
                                                        if (txtItemClass.Tag.ToString().Trim() != oItem.ItemClass_ID)
                                                            continue;
                                                    }
                                                    ...
                                                    else if (bItemcatagorySelected)
                                                    {
                                                        //  txtItemSubCategory
                                                        //  TxtItemCat.Tag
                                                        if (TxtItemCat.Tag.ToString().Trim() != oItem.ItemCategory_ID)
                                                            continue;
                                                    }
                                                    #endregion
                                ...
                                else if (Report == enum_ReportName.ST_Invoice_Listing_Report)
                                {
                                    ...
                                    foreach (tbl_sasInvoice oInvoice in Query)
                                    {
                                        ...
                                        sPONo = clsHelpMethods.GetPONoByDeliveryOrderID(oInvoice.DeliveryOrder_ID);
                                        sPONo = sPONo == "default" ? "N/A" : sPONo;
                                        string sJobType = "N/A";
                                        ...
                                        if (oCustomer != null)
                                        {
                                            ...
                                            foreach (tbl_sasInvoice_Detail detail in tbl_sasInvoice_Detail.SelectAllByInvoice_ID(oInvoice.Invoice_ID))
                                            {
                                                ...
                                                if (oItem != null && dtDO != null)
                                                {
                                                    ...
                                                    string sItemSize = clsHelpMethods.GetItemSizeByItemID(detail.Item_ID);
                                    ...
                                    try
                                    {
                                        ...
                                        foreach (tbl_pmsProductionJobRegister oJob in tbl_pmsProductionJobRegister.SelectAll().Where(p => p.ProductionJob_ID != "default" && !p.IsDeleted && p.ProductionOrderDate.Date >= dtpFrom.Value.Date && p.ProductionOrderDate.Date <= dtpTo.Value.Date))
                                        {
                                            ...
                                            if (dJobQty > 0)
                                            {
                                                ...
                                                List<string> sMaterials = clsHelpMethods.getCombinationMaterialListByProductionJobID(oJob.ProductionJob_ID, true);
                                                ...
                                                glb_dtsSasDeliveryTracking.dt_sasDeliveryTrackingJobWise.Adddt_sasDeliveryTrackingJobWiseRow(oJob.ProductionJob_ID, oJob.ProductionOrderDate, clsGenaralName.getName_Customer(oJob.Customer_ID), oJob.OrderRefNo_ID, dJobQty, clsGenaralName.getName_Uom(oJob.Uom_ID), clsGenaralName.getName_ProductionJobType(oJob.ProductionJobType_ID), clsGenaralName.getName_Item(oJob.Item_ID),
                                                    (dPresentage == 100) ? true : false, sSalesman_ID != "default" ? clsGenaralName.getName_Employee(sSalesman_ID) : "", sCombinationMatirials, clsCommon.fncsetstring(clsHelpMethods.GetItemSizeByItemID(oJob.Item_ID)));
                                ...
                                else if (Report == enum_ReportName.ST_CustomerOrderTrackingReport)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        foreach (tbl_sasCustomerOrder oCO in oCOList)
                                        {
                                            ...
                                            clsHelpMethods.startProgressBar(0, oCOList.Count + 2, 1, ProgressBar);
                                        }
                                ...
                                else if (Report == enum_ReportName.ST_SalesReturnValue)
                                {
                                    try
                                    {
                                        ...
                                        foreach (tbl_sasSalesReturnedNote oSRN in oSRNs)
                                        {
                                            ...
                                            {
                                                tbl_genItemMaster oItem = tbl_genItemMaster.Select(oSalesReturnNoteDetail.Item_ID);
                                                if (oItem != null)
                                                {
                                                    #region Item Filter
                                                    if (bItemSelected)
                                                    {
                                                        if (txtItemID.Tag.ToString().Trim() != oSalesReturnNoteDetail.Item_ID.Trim())
                                                            continue;
                                                    }
                                                    else if (bItemClassSelected)
                                                    {
                                                    ...
                                                    bNoValue = true;
                                                    sUom = clsGenaralName.getName_Uom(oItem.Uom_ID);
                                    ...
                                    try
                                    {
                                        ...
                                        if (!chkShowUpdatedReport.Checked)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("isQtyReport", chk_Qty.Checked ? "1" : "0", true, false);
                                        ...
                                        foreach (tbl_sasInvoice oInvoice in oInvoices)
                                        {
                                            ...
                                            foreach (tbl_sasInvoice_Detail oDetail in tbl_sasInvoice_Detail.SelectAllByInvoice_ID(oInvoice.Invoice_ID))
                                            {
                                                ...
                                                if (oItem != null && oItem.Item_ID != "default")
                                                {
                                                    ...
                                                    decimal dCostPriceValue = clsProcessMethods.GetCostPrice_ByCostType(oItem.Item_ID, "default", "default", "0", "0", eCostType);
                                ...
                                #region Sales Reports Sales Route wise - Indika
                                else if (Report == enum_ReportName.ST_SalesReport_RouteWise)
                                {
                                    try
                                    {
                                        glb_dtsSales.Clear();
                                        ...
                                        foreach (tbl_sasInvoice oInvoice in oInvoices)
                                        {
                                            ...
                                            if (oCustomer != null)
                                            {
                                                ...
                                                sRouteName = iRouteID == -1 ? "-" : clsGenaralName.get_RouteName(iRouteID);
                                            }
                                            #endregion

                                            glb_dtsSales.dt_sasSalesReport.Adddt_sasSalesReportRow("", oInvoice.Invoice_ID, oInvoice.InvoiceDate, "", "", iRouteID.ToString(), sRouteName, "", "", oInvoice.GrandTotal, 0, 0);
                                        ...
                                        foreach (tbl_bpsCreditNote oCRN in CRNs)
                                        {
                                            ...
                                            if (oCustomer != null)
                                            {
                                                foreach (tbl_genCustomerMaster_Branches oRoute in tbl_genCustomerMaster_Branches.SelectAllByCustomer_ID(oCRN.Customer_ID))
                                                {
                                                    iRouteID = oRoute.Route_ID;
                                                }
                                            ...
                                            // Sales Return Credit Notes
                                            if (oCRN.CreditNoteType_ID == "TP/002")
                                                glb_dtsSales.dt_sasSalesReport.Adddt_sasSalesReportRow("", oCRN.CreditNote_ID, oCRN.CreditNoteDate, "", "", iRouteID.ToString(), sRouteName, "", "", 0, oCRN.TotalAmount, 0);
                                            else
                                                glb_dtsSales.dt_sasSalesReport.Adddt_sasSalesReportRow("", oCRN.CreditNote_ID, oCRN.CreditNoteDate, "", "", iRouteID.ToString(), sRouteName, "", "", 0, 0, oCRN.TotalAmount);
                                ...
                                else if (Report == enum_ReportName.ST_ItemSalesReport_SalesRepWise)
                                {
                                    try
                                    {
                                        ...
                                        {
                                            decimal dQty = 0;
                                            foreach (srh_scsFlowStock oStocktxn in srh_scsFlowStock.Select(dtpFrom.Value.Date, sItemList.ElementAt(i), "0", txtBranch.Tag.ToString(), true))
                                            {
                                                dQty += oStocktxn.Qty;
                                            }
                                        ...
                                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("FromDate", clsFormatter.FormatDate_SL(dtpFrom.Value.Date), true, false);
                                        glb_dtsSales.dt_Company.Adddt_CompanyRow(clsSecurity.DigiteqName, clsSecurity.DigiteqEmail, clsSecurity.CompanyName, clsSecurity.CompanyAddress1, clsSecurity.CompanyAddress2, clsCommon.getCompanyImage(), sReportTitle_Main, sReportTitle_Sub, sDateRange, clsSecurity.UserNameLoged, sFilter);
                                ...
                                else if (Report == enum_ReportName.ST_MonthlySalesCalendar_RouteSalesRepWise)
                                {
                                    try
                                    {
                                        ...
                                        List<tbl_bpsCreditNote> oCRNList = tbl_bpsCreditNote.SelectAll().Where(p => p.CompanyBranch_ID == txtBranch.Tag.ToString() && !p.IsDeleted && p.CreditNoteType_ID != "default" && p.PosReturnTransaction_Index == -1 && p.CreditNoteDate.Date >= dtpFrom.Value.Date && p.CreditNoteDate.Date <= dtpTo.Value.Date).ToList();
                                        ...
                                        #region CRN
                                        foreach (tbl_bpsCreditNote oCRN in oCRNList)
                                        {
                                            #region Route
                                ...
                                else if (Report == enum_ReportName.CU_SalesDetailReport_InvoiceItemWiseCR)
                                {
                                    glb_dtsSasSalesReports.Clear();
                                    glb_dtsReportExport.Clear();
                                    ...
                                    //   if (Report == enum_ReportName.CU_SalesDetailReport_InvoiceItemWise || Report == enum_ReportName.CU_SalesDetailReport_InvoiceWise)
                                    {
                                        ...
                                        foreach (tbl_sasInvoice oInvoice in oInvoiceList)
                                        {
                                            ...
                                            if (oCustomer != null)
                                            {
                                                ...
                                                foreach (tbl_sasInvoice_Detail oDetail in tbl_sasInvoice_Detail.SelectAllByInvoice_ID(oInvoice.Invoice_ID).OrderBy(r => r.Line_No))
                                                {
                                                    ...
                                                    glb_dtsSasSalesReports.dt_salesReport.Adddt_salesReportRow("1-Sales",
                                                        clsGenaralName.getName_CompanyBranchMaster(oInvoice.CompanyBranch_ID),
                                                       oInvoice.Invoice_ID,
                                                         oInvoice.InvoiceDate.Date,
                                                       sSalesmanID == "default" ? "-" : clsGenaralName.getName_SalesRep(sSalesmanID),
                                                        clsGenaralName.getName_Customer(oInvoice.Customer_ID),
                                                         clsGenaralName.getName_CustomerClass(oCustomer.CustomerClass_ID),
                                                       clsGenaralName.getName_CustomerType(oCustomer.CustomerType_ID),
                                                        clsGenaralName.getName_CustomerCategory(oCustomer.CustomerCategory_ID),

                                                      oDetail.Item_ID,
                                                       clsGenaralName.getName_Item(oDetail.Item_ID),

                                                        dUnitPrice,
                                                     oDetail.Qty,

                                                      dAmountBeforeLineDiscount,
                                                     dBulkDiscount + dLineDiscount,
                                               dAmountBeforeLineDiscount - (dBulkDiscount + dLineDiscount), //dSubTotal,

                                                    dNBTAmount,
                                                    dVATAmount,
                                                    dItemWise_GrandTotal,
                                                 dSVATAmount,
                                                ...
                                                clsHelpMethods.startProgressBar(0, oInvoiceList.Count, 1, ProgressBar);
                                            }
                                        }
                                        ProgressBar.Value = 0;
                                        #endregion
                                        ...
                                        foreach (tbl_sasSalesReturnedNote oSrn in oSrnList)
                                        {
                                            ...
                                            if (oCustomer != null)
                                            {
                                                ...
                                                foreach (tbl_sasSalesReturnedNote_Detail oDetail in tbl_sasSalesReturnedNote_Detail.SelectAllBySalesReturnedNote_ID(oSrn.SalesReturnedNote_ID).OrderBy(r => r.Line_No))
                                                {
                                                    ...
                                                    decimal dUnitPrice = dAmountBeforeLineDiscount / oDetail.Qty;
                                                    glb_dtsSasSalesReports.dt_salesReport.Adddt_salesReportRow("2-Sales Return",
                                                        clsGenaralName.getName_CompanyBranchMaster(oSrn.CompanyBranch_ID),
                                                       oSrn.SalesReturnedNote_ID,
                                                       oSrn.SalesReturnedNoteDate.Date,
                                                       sSalesmanID == "default" ? "-" : clsGenaralName.getName_SalesRep(sSalesmanID),
                                                       clsGenaralName.getName_Customer(oSrn.Customer_ID),
                                                      clsGenaralName.getName_CustomerClass(oCustomer.CustomerClass_ID),
                                                       clsGenaralName.getName_CustomerType(oCustomer.CustomerType_ID),
                                                       clsGenaralName.getName_CustomerCategory(oCustomer.CustomerCategory_ID),
                                                   ...
                                                   oDetail.Item_ID,
                                                      clsGenaralName.getName_Item(oDetail.Item_ID),
                                                       ...
                                                       (dUnitPrice * -1),
                                                   (oDetail.Qty * -1),
                                                   (dAmountBeforeLineDiscount * -1),

                                                   (dBulkDiscount + dLineDiscount * -1),
                                                      ((dAmountBeforeLineDiscount - (dBulkDiscount + dLineDiscount)) * -1),

                                                      (dNBTAmount * -1),
                                                   (dVATAmount * -1),
                                                   (dItemWise_GrandTotal * -1),
                                                     (dSVATAmount * -1),
                                                ...
                                                clsHelpMethods.startProgressBar(0, oSrnList.Count, 1, ProgressBar);
                                            }
                                        }
                                        ProgressBar.Value = 0;
                                        #endregion
                                        ...
                                        foreach (tbl_bpsCreditNote oCrn in oCrnList.Where(p => p.SalesReturnedNote_ID == "default" && p.PosReturnTransaction_Index == -1 && p.AdvanceReceived_Index == -1 && p.CreditNoteType_ID == "TP/002" || p.CreditNoteType_ID == "TP/007" || p.CreditNoteType_ID == "TP/004" || p.CreditNoteType_ID == "TP/005"))
                                        {
                                            ...
                                            if (oCustomer != null)
                                            {
                                                ...
                                                glb_dtsSasSalesReports.dt_salesReport.Adddt_salesReportRow("3-Credit Note",
                                               clsGenaralName.getName_CompanyBranchMaster(oCrn.CompanyBranch_ID),
                                              oCrn.CreditNote_ID,
                                               oCrn.CreditNoteDate.Date,
                                                 sSalesmanID == "default" ? "-" : clsGenaralName.getName_SalesRep(sSalesmanID),
                                              clsGenaralName.getName_Customer(oCrn.Customer_ID),
                                                clsGenaralName.getName_CustomerClass(oCustomer.CustomerClass_ID),
                                                clsGenaralName.getName_CustomerType(oCustomer.CustomerType_ID),
                                                 clsGenaralName.getName_CustomerCategory(oCustomer.CustomerCategory_ID),
                                              ...
                                              0,
                                               (dItemTotal * -1),
                                                ...
                                                clsHelpMethods.startProgressBar(0, oCrnList.Count, 1, ProgressBar);
                                            }
                                        }
                                        ProgressBar.Value = 0;
                                        #endregion
                                        ...
                                        foreach (tbl_bpsDebitNote oDbn in oDbnList.Where(p => p.DebitNoteType_ID == "TP/003"))
                                        {
                                            ...
                                            if (oCustomer != null)
                                            {
                                                ...
                                                glb_dtsSasSalesReports.dt_salesReport.Adddt_salesReportRow("4-Debit Note",
                                                clsGenaralName.getName_CompanyBranchMaster(oDbn.CompanyBranch_ID),
                                             oDbn.DebitNote_ID,
                                                 oDbn.DebitNoteDate.Date,
                                                  sSalesmanID == "default" ? "-" : clsGenaralName.getName_SalesRep(sSalesmanID),
                                               clsGenaralName.getName_Customer(oDbn.Customer_ID),
                                                  clsGenaralName.getName_CustomerClass(oCustomer.CustomerClass_ID),
                                                 clsGenaralName.getName_CustomerType(oCustomer.CustomerType_ID),
                                                clsGenaralName.getName_CustomerCategory(oCustomer.CustomerCategory_ID),
                                             ...
                                             dNBTAmount,
                                              dVATAmount,
                                                ...
                                                clsHelpMethods.startProgressBar(0, oDbnList.Count, 1, ProgressBar);
                                            }
                                        }
                                        ProgressBar.Value = 0;
                                        #endregion
                                        ...
                                        foreach (tbl_posTransaction oPos in oPosList)
                                        {
                                            ...
                                            if (oCustomer != null)
                                            {
                                                ...
                                                foreach (tbl_posTransaction_Detail oDetail in tbl_posTransaction_Detail.SelectAllByPosTransaction_Index(oPos.PosTransaction_Index))
                                                {
                                                    ...
                                                    glb_dtsSasSalesReports.dt_salesReport.Adddt_salesReportRow(
                                                       "5-POS Sales & Return",
                                                     clsGenaralName.getName_CompanyBranchMaster(oPos.CompanyBranch_ID),
                                                      oPos.PosTransaction_ID,
                                                      oPos.PosTransactiondate.Date,
                                                        sSalesmanID == "default" ? "-" : clsGenaralName.getName_SalesRep(sSalesmanID),
                                                     clsGenaralName.getName_Customer(oPos.Customer_ID),
                                                       clsGenaralName.getName_CustomerClass(oCustomer.CustomerClass_ID),
                                                       clsGenaralName.getName_CustomerType(oCustomer.CustomerType_ID),
                                                        clsGenaralName.getName_CustomerCategory(oCustomer.CustomerCategory_ID),
                                                    ...
                                                    dBulkDiscount + dLineDiscount,
                                                     dAmountBeforeLineDiscount - (dBulkDiscount + dLineDiscount),
                                                ...
                                                foreach (tbl_posReceipt oReceipt in tbl_posReceipt.SelectAllByPosTransaction_Index(oPos.PosTransaction_Index))
                                                {
                                                    foreach (tbl_bpsChequeRegister oChequeRegister in tbl_bpsChequeRegister.SelectAllByPosReceipt_ID(oReceipt.PosReceipt_ID).Where(p => p.PaymentMethod_ID == (int)PaymentMethod.Gift_Voucher))
                                                    {
                                                        ...
                                                        glb_dtsSasSalesReports.dt_salesReport.Adddt_salesReportRow(
                                                            "5-POS Sales & Return",
                                                          clsGenaralName.getName_CompanyBranchMaster(oPos.CompanyBranch_ID),
                                                          oPos.PosTransaction_ID + "(GVR)",
                                                       oPos.PosTransactiondate.Date,
                                                           sSalesmanID == "default" ? "-" : clsGenaralName.getName_SalesRep(sSalesmanID),
                                                     clsGenaralName.getName_Customer(oPos.Customer_ID),
                                                      clsGenaralName.getName_CustomerClass(oPos.Customer_ID),
                                                            clsGenaralName.getName_CustomerType(oCustomer.CustomerType_ID),
                                                           clsGenaralName.getName_CustomerCategory(oCustomer.CustomerCategory_ID),

                                                             oChequeRegister.GiftVoucherID.ToString(),
                                                           "Gift Voucher",

                                                           0,
                                                            0,
                                                            -dGrandTotalWithout_Tax2,
                                               ...
                                               0,
                                                        ...
                                                        //       TxType = "5-POS Sales & Return",
                                                        //       Branch = clsGenaralName.getName_CompanyBranchMaster(oPos.CompanyBranch_ID),
                                                        //       Tx_ID = oPos.PosTransaction_ID + "(GVR)",
                                                        //       TxDate = oPos.PosTransactiondate.Date,
                                                        //       SalesRep = sSalesmanID == "default" ? "-" : clsGenaralName.getName_SalesRep(sSalesmanID),
                                                        //       Customer = clsGenaralName.getName_Customer(oPos.Customer_ID),
                                                        //       CustomerType = clsGenaralName.getName_CustomerType(oCustomer.CustomerType_ID),
                                                ...
                                                clsHelpMethods.startProgressBar(0, oPosList.Count, 1, ProgressBar);
                                            }
                                        }
                                        ProgressBar.Value = 0;
                                        #endregion
                                        ...
                                        glb_dtsSasSalesReports.dt_Company.Adddt_CompanyRow(clsSecurity.DigiteqName, clsSecurity.DigiteqEmail, clsSecurity.CompanyName, clsSecurity.CompanyAddress1, clsSecurity.CompanyAddress2, clsCommon.getCompanyImage(), sReportTitle_Main, sReportTitle_Sub, sDateRange, clsSecurity.UserNameLoged, sFilter);

                                        frm_ReportViewer_New rpt = new frm_ReportViewer_New();
                                        rpt.print(sReportPath, glb_dtsSasSalesReports, glb_dtsReportExport.dt_rptParameter, clsAutocode.getReportID(Report));
        ...
        private void clearField()
        {
            ...
            //clsCommon.SetEnableDisable_NormalLabel(lblItemCat, false);
            //clsCommon.SetEnableDisable_ForeignKeyTextboxOptional(TxtItemCat, false);
            ...
            //clsCommon.SetEnableDisable_NormalComboBox(cmbDOType, false);
            ...
            //clsCommon.SetEnableDisable_ForeignKeyTextboxOptional(txtCusClass, false);
            //clsCommon.SetEnableDisable_NormalLabel(lblCusClass, false);
            //clsCommon.SetEnableDisable_ForeignKeyTextboxOptional(txtCusType, false);
            //clsCommon.SetEnableDisable_NormalLabel(lblCusType, false);
            //clsCommon.SetEnableDisable_ForeignKeyTextboxOptional(txtCusCategory, false);
            //clsCommon.SetEnableDisable_NormalLabel(lblCusCategory, false);

            //clsCommon.SetEnableDisable_ForeignKeyTextboxOptional(txtJobCode, false);
            //clsCommon.SetEnableDisable_NormalLabel(lblInquiryID, false);
            //clsCommon.SetEnableDisable_NormalComboBox(cmbTaxType, false);
            //clsCommon.SetEnableDisable_NormalLabel(lblTaxType, false);
            //clsCommon.SetEnableDisable_NormalComboBox(cmbPriceCategory, false);
            //clsCommon.SetEnableDisable_NormalLabel(lblPriceCat, false);
            //clsCommon.SetEnableDisable_NormalComboBox(cmbCostPrice, false);
            //clsCommon.SetEnableDisable_NormalLabel(lblCostPrice, false);
            ...
            //clsCommon.SetEnableDisable_NormalCheckBox(chkIsReplasement, false);
            //clsCommon.SetEnableDisable_NormalRadioButton(rdoRupee, false);
            //clsCommon.SetEnableDisable_NormalRadioButton(rdoDollars, false);
        ...
        //private void print(string path, string sReportTitle, DataSet objDataSet, string sFilter)
        //{
        //    try
        //    {
        //        Cursor = Cursors.WaitCursor;
        ...
        //        //sSeperator = sFilter.Length > 0 ? " | " : "";
        //        //if (txtCustomer.Tag != null && txtCustomer.Tag.ToString().Length > 0)
        //        //    sFilter += sSeperator + "Customer : " + txtCustomer.Text.Trim();
        //        //sSeperator = sFilter.Length > 0 ? " | " : "";
        //        //if (txtSalesNoteType.Tag != null && txtSalesNoteType.Tag.ToString().Length > 0)
        //        //    sFilter += sSeperator + "Note Type : " + txtSalesNoteType.Text.Trim();
        //        //sSeperator = sFilter.Length > 0 ? " | " : "";
        //        //if (txtSalesRep.Tag != null && txtSalesRep.Tag.ToString().Trim().Length > 0)
        //        //    sFilter += sSeperator + "Salesman : " + txtSalesRep.Text.Trim();
        //        //sSeperator = sFilter.Length > 0 ? " | " : "";
        //        //if (txtTown.Tag != null && txtTown.Tag.ToString().Length > 0)
        //        //    sFilter += sSeperator + "Town Name : " + txtTown.Text.Trim();
        //        //sSeperator = sFilter.Length > 0 ? " | " : "";

        //        //if (txtItemClass.Tag != null && txtItemClass.Tag.ToString().Length > 0)
        //        //    sFilter += sSeperator + "Item Class : " + txtItemClass.Text.Trim();
        //        //sSeperator = sFilter.Length > 0 ? " | " : "";

        //        //if (TxtItemType.Tag != null && TxtItemType.Tag.ToString().Length > 0)
        //        //    sFilter += sSeperator + "Item type: " + TxtItemType.Text.Trim();
        //        //sSeperator = sFilter.Length > 0 ? " | " : "";

        //        //if (TxtItemCat.Tag != null && TxtItemCat.Tag.ToString().Length > 0)
        //        //    sFilter += sSeperator + "Item Category : " + TxtItemCat.Text.Trim();
        //        //sSeperator = sFilter.Length > 0 ? " | " : "";

        //        //if (txtItemID.Tag != null && txtItemID.Tag.ToString().Length > 0)
        //        //    sFilter += sSeperator + "Item : " + txtItemID.Text.Trim();
        //        //sSeperator = sFilter.Length > 0 ? " | " : "";
        ...
        //        //sSeperator = sFilter.Length > 0 ? " | " : "";
        //        //if (txtRoute.Tag != null && txtRoute.Tag.ToString().Length > 0)
        //        //    sFilter += sSeperator + "Route Name : " + txtRoute.Text.Trim();
        //        //sSeperator = sFilter.Length > 0 ? " | " : "";
        //        //if (txtJobCode.Tag != null && txtJobCode.Tag.ToString().Length > 0)
        //        //    sFilter += sSeperator + "Job Code : " + txtJobCode.Text.Trim();
        ...
        private void txtItemID_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F1)
                ...
                clsHelpMethods.SearchItemAdvanceByKeyPress(ref txtItemID, ref TxtItemCat, ref txtItemSerialNo, e);
        }
        #endregion
        ...
        private void Search_CustomerID()
        {
            ...
            if (chkShowAll_Customers.Checked)
                lstParameeters.Add("");
            else
                lstParameeters.Add("0");
            ...
            try
            {
                ...
                foreach (tbl_sasCustomerOrder oCO in Query)
                {
                    ...
                    if (bCustomerOK && bSalesRep)
                    {
                        ...
                        {
                            decimal dStanderdWeight = clsHelpMethods.GetJobStandardWeightBy_SalesJobID(oCO.Job_ID);
                            decimal dProductionWegiht = 0, dProductionQty = 0, dDeliveryQty = 0, dSRNQty = 0;
                            string dProductionUOM = "N/A";
                            string sItemSize = clsHelpMethods.GetItemSizeByItemID(oJob.Item_ID);
