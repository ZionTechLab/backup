                    ...
                    try
                    {
                        ...
                        if (clsSecurity.PermissionToPrint_WithMessage(clsAutocode.getReportID(Report)))
                        {
                            ...
                            if (clsHelpMethods.GetReportPath(clsAutocode.getReportID(Report), ref sReportTitle_Main, ref sReportTitle_Sub, ref sReportPath))
                            {
                                    ...
                                    try
                                    {
                                        ...
                                        foreach (tbl_bpsCreditNote oCRN in oCRNL)
                                        {
                                            ...
                                            if (oCustMas != null)
                                            {
                                                ...
                                                #region Progress Bar
                                                clsHelpMethods.startProgressBar(0, oCRNL.Count + 2, 1, ProgressBar);
                                                #endregion
                                            }
                                        }
                                ...
                                if (Report == enum_ReportName.CU_BankAccountWisePaymentVoucher)
                                {
                                    try
                                    {
                                        ...
                                        foreach (tbl_accPaymentVoucher oPayment in oPayments)
                                        {
                                            ...
                                            {
                                                glb_dtsUnSpecified.dt_Unspecified_01.Adddt_Unspecified_01Row(oPayment.PaymentVoucher_ID,//Payment Voucher No-S1
                                            ...
                                            #region Progress Bar
                                            clsHelpMethods.startProgressBar(0, oPayments.Count + 2, 1, ProgressBar);
                                            #endregion
                                        }
                                        glb_dtsUnSpecified.dt_Company.Adddt_CompanyRow(clsSecurity.DigiteqName, clsSecurity.DigiteqEmail, clsSecurity.CompanyName, clsSecurity.CompanyAddress1, clsSecurity.CompanyAddress2, clsCommon.getCompanyImage(), sReportTitle_Main, sReportTitle_Sub, sDateRange, clsSecurity.UserNameLoged, sFilter);
                                ...
                                if (Report == enum_ReportName.CU_CollectionReportRouteWise)
                                {
                                    try
                                    {
                                        Cursor = Cursors.WaitCursor;
                                        glb_dtsUnSpecified.Clear();
                                        string sRouteName = "", sSalesmanID = "";
                                        //, sRouteID = "";
                                        int iRouteID = -1;
                                        ProgressBar.Value = 0;
                                        ...
                                        List<tbl_genCustomerMaster_Branches> oRouts = tbl_genCustomerMaster_Branches.SelectAll().Where(p => p.Route_ID != -1).ToList();
                                        foreach (tbl_genCustomerMaster_Branches oRoute in oRouts)
                                        {
                                            iRouteID = oRoute.Route_ID;

                                            //List<tbl_bpsChequeRegister> oCheques = tbl_bpsChequeRegister.SelectAll().Where(p => p.ChequeRegister_ID != "default" && p.DateRegister.Date >= dtpFrom.Value.Date && p.DateRegister.Date <= dtpTo.Value.Date && p.CompanyBranch_ID == clsSecurity.BranchID).ToList();
                                            List<tbl_bpsChequeRegister> oCheques = tbl_bpsChequeRegister.SelectAllByCustomer_ID(oRoute.Customer_ID).Where(p => p.ChequeRegister_ID != "default" && !p.IsDeleted && p.DateRegister.Date >= dtpFrom.Value.Date && p.DateRegister.Date <= dtpTo.Value.Date && p.CompanyBranch_ID == clsSecurity.BranchID).ToList();
                                            foreach (tbl_bpsChequeRegister oCheque in oCheques)
                                            {
                                                decimal dchequeCollection = 0, dcashCollection = 0, dTotalCollection = 0, dReturnCollection = 0, dNetCollection = 0;

                                                #region Common Filter                                               
                                                //if (rdoDeleted.Checked)
                                                //    if (oCheque.IsDeleted != true)
                                                //        continue;

                                                //if (rdoActive.Checked)
                                                //    if (oCheque.IsDeleted != false)
                                                //        continue;
                                                #endregion

                                                tbl_bpsReceipt oCollector = tbl_bpsReceipt.Select_FromAllReciepts(oCheque.Receipt_ID);
                                                if (oCollector != null)
                                                {
                                                    if (bCollectorSelected)
                                                        if (oCollector.Collector_ID != txtCollector.Tag.ToString().Trim())
                                                            continue;
                                                }

                                                tbl_genCustomerMaster oCuMaster = tbl_genCustomerMaster.Select(oCheque.Customer_ID);
                                                if (oCuMaster != null)
                                                {
                                                    if (bAreamManagerSelected)
                                                        if (oCuMaster.AreaManager_ID != txtAreaManager.Tag.ToString().Trim())
                                                            continue;
                                                }
                                                ...
                                                if (!chkUseCustomerMastorSaleRep.Checked)
                                                {
                                                    ...
                                                    sSalesmanID = oCuMaster.SalesRep_ID;
                                                ...
                                                //if (bRouteSelected)
                                                //{
                                                //    foreach (tbl_genCustomerMaster_Branches oRoute in tbl_genCustomerMaster_Branches.SelectAllByCustomer_ID(oCheque.Customer_ID).Where(p => p.Route_ID != -1))
                                                //    {
                                                //        sRouteID = oRoute.Route_ID.ToString();
                                                //        if (txtRoute.Tag.ToString() == sRouteID)
                                                //            break;
                                                ...
                                                #region Cal Amount Details
                                                if (oCheque.PaymentMethod_ID == (int)PaymentMethod.Cash)
                                                    dcashCollection += oCheque.Amount;

                                                if (oCheque.PaymentMethod_ID == (int)PaymentMethod.Cheque)
                                                    dchequeCollection += oCheque.Amount;

                                                if (oCheque.PaymentMethod_ID == (int)PaymentMethod.Cheque && oCheque.ChequeStatus_ID == clsAutocode.getChequeStatusID(ChequeStatus.Returned_R) || oCheque.ChequeStatus_ID == clsAutocode.getChequeStatusID(ChequeStatus.Returned_NR_O) || oCheque.ChequeStatus_ID == clsAutocode.getChequeStatusID(ChequeStatus.Returned_NR_C))
                                                    dReturnCollection += oCheque.Amount;

                                                dTotalCollection = dcashCollection + dchequeCollection;
                                                dNetCollection = dTotalCollection - dReturnCollection;
                                                #endregion

                                                #region Fill Dataset
                                                glb_dtsUnSpecified.dt_Unspecified_01.Adddt_Unspecified_01Row(clsGenaralName.getCode_Route(iRouteID),//Route Name-S1
                                                    "", "", "", "", "", "", "", "", "",
                                                    dchequeCollection,//Cheque Collection-D1
                                                    dcashCollection,//Cash Collection-D2
                                                    dTotalCollection,//Total Collection-D3
                                                    dReturnCollection,//Return Cheque Collection-D4
                                                    dNetCollection,//Net Collection-D5
                                                    0, 0, 0, 1, 1, 1, 1, DateTime.MinValue, DateTime.MinValue, DateTime.MinValue, DateTime.MinValue, oCheque.IsReturned, oCheque.IsDeleted, false, false);
                                                #endregion
                                                                                                                                       
                                            }
                                            #region Progress Bar
                                            clsHelpMethods.startProgressBar(0, oRouts.Count + 2, 1, ProgressBar);
                                            #endregion
                                ...
                                if (Report == enum_ReportName.CU_DepositedCheque)
                                {
                                    try
                                    {
                                        ...
                                        foreach (tbl_bpsChequeRegister oDeposited in oDepositeds)
                                        {
                                            ...
                                            #region Progress Bar
                                            clsHelpMethods.startProgressBar(0, oDepositeds.Count + 2, 1, ProgressBar);
                                            #endregion
                                        }


                                        #region Fill Columns Header 
                                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("Acc1Bank", clsGenaralName.getShortName_Bank(oAccount[0].Bank_ID), true,false);
                                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("Acc1", oAccount[0].AccountNumber.Substring(oAccount[0].AccountNumber.Length - 4, 4), true,false);
                                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("Acc2Bank", clsGenaralName.getShortName_Bank(oAccount[1].Bank_ID), true,false);
                                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("Acc2", oAccount[1].AccountNumber.Substring(oAccount[1].AccountNumber.Length - 4, 4), true,false);
                                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("Acc3Bank", clsGenaralName.getShortName_Bank(oAccount[2].Bank_ID), true,false);
                                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("Acc3", oAccount[2].AccountNumber.Substring(oAccount[2].AccountNumber.Length - 4, 4), true,false);
                                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("Acc4Bank", clsGenaralName.getShortName_Bank(oAccount[3].Bank_ID), true,false);
                                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("Acc4", oAccount[3].AccountNumber.Substring(oAccount[3].AccountNumber.Length - 4, 4), true,false);
                                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("Acc5Bank", clsGenaralName.getShortName_Bank(oAccount[4].Bank_ID), true,false);
                                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("Acc5", oAccount[4].AccountNumber.Substring(oAccount[4].AccountNumber.Length - 4, 4), true,false);
                                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("Acc6Bank", clsGenaralName.getShortName_Bank(oAccount[5].Bank_ID), true,false);
                                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("Acc6", oAccount[5].AccountNumber.Substring(oAccount[5].AccountNumber.Length - 4, 4), true,false);
                                        #endregion
