...
namespace SEACC_POS.UserManagement
{
    ...
    public partial class UC_UserPermission : UserControl
    {
            ...
            if (CheckValidity())
                try
                {
                    ...
                    foreach (DataRow row in dgr_Main.dt.Rows)
                    {
                        ...
                        tbl_securityFunctionMaster_Permission oldRecord = tbl_securityFunctionMaster_Permission.Select(txtComBranch.Tag.ToString(), txtUserID.Tag.ToString(), function_ID);
                        if (oldRecord != null)
                        {
                            tbl_securityFunctionMaster_Permission ouserPermission = new tbl_securityFunctionMaster_Permission(txtComBranch.Tag.ToString(), txtUserID.Tag.ToString(), function_ID, isReadEnable, isWriteEnable, isDeleteEnable, isApproveEnable, isCheckEnable, isEditEnable, isPrintEnable, isReprintEnable, isExportEnable, isViewEnable);
            ...
            if (chkUIs.IsChecked || chkReports.IsChecked)
            {
                if (CheckValidity())
                    if (txtModule.Tag != null)
                        RefreshGrid(txtComBranch.Tag.ToString(), txtUserID.Tag.ToString(), txtModule.Tag.ToString(), chkReports.IsChecked);
                    else
                        RefreshGrid(txtComBranch.Tag.ToString(), txtUserID.Tag.ToString(), "%", chkReports.IsChecked);
            }
            else
            {
                dgr_Main.dt.Clear();
        ...
        private void ClearFields()
        {
            ...
            cls_Formater.SetEnableDisable_ForigenKeyLabelTextBox(txtComBranch, false, false, false);
            ...
            txtComBranch.Tag = clsSecurity.BranchID;
            txtUserID.Tag = null;
            txtModule.Tag = null;

            txtComBranch.Text = clsGenaralName.getName_CompanyBranchMaster(clsSecurity.BranchID);
            txtUserID.Text = "";
            txtModule.Text = "";
        ...
        private bool CheckValidity_EmptyField()
        {
            ...
            if (!clsValidation.Validate_EmptyValue(txtComBranch))
                bStatus = false;
        ...
        private void txtComBranch_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            ...
            {
                txtComBranch.Tag = lstResult[0];
                txtComBranch.Text = lstResult[1];
            }
        }
