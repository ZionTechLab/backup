...
namespace SEACC_POS
{
    ...
    {
        #region variables
        public FormName enmFormName = Digiteq_Logic.FormName.accAccount;
        ...
        {
            return clsCommon_SeaccForm.getAutoGeneratedCode_Advanced(enmFormName, "default");
        }

        public string getAutoGeneratedCode(string sSalesNotypeID)
        {
            return clsCommon_SeaccForm.getAutoGeneratedCode_Advanced(enmFormName, sSalesNotypeID);
        }

    }

    static class clsCommon_SeaccForm
    {
        public static string getAutoGeneratedCode_Advanced(FormName enmFormName, string sSalesNote_ID)
        {
            string sCode = "";
            tbl_securityFunctionMaster_Form oFunction = tbl_securityFunctionMaster_Form.Select((int)enmFormName);
            if (oFunction != null && oFunction.IsAutoGenerate)
            {
                enum_SerialType ST = (enum_SerialType)oFunction.SerialType;
                if (ST == enum_SerialType.BranchWice)
                {
                    #region Branch Wice
                    tbl_securityFunctionMaster_Form_Advanced oAdv = tbl_securityFunctionMaster_Form_Advanced.Select(clsSecurity.CompanyID, clsSecurity.BranchID, oFunction.Function_ID, "default");
                ...
                else if (ST == enum_SerialType.NoteType)
                {
                    ...
                    tbl_securityFunctionMaster_Form_Advanced oAdv = tbl_securityFunctionMaster_Form_Advanced.Select(clsSecurity.CompanyID, "default", oFunction.Function_ID, sSalesNote_ID);
                    if (oAdv != null && oAdv.IsAutoGenerate)
                    {
                        sCode += oAdv.Prefix;
                        sCode += oAdv.Counter.ToString(getWidthFormat(oAdv.Length));
                ...
                else if (ST == enum_SerialType.BranchWise_NoteType)
                {
                    ...
                    tbl_securityFunctionMaster_Form_Advanced oAdv = tbl_securityFunctionMaster_Form_Advanced.Select(clsSecurity.CompanyID, clsSecurity.BranchID, oFunction.Function_ID, sSalesNote_ID);
                    if (oAdv != null && oAdv.IsAutoGenerate)
                    {
                        sCode += oAdv.Prefix;
                        sCode += oAdv.Counter.ToString(getWidthFormat(oAdv.Length));
                ...
                {
                    #region Standerd
                    if (oFunction.Prefix1 != null && oFunction.Prefix1.Length > 0)
                        sCode += oFunction.Prefix1;

                    sCode += oFunction.Counter.ToString(getWidthFormat(oFunction.Length));
                    oFunction.Counter++;
                    oFunction.Update();
