    ...
    public partial class frmMain : Form
    {
        ...
        private void btnModuleButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string sExeLocation = clsGenaralName_Login.getLocation_ModuleExe(iModule_index);
        ...
        private void btnModuleButton_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right && clsSecurity_Login.UserIDLoged == "digiteq")
            {
                Clipboard.SetText(sDB_Con + " " +
                    ...
                    sComBranchID + " " +
                    sServer + " " +
                    sDomain+" "+clsSecurity_Login.color.Name
                    );
            }
        }
        ...
        private void SetInitializeConfigs()
        {
            try
            {
                sDB_Con = clsSecurity_Login.encryptPassword(DBHandling.DBConnection);
                sTerminal = clsSecurity_Login.encryptPassword(clsSecurity_Login.TerminalID);
                sUserID = clsSecurity_Login.encryptPassword(clsSecurity_Login.UserIDLoged);
                sSession_Index = clsSecurity_Login.encryptPassword(sSession_Index);
                sComID = clsSecurity_Login.encryptPassword(clsSecurity_Login.CompanyID);
                sComBranchID = clsSecurity_Login.encryptPassword(clsSecurity_Login.CompanyBranchID);
                sServer = clsSecurity_Login.encryptPassword(clsSecurity_Login.Server);
                sDomain = clsSecurity_Login.encryptPassword(clsSecurity_Login.Domain);
            }
            catch (Exception ex)
            {
                clsValidate_Login.WriteErrorLog(ex.Message, "SetInitializeConfigs");
                SEACCException.Show(ex);
            }
        }
        #endregion
        ...
        private void SetUser_Details(string sUserID)
        {
            ...
            if (oMasUser != null)
            {
                ...
                ucUserIndicator.UserID = clsGenaralName_Login.getName_Group(oMasUser.Group_ID);
