...
namespace Digiteq
{
 ...
 ClearFields(); 
            ...
            {
                z2.Visible = false;
                chkUnitPricing.Visible = false;
                chkReverseCalculation.Visible = false;
                chkPrintOriginal.Visible = false;

                dgvDetail.Columns["UnitPrice"].Visible = false;
                dgvDetail.Columns["Amount"].Visible = false;
                dgvDetail.Columns["WeightAvg"].Visible = false;


                SetVisibility_ActionButons(true, false, false, true, true, false, false, true, true);
            }
                                                    ...
                                                    if (CheckStockValidity(detail.ExternalGoodReceivedNote_ID, false))
                                                    {
                                                        if (CheckCancelValidity_WATollarance())
                                                        {
                                                            ...
                                                            foreach (tbl_scsExternalGoodReceivedNote_Detail Olddetail in tbl_scsExternalGoodReceivedNote_Detail.SelectAllByExternalGoodReceivedNote_ID(txtGRNID.Text.Trim()))
                                                            {
                                                                ...
                                                                //    clsHelpMethods_Local.UpdateStoreStock(iFormID, Olddetail.ExternalGoodReceivedNote_ID, detail.ExternalGoodReceivedNoteDate, Olddetail.Item_ID, "0", txtStoreID.Tag.ToString(), Olddetail.Qty, Olddetail.Weight, Olddetail.TatalAmount, true, true, true, ref dWeightedAverageCostPrice);
                                                        ...
                                                        var responce=    oData.Delete_InventoryTxn(iFormID, txtGRNID.Text.Trim());
                                                            if (!responce.IsSuccess)
                                                            {
                                                                clsValidate.WriteErrorLog(txtGRNID.Text.Trim() + " - " + responce.OutMsg, iFormID, null);
                                                            }
                                                            //   clsHelpMethods.Delete_Inventory(iFormID, 0, txtGRNID.Text.Trim());
                                                        
                                                            MessageBox.Show(clsFormatter.GetMessageFrom(MessageType.DeleteDone), clsFormatter.GetMessageCaption(), MessageBoxButtons.OK, MessageBoxIcon.Information);
                                                            email.createEmail_GRN(txtGRNID.Text.Trim(), enum_Alerts.Good_RecivedNote_Cancel);
                                                            ClearFields();
                                                        }
                                                    }
                                                }
                                    ...
                                    if (!oldRecord.IsChecked || (oldRecord.IsChecked && clsSecurity.PermissionToApproved(clsSecurity.UserIDLoged, iFormID)))
                                    {
                                        ...
                                        //    List<tbl_scsInventoryTxnDetail> oListInventory = new List<tbl_scsInventoryTxnDetail>();
...

                                            {
                                                ...
                                                {
                                                    //  clsHelpMethods_Local.UpdateStoreStock(oUpdatedRecord.Item_ID, txtStoreID.Tag.ToString(), oUpdatedRecord.Qty, oUpdatedRecord.Weight, true, true);
                                                }
                                                ...
                                                foreach (tbl_scsExternalGoodReceivedNote_DetailBreakdown oUpdatedRecord in tbl_scsExternalGoodReceivedNote_DetailBreakdown.SelectAllByExternalGoodReceivedNote_ID(txtGRNID.Text.Trim()))
                                                {
                                                    //   clsHelpMethods_Local.UpdateStoreStock(oUpdatedRecord.Item_ID, txtStoreID.Tag.ToString(), oUpdatedRecord.Qty, oUpdatedRecord.Weight, false, true);
                                                }
                                            ...
                                            {
                                                decimal dWeightedAverageCostPrice = 0;
                                              //  clsHelpMethods_Local.UpdateStoreStock(iFormID, oUpdatedRecord.ExternalGoodReceivedNote_ID, oldRecord.ExternalGoodReceivedNoteDate, oUpdatedRecord.Item_ID, "0", txtStoreID.Tag.ToString(), oUpdatedRecord.Qty, oUpdatedRecord.Weight, oUpdatedRecord.TatalAmount, true, true, true, ref dWeightedAverageCostPrice);
                                            ...
                                            var responce = oData.Update_InventoryTxn(iFormID,txtGRNID.Text.Trim(), IsUpdate);
                                            if (!responce.IsSuccess)
                                            {
                                                clsValidate.WriteErrorLog(txtGRNID.Text.Trim() + " - " + responce.OutMsg, iFormID, null);
                                            ...
                                            {
                                                decimal dWeightedAverageCostPrice = 0;
                                             //   clsHelpMethods_Local.UpdateStoreStock(iFormID, EGRN.ExternalGoodReceivedNote_ID, EGRN.ExternalGoodReceivedNoteDate, oUpdatedRecord.Item_ID, "0", txtStoreID.Tag.ToString(), oUpdatedRecord.Qty, oUpdatedRecord.Weight, oUpdatedRecord.TatalAmount, false, true, true, ref dWeightedAverageCostPrice);
                                                oUpdatedRecord.WeightedAvgCost = dWeightedAverageCostPrice;
                                                oUpdatedRecord.Update();
                                            }
                    ...
                    else
                    {
                        ...
                        if (clsValidate.CheckValidity_TransactionCodeLength(txtGRNID.Text)) //if (txtGRNID.Text.Trim().Length > 0)
                        {
                            ...
                            foreach (DataGridViewRow row in dgvDetail.Rows)
                            {
                                ...
                                decimal dWeightedAverageCostPrice = 0;
                             //   clsHelpMethods_Local.UpdateStoreStock(iFormID, EGRN.ExternalGoodReceivedNote_ID, EGRN.ExternalGoodReceivedNoteDate, sItemCode, "0", txtStoreID.Tag.ToString(), dQuantity, dWeight, EGRNdetail.TatalAmount, false, true, true, ref dWeightedAverageCostPrice);
                            ...
                            email.createEmail_GRN(txtGRNID.Text.Trim(), enum_Alerts.Good_RecivedNote_Created);
                        }
                        //else
        ...
        {
            PrintDataset(false);
            email.createEmail_GRN(txtGRNID.Text.Trim(), enum_Alerts.Good_RecivedNote_Print);
        }
        #endregion
            ...
            try
            {
                if (sID.Length > 0)
                {
                    ...
                    if (detail != null)
                    {
                        ...
                        //Fill Process Flow
                        clsHelpMethods_Local.SetProcessFlow_Stock_External(detail.IssuedRefNo_ID, txtFlowSR, txtFlowPO, txtFlowGRN, txtFlowPRN);
        ...
        private void FillTaxDetailByPO(string sPurchaseOrderID)
        {
            try
            {
                ...
                if (detail != null)
                {
                    ...
                    txtPercentageOtherTax.Text = clsFormatter.FormatToNumberWithTwoDecimalPlaces(detail.OtherTaxPercentage);
                    txtPercentageVat.Text = clsFormatter.FormatToNumberWithTwoDecimalPlaces(detail.VatPercentage);
                    txtSubTotal.Text = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods_Local.getDisplayPrice(detail.SubTotal, detail.ForexRate));
                    txtDiscount.Text = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods_Local.getDisplayPrice(detail.DiscountTotal, detail.ForexRate));
        ...
        private void RefreshGridByItemID(string sItemrID)
        {
            ...
            {
                int iRow;
                tbl_genItemMaster oItem = tbl_genItemMaster.Select(sItemrID);
                tbl_genItemMaster_Pricing oPriceing = tbl_genItemMaster_Pricing.Select(sItemrID);
                if (oItem != null && oPriceing != null)
                {
                    if (!(oItem.IsFinishGood || oItem.IsRawMeterial))
                    {
                        MessageBox.Show("Selected item is not a purchace item", clsFormatter.GetMessageCaption(), MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                    }
                    ...
                    Fill_Datagrid(iRow, maxLineNo + 1, oItem.Item_ID, txtItemSubCategory.Tag.ToString(), txtItemSubCategory.Text.Trim(), txtItemSerialNo.Tag.ToString(), txtItemSerialNo.Text.Trim(),
                        "default", "default", "", oItem.IsTIEPItem, 0, oPriceing.CostPrice1, oPriceing.SellingPrice6, 0, oPriceing.WeightedAverageCostPrice, 0, 0, oItem.Description, dExRate, clsConfig.Font_Grid_Active);
        ...
        private bool CheckValidity_Posting()
        {
            ...
            if (clsConfig.bAutoPostingEnable)
            {
                ...
                var responce = oData.validate_ItemArray(Items);
                if (!responce.IsSuccess)
                {
        ...
        private bool CheckValidityQty(bool IsUpdate)
        {
            ...
            try
            {
                ...
                if (clsConfig.isEnable_QuantityExceedPercentageLock_GRN)
                {
                    ...
                    foreach (DataGridViewRow row in dgvDetail.Rows)
                    {
                        string sPurchaceOrder = clsValidate.ValidateGridValue(dgvDetail, "POID", row.Index, "");
                        string sItemCode = clsValidate.ValidateGridValue(dgvDetail, "ItemCode", row.Index, "");
                        decimal dQty = clsValidate.ValidateGridValue(dgvDetail, "Quantity", row.Index, decimal.Parse("0.00"));
                        decimal dWeight = clsValidate.ValidateGridValue(dgvDetail, "Weight", row.Index, decimal.Parse("0.00"));
                        string sItemSubCategoryID1 = clsValidate.ValidateGridTag(dgvDetail, "ItemSubCategoryID1", row.Index, "default");
                        string sItemSubCategoryID2 = clsValidate.ValidateGridTag(dgvDetail, "ItemSubCategoryID2", row.Index, "default");
                        ...
                        string sPOID = clsValidate.ValidateGridValue(dgvDetail, "POID", row.Index, "default");
                        string sPRNID = clsValidate.ValidateGridValue(dgvDetail, "PRNID", row.Index, "default");
                        string sBatch = clsValidate.ValidateGridValue(dgvDetail, "Batch", row.Index, "");
                        string sUom = clsValidate.ValidateGridValue(dgvDetail, "UOM", row.Index, "default");
                        ...
                        if (IsUpdate)
                        {
                            ...
                            if (chkUnitPricing.Checked)  // Qty
                            {
                                ...
                                foreach (tbl_scsExternalGoodReceivedNote_Detail oldPrnDetail in oldPrnDetails)
                                {
                                    ...
                                    {
                                        decimal dDeliveryQty = (oldPrnDetail.QtySettle - dOldPOQty) + dQty;
                                        if (dDeliveryQty > oldPrnDetail.Qty) //qty is exceeding the order qty
                                        {
                                            decimal dMaxValue = ((dExceedPacentage + 100) / 100) * oldPrnDetail.Qty;
                        ...
                        else // insert
                        {
                            ...
                            if (chkUnitPricing.Checked)  // Qty
                            {
                                ...
                                foreach (tbl_scsPurchaseOrder_Detail oldPoDetail in oldPoDetails)
                                {
                                    if (oldPoDetail.Item_ID == sItemCode && oldPoDetail.ItemSubCategory_ID == sItemSubCategoryID1 && oldPoDetail.ItemSubCategory2_ID == sItemSubCategoryID2 && oldPoDetail.ItemSerialNo == sItemSerialNo1 && oldPoDetail.ItemSerialNo2 == sItemSerialNo2)
                                    {
                                        decimal dDeliveryQty = dQty + oldPoDetail.QtySettle;
                    ...
                    try
                    {
                        ...
                        if (detail != null && detail.ExternalGoodReceivedNote_ID != "default")
                        {
                            ...
                            if (bApprovalDone && bCheckDone)
                            {
                                ...
                                #region Parameter
                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("ReportTitle", "Good Received Note", true);
                                ...
                                string s_Path = clsHelpMethods_Local.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_GoodsReceivedNote));
                            ...
                            if (msgResult == DialogResult.Yes)
                            {
                                ...
                                if (frmSetApproved.bChecked)
                                {
                                    ...
                                    if (IsUpdate)
                                    {
                                        ...
                                        var responce = oData.Approve_GRN( txtGRNID.Text.Trim(), frmSetApproved.sApprovedUserID);
                                        if (!responce.IsSuccess)
                                        {
                                            clsValidate.WriteErrorLog(txtGRNID.Text.Trim() + " - " + responce.OutMsg, iFormID, null);
                                            MessageBox.Show(responce.OutMsg);
                                        ...
                                        {
                                            clsMethods_GL.PostTransaction_APN(txtGRNID.Text.Trim());
                                        }
