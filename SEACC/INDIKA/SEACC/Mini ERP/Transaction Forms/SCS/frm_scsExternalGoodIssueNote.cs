                                        ...
                                        if (CheckSupplierSaveValidity())
                                        {
                                            ...
                                            if (msgResult == DialogResult.Yes)
                                            {
                                                ...
                                                foreach (tbl_scsExternalGoodIssueNote_Detail Olddetail in tbl_scsExternalGoodIssueNote_Detail.SelectAllByExternalGoodIssueNote_ID(txtGINID.Text.Trim()))
                                                {
                                                  ...
                                                  //      clsHelpMethods_Local.UpdateStoreStock(iFormID, Olddetail.ExternalGoodIssueNote_ID, detail.ExternalGoodIssueNoteDate, Olddetail.Item_ID, "0", txtStoreID.Tag.ToString(), Olddetail.Qty, Olddetail.Weight, Olddetail.TatalAmount, true, false, false, ref dWeightedAverageCostPrice);
                                                ...
                                                var responce = oData.Delete_InventoryTxn(iFormID, txtGINID.Text.Trim());
                                                if (!responce.IsSuccess)
                                                {
                                                    clsValidate.WriteErrorLog(txtGINID.Text.Trim() + " - " + responce.OutMsg, iFormID, null);
                                                }
                                                        ...
                                                        if (!oldRecord.IsLocked && !oldRecord.IsApproved && !oldRecord.IsFinished && !oldRecord.IsDeleted)
                                                        {
                                                            if (!oldRecord.IsChecked ||  (oldRecord.IsChecked && clsSecurity.PermissionToApproved(clsSecurity.UserIDLoged, iFormID)))
                                                            {
                                                             ...
                                                             //       List<tbl_scsInventoryTxnDetail> oListInventory = new List<tbl_scsInventoryTxnDetail>();
                                                                    ...
                                                                    var responce = oData.Update_InventoryTxn(iFormID, txtGINID.Text.Trim());
                                                                    if (!responce.IsSuccess)
                                                                    {
                                                                        clsValidate.WriteErrorLog(txtGINID.Text.Trim() + " - " + responce.OutMsg, iFormID, null);
                                                                    }
                                                    ...
                                                    if (clsValidate.CheckValidity_TransactionCodeLength(txtGINID.Text)) //if (txtGINID.Text.Trim().Length > 0)
                                                    {
                                                      //  List<tbl_scsInventoryTxnDetail> oListInventory = new List<tbl_scsInventoryTxnDetail>();
                                                        ...
                                                        foreach (DataGridViewRow row in dgvDetail.Rows)
                                                        {
                                                            ...
                                                            #region Update Store Stock
                                                            decimal dWeightedAverageCostPrice = 0;
                                                       //     decimal dCostFifo = clsHelpMethods_Local.UpdateStoreStock(iFormID, EGIN.ExternalGoodIssueNote_ID, EGIN.ExternalGoodIssueNoteDate, EGINdetail.Item_ID, "0", txtStoreID.Tag.ToString(), EGINdetail.Qty, EGINdetail.Weight, EGINdetail.TatalAmount, false, false, false, ref dWeightedAverageCostPrice);
            ...
            try
            {
                ...
                foreach (DataRow row in dt_ItemGrouped.Rows)
                {
                    ...
                    if (!clsHelpMethods_Local.IsNonInventoryItem(sItemCode))
                    {
                        ...
                        {
                            if ((clsConfig.bStockValidateQty_eGIN || clsConfig.bStockValidateWeight_eGIN) && !clsHelpMethods_Local.IsNonInventoryItem(sItemCode) && (clsConfig.sSoftwareModel.Trim() != SoftwareModel_Sales.akt.ToString()))
                            {
                    ...
                    if (oGIN != null && oGIN.ExternalGoodIssueNote_ID != "default")
                    {
                        ...
                        if (bZeroQtyValidate)
                        {
                            ...
                            if (clsConfig.bDataSetActive_GIN)
                            {
                                ...
                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("IsSample", sSample, true);
                                ...
                                string sGetRptPath = clsHelpMethods_Local.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_eGIN));
