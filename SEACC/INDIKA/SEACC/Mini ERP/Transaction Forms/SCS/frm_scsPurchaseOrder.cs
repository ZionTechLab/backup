...
namespace Digiteq
{
    public partial class frm_scsPurchaseOrder : SEACC_Form
    {
        ...
        {
            // sFormConfigCode = clsAutocode.getFormConfigCode(FormName.scsPOSupplier);
            //iFormID = clsSecurity.getFormID(FormName.scsPOSupplier);
        ...
        {
            //format Form
            //clsFormatter.setFormatForm(this, "Purchase Order [PO]", 4, iFormID);
            ...
            if (CheckValidity())
            {
                ...
                try
                {
                                    ...
                                    )
                                {
                                         ...
                                         clsSecurity.PermissionToApproved(clsSecurity.UserIDLoged, iFormID)))
                                    {
                                        if (clsValidate.CheckValidity_TransactionCodeLength(txtPOID.Text))
                                        {
                                            ...
                                            List<tbl_scsPurchaseOrder_Detail> oldDetails =
                                                tbl_scsPurchaseOrder_Detail.SelectAllByPurchaseOrder_ID(
                                                    txtPOID.Text.Trim());
                                            foreach (tbl_scsPurchaseOrder_Detail oldDetail in oldDetails)
                                            {
                                                ...
                                                foreach (DataGridViewRow row in dgvDetail.Rows)
                                                {
                                                    ...
                                                    dQty = clsValidate.ValidateGridValue(dgvDetail, "Quantity",
                                                    ...
                                                    dWeight = clsValidate.ValidateGridValue(dgvDetail, "Weight",
                                                        row.Index, decimal.Parse("0.00"));
                                                    dWeidhtPrice = clsValidate.ValidateGridTag(dgvDetail, "WeightPrice",
                                                        row.Index, decimal.Parse("0.00"));
                                                    dMeter = clsValidate.ValidateGridValue(dgvDetail, "Meter",
                                                        row.Index, decimal.Parse("0.00"));
                                                    ...
                                                    if (oldDetail.PurchaseOrder_ID == txtPOID.Text.Trim() &&
                                                ...
                                                if (bHasItemInDB)
                                                {
                                                    ...
                                                    if (txtPurchaseRequisitionID.Tag != null &&
                                                        txtPurchaseRequisitionID.Tag.ToString().Trim() != "default")
                                                    {
                                                        foreach (tbl_scsPurchaseRequisition_Detail oPR in
                                                            tbl_scsPurchaseRequisition_Detail
                                                                .SelectAllByPurchaseRequisitionNote_ID(
                                                                    txtPurchaseRequisitionID.Tag.ToString().Trim())
                                                                .Where(p => p.Item_ID == sItemCode))
                                                        {
                                                            oPR.QtySettle = (oldDetail.Qty);
                                                            oPR.WeightSettle = (oldDetail.Weight);
                                                            oPR.Update();
                                                            clsProcessMethods.SetSettle_PurchaseRequisition(
                                                                txtPurchaseRequisitionID.Tag.ToString().Trim());
                                                        }
                                                    }
                                                    ...
                                                    if (txtQuotationID.Tag != null &&
                                                        txtQuotationID.Tag.ToString().Trim() != "default")
                                                    {
                                                        //tbl_sasQuotation_Detail inqItem = tbl_sasQuotation_Detail.Select(txtQuotationID.Tag.ToString().Trim(), sItemCode, sItemSubCategoryID1, sItemSubCategoryID2, sItemSerialNo1, sItemSerialNo2);
                                                        tbl_sasQuotation_Detail inqItem = tbl_sasQuotation_Detail
                                                            .SelectAllByQuotation_ID(txtQuotationID.Tag.ToString())
                                                            .Where(r => r.Item_ID == sItemCode &&
                                                                        r.ItemSubCategory_ID == sItemSubCategoryID1 &&
                                                                        r.ItemSubCategory2_ID == sItemSubCategoryID2 &&
                                                                        r.ItemSerialNo == sItemSerialNo1 &&
                                                                        r.ItemSerialNo2 == sItemSerialNo2)
                                                            .FirstOrDefault();
                                                        if (inqItem != null)
                                                        {
                                                            if (chkUnitPricing.Checked)
                                                                inqItem.QtySettle_Invoice =
                                                                    (inqItem.QtySettle_Invoice - oldDetail.Qty) + dQty;
                                                            else
                                                                inqItem.WeightSettle_Invoice =
                                                                    (inqItem.WeightSettle_Invoice - oldDetail.Weight) +
                                                                    dWeight;
                                                            inqItem.Update();
                                                            clsProcessMethods.SetSettle_QuotationFrom_Invoice(
                                                                txtQuotationID.Tag.ToString().Trim(), chkUnitPricing);
                                                        }
                                                    }
                                                ...
                                                else //Delete the old item by user so this else part use to delete that recode from database
                                                {
                                            ...
                                            foreach (DataGridViewRow row in dgvDetail.Rows)
                                            {
                                                ...
                                                iLineNo = clsValidate.ValidateGridValue(dgvDetail, "LineNo", row.Index,
                                                    int.Parse("0"));
                                                sItemCode = clsValidate.ValidateGridValue(dgvDetail, "ItemCode",
                                                    row.Index, "");
                                                sItemSubCategoryID1 = clsValidate.ValidateGridTag(dgvDetail,
                                                ...
                                                sItemSerialNo2 = clsValidate.ValidateGridValue(dgvDetail,
                                                    "ItemSerialNo2", row.Index, "0");
                                                dQty = clsValidate.ValidateGridValue(dgvDetail, "Quantity", row.Index,
                                                    decimal.Parse("0.00"));
                                                dUnitPrice = clsValidate.ValidateGridTag(dgvDetail, "UnitPrice",
                                                    row.Index, decimal.Parse("0.00"));
                                                ...
                                                dWeidhtPrice = clsValidate.ValidateGridTag(dgvDetail, "WeightPrice",
                                                    row.Index, decimal.Parse("0.00"));
                                                dMeter = clsValidate.ValidateGridValue(dgvDetail, "Meter", row.Index,
                                                    decimal.Parse("0.00"));
                                                ...
                                                {
                                                    tbl_scsPurchaseOrder_Detail poDetail =
                                                        new tbl_scsPurchaseOrder_Detail(iLineNo, txtPOID.Text.Trim(),
                                                            sItemCode, sItemSubCategoryID1, sItemSubCategoryID2,
                                                            sItemSerialNo1, sItemSerialNo2,
                                                            dQty, 0, dWeight, 0, dMeter, dWeidhtPrice, dUnitPrice, 0, 0,
                                                            dAmount, sRemarks);
                                                    poDetail.Insert();
                                                }
                                                ...
                                                if (txtQuotationID.Tag != null &&
                                                    txtQuotationID.Tag.ToString().Trim() != "default")
                                                {
                                                    //tbl_sasQuotation_Detail inqItem = tbl_sasQuotation_Detail.Select(txtQuotationID.Tag.ToString(), sItemCode, sItemSubCategoryID1, sItemSubCategoryID2, sItemSerialNo1, sItemSerialNo2);
                                                    tbl_sasQuotation_Detail inqItem = tbl_sasQuotation_Detail
                                                        .SelectAllByQuotation_ID(txtQuotationID.Tag.ToString()).Where(
                                                            r => r.Item_ID == sItemCode &&
                                                                 r.ItemSubCategory_ID == sItemSubCategoryID1 &&
                                                                 r.ItemSubCategory2_ID == sItemSubCategoryID2 &&
                                                                 r.ItemSerialNo == sItemSerialNo1 &&
                                                                 r.ItemSerialNo2 == sItemSerialNo2).FirstOrDefault();
                                                    if (inqItem != null)
                                                    {
                                                        if (chkUnitPricing.Checked)
                                                            inqItem.QtySettle_Invoice =
                                                                inqItem.QtySettle_Invoice + dQty;
                                                        else
                                                            inqItem.WeightSettle_Invoice =
                                                                inqItem.WeightSettle_Invoice + dWeight;
                                                        inqItem.Update();
                                                        clsProcessMethods.SetSettle_QuotationFrom_Invoice(
                                                            txtQuotationID.Tag.ToString(), chkUnitPricing);
                                                    }
                                                }
                                            ...
                                            tbl_scsPurchaseOrder po = new tbl_scsPurchaseOrder(
                                                oldRecord.PurchaseOrder_ID, dtpPoDate.Value,
                                                txtIssuedRefNo.Tag.ToString(), txtPayMode.Tag.ToString(),
                                                txtPurchaseRequisitionID.Tag.ToString(), txtQuotationID.Tag.ToString(),
                                                txtStockNoteType.Tag.ToString(), txtRemark.Text.Trim(),
                                                txtDeliveryAddress.Text.Trim(), dtpDueDate.Value,
                                                txtDeliveryTerms.Text.Trim(), txtOrderBy.Text.Trim(), "", "",
                                                dAdvanceAmount, dOnDelivery, dBalanceDays, dForexRate,
                                                txtCurrency.Tag.ToString(), txtSupplierID.Tag.ToString(),
                                                txtQuotationNo.Text.Trim(), oldRecord.GlPosting_ID,
                                                txtCostCenter.Tag.ToString(), oldRecord.PostingStatus_ID,
                                                ...
                                                getSavePrice(decimal.Parse(txtSubTotal.Tag.ToString().Trim())),
                                                getSavePrice(decimal.Parse(txtDiscount.Tag.ToString().Trim())),
                                                getSavePrice(decimal.Parse(txtNBT.Tag.ToString().Trim())),
                                                getSavePrice(decimal.Parse(txtVat.Tag.ToString().Trim())),
                                                getSavePrice(decimal.Parse(txtOtherTax.Tag.ToString().Trim())),
                                                getSavePrice(decimal.Parse(txtGrandTotal.Text.Trim())),
                                                ...
                                                glbApprovedDate, bHasChecked, bHasApproved, oldRecord.IsFinished,
                                                oldRecord.IsDeleted, oldRecord.IsLocked,
                                                oldRecord.SeattleAmount, oldRecord.IsSeattled, oldRecord.PrintCount,
                                                oldRecord.IsWeightCalculation, clsHelpMethods.isTaxActiveNote(txtVat),
                                                clsHelpMethods.isTaxActiveNote(txtOtherTax), chkTIEP.Checked,
                                                oldRecord.CompanyID, oldRecord.CompanyBranch_ID);
                                            po.Update();

                                            #endregion

                                            //Attachments.Insert(iFormID, oldRecord.PurchaseOrder_ID);
                                            //Attachments.Remove(iFormID, oldRecord.PurchaseOrder_ID);
                    ...
                    else
                    {
                        ...
                        if (clsValidate.CheckValidity_TransactionCodeLength(txtPOID.Text)) //if (txtPOID.Text.Trim().Length > 0)
                        {
                            ...
                            if (oIGIN == null)
                            {
                                ...
                                foreach (DataGridViewRow row in dgvDetail.Rows)
                                {
                                    ...
                                    if (sItemCode.Trim().Length > 0)
                                    {
                                        //tbl_scsPurchaseOrder_Detail poDetail = new tbl_scsPurchaseOrder_Detail(clsHelpMethods_Local.GetMaxzimumLineNoPurchaseOrder(txtPOID.Text.Trim()), txtPOID.Text.Trim(), sItemCode, sItemSubCategoryID1, sItemSubCategoryID2, sItemSerialNo1, sItemSerialNo2,
                        ...
                        //else
                        //{
                        //    MessageBox.Show("PO " + clsFormatter.GetMessageFrom(MessageType.IDIsEmpty), clsFormatter.GetMessageCaption(), MessageBoxButtons.OK, MessageBoxIcon.Information);
                        //}
                        #endregion
                    }
                }
            ...
            try
            {
                ...
                foreach (tbl_scsPurchaseRequisition_Detail detail in tbl_scsPurchaseRequisition_Detail.SelectAllByPurchaseRequisitionNote_ID(sPR_ID).OrderBy(p => p.Line_No))
                {
                    ...
                    if (item != null)
                    {
                        ...
                        //Fill_Datagrid(iRow, detail.Item_ID, detail.ItemSubCategory_ID, detail.ItemSubCategory2_ID, detail.ItemSerialNo, detail.ItemSerialNo2, dQty, 0, dWeight, 0, 0, item.WeightedAverageCostPrice, 0, "", dExRate);
                        Fill_Datagrid(iRow, detail.Line_No, detail.Item_ID, detail.ItemSubCategory_ID, detail.ItemSubCategory2_ID, detail.ItemSerialNo, detail.ItemSerialNo2, dQty, item.CostPrice1, dWeight, 0, 0, item.WeightedAverageCostPrice, 0, "", dExRate);
                    }
                }
            }
            catch (Exception ex)
        ...
        private bool CheckValidity_EmptyField()
        {
            ...
            {
                //if (clsValidate.ValidateTextBox_EmptyValue(txtIssuedRefNo, "Tracking No"))
                //{
        ...
        private bool CheckValidity_CostPrice()
        {
            ...
            try
            {
                ...
                //if (clsConfig.isEnable_CostPriceExceed_WAPercentage)
                //{
                //    if (decimal.Parse(clsConfig.sWeightedAvg_Percentage) > 0)
                //    {
                foreach (DataGridViewRow row in dgvDetail.Rows)
                {
                ...
                if (bShowMessage == true)
                {
                    ...
                    //MessageBox.Show(strMessage + sItem, clsFormatter.GetMessageCaption(), MessageBoxButtons.YesNo, MessageBoxIcon.Information);
        ...
        private void txtSrID_DoubleClick(object sender, EventArgs e)
        {
            ...
            Search_PurchaseRequestionNote();
            //   else
            //       MessageBox.Show("Please select a Supplier..", clsFormatter.GetMessageCaption(), MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private void Search_SupplierID()
        {
            try
            {
                ...
                //  }
                //  else
                //      MessageBox.Show("Please remove items to change Supplier..", clsFormatter.GetMessageCaption(), MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            decimal Meter, decimal WeightAvg, decimal Amount, string sRemars, decimal dExRate)
        {
            try
            {
                clsHelpMethods_Local.AddMultipleItems_Grid(dgvDetail, ItemID, ref iRow, ref iLineNo, ref Quantity, ref UnitPrice, ref Weight, ref WeightAvg);
                ...
                else
                {
                    ...
                    dgvDetail["Amount", iRow].Tag = Amount;
                    dgvDetail["Meter", iRow].Value = Meter.ToString();//add by thilina
                }
                ...
                if (txtPOID.TextLength > 0 && txtPOID.Text != "<Auto Generate>")
                {
                    ...
                    if (order != null)
                    {
                        ...
                        if (bApprovalDone && bCheckingDone)
                        {
                            ...
                            if (clsConfig.bDateSetActive_PurchaseOrderPrint) //This is needed to true for LWC *****************************
                            {
                                #region DataSet
                                ...
                                if (oSupplier != null && oCurrency != null)
                                    glb_dtsSasPurchaseOrder.dt_scsPurchaseOrder.Adddt_scsPurchaseOrderRow(order.PurchaseOrder_ID, order.PurchaseOrderDate,
                                        ...
                                        getDisplayUnitPrice(order.SubTotal, order.ForexRate), getDisplayUnitPrice(order.DiscountTotal, order.ForexRate),
                                        getDisplayUnitPrice(order.VatTotal, order.ForexRate), getDisplayUnitPrice(order.GrandTotal, order.ForexRate),
                                        oSupplier.Supplier_ID, oSupplier.SupplierName, oSupplier.AddressRegister, oSupplier.Telephone, oSupplier.Fax, oSupplier.Remark, oCurrency.CurrencyCode,
                                        ...
                                        order.OrderdBy, order.DueDate, order.QuotaionNo, order.PurchaseRequisitionNote_ID, oRef.IssuedRefNo, order.IsDeleted, order.CreateUser_ID, order.ApprovedUser_ID, order.DateCreate, order.DateApproved, 0, order.NbtTotal, order.OtherTaxTotal);

                                #endregion

                                #region Purchase Order Detail
                                List<tbl_scsPurchaseOrder_Detail> oOrderDetails = tbl_scsPurchaseOrder_Detail.SelectAllByPurchaseOrder_ID(order.PurchaseOrder_ID);
                                foreach (tbl_scsPurchaseOrder_Detail Detail1 in oOrderDetails.OrderBy(p => p.Line_No))
                                {
                                    ...
                                    if (oItem != null && oUOM != null)
                                    {
                                        glb_dtsSasPurchaseOrder.dt_scsPurchaseOrder_Detail.Adddt_scsPurchaseOrder_DetailRow(Detail1.PurchaseOrder_ID, Detail1.Qty,
                                            oUOM.UomCode, getDisplayUnitPrice(Detail1.UnitPrice, order.ForexRate), getDisplayUnitPrice(Detail1.TatalAmount, order.ForexRate),
                                ...
                                string sGetRptPath = clsHelpMethods_Local.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_PurchaseOrder));
