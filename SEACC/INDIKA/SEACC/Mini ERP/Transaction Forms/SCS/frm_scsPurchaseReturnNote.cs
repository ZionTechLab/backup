                                                        ...
                                                        if (CheckCancelValidity_WATollarance())
                                                        {
                                                            ...
                                                            foreach (tbl_scsPurchaseReturnedNote_Detail Olddetail in tbl_scsPurchaseReturnedNote_Detail.SelectAllByPurchaseReturnedNote_ID(txtPRNID.Text.Trim()))
                                                            {
                                                                if (Olddetail.Item_ID != null)
                                                                {
                                                                    ...
                                                                    // clsHelpMethods_Local.UpdateStoreStock(Olddetail.Item_ID, txtStoreID.Tag.ToString(), Olddetail.Qty, Olddetail.Weight, true, false);
                                                                ...
                                                                //    clsHelpMethods_Local.UpdateStoreStock(iFormID, Olddetail.PurchaseReturnedNote_ID, detail.PurchaseReturnedNoteDate, Olddetail.Item_ID, "0", txtStoreID.Tag.ToString(), Olddetail.Qty, Olddetail.Weight, Olddetail.TatalAmount, true, false, true, ref dWeightedAverageCostPrice);
                                                            ...
                                                            var responce = oData.Delete_InventoryTxn(iFormID, txtPRNID.Text.Trim());
                                                            if (!responce.IsSuccess)
                                                            {
                                                                clsValidate.WriteErrorLog(txtPRNID.Text.Trim() + " - " + responce.OutMsg, iFormID, null);
                                        ...
                                        if (clsValidate.CheckValidity_TransactionCodeLength(txtPRNID.Text))
                                        {
                                         //   List<tbl_scsInventoryTxnDetail> oListInventory = new List<tbl_scsInventoryTxnDetail>();
                                                        ...
                                                        txtPRNID.Text.Trim()))
                                            {
                                                ...
                                                //   clsHelpMethods_Local.UpdateFifo_Stock(iFormID, PRN.PurchaseReturnedNote_ID, PRN.PurchaseReturnedNoteDate, PRN.Store_ID, oUpdatedRecord.Item_ID, oUpdatedRecord.ItemSerialNo, oUpdatedRecord.Qty, oUpdatedRecord.UnitPrice, false, dCostFifo);
                                           ...
                                           //     oUpdatedRecord.Cost_FIFO = dCostFifo;
                                            ...
                                            var responce = oData.Update_InventoryTxn(iFormID, txtPRNID.Text.Trim(),IsUpdate);
                                            if (!responce.IsSuccess)
                                            {
                                                clsValidate.WriteErrorLog(txtPRNID.Text.Trim() + " - " + responce.OutMsg, iFormID, null);
                                                MessageBox.Show(responce.OutMsg);
                        ...
                        if (clsValidate.CheckValidity_TransactionCodeLength(txtPRNID.Text))// if (txtPRNID.Text.Trim().Length > 0)
                        {
                           // List<tbl_scsInventoryTxnDetail> oListInventory = new List<tbl_scsInventoryTxnDetail>();
                            ...
                            foreach (DataGridViewRow row in dgvDetail.Rows)
                            {
                                ...
                                //  decimal dCostFifo = clsHelpMethods_Local.GetFifoCost(sItemCode , dQty);
                                ...
                                #region Update Store Stock
                                decimal dWeightedAverageCostPrice = 0;
                            //    decimal dCostFifo = clsHelpMethods_Local.UpdateStoreStock(iFormID, PRN.PurchaseReturnedNote_ID, PRN.PurchaseReturnedNoteDate, sItemCode, "0", txtStoreID.Tag.ToString(), dQty, dWeight, PRNDetail.TatalAmount, false, false, true, ref dWeightedAverageCostPrice);
                                //   clsHelpMethods_Local.UpdateFifo_Stock(iFormID, PRN.PurchaseReturnedNote_ID, PRN.PurchaseReturnedNoteDate, PRN.Store_ID, PRNDetail.Item_ID, PRNDetail.ItemSerialNo, PRNDetail.Qty, PRNDetail.UnitPrice, false, dCostFifo);
                    ...
                    if (bPermissinOkToPrint)
                    {
                        ...
                        if (detail != null && detail.PurchaseReturnedNote_ID != "default")
                        {
                            ...
                            if (bApprovalDone && bCheckingDone)
                            {
                                ...
                                string s_Path = "";
                                string sGetRptPath = clsHelpMethods_Local.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_PurchaseReturnNote));
                                ...
                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("SupplierAddressRegister", clsGenaralName.getSupplierAddressRegister(detail.Supplier_ID), true);
                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("TrackingNo", txtSupplierRefNo.Text, true);
                                ...
                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("ReportTitle", "Purchase Return Note", true);
                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("TelphoneFax", clsCommon.getSupplerTelephoneAndFax(detail.Supplier_ID), true);
                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("SupplierAddressRegister", clsGenaralName.getSupplierAddressRegister(detail.Supplier_ID), true);
                                ...
                                if (bIsDraft)
                                {
                                    if (!clsConfig.isVisibleCompanyInfoInDraftPrint)
                                    {
                                        ...
                                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("TelphoneFax", "", true);
