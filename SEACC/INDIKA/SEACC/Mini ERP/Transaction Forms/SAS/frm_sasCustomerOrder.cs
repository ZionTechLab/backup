    ...
    public partial class frm_sasCustomerOrder : SEACC_Form
    {
                ...
                try
                {
                                    ...
                                    )
                                {
                                         ...
                                         clsSecurity.PermissionToApproved(clsSecurity.UserIDLoged, iFormID)))
                                    {
                                        if (clsValidate.CheckValidity_TransactionCodeLength(txtCustomerOrderID.Text))
                                        {
                                            ...
                                            if (bDisAmnt)
                                                email.Email_CO_DiscountedItem(txtCustomerOrderID.Text.Trim(),
                    ...
                    else
                    {
                        ...
                        if (clsValidate.CheckValidity_TransactionCodeLength(txtCustomerOrderID.Text)) //if (txtCustomerOrderID.TextLength > 0 && txtCustomerOrderID.Text != "<Auto Generate>")
                        {
                            ...
                            clsHelpMethods_Local.getSavePrice(decimal.Parse(txtNBT.Text.Trim()), txtCurrencyRate), clsHelpMethods_Local.getSavePrice(decimal.Parse(txtVat.Text.Trim()), txtCurrencyRate), clsHelpMethods_Local.getSavePrice(decimal.Parse(txtOtherTax.Text.Trim()), txtCurrencyRate), clsHelpMethods_Local.getSavePrice(decimal.Parse(txtGrandTotal.Text.Trim()), txtCurrencyRate),
                            clsHelpMethods_Local.getSavePrice(decimal.Parse(txtAdvanceAmount.Text.Trim()), txtCurrencyRate), clsHelpMethods_Local.getSavePrice(decimal.Parse(txtSubTotal_Rec.Text.Trim()), txtCurrencyRate), clsHelpMethods_Local.getSavePrice(decimal.Parse(txtGrandTotal_Rec.Text.Trim()), txtCurrencyRate), clsSecurity.UserIDLoged, clsSecurity.UserIDLoged, "default", "default",
                            ...
                            !chkUnitPricing.Checked, 0, chkReverseCalculation.Checked, chkFreeOrder.Checked, clsHelpMethods.isTaxActiveNote(txtVat), clsHelpMethods.isTaxActiveNote(txtOtherTax), txtCustomerBranchID.Tag.ToString(), decimal.Parse(txtComRate.Text.Trim()), ((ComboBoxItem)cmbItemPrice.SelectedItem).Value, clsSecurity.CompanyID, clsSecurity.BranchID, int.Parse(lblRoute.Tag.ToString()));
                                ...
                                email.Email_CO(txtCustomerOrderID.Text.Trim(), enum_Alerts.CustomerOrderCreate);
                                ...
                                if (bDisAmnt)
                                    email.Email_CO_DiscountedItem(txtCustomerOrderID.Text.Trim(), enum_Alerts.CustomerOrderDiscountedItemCreate);
                            }
                            else
        ...
        private void RefreshGridByItemID(string sItemID, string sItemSubCategoryID, string sItemSubCategoryID2, string sSerialNo, string sSerialNo2)
        {
            try
            {
                ...
                if (detail != null && oItemF != null)
                {
                    ...
                    decimal dUnitPrice = new masRouteWiseItemPricingData().GetRouteWisePrice(iRoute,sItemID);       // clsProcessMethods.GetRecommendedUnitPrice_Advance(sItemID, sItemSubCategoryID, sItemSubCategoryID2, sSerialNo, sSerialNo2, txtCustomerID.Tag.ToString());
                    #endregion
            ...
            try
            {
                if (sID.Length > 0)
                {
                    ...
                    if (detail != null)
                    {
                        ...
                        txtInquiryCode.Tag = detail.Inquiry_ID;
                        txtInquiryCode.Text = detail.Inquiry_ID;
            ...
            decimal Width, decimal Height, decimal Gauge, decimal Gusset, decimal Weight, decimal Qty, string sItemStatus, string ItemSubCategoryID, string ItemSubCategoryID2, string SerialNo, string SerialNo2, string Remark, bool bHasSettled, decimal dExRate)
        {
            try
            {
                ...
                //Get Unit Price with Exchange rate to save
                UnitPrice = clsFormatter.RoundDecimalPlaces_UnitPrice(clsHelpMethods_Local.getDisplayPrice(UnitPrice, dExRate));
                                                ...
                                                if (clsMethods_GL.CheckValidity_FinancialYear(dtpCustomerOrderDate.Value.Date))
                                                {
                                                    if (clsSecurity.PermissionToSave(clsSecurity.UserIDLoged, iFormID, IsUpdate))
                                                    {
                                                        if (clsSecurity.permissionToSave_Store(clsSecurity.UserIDLoged, txtStoreID.Tag.ToString(), IsUpdate))
                                                        {
                                                            ...
                                                            {
                                                             //   if (CheckValidity_ReturnedCheques())
                                                              //  {
        ...
        private bool CheckStockValidity()
        {
            ...
            try
            {
                ...
                foreach (DataRow row in dt_ItemGrouped.Rows)
                {
                    ...
                    if (stock != null)
                    {
                        if (sItemStatus.ToLower() == "o")//if the item is old and check stock for more than one time
                        {
                            ...
                            foreach (tbl_sasCustomerOrder_Detail oldDoDetail in oldDoDetails)
                            {
                                ...
                                {
                                    decimal dVeriance = 0;
                                    if (clsConfig.bStockValidateQty_CustomerOrder && !clsHelpMethods_Local.IsNonInventoryItem(oldDoDetail.Item_ID))
                                    {
                                    ...
                                    if (clsConfig.bStockValidateWeight_CustomerOrder && !clsHelpMethods_Local.IsNonInventoryItem(oldDoDetail.Item_ID))
                                    {
                                        ////weight part
                        ...
                        else//first time added item ant have to check stock
                        {
                            ...
                            if (stock.Weight < dWeight && clsConfig.bStockValidateWeight_CustomerOrder && !clsHelpMethods_Local.IsNonInventoryItem(sItemCode))
                            {
                            ...
                            if (stock.Qty < dQty && clsConfig.bStockValidateQty_CustomerOrder && !clsHelpMethods_Local.IsNonInventoryItem(sItemCode))
                            {
                                strMessage += "Item: " + clsGenaralName.getName_Item(sItemCode) + " Required quantity is not currently available in " + clsGenaralName.getName_Store(txtStoreID.Tag.ToString()) + " Stock. You may try this item in another stores or a replacement item.\n";
                                bStatus = false;
                            }
                    ...
                    {
                        if ((clsConfig.bStockValidateQty_CustomerOrder || clsConfig.bStockValidateWeight_CustomerOrder) && !clsHelpMethods_Local.IsNonInventoryItem(sItemCode))
                        {
        ...
        private bool CheckValidity_ReturnedCheques()
        {
            bool bValue = true;
            try
            {
                foreach (tbl_sasInvoice oInvoice in tbl_sasInvoice.SelectAllByCustomer_ID(txtCustomerID.Tag.ToString()).Where(p => p.Invoice_ID != "default" && !p.IsDeleted && !p.IsSeattled && p.GrandTotal > 0 && p.IsReturnedCheque))
                {
                    bValue = false;
                    MessageBox.Show("This customer has unsettled returned cheque/s", clsFormatter.GetMessageCaption(), MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    break;
                }
            }
            catch (Exception ex)
            ...
            try
            {
                if (txtCustomerOrderID.TextLength > 0 && txtCustomerOrderID.Text != "<Auto Generate>")
                {
                    ...
                    try
                    {
                        ...
                        if (clsHelpMethods_Local.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_CustomerOrder), ref sReportTitle_Main, ref sReportTitle_Sub, ref sReportPath))
                        {
                                ...
                                if (oCusOrder != null)
                                {
                                    ...
                                    if (bApprovalDone && bCheckingDone)
                                    {
                                        ...
                                        if (oCustomer != null)
                                        {
                                            ...
                                            glb_dts_sasCustomerOrder.dt_sasCustomerOrder.Adddt_sasCustomerOrderRow(oCusOrder.CustomerOrder_ID, oCusOrder.CustomerOrderDate, oCusOrder.DeliveryDate.Date, sDeliveryAddress, sDeliveryTel, oCustomer.CustomerName, oCustomer.AddressRegister, oCustomer.Telephone, clsGenaralName.getName_BranchCustomer(oCusOrder.Customer_ID, int.Parse(oCusOrder.Branch_ID)), dCreditLimit, Convert.ToInt32(dCreditPeriod), sBranchId, clsGenaralName.getName_Employee(oCusOrder.Employee_ID), oCusOrder.Remark, oCusOrder.Customer_ID, "p_Date",
                                                oCusOrder.GrandTotal, oCusOrder.SubTotal, oCusOrder.DiscountTotal, oCusOrder.NbtTotal, oCusOrder.VatTotal, oCusOrder.OtherTaxTotal, oCusOrder.AdvanceAmount, clsCommon.CurrencyToWord(decimal.Parse(txtGrandTotal.Text)),
                                                ...
                                                oCustomer.CustomerType_ID, sCusType, oCustomer.CustomerCategory_ID, sCusCategory, ((ComboBoxItem)cmbItemPrice.SelectedItem).ToString());
                                            ...
                                            {
                                                glb_dts_sasCustomerOrder.dt_sasCustomerOrderDetail.Adddt_sasCustomerOrderDetailRow(oDetails_CO.CustomerOrder_ID, oDetails_CO.Item_ID, clsGenaralName.getName_Item(oDetails_CO.Item_ID), oDetails_CO.Qty, oDetails_CO.Weight, oDetails_CO.UnitPrice, oDetails_CO.BIsFreeItem, oDetails_CO.DiscountPresentage, oDetails_CO.DiscountAmount, oDetails_CO.Remark, oDetails_CO.TatalAmount, "", 0, 0, 0, clsGenaralName.getName_ItemUOMName(oDetails_CO.Item_ID), oDetails_CO.WeightPrice, oDetails_CO.QtySettle_DeliveryOrder, clsGenaralName.getName_ItemCategorySub(oDetails_CO.ItemSubCategory_ID), clsHelpMethods.GetPLU(oCusOrder.Customer_ID, oDetails_CO.Item_ID));
                                            }

                                            tbl_zCurrency oCurrency = tbl_zCurrency.Select(oCusOrder.Currency_ID);
                                            sExRate = clsFormatter.FormatDecimalPlaces_Price(oCurrency.CurrencyRate);

                                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("isDel", oCusOrder.IsDeleted ? "CANCELLED" : "", true);
