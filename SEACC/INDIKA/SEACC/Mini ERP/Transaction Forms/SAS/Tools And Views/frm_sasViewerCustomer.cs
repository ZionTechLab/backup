    ...
    public partial class frm_sasViewerCustomer : Form
    {
        ...
        private void FillDetails(string sItemCode)
        {
            ...
            if (detail != null)
            {
                ...
                lblOpeningBalance.Text = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods_Local.GetCustomerTotalDues_OpeningBalance(detail.Customer_ID));
                lblRCOutstandings.Text = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods_Local.GetCustomerTotalDues_ReturnedCheque(detail.Customer_ID));
                lblSalseDues.Text = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods_Local.GetCustomerTotalDues_Invoice(detail.Customer_ID));
                lblOverPayments.Text = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods_Local.GetCustomerTotal_UnsettledPayements(detail.Customer_ID));
                lblTotalDues.Text = clsFormatter.FormatToCurrecyWithThousendSep(clsHelpMethods_Local.GetCustomerTotalDues_All(detail.Customer_ID));
                lblTotalBalance.Text = clsFormatter.FormatToCurrecyWithThousendSep(decimal.Parse(lblCreditLimit.Text.Trim()) - clsHelpMethods_Local.GetCustomerTotalDues_All(detail.Customer_ID) - clsHelpMethods_Local.GetCustomerChequesInHand(detail.Customer_ID));
        ...
        private void RefreshGrid_Ageing()
        {
            ...
            lblOverDues.Text = clsFormatter.FormatToCurrecyWithThousendSep((dSalesOutstanding - clsHelpMethods_Local.GetCustomerTotal_UnsettledPayements(glbCustomerID)));
