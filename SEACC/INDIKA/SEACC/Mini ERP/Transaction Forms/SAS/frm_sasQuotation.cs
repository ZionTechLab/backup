                                         ...
                                         clsSecurity.PermissionToApproved(clsSecurity.UserIDLoged, iFormID)))
                                    {
                                        if (clsValidate.CheckValidity_TransactionCodeLength(txtQuotationID.Text))
                                        {
                                            ...
                                            tbl_sasQuotation detail = new tbl_sasQuotation(txtQuotationID.Text.Trim(),
                                                ...
                                                txtCustomerAddress.Text.Trim(), txtQuotationTerms.Tag.ToString(),
                    ...
                    else  //insert records
                    {
                        ...
                        if (clsValidate.CheckValidity_TransactionCodeLength(txtQuotationID.Text)) //if (txtQuotationID.TextLength > 0)
                        {
                            ...
                            tbl_sasQuotation detail = new tbl_sasQuotation(txtQuotationID.Text.Trim(), dtpQuotationDate.Value, txtRemark.Text.Trim(),
                                                    ...
                                                    !chkUnitPricing.Checked, 0, chkReverseCalculation.Checked, chkFreeOrder.Checked, clsHelpMethods.isTaxActiveNote(txtVat), clsHelpMethods.isTaxActiveNote(txtOtherTax), txtBranch.Tag.ToString(), txtCustomerAddress.Text.Trim(), txtQuotationTerms.Tag.ToString(), lblAccountNo.Tag.ToString(), ((ComboBoxItem)cmbItemPrice.SelectedItem).Value);
                            detail.Insert();
                            #endregion
            ...
            try
            {
                if (sID.Length > 0)
                {
                    ...
                    if (detail != null)
                    {
                        ...
                        txtQuotationTerms.Tag = detail.QuotationTerms;
                            ...
                            if (ValidateForDependancies(detail.Quotation_ID))
                            {
                                if (!detail.IsLocked)
                                {
                                    if (!detail.IsDeleted)
                                    {
                                        ...
                                        {
                                            DialogResult msgResult = MessageBox.Show(clsFormatter.GetMessageFrom(MessageType.AskForDelete, " Customer Order : " + detail.Quotation_ID), clsFormatter.GetMessageCaption(), MessageBoxButtons.YesNo, MessageBoxIcon.Stop);
                                            if (msgResult == DialogResult.Yes)
                                            {
                                                //////Update Other Tables 
        ...
        private void Print(bool bIsDraft)
        {
            ...
            if (bTaxTypeSelection_OK)
            {
                ...
                if (false)
                {
                    ...
                    //        string sGetRptPath = clsHelpMethods_Local.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_Quotation));
                    ...
                    //            clsHelpMethods_Local.SetPrinterSetting(clsAutocode.getReportID(enum_ReportName.NP_CustomerQuotation), ref RD);
                ...
                else
                {
                    try
                    {
                        ...
                        string sDuplicate = "";
                        if (clsHelpMethods_Local.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_Quotation), ref sReportTitle_Main, ref sReportTitle_Sub, ref sReportPath))
                        {
                            ...
                            if (oQuotation != null)
                            {
                                ...
                                if (clsConfig.benable_TaxSelection_Quotation)
                                {
                                    decimal dSubTotal = clsHelpMethods_Local.getDisplayPrice(oQuotation.SubTotal, oQuotation.CurrencyRate);
                                    decimal dDiscountTotal = clsHelpMethods_Local.getDisplayPrice(oQuotation.DiscountTotal, oQuotation.CurrencyRate);
                                    decimal dNbtAmout = clsHelpMethods_Local.getDisplayPrice(oQuotation.NbtTotal, oQuotation.CurrencyRate);
                                    decimal dvatAmount = clsHelpMethods_Local.getDisplayPrice(oQuotation.VatTotal, oQuotation.CurrencyRate);
                                    decimal dSvatAmount = clsHelpMethods_Local.getDisplayPrice(oQuotation.OtherTaxTotal, oQuotation.CurrencyRate);
                                    decimal dGrandToatal = clsHelpMethods_Local.getDisplayPrice(oQuotation.GrandTotal, oQuotation.CurrencyRate);
                                    //decimal dnbtPresentage = clsHelpMethods_Local.getDisplayPrice(oQuotation.NbtPercentage, oQuotation.CurrencyRate);
                                    //decimal dVatPreasentage = clsHelpMethods_Local.getDisplayPrice(oQuotation.VatPercentage, oQuotation.CurrencyRate);
                                    //decimal dSvatPresentage = clsHelpMethods_Local.getDisplayPrice(oQuotation.OtherTaxPercentage, oQuotation.CurrencyRate);
                                    ...
                                    decimal dInvoiceSubTotal = clsHelpMethods_Local.getDisplayPrice(oQuotation.SubTotal, oQuotation.CurrencyRate);
                                    ...
                                    foreach (tbl_sasQuotation_Detail oQDetail in tbl_sasQuotation_Detail.SelectAllByQuotation_ID(oQuotation.Quotation_ID))
                                    {
                                        ...
                                        {
                                            decimal dAmount = clsHelpMethods_Local.getDisplayPrice(oQDetail.TatalAmount, oQuotation.CurrencyRate);
                                ...
                                {
                                    glb_dts_sasQuotation.Quotation.AddQuotationRow(oQuotation.Quotation_ID, oQuotation.QuotationDate, "", clsGenaralName.getName_SalesRep(oQuotation.Employee_ID), "", oQuotation.OrderRefNo_ID, oQuotation.ContactName, oQuotation.Customer_ID, clsGenaralName.getName_Customer(oQuotation.Customer_ID), clsGenaralName.getName_CustomerDeliveryAddress(oQuotation.Customer_ID), clsGenaralName.getName_BranchCustomer(oQuotation.Customer_ID, int.Parse(oQuotation.Branch_ID)), "", "", "", oQuotation.ValiedPeriod, oQuotation.PaymentPeriod, oQuotation.DeliveryPeriod, oQuotation.SubTotal, oQuotation.DiscountPercentage, oQuotation.DiscountTotal, 0, oQuotation.NbtPercentage, oQuotation.NbtTotal, oQuotation.VatPercentage, oQuotation.VatTotal, 0, 0, oQuotation.GrandTotal, oQuotation.Remark, 0, true);
                                    tbl_genEmployeeMaster oSalesman = tbl_genEmployeeMaster.Select(oQuotation.Employee_ID);
                                    sSalesMan = "[ " + oSalesman.EmployeeName + ", " + oSalesman.Telephone + " | " + oSalesman.Email + " ]";
                            ...
                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("Salesman", sSalesMan, true);
                            ...
                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("isDel", oQuotation.IsDeleted ? "CANCELLED" : "", true);
