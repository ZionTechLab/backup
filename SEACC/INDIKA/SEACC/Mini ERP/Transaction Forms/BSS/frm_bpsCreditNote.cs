                                ...
                                if (!oldRecord.IsChecked || (oldRecord.IsChecked && clsSecurity.PermissionToApproved(clsSecurity.UserIDLoged, iFormID)))
                                {
                                    if (clsValidate.CheckValidity_TransactionCodeLength(txtCreditNoteID.Text))
                                    {
                                        ...
                                        tbl_bpsCreditNote detail = new tbl_bpsCreditNote(txtCreditNoteID.Text.Trim(),
                                            ...
                                            clsHelpMethods_Local.getSavePrice(dSubTotal, txtCurrencyRate),
                                            clsHelpMethods_Local.getSavePrice(dDiscount, txtCurrencyRate),
                                            clsHelpMethods_Local.getSavePrice(dNbt, txtCurrencyRate),
                                            clsHelpMethods_Local.getSavePrice(dVat, txtCurrencyRate),
                                            clsHelpMethods_Local.getSavePrice(dOtherTax, txtCurrencyRate),
                                            clsHelpMethods_Local.getSavePrice(dAllocatedAmount, txtCurrencyRate),
                    ...
                    else
                    {
                        ...
                        if (clsValidate.CheckValidity_TransactionCodeLength(txtCreditNoteID.Text)) //if (txtCreditNoteID.TextLength > 0)
                        {
                            ...
                            tbl_bpsCreditNote detail = new tbl_bpsCreditNote(txtCreditNoteID.Text.Trim(), dtpCreditNoteDate.Value, txtRemark.Text.Trim(), sSRNID, sInvoiceId, txtCustomerID.Tag.ToString(), sDoID, sOrderRefNo, sChequeRegisterId, txtCreditNoteType.Tag.ToString(), "default",
                                                    ...
                                                    decimal.Parse(txtPercentageDiscount.Text.Trim()), decimal.Parse(txtPercentageNBT.Text.Trim()), decimal.Parse(txtPercentageVat.Text.Trim()), decimal.Parse(txtPercentageOtherTax.Text.Trim()), clsHelpMethods_Local.getSavePrice(dSubTotal, txtCurrencyRate), clsHelpMethods_Local.getSavePrice(dDiscount, txtCurrencyRate),
                                                    clsHelpMethods_Local.getSavePrice(dNbt, txtCurrencyRate), clsHelpMethods_Local.getSavePrice(dVat, txtCurrencyRate), clsHelpMethods_Local.getSavePrice(dOtherTax, txtCurrencyRate), clsHelpMethods_Local.getSavePrice(dAllocatedAmount, txtCurrencyRate), clsSecurity.UserIDLoged, "default", "default", "default",
                    ...
                    if (bPermissinOkToPrint)
                    {
                        ...
                        if (CreditNote != null)
                        {
                            ...
                            if (bApprovalDone)
                            {
                                ...
                                string sDbPath = clsHelpMethods_Local.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_CreditNote));
        ...
        private DataSet AdvancedPrint_CreditNote(tbl_bpsCreditNote oCreditNote)
        {
            ...
            if (oCreditNote != null)
            {
                ...
                glb_dtsBills.dt_bpsCreditNote.Adddt_bpsCreditNoteRow(oCreditNote.CreditNote_ID, oCreditNote.CreditNoteDate, oCreditNote.Customer_ID, clsGenaralName.getName_Customer(oCreditNote.Customer_ID),
                                                ...
                                                sPoNO,
                                                clsHelpMethods_Local.getDisplayPrice(oCreditNote.SubTotal, oCreditNote.CurrencyRate),
                                                oCreditNote.DiscountPercentage, clsHelpMethods_Local.getDisplayPrice(oCreditNote.DiscountTotal, oCreditNote.CurrencyRate),
                                                clsHelpMethods_Local.getDisplayPrice(oCreditNote.SubTotal, oCreditNote.CurrencyRate),
                                                oCreditNote.NbtPercentage, clsHelpMethods_Local.getDisplayPrice(oCreditNote.NbtTotal, oCreditNote.CurrencyRate),
                                                oCreditNote.VatPercentage, clsHelpMethods_Local.getDisplayPrice(oCreditNote.VatTotal, oCreditNote.CurrencyRate),
                                                oCreditNote.OtherTaxPercentage, clsHelpMethods_Local.getDisplayPrice(oCreditNote.OtherTaxTotal, oCreditNote.CurrencyRate),
                                                clsHelpMethods_Local.getDisplayPrice(oCreditNote.TotalAmount, oCreditNote.CurrencyRate),
            ...
            if (CheckInvoiceValidity())
            {
                ...
                if (detail != null && detail.Invoice_ID != "default")
                {
                    ...
                    if (bCurrencyTypeOk)
                    {
                        ...
                        CalcualteSubTotal(clsHelpMethods_Local.getDisplayPrice((detail.GrandTotal - detail.SeattleAmount), detail.CurrencyRate));
                        CalculateTaxesAndGrandTotal();
