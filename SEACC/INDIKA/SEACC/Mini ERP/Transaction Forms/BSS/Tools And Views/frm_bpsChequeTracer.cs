...
namespace Digiteq
{
    ...
    //    DateTime glbCheckedDate = clsSecurity.getServerDateTime();
        ...
        private void txtCashDepID_KeyUp(object sender, KeyEventArgs e)
        {
            createFilterQuary_Inward_Cash(txtCashDepID, "cashDeposit_ID");
        }
        ...
        private void dgvDetail_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                if (e.RowIndex >= 0)
                {
                    ...
                    {
                        string sChequeRegister_ID = clsValidate.ValidateGridValue(dgvInward, "RegisterCode", e.RowIndex, "");
                        if (sChequeRegister_ID.Length > 0)
                        {
                            var frm = new frm_bpsChequeViewer();
                            frm.glbChequeRegisterID = sChequeRegister_ID;
