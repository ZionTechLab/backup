            ...
            if (CheckValidity())
            {
                if (CheckValidityForBank())
                {
                    if (!CheckGrid())
                    {
                       ...
                       clsHelpMethods_Local.getSavePrice(decimal.Parse(txtAmount.Text.ToString()), txtExRate),
        ...
        private void RefreshGrid()
        {
            ...
            {                         
                ...
                foreach (DataRow row in dtRecodes.Rows)
                {
                    ...
                    txtAmount.Text = clsHelpMethods_Local.getDisplayPrice(decimal.Parse(row["Amount"].ToString().Trim()),txtExRate).ToString();
        ...
        private void AsignDetails_SubTotals()
        {
            try
            {
                if (dgvDetail.Rows.Count > 0)
                {
                    ...
                    dTotal += clsHelpMethods_Local.getDisplayPrice(dAmount,txtExRate);
                }
            }
            catch (Exception ex)
            {
