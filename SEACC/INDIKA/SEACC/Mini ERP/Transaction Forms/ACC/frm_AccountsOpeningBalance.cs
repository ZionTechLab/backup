...
namespace Digiteq
{
    public partial class frm_AccountsOpeningBalance : MettroForm
    {
        ...
        private void SearchSubledgerSubGLCode()
        {
            try
            {
                clsSearch.Search_SubGLCode(txtGLCode, null, "", false);
            }
            catch (Exception ex)
            {
                clsValidate.WriteErrorLog("", iFormID, ex);
        ...
        private void btnAutoUpdate_Click(object sender, EventArgs e)
        {
            ...
            FYValidationData odata = new FYValidationData();
            var x = odata.FYupdate_opbl(txtFinYear.Tag.ToString());
            if (x.IsSuccess)
            {
                MessageBox.Show("Successfully updated", clsFormatter.GetMessageCaption(), MessageBoxButtons.OK, MessageBoxIcon.Information);

                RefreshGridForMonth(txtFinYear.Tag.ToString(), "April");
            }
            else
                MessageBox.Show(x.OutMsg, clsFormatter.GetMessageCaption(), MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
}
