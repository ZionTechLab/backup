        ...
        private void PntDocCheque_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                if (txtBank.Tag.ToString().Length > 0)
                {
                    ...
                    #region Celcius
                    //   if (clsConfig.sCustomChequeFormat == "Celcius")
                    // {

                    bool bShowCounterBook = false;
                  ...
                  //  if (txtBank.Tag.ToString() == "7463" || txtBank.Tag.ToString() == "7056" || txtBank.Tag.ToString() == "7162" || txtBank.Tag.ToString() == "7010")
                        ...
                        if (sPayee.Length <= 56)
                            ...
                            {
                                x[oElament.ElementID] = i_Xvalue+ oElament.XValue;
                                y[oElament.ElementID] = i_Yvalue+ oElament.YValue;
                            }

                            if (chkAccPayee.Checked)
                            {
                                e.Graphics.DrawString(sUndeline, Font_Title1, Brushes.Black, new Point(x[15], y[15]-12), sf);
                                e.Graphics.DrawString(sAccountPayee, Font_Title1, Brushes.Black, new Point(x[15], y[15]), sf);
                                e.Graphics.DrawString(sUndeline, Font_Title1, Brushes.Black, new Point(x[15], y[15]), sf);
                            }

                            #region Counter Book
                            if (bShowCounterBook)
                            {
                                e.Graphics.DrawString(sDate, Font_Title1, Brushes.Black, new Point(x[0], y[0]), sf);//Date

                                if (sPayee.Length < 20)
                                    e.Graphics.DrawString(sPayee, Font_Title2, Brushes.Black, new Point(x[1], y[1]), sf);
                                else if (sPayee.Length < 40)
                                {
                                    e.Graphics.DrawString(sPayee.Substring(0, 20), Font_Title2, Brushes.Black, new Point(x[2], y[2]), sf);//payee1
                                    e.Graphics.DrawString(sPayee.Substring(20, sPayee.Length - 20), Font_Title2, Brushes.Black, new Point(-15, 85));//payee2
                                }
                                else
                                {
                                    e.Graphics.DrawString(sPayee.Substring(0, 20), Font_Title2, Brushes.Black, new Point(x[2], y[2]), sf);//payee1
                                    e.Graphics.DrawString(sPayee.Substring(20, 20), Font_Title2, Brushes.Black, new Point(-15, 85));//payee2
                                    e.Graphics.DrawString(sPayee.Substring(40, iLastStr), Font_Title2, Brushes.Black, new Point(-15, +110));//payee2
                                }

                                e.Graphics.DrawString(sAmount, Font_Title1, Brushes.Black, new Point(x[3], y[3]));//amount 
                            }
                            #endregion

                            #region Cheque Area
                            e.Graphics.DrawString(sPayee, Font_Title2, Brushes.Black, new Point( x[4],  y[4]));//Payee //ok

                            e.Graphics.DrawString(sRupee[0], Font_Title2, Brushes.Black, new Point( x[5],  y[5]));//rupee1 ok              
                            e.Graphics.DrawString(sRupee[1], Font_Title2, Brushes.Black, new Point( x[6],  y[6]));//rupee2 ok
                            e.Graphics.DrawString(sRupee[2], Font_Title2, Brushes.Black, new Point( x[7],  y[7]));//rupee3 ok

                            e.Graphics.DrawString(d1, Font_Title3, Brushes.Black, new Point( x[8],  y[8]));//day1
                            e.Graphics.DrawString(d2, Font_Title3, Brushes.Black, new Point( x[9],  y[9]));//day2
                            e.Graphics.DrawString(m1, Font_Title3, Brushes.Black, new Point( x[10],  y[10]));//month1
                            e.Graphics.DrawString(m2, Font_Title3, Brushes.Black, new Point( x[11],  y[11]));//month2                
                            e.Graphics.DrawString(y3, Font_Title3, Brushes.Black, new Point( x[12],  y[12]));//year3
                            e.Graphics.DrawString(y4, Font_Title3, Brushes.Black, new Point( x[13],  y[13]));//year4

                            e.Graphics.DrawString(sAmount, Font_Title3, Brushes.Black, new Point( x[14],  y[14]));//amount
                            #endregion
                        }
                        #endregion
                        #region HNB
