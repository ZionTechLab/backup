                        ...
                        else
                        {
                            ...
                            if (clsValidate.CheckValidity_TransactionCodeLength(txtDebitNoteID.Text)) //if (txtDebitNoteID.Text.Trim().Length > 0)
                            {
                                ...
                                foreach (DataGridViewRow row in dgvAPN.Rows)
                                {
                                    ...
                                    dApnAmount = clsHelpMethods_Local.getSavePrice(decimal.Parse(clsValidate.ValidateGridValue(dgvAPN, "APNAmount", row.Index, "0.00")), uC_ExchangeRate1.ExchangeRate);
                                    dTotalApnAmount += dApnAmount;
                ...
                if (sID.Length > 0)
                {
                    ...
                    if (detail != null && detail.DebitNote_ID != "default")
                    {
                        ...
                        foreach (tbl_accPaymentVoucher_Detail APNdetail in tbl_accPaymentVoucher_Detail.SelectAllByDebitNote_ID(detail.DebitNote_ID))
                        {
                            ...
                            dAmount = clsHelpMethods_Local.getDisplayPrice(APNdetail.SettleAmount, uC_ExchangeRate1.ExchangeRate);
            ...
            try
            {
                if (sID.Length > 0)
                {
                    ...
                    if (detail != null)
                    {
                        ...
                        dgvAPN.Rows.Add(detail.AccountPayableNote_ID, clsFormatter.FormatDate_Short(detail.AccountPayableNoteDate), clsFormatter.FormatDecimalPlaces_Price(clsHelpMethods_Local.getDisplayPrice(dUnSettledAmount, uC_ExchangeRate1.ExchangeRate)));
