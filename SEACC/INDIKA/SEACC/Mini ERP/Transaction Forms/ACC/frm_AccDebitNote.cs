    ...
    public partial class frm_AccDebitNote : SEACC_Form
    {
                                         ...
                                         clsSecurity.PermissionToApproved(clsSecurity.UserIDLoged, iFormID)))
                                    {
                                        if (clsValidate.CheckValidity_TransactionCodeLength(txtDebitNoteID.Text))
                                        {
                                            ...
                                            foreach (DataGridViewRow row in dgvAPN.Rows)
                                            {
                                                ...
                                                dApnAmount = clsHelpMethods_Local.getSavePrice(
                        ...
                        else
                        {
                            ...
                            if (clsValidate.CheckValidity_TransactionCodeLength(txtDebitNoteID.Text)) //if (txtDebitNoteID.Text.Trim().Length > 0)
                            {
                                ...
                                tbl_accDebitNote AccDBN = new tbl_accDebitNote(txtDebitNoteID.Text.Trim(), dtpDabitNoteDate.Value, txtNarration.Text, sPRNID, sAPNID, "default", "default", "default", txtSupplierID.Tag.ToString().Trim(), txtCurCode.Tag.ToString(),
                                    decimal.Parse(txtCurrencyRate.Text.Trim()), clsHelpMethods_Local.getSavePrice(decimal.Parse(txtGrandTotal.Text.Trim()), dExRate), clsHelpMethods_Local.getSavePrice(decimal.Parse(txtOtherTax.Text.Trim()), dExRate), clsHelpMethods_Local.getSavePrice(decimal.Parse(txtVat.Text.Trim()), dExRate), clsHelpMethods_Local.getSavePrice(decimal.Parse(txtNBT.Text.Trim()), dExRate), clsHelpMethods_Local.getSavePrice(decimal.Parse(txtDiscount.Text.Trim()), dExRate),
                                    clsHelpMethods_Local.getSavePrice(decimal.Parse(txtSubTotal.Text.Trim()), dExRate), decimal.Parse(txtPercentageOtherTax.Text.Trim()), decimal.Parse(txtPercentageVat.Text.Trim()), decimal.Parse(txtPercentageNBT.Text.Trim()), decimal.Parse(txtPercentageDiscount.Text.Trim()),
                                ...
                                foreach (DataGridViewRow row in dgvAPN.Rows)
                                {
                                    ...
                                    dApnAmount = clsHelpMethods_Local.getSavePrice(decimal.Parse(clsValidate.ValidateGridValue(dgvAPN, "APNAmount", row.Index, "0.00")), dExRate);
                ...
                if (sID.Length > 0)
                {
                    ...
                    if (detail != null && detail.DebitNote_ID != "default")
                    {
                        ...
                        foreach (tbl_accPaymentVoucher_Detail APNdetail in tbl_accPaymentVoucher_Detail.SelectAllByDebitNote_ID(detail.DebitNote_ID))
                        {
                            ...
                            dAmount = clsHelpMethods_Local.getDisplayPrice(APNdetail.SettleAmount, dExRate);
            ...
            try
            {
                if (sID.Length > 0)
                {
                    ...
                    if (detail != null)
                    {
                        ...
                        txtSubTotal.Text = clsFormatter.FormatDecimalPlaces_Price(clsHelpMethods_Local.getDisplayPrice(detail.SubTotal, detail.CurrencyRate));
                        txtDiscount.Text = clsFormatter.FormatDecimalPlaces_Price(clsHelpMethods_Local.getDisplayPrice(detail.DiscountTotal, detail.CurrencyRate));
                        txtNBT.Text = clsFormatter.FormatDecimalPlaces_Price(clsHelpMethods_Local.getDisplayPrice(detail.NbtTotal, detail.CurrencyRate));
                        txtVat.Text = clsFormatter.FormatDecimalPlaces_Price(clsHelpMethods_Local.getDisplayPrice(detail.VatTotal, detail.CurrencyRate));
                        txtOtherTax.Text = clsFormatter.FormatDecimalPlaces_Price(clsHelpMethods_Local.getDisplayPrice(detail.OtherTaxTotal, detail.CurrencyRate));
                        txtGrandTotal.Text = clsFormatter.FormatDecimalPlaces_Price(clsHelpMethods_Local.getDisplayPrice(detail.GrandTotal, detail.CurrencyRate));
                        ...
                        dgvAPN.Rows.Add(detail.AccountPayableNote_ID, clsFormatter.FormatDate_Short(detail.AccountPayableNoteDate), clsFormatter.FormatDecimalPlaces_Price(clsHelpMethods_Local.getDisplayPrice(dUnSettledAmount, dExRate)));
                    }
                }
            }
            catch (Exception ex)
        ...
        private bool CheckValidity_GrandTotalVsDebit()
        {
            ...
            try
            {
                ...
                //if (decimal.Parse(txtDebitAmount.Text) != clsHelpMethods_Local.getSavePrice(decimal.Parse(clsFormatter.FormatDecimalPlaces_Price(decimal.Parse(txtGrandTotal.Text.Trim()))), txtCurrencyRate))
                //if (decimal.Parse(txtDebitAmount.Text) != decimal.Parse(clsFormatter.FormatDecimalPlaces_Price(clsHelpMethods_Local.getSavePrice(decimal.Parse(txtGrandTotal.Text.Trim()), txtCurrencyRate))))
            ...
            try
            {
                ...
                if (txtDebitNoteID.Text.Trim().Length > 0 && txtDebitNoteID.Text.Trim() != "<Auto Generate>")
                {
                    ...
                    if (bPermissinOkToPrint)
                    {
                        ...
                        string s_Path = "";
                        string sGetRptPath = clsHelpMethods_Local.GetReportPath(clsAutocode.getReportID(enum_ReportName.NP_AccountDebitNote));
        ...
        private void print(string path, string sReportTitle, DataSet ojbDataSet, string sCreateUserNameAndDate, string sChekcedUserNameAndDate, string sApprovedUserNameAndDate, string sDuplicate, bool isCancel, bool bIsDraft, string sReportID)
        {
            try
            {
                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("Cancel", isCancel ? "CANCELLED" : "", true);
                ...
                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CreateUserName", sCreateUserNameAndDate, true);
                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CheckUserName", sChekcedUserNameAndDate, true);
                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("ApproveUserName", sApprovedUserNameAndDate, true);
                            ...
                            try
                            {
                                ...
                                if ((oPRN != null && oPRN.PurchaseReturnedNote_ID != "default") && (oDBN != null && oDBN.DebitNote_ID != "default"))
                                {
                                    ...
                                    foreach (DataGridViewRow row in dgvAPN.Rows)
                                    {
                                        ...
                                        if (clsHelpMethods_Local.RemoveSattlementsFrom_DebitNoteWithAPN("default", sApnNo, "default", oDBN.DebitNote_ID, "default"))
                                        {
                                            dAllocatedAmount += clsHelpMethods_Local.AutoSettleDebitNoteWithAPN(sApnNo, oDBN.DebitNote_ID, DateTime.Now.Date, dApnAmountTobeSettle, "", row.Index + 1, ref bIsEnoughDbnAmountToSettledApn);
