                        ...
                        if (oldRecord != null && clsValidate.CheckPrintingValidity(oldRecord.PrintCount))
                        {
                            if (!oldRecord.IsLocked && !oldRecord.IsApproved && !oldRecord.IsFinished && !oldRecord.IsDeleted)
                            {
                                if (clsValidate.CheckValidity_TransactionCodeLength(txtAPNID.Text))
                                {
                                    ...
                                    //    clsProcessMethods.SetSettle_GRN_From_APN(txtGRN.Tag.ToString().Trim(), clsHelpMethods_Local.getDisplayPrice(dSettleAmount - oldRecord.GrandTotal, dExRate));
                    ...
                    else
                    {
                            ...
                            .Select(g =>
                      {
                        ...
                        foreach (DataRow row in newDt.Rows)
                        {
                            ...
                            decimal sUnsettledAmount = clsHelpMethods_Local.getSavePrice(clsValidate.ValidateRowValue(row, "AllocatedAmount", (decimal)0), dExRate);
