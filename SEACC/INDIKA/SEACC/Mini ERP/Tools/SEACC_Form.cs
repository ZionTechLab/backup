...
namespace Digiteq
{
    public partial class SEACC_Form : UserControl
    {
        ...
        {
          //  if (!button_Print)
                btnPrint.Visible = button_Print;
          //  if (!button_Draft)
                btnDraft.Visible = button_Draft;
           // if (!button_New)
                btnNew.Visible = button_New;
           // if (!button_Save)
                btnSave.Visible = button_Save;
           // if (!button_Cancle)
                btnCancel.Visible = button_Cancle;
           // if (!button_Approve)
                btnApproved.Visible = button_Approve;
           // if (!button_Approve)
                btnChecked.Visible = button_Approve;
            if (!button_Approve || !button_Check)
                btnUserDetails.Visible = false;
           // if (!button_Attachment)
                Attachments.Visible = button_Attachment;
            //if (!button_Temp)
                btnTemp.Visible = button_Temp;
        }
