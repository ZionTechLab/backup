...
namespace Digiteq
{
    public class clsHelpMethods_Local
    {
        ...
        public static decimal GetUnitPrice(decimal dWidth, decimal dLength, decimal dGauge, decimal dGussest, decimal dKiloPrice, string sUomID)
        {
            ...
            try
            {
                if (clsHelpMethods_Local.IsUomCalculationBag(sUomID))
                    dUnitPrice = (((dWidth * dLength * dGauge * dGussest) / 3300) * dKiloPrice) / 1000;
                else if (clsHelpMethods_Local.IsUomCalculationKilogram(sUomID))
        ...
        public static decimal GetWeightByItemID(string sItemID, decimal dQuantity)
        {
            ...
            try
            {
                ...
                if (detail != null)
                {
                    if (clsHelpMethods_Local.IsUomCalculationBag(detail.Uom_ID))
                    {
           ...
           string sSerial1, string sSerial2, string sItemStatus, decimal dUnitPrice, decimal dTotalAmount, string sRemarks, decimal dFlowStockQty)
        {
            ...
            clsHelpMethods_Local.AddMultipleItems_Grid(dgvDetail, sItemID, ref iRow, ref iLineNo, ref dQuantity, ref dUnitPrice, ref dWeight, ref dWeightAvg);
        ...
        //    decimal  dLowerstCostPrice = 0, dHighestPurchaseCostPrice = 0;//dWeightedAverageCostPrice = 0,
        //    decimal dUnitCost = 0;
        ...
        //    foreach (tbl_genStore_Stock stock in tbl_genStore_Stock.SelectAllByItem_ID(Item_Code))//  .Select(StoreID, Item_Code, "default", "default", "default", "0", "0");
        //    {
        ...
        //        if (oItem != null)
        //        {
        //            FifoCost = GetFifoCost(Item_Code, 0);
        ...
        //                dWeightedAverageCostPrice =
        //                    ((dFlowStockQty_Old * oItem.WeightedAverageCostPrice) + (Qty * dUnitCost)) 
        //                    / (dFlowStockQty_Old + Qty);


        //            oItem.FifoCostPrice = FifoCost;
        ...
        {
            string sValue = "";
            decimal dTranslateValue = clsHelpMethods_Local.GetJobMeasurementUomConvertValue(sItem_ID);
                    ...
                    if (JournalEntry != null && oReceipt != null)
                    {
                        ...
                        if (dReceiptAvailableAmount > 0 && dJeToBeSettled > 0)
                        {
                            if (dJeToBeSettled <= dReceiptAvailableAmount) //if Receipt has enough cash to settled
                            {
                                ...
                                JournalEntry.Update();
                                oReceipt.Update();
                                clsHelpMethods_Local.InsertInvoiceSettlementRecord("default", JournalEntry.JournalEntry_ID, JournalEntry.Line_No, "default", sChqRegisterID, "default", "default", "default", -1, ((int)PaymentMethod.Cash).ToString(), dtmAllocationDate, dJeToBeSettled, sAllocationID, bIsAdvancePayment, bIsOverPayment);
                            }
                            else //if Invoice amount is greter than receipt amount
                            {
                                ...
                                clsHelpMethods_Local.InsertInvoiceSettlementRecord("default", JournalEntry.JournalEntry_ID, JournalEntry.Line_No, "default", sChqRegisterID, "default", "default", "default", -1, ((int)PaymentMethod.Cash).ToString(), dtmAllocationDate, dReceiptAvailableAmount, sAllocationID, bIsAdvancePayment, bIsOverPayment);
                            }
                        }
                    }
                }
                    ...
                    if (JournalEntry != null && oCheque != null)
                    {
                        ...
                        if (dChequeAvailableAmount > 0 && dJEToBeSettled > 0)
                        {
                            if (dJEToBeSettled <= dChequeAvailableAmount) //if Receipt has enough cash to settled
                            {
                                ...
                                JournalEntry.Update();
                                oCheque.Update();
                                clsHelpMethods_Local.InsertInvoiceSettlementRecord("default", JournalEntry.JournalEntry_ID, JournalEntry.Line_No, oCheque.Receipt_ID, sChequeRegisterID, "default", "default", "default", -1, ((int)PaymentMethod.Cheque).ToString(), dtmAllocationDate, dJEToBeSettled, sAllocationID, bIsAdvancePayment, bIsOverPayment);
                            }
                            else //if Invoice amount is greter than receipt amount
                            {
                                ...
                                JournalEntry.Update();
                                oCheque.Update();
                                clsHelpMethods_Local.InsertInvoiceSettlementRecord("default", JournalEntry.JournalEntry_ID, JournalEntry.Line_No, oCheque.Receipt_ID, sChequeRegisterID, "default", "default", "default", -1, ((int)PaymentMethod.Cheque).ToString(), dtmAllocationDate, dChequeAvailableAmount, sAllocationID, bIsAdvancePayment, bIsOverPayment);
                            }
                        }
                    }
                }
                    ...
                    if (JournalEntry != null && oCreditNote != null)
                    {
                        ...
                        if (dCreditNoteAvailableAmount > 0 && dJEToBeSettled > 0)
                        {
                            if (dJEToBeSettled <= dCreditNoteAvailableAmount) //if Credit Note has enough cash to settled
                            {
                                ...
                                sSettlement_ID = clsHelpMethods_Local.InsertInvoiceSettlementRecord("default", JournalEntry.JournalEntry_ID, JournalEntry.Line_No, "default", "default", sCreditNoteID, "default", "default", -1, "default", dtmAllocationDate, dJEToBeSettled, sAllocationID, bIsAdvancePayment, bIsOverPayment);
                            ...
                            else //if Invoice amount is greter than credit note amount
                            {
                                ...
                                sSettlement_ID = clsHelpMethods_Local.InsertInvoiceSettlementRecord("default", JournalEntry.JournalEntry_ID, JournalEntry.Line_No, "default", "default", sCreditNoteID, "default", "default", -1, "default", dtmAllocationDate, dCreditNoteAvailableAmount, sAllocationID, bIsAdvancePayment, bIsOverPayment);
            ...
            try
            {
                if (clsAutocode.getConfigStatus(ConfigStatus.AutoInvoiceSettleWhenCashReceipt))
                {
                    ...
                    if (JournalEntryDR != null && JournalEntryCR != null)
                    {
                        ...
                        tbl_accJournalEntry oJEDR = tbl_accJournalEntry.Select(JournalEntryCR.JournalEntry_ID);
                        if (oJEDR != null)
                        ...
                        {
                            if (dJEDRToBeSettled <= dJEAvailableAmount) //if Receipt has enough cash to settled
                            {
                                ...
                                clsHelpMethods_Local.InsertInvoiceSettlementRecord("default", JournalEntryDR.JournalEntry_ID, JournalEntryDR.Line_No, "default", "default", "default", "default", JournalEntryCR.JournalEntry_ID, LineNo_JECR, ((int)PaymentMethod.Cheque).ToString(), dtmAllocationDate, dAllocatedAmount, sAllocationID, bIsAdvancePayment, bIsOverPayment);
                            }
                            else //if Invoice amount is greter than receipt amount
                            {
        ...
        //                        oInvoice.Update();
        //                        oReceipt.Update();
        //                        clsHelpMethods_Local.InsertInvoiceSettlementRecord(sInvoiceID,"default",-1, sReceiptID, "default", "default", "default", "default",-1, clsConfig.sPaymentMethod_Cash, dtmAllocationDate, dInvoiceToBeSettled, sAllocationID, bIsAdvancePayment, bIsOverPayment);
        //                    }
        //                    else //if Invoice amount is greter than receipt amount
        //                    {
        //                        dAllocatedAmount = dReceiptAvailableAmount;
        ...
        //                        oInvoice.Update();
        //                        oReceipt.Update();
        //                        clsHelpMethods_Local.InsertInvoiceSettlementRecord(sInvoiceID, "default", -1, sReceiptID, "default", "default", "default", "default",-1, clsConfig.sPaymentMethod_Cash, dtmAllocationDate, dReceiptAvailableAmount, sAllocationID, bIsAdvancePayment, bIsOverPayment);
        //                    }
        //                }
        //            }
        //        }
                    ...
                    if (oInvoice != null && oCheque != null)
                    {
                        ...
                        if (dChequeAvailableAmount > 0 && dInvoiceToBeSettled > 0)
                        {
                            if (dInvoiceToBeSettled <= dChequeAvailableAmount) //if Receipt has enough cash to settled
                            {
                                ...
                                oInvoice.Update();
                                oCheque.Update();
                                clsHelpMethods_Local.InsertInvoiceSettlementRecord(sInvoiceID, "default", -1, oCheque.Receipt_ID, sChequeRegisterID, "default", "default", "default", -1, ((int)PaymentMethod.Cheque).ToString(), dtmAllocationDate, dInvoiceToBeSettled, sAllocationID, bIsAdvancePayment, bIsOverPayment);
                            }
                            else //if Invoice amount is greter than receipt amount
                            {
                                dAllocatedAmount = dChequeAvailableAmount;
                                ...
                                oInvoice.Update();
                                oCheque.Update();
                                clsHelpMethods_Local.InsertInvoiceSettlementRecord(sInvoiceID, "default", -1, oCheque.Receipt_ID, sChequeRegisterID, "default", "default", "default", -1, ((int)PaymentMethod.Cheque).ToString(), dtmAllocationDate, dChequeAvailableAmount, sAllocationID, bIsAdvancePayment, bIsOverPayment);
                            }
                        }
                    }
                }
            ...
            try
            {
                if (clsAutocode.getConfigStatus(ConfigStatus.AutoInvoiceSettleWhenCashReceipt))
                {
                    ...
                    if (oInvoice != null && JournalEntry != null)
                    {
                        ...
                        {
                            if (dInvoiceToBeSettled <= dJEAvailableAmount) //if Receipt has enough cash to settled
                            {
                                dAllocatedAmount = dInvoiceToBeSettled;
                                oInvoice.SeattleAmount += dInvoiceToBeSettled;
                                ...
                                oInvoice.Update();
                                JournalEntry.Update();
                                clsHelpMethods_Local.InsertInvoiceSettlementRecord(sInvoiceID, "default", -1, "default", "default", "default", "default", JournalEntry.JournalEntry_ID, LineNo_JE, ((int)PaymentMethod.Cheque).ToString(), dtmAllocationDate, dAllocatedAmount, sAllocationID, bIsAdvancePayment, bIsOverPayment);
                            }
                            else //if Invoice amount is greter than receipt amount
                            {
                                dAllocatedAmount = dJEAvailableAmount;
                    ...
                    if (oInvoice != null && oCreditNote != null)
                    {
                        ...
                        if (dCreditNoteAvailableAmount > 0 && dInvoiceToBeSettled > 0)
                        {
                            if (dInvoiceToBeSettled <= dCreditNoteAvailableAmount) //if Credit Note has enough cash to settled
                            {
                                ...
                                sSettlement_ID = clsHelpMethods_Local.InsertInvoiceSettlementRecord(sInvoiceID, "default", -1, "default", "default", sCreditNoteID, "default", "default", -1, "default", dtmAllocationDate, (dSetaleAmount != 0) ? dSetaleAmount : dInvoiceToBeSettled, sAllocationID, bIsAdvancePayment, bIsOverPayment);
                            ...
                            else //if Invoice amount is greter than credit note amount
                            {
                                ...
                                sSettlement_ID = clsHelpMethods_Local.InsertInvoiceSettlementRecord(sInvoiceID, "default", -1, "default", "default", sCreditNoteID, "default", "default", -1, "default", dtmAllocationDate, dCreditNoteAvailableAmount, sAllocationID, bIsAdvancePayment, bIsOverPayment);
                ...
                if (oLoanIn != null && oLoanOut != null && oLoanIn.IsWeightCalculation == oLoanOut.IsWeightCalculation)
                {
                    ...
                    if (dLoanOutQty > 0 && dLoanInQty > 0)
                    {
                        if (dLoanInQty <= dLoanOutQty) //if Receipt has enough cash to settled
                        {
                            ...
                            oLoanIn.Update();
                            oLoanOut.Update();
                            clsHelpMethods_Local.InsertLoanInOutSettlementRecord(sAllocationID, sLoanIN_ID, sLoanOut_ID, 0, 0, oLoanIn.IsWeightCalculation ? 0 : dLoanInQty, oLoanIn.IsWeightCalculation ? dLoanInQty : 0);
                        }
                        else //if Invoice amount is greter than receipt amount
                        {
                            ...
                            oLoanIn.Update();
                            oLoanOut.Update();
                            clsHelpMethods_Local.InsertLoanInOutSettlementRecord(sAllocationID, sLoanIN_ID, sLoanOut_ID, 0, 0, oLoanIn.IsWeightCalculation ? 0 : dLoanOutQty, oLoanIn.IsWeightCalculation ? dLoanOutQty : 0);
                        }
                    }
                }
       ...
       */
        ...
        {
            clsHelpMethods_Local.FormatGridColumns_Sales(dgv, clsConfig.SoftwareModel);

      //      if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.ePackWithDimension.ToString())
      //          clsHelpMethods_Local.FormatGridColumns_Sales(dgv, SoftwareModel_Sales.ePackWithDimension);
            //else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.akt.ToString())
            //    clsHelpMethods_Local.FormatGridColumns_Sales(dgv, SoftwareModel_Sales.akt);
            //else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.aktN2.ToString())
            //    clsHelpMethods_Local.FormatGridColumns_Sales(dgv, SoftwareModel_Sales.aktN2);
            //else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.ePackWithoutDimension.ToString())
            //    clsHelpMethods_Local.FormatGridColumns_Sales(dgv, SoftwareModel_Sales.ePackWithoutDimension);
            //else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.ePackWithSubCategory.ToString())
            //    clsHelpMethods_Local.FormatGridColumns_Sales(dgv, SoftwareModel_Sales.ePackWithSubCategory);
            //else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.ePackWithSerialNumber.ToString())
            //    clsHelpMethods_Local.FormatGridColumns_Sales(dgv, SoftwareModel_Sales.ePackWithSerialNumber);
            //else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.ceilingAndWallPanal.ToString())
            //    clsHelpMethods_Local.FormatGridColumns_Sales(dgv, SoftwareModel_Sales.ceilingAndWallPanal);
            //else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.idealWheels.ToString())
            //    clsHelpMethods_Local.FormatGridColumns_Sales(dgv, SoftwareModel_Sales.idealWheels);
            //else
            //    clsHelpMethods_Local.FormatGridColumns_Sales(dgv);
        }
        ...
        {
            if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.ePackWithDimension.ToString())
                clsHelpMethods_Local.FormatGridColumns_Stock(dgv, SoftwareModel_Sales.ePackWithDimension);
            else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.akt.ToString())
                clsHelpMethods_Local.FormatGridColumns_Stock(dgv, SoftwareModel_Sales.akt);
            else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.aktN2.ToString())
                clsHelpMethods_Local.FormatGridColumns_Stock(dgv, SoftwareModel_Sales.aktN2);
            else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.ePackWithoutDimension.ToString())
                clsHelpMethods_Local.FormatGridColumns_Stock(dgv, SoftwareModel_Sales.ePackWithoutDimension);
            else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.ePackWithSubCategory.ToString())
                clsHelpMethods_Local.FormatGridColumns_Stock(dgv, SoftwareModel_Sales.ePackWithSubCategory);
            else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.ePackWithSerialNumber.ToString())
                clsHelpMethods_Local.FormatGridColumns_Stock(dgv, SoftwareModel_Sales.ePackWithSerialNumber);
            else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.ceilingAndWallPanal.ToString())
                clsHelpMethods_Local.FormatGridColumns_Stock(dgv, SoftwareModel_Sales.ceilingAndWallPanal);
            else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.idealWheels.ToString())
                clsHelpMethods_Local.FormatGridColumns_Stock(dgv, SoftwareModel_Sales.idealWheels);
            //else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.gem.ToString())
            //    clsHelpMethods_Local.FormatGridColumns_Stock(dgv, SoftwareModel_Sales.gem);
        }
        ...
        {
            if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.ePackWithDimension.ToString())
                clsHelpMethods_Local.FormatGridColumns_Stock_External(dgv, SoftwareModel_Sales.ePackWithDimension);
            else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.akt.ToString())
                clsHelpMethods_Local.FormatGridColumns_Stock_External(dgv, SoftwareModel_Sales.akt);
            else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.aktN2.ToString())
                clsHelpMethods_Local.FormatGridColumns_Stock_External(dgv, SoftwareModel_Sales.aktN2);
            else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.ePackWithoutDimension.ToString())
                clsHelpMethods_Local.FormatGridColumns_Stock_External(dgv, SoftwareModel_Sales.ePackWithoutDimension);
            else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.ePackWithSubCategory.ToString())
                clsHelpMethods_Local.FormatGridColumns_Stock_External(dgv, SoftwareModel_Sales.ePackWithSubCategory);
            else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.ePackWithSerialNumber.ToString())
                clsHelpMethods_Local.FormatGridColumns_Stock_External(dgv, SoftwareModel_Sales.ePackWithSerialNumber);
            else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.ePackWithRemark.ToString())
                clsHelpMethods_Local.FormatGridColumns_Stock_External(dgv, SoftwareModel_Sales.ePackWithRemark);
            else if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.idealWheels.ToString())
                clsHelpMethods_Local.FormatGridColumns_Stock_External(dgv, SoftwareModel_Sales.idealWheels);
        }
            ...
            switch (pn)
            {
                case ProcessNote.Inquiry:
                    foreach (tbl_sasInquiry objinquiry in tbl_sasInquiry.SelectAllByOrderRefNo_ID(sOrderRefNo))
                    {
                        ...
                        clsHelpMethods_Local.Fill_NoteDatagrid(iRow, "", objinquiry.Inquiry_ID, objinquiry.InquiryDate.ToShortDateString(), objinquiry.GrandTotal, dgv);
                    }
                    break;
                case ProcessNote.Quotation:
                    foreach (tbl_sasQuotation objQuotation in tbl_sasQuotation.SelectAllByOrderRefNo_ID(sOrderRefNo))
                    {
                        dgv.Rows.Add();
                        iRow = dgv.Rows.Count - 1;
                        clsHelpMethods_Local.Fill_NoteDatagrid(iRow, "", objQuotation.Quotation_ID, objQuotation.QuotationDate.ToShortDateString(), objQuotation.GrandTotal, dgv);
                    }
                    break;
                case ProcessNote.ProforemaInvoice:
                    foreach (tbl_sasProformaInvoice objPInvoice in tbl_sasProformaInvoice.SelectAllByOrderRefNo_ID(sOrderRefNo))
                    {
                        dgv.Rows.Add();
                        iRow = dgv.Rows.Count - 1;
                        clsHelpMethods_Local.Fill_NoteDatagrid(iRow, "", objPInvoice.ProformaInvoice_ID, objPInvoice.ProformaInvoiceDate.ToShortDateString(), objPInvoice.GrandTotal, dgv);
                    }
                    break;
                case ProcessNote.CustomerOrder:
                    foreach (tbl_sasCustomerOrder objCustomerOrder in tbl_sasCustomerOrder.SelectAllByOrderRefNo_ID(sOrderRefNo))
                    {
                        dgv.Rows.Add();
                        iRow = dgv.Rows.Count - 1;
                        clsHelpMethods_Local.Fill_NoteDatagrid(iRow, "", objCustomerOrder.CustomerOrder_ID, objCustomerOrder.CustomerOrderDate.ToShortDateString(), objCustomerOrder.GrandTotal, dgv);
                    }
                    break;
                case ProcessNote.DeliveryOrder:
                    if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.idealWheels.ToString())
                    {
                        foreach (tbl_sasInvDeliveryOrder objDeliveryOrder in tbl_sasInvDeliveryOrder.SelectAllByOrderRefNo_ID(sOrderRefNo))
                        {
                            dgv.Rows.Add();
                            iRow = dgv.Rows.Count - 1;
                            clsHelpMethods_Local.Fill_NoteDatagrid(iRow, "", objDeliveryOrder.IDeliveryOrder_ID, objDeliveryOrder.IDeliveryOrderDate.ToShortDateString(), objDeliveryOrder.GrandTotal, dgv);
                        }
                    }
                    else
                    {
                        foreach (tbl_sasDeliveryOrder objDeliveryOrder in tbl_sasDeliveryOrder.SelectAllByOrderRefNo_ID(sOrderRefNo))
                        {
                            dgv.Rows.Add();
                            iRow = dgv.Rows.Count - 1;
                            clsHelpMethods_Local.Fill_NoteDatagrid(iRow, objDeliveryOrder.CustomerOrder_ID, objDeliveryOrder.DeliveryOrder_ID, objDeliveryOrder.DeliveryOrderDate.ToShortDateString(), objDeliveryOrder.GrandTotal, dgv);
                        }
                    }
                    break;
                case ProcessNote.Invoice:
                    foreach (tbl_sasInvoice objInvoice in tbl_sasInvoice.SelectAllByOrderRefNo_ID(sOrderRefNo))
                    {
                        dgv.Rows.Add();
                        iRow = dgv.Rows.Count - 1;
                        clsHelpMethods_Local.Fill_NoteDatagrid(iRow, objInvoice.DeliveryOrder_ID, objInvoice.Invoice_ID, objInvoice.InvoiceDate.ToShortDateString(), objInvoice.GrandTotal, dgv);
                    }
                    break;
                case ProcessNote.Receipt:
                    foreach (tbl_bpsReceipt_Invoice objReceipt in tbl_bpsReceipt_Invoice.SelectAllByOrderRefNo_ID(sOrderRefNo))
                    {
                        ...
                        if (detail != null && detail.Receipt_ID != "default")
                        {
                            dgv.Rows.Add();
                            iRow = dgv.Rows.Count - 1;
                            clsHelpMethods_Local.Fill_NoteDatagrid(iRow, detail.Invoice_ID, detail.Receipt_ID, detail.ReceiptDate.ToShortDateString(), detail.TotalAmount, dgv);
                        }
                    }
                    break;
                case ProcessNote.SalesReturned:
                    foreach (tbl_sasSalesReturnedNote objSalesReturned in tbl_sasSalesReturnedNote.SelectAllByOrderRefNo_ID(sOrderRefNo))
                    {
                        dgv.Rows.Add();
                        iRow = dgv.Rows.Count - 1;
                        clsHelpMethods_Local.Fill_NoteDatagrid(iRow, objSalesReturned.Invoice_ID, objSalesReturned.SalesReturnedNote_ID, objSalesReturned.SalesReturnedNoteDate.ToShortDateString(), objSalesReturned.GrandTotal, dgv);
                    }
                    break;
                ...
                case ProcessNote.PaymentVoucher:
                    foreach (tbl_accPaymentVoucher_Detail objPaymentVoucher in tbl_accPaymentVoucher_Detail.SelectAllByPaymentVoucher_ID(sOrderRefNo))
                    {
                        ...
                        clsHelpMethods_Local.Fill_NoteDatagrid(iRow, objPaymentVoucher.PaymentVoucher_ID, objPaymentVoucher.AccountPayableNote_ID, oAPN.BillDate.ToShortDateString(), objPaymentVoucher.SettleAmount, dgv);
                    }
                    break;
            }
        }
                        ...
                        if (sOrderRefNo != "default")
                        {
                            ...
                            else
                            {
                                ...
                                foreach (tbl_sasDeliveryOrder objDeliveryOrder in objDeliveryOrders)
                                {
                                    dgv.Rows.Add();
                                    iRow = dgv.Rows.Count - 1;
                                    clsHelpMethods_Local.Fill_NoteDatagrid(iRow, "", objDeliveryOrder.DeliveryOrder_ID, objDeliveryOrder.DeliveryOrderDate.ToShortDateString(), objDeliveryOrder.GrandTotal, dgv);
                                }
                            }
                        }
                    }
            ...
            switch (pn)
            {
                case ProcessNote.Invoice:
                    ...
                    foreach (tbl_sasInvoice objInvoice in objInvoices)
                    {
                        dgv.Rows.Add();
                        iRow = dgv.Rows.Count - 1;
                        clsHelpMethods_Local.Fill_NoteDatagrid(iRow, objInvoice.Invoice_ID, objInvoice.InvoiceDate.ToShortDateString(), objInvoice.GrandTotal, bSelect, dgv);
                    }
                    break;
            }
        }
        ...
        //    if (!frm.bNoAccess)
        //    {
        //        if (clsConfig.Theme_ID == 0)
        //        {
        //            LegessyForm mf = new LegessyForm() { };
        //            mf.Settings_Click += delegate (object sender, EventArgs e)
        //            {
        //                frm.SettingsClick();
        //            };
        ...
        //            mf.Height = mf.DefaultHight;
        //            mf.Controls.Add(frm);
        //            //    frm.BackColor = System.Drawing.Color.Transparent;
        //            frm.Dock = System.Windows.Forms.DockStyle.Fill;
        //            mf.Text = frm.Name;
        ...
        public static decimal GetFlowStock_Qty(DateTime dtTodate, string sItem_ID, string sStore_ID)
        {
            ...
            List<srh_scsFlowStock> oDetail = srh_scsFlowStock.Select(dtTodate.Date, sItem_ID, "0", clsSecurity.BranchID);
        ...
        public static bool CheckOutstandingValidity_CreditPeriodAndLimit( TextBox txtCustomer,  TextBox txtGrandTotal,decimal oldValue)
        {
                ...
                else
                {
                    ...
                    if (bOk_CreditPeriod)
                    {
                        if (clsConfig.bValidate_CreditBalance_Message || clsConfig.bValidate_CreditBalance_Block)
                        {
                            ...
                            if ((total + dAmountDue- oldValue) > dCreditLimit)
                            {
