...
namespace Digiteq
{
    public partial class frmUserPermission_StorWise : MettroForm
    {
        ...
        #region Form Load
        public frmUserPermission_StorWise()
        {
            iFormID = clsSecurity.getFormID(Digiteq_Logic.FormName.UserPermissionStoreWise);
        ...
        {
            clsFormatter.setFormatForm(this, "Store Wise User Permission", 1,0);
                        ...
                        if (txtUserID.TextLength > 0)
                        {
                            ...
                            {                                
                                for (int x = 0; x < dgvDetail.Rows.Count; x++)
                                {
                                    try
                                    {
                                        string sStocCode = dgvDetail["FormCode", x].Value.ToString();
                                        ...
                                        tbl_securityStorePermission oldRecord = tbl_securityStorePermission.Select(txtUserID.Tag.ToString(), sStocCode);
                                        if (oldRecord != null)
                                        {
                                            tbl_securityStorePermission oldRecordNew = new tbl_securityStorePermission(txtUserID.Tag.ToString(),
                                               sStocCode, bAllowRead, bAllowWrite, bAllowDelete, bAllowApprovable, bAllowCheckable, bAllowUpdate);
                                            oldRecordNew.Update();
                                        }
                                        else
                                        {
                                            tbl_securityStorePermission detail = new tbl_securityStorePermission(txtUserID.Tag.ToString(),
                                                sStocCode, bAllowRead, bAllowWrite, bAllowDelete, bAllowApprovable, bAllowCheckable, bAllowUpdate);
                                            detail.Insert();
                                        }
                                    }
                            ...
                            else  //insert records
                            {
                                for (int x = 0; x < dgvDetail.Rows.Count; x++)
                                {
                                    ...
                                    catch (Exception ) { /*MessageBox.Show(ex.Message);*/ } //if last raw is null
                                }
        ...
        private void FillDataGrid()
        {
            dtAllRecodes.Clear();
            dgvDetail.DataSource = source;
            List<tbl_genStoreMaster> details = tbl_genStoreMaster.SelectAll();
            foreach (tbl_genStoreMaster detail in details)
            {
                string sStocID = detail.Store_ID;
                string sFormName, sCategoryId;
                bool bAllowRead, bAllowWrite, bAllowDelete, bAllowEdit, bAllowCheckable, bAllowApprovable;

                if (detail != null && !detail.IsDeleted && detail.Store_ID != "default")
                {
                    sStocID = detail.Store_ID;
                    sFormName =detail.StoreName;                  
                    bAllowRead = false;
                    bAllowWrite = false;
                    bAllowDelete = false;
                    ...
                    dtAllRecodes.Rows.Add(sStocID, sFormName, bAllowRead, bAllowWrite, bAllowDelete, bAllowEdit, bAllowCheckable, bAllowApprovable);
                }
            }
        ...
        private void FillDetailsUser(string sUserID,bool bIsTemplate)
        {
            if (sUserID.Length > 0)
            {
                ...
                List<tbl_securityStorePermission> permission = tbl_securityStorePermission.SelectAllByUser_ID(sUserID);
        ...
        private void RefreshGrid_UserPermission(string sUserID)
        {
            ...
            foreach (tbl_genStoreMaster detail in details)
            {
                if (detail != null && !detail.IsDeleted && detail.Store_ID != "default")
                {
                     ...
                     sFormName = detail.StoreName;
                     tbl_securityStorePermission permission = tbl_securityStorePermission.Select(sUserID, detail.Store_ID);
                    if (permission != null)
                    {
                    ...
                    dtAllRecodes.Rows.Add(sStoreId, sFormName, bAllowRead, bAllowWrite, bAllowDelete, bAllowUpdate, bAllowCheckable, bAllowApprovable);
                }
            }
            source.DataSource = dtAllRecodes;           
        }
        ...
        private void CreateDataTable()
        {
            ...
            dtAllRecodes.Columns.Add("FormCode", typeof(string));
