...
namespace Digiteq
{
    public partial class frmAuditPermission : Form
    {
        ...
        {
            //format Form
            clsFormatter.setFormatForm(this, "Audit Permission", 1,0);
                            ...
                            if (IsUpdate)  //update records
                            {
                                for (int x = 0; x < dgvDetail.Rows.Count; x++)
                                {
                                    try
                                    {
                                        ...
                                        tbl_securityAuditPermission oldRecord = tbl_securityAuditPermission.Select(txtUserID.Tag.ToString(), int.Parse(sFormCode));
                                        if (oldRecord != null)
                                        {
                                            tbl_securityAuditPermission oldRecordNew = new tbl_securityAuditPermission(txtUserID.Tag.ToString(), int.Parse(sFormCode), bAllowRead);
                                            oldRecordNew.Update();
                                        }
                                        else
                                        {
                                            tbl_securityAuditPermission detail = new tbl_securityAuditPermission(txtUserID.Tag.ToString(), int.Parse(sFormCode), bAllowRead);
                                            detail.Insert();
                                        }
                                    }
        ...
        private void FillDetailsUser(string sUserID)
        {
            if (sUserID.Length > 0)
            {
                ...
                List<tbl_securityAuditPermission> permission = tbl_securityAuditPermission.SelectAllByUser_ID(sUserID);
        ...
        private void RefreshGrid_UserPermission(string sUserID)
        {
            ...
            foreach (tbl_securityProcessNoteMaster detail in details)
            {
                if (detail != null)
                {
                     ...
                     iCategoryId = detail.ProcessNoteCategory_ID;
                     sCategoryName = clsGenaralName.getName_ProcessNoteCategory(detail.ProcessNoteCategory_ID);
                     tbl_securityAuditPermission permission = tbl_securityAuditPermission.Select(txtUserID.Tag.ToString(), detail.ProcessNote_ID);
                    if (permission != null)                    
