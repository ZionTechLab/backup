...
namespace Digiteq
{
    public partial class frmReportPermission : MettroForm
    {
        ...
        private void ClearFields()
        {
            ...
            txtCompany.Text = clsSecurity.CompanyName;
        ...
        private void FillDataGrid()
        {
            try
            {
                ...
                if (txtUserID.Tag != null && txtUserID.Tag.ToString().Trim().Length > 0)
                    sUserID = txtUserID.Tag.ToString();

                dtAllRecodes.Merge(DBHandling.ExecQuery("exec sp_ReportMasterLoad '" + sUserID + "','" + txtCompany.Tag.ToString() + "','" + txtBranch.Tag.ToString() + "'").Tables[0]);
        ...
        private void createFilterQuary()
        {
            ...
            if(txtModule.Tag!=null)
                sFinalQuary = "formCategory_ID = '" + txtModule.Tag.ToString() + "'";

            if (txtCategory.Tag != null)
                sFinalQuary = "ReportCategory = '" + txtCategory.Tag.ToString() + "'";

            if (txtSearch.TextLength > 0)
                sFinalQuary += (sFinalQuary != "" ? "AND " : "") + "(ReportName LIKE '%" + txtSearch.Text.Trim() + "%' OR ReportID LIKE '%" + txtSearch.Text.Trim() + "%')";
        ...
        {
            clsSearch.Search_SecurityFormCategory(ref txtModule);
