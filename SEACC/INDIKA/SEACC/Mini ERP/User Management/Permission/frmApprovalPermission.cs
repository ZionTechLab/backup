...
namespace Digiteq
{
    public partial class frmApprovalPermission : Form
    {
        ...
        #region Form Load
        public frmApprovalPermission()
        {
            iFormID = clsSecurity.getFormID(Digiteq_Logic.FormName.UserPermission_PendingApproval);
        ...
        {
            //format Form
            clsFormatter.setFormatForm(this, "User Approval Permission", 1, 0);
                            ...
                            if (IsUpdate)  //update records
                            {
                                for (int x = 0; x < dgvDetail.Rows.Count; x++)
                                {
                                    try
                                    {
                                        ...
                                        tbl_securityApprovalPermission oldRecord = tbl_securityApprovalPermission.Select(txtUserID.Tag.ToString(), int.Parse(sFormCode));
                                        if (oldRecord != null)
                                        {
                                            tbl_securityApprovalPermission oldRecordNew = new tbl_securityApprovalPermission(txtUserID.Tag.ToString(), int.Parse(sFormCode), bAllowRead);
                                            oldRecordNew.Update();
                                        }
                                        else
                                        {
                                            tbl_securityApprovalPermission detail = new tbl_securityApprovalPermission(txtUserID.Tag.ToString(), int.Parse(sFormCode), bAllowRead);
                                            detail.Insert();
                                        }
                                    }
        ...
        private void FillDetailsUser(string sUserID)
        {
            if (sUserID.Length > 0)
            {
                ...
                List<tbl_securityApprovalPermission> permission = tbl_securityApprovalPermission.SelectAllByUser_ID(sUserID);
        ...
        private void RefreshGrid_UserPermission(string sUserID)
        {
            ...
            foreach (tbl_securityProcessNoteMaster detail in details)
            {
                if (detail != null)
                {
                     ...
                     iCategoryId = detail.ProcessNoteCategory_ID;
                     sCategoryName = clsGenaralName.getName_ProcessNoteCategory(detail.ProcessNoteCategory_ID);
                     tbl_securityApprovalPermission permission = tbl_securityApprovalPermission.Select(txtUserID.Tag.ToString(), detail.ProcessNote_ID);
                    if (permission != null)                    
