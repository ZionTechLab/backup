...
namespace Digiteq
{
    public partial class frmItemViewer : Form
    {
        ...
        private void FillDetails(string sItemCode, string sSubCategoryID1, string sSubCategoryID2, string sSerial1, string sSerial2)
        {
            ...
            if (Details != null)
            {
                RefreshGridStoreStock(sItemCode);
             ...
             //   RefreshGridDepartmentStock(sItemCode, sSubCategoryID1, sSubCategoryID2, sSerial1, sSerial2);
                ...
                lblAllStoreBalance.Text = clsFormatter.FormatDecimalPlaces_Quantity(clsHelpMethods_Local.GetQty_AllStoresBalance(Details.Item_ID, sSubCategoryID1, sSubCategoryID2, sSerial1, sSerial2));
                lblMinPRQty.Text = clsFormatter.FormatDecimalPlaces_Quantity(clsHelpMethods_Local.GetQty_MinimumPR(Details.Item_ID, sSubCategoryID1, sSubCategoryID2, sSerial1, sSerial2));
        ...
        #region Refresh Grid
        private void RefreshGridStoreStock(string sItemID)
        {
            try
            {
                dgvDetailStore.DataSource = null; ;

                var dt = DBHandling.ExecQuery("exec sp_Get_StoreStock '" + sItemID + "'").Tables[0];
                dgvDetailStore.DataSource = dt;
