...
namespace Digiteq
{
    public partial class frmSecurityConfigValue : Form
    {
        ...
        private void frm_mtrSecurityConfigValue_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if (txtValueID.TextLength > 0)
                {
                    if (clsSecurity.PermissionToDelete(clsSecurity.UserIDLoged, iFormID))
                    {
                        ...
                        tbl_securityConfigValue detail = tbl_securityConfigValue.Select(Convert.ToInt32(txtValueID.Text.Trim()));
                        DialogResult msgResult = MessageBox.Show(clsFormatter.GetMessageFrom(MessageType.AskForDelete, " Security Configeration Value " + detail.ValueName), clsFormatter.GetMessageCaption(), MessageBoxButtons.YesNo, MessageBoxIcon.Stop);
                        ...
                        try
                        {
                            ...
                            if (txtValueID.TextLength > 0)
                            {
                                if (IsUpdate)  //update records
                                {
                                    ...
                                    tbl_securityConfigValue oldRecord = tbl_securityConfigValue.Select(Convert.ToInt32(txtValueID.Text.Trim()));
                                    if (oldRecord != null)
                                    {
                                        //Country Header
                                        tbl_securityConfigValue detail = new tbl_securityConfigValue(Convert.ToInt32(txtValueID.Text.Trim()), txtValueName.Text.Trim(), txtConfigValue.Text.Trim(), txtTypeValueID.Tag.ToString(), oldRecord.Form_ID);
                                        detail.Update();
                ...
                if (iID > 0)
                {
                    tbl_securityConfigValue detail = tbl_securityConfigValue.Select(iID);
                        if (detail != null)
                        {
                            ...
                            //asign values
                            txtTypeValueID.Tag = detail.ConfigTypeValue_ID;
                            txtTypeValueID.Text = clsCommon.GetForeignKeyValue(clsGenaralName.getName_SecurityConfigType_Value(detail.ConfigTypeValue_ID));
        ...
        private void Search_ConfigValue()
        {
            try
            {
                ...
                clsSearch.Search_MasterSecurityConfig_Value(ref txtValueID);
        ...
        private void Search_ConfigValueType()
        {
            try
            {                
                clsSearch.Search_MasterSecurityConfigType_Value(ref txtTypeValueID);              
            }
            catch (Exception ex)
            {
                clsValidate.WriteErrorLog("", iFormID,ex);
        ...
        private void ValidateEmptyForeignKey()
        {
            try
            {
                clsCommon.ValidateForeignKey(ref txtTypeValueID);
            }
            catch (Exception ex)
            {
                clsValidate.WriteErrorLog("", iFormID,ex);
