...
namespace Digiteq
{
    public partial class frm_rpt_AccountStandardReport : MettroForm
    {
                            ...
                            if (clsHelpMethods_Local.GetReportPath(clsAutocode.getReportID(Report), ref sReportTitle_Main, ref sReportTitle_Sub, ref sReportPath))
                            {
                                ...
                                if (Report == enum_ReportName.ST_ACC_Trail_Balance || Report == enum_ReportName.ST_ACC_Trail_Balance_Advance)
                                {
                                    try
                                    {
                                        ...
                                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("ShowZeros", (chk_HideZeroAmount.Checked ? "0" : "1"), true);
                                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("HideOPBL", (chkHideopbl.Checked ? "0" : "1"), true);
                                ...
                                else if (Report == enum_ReportName.ST_Acc_ProfitAndLoss_Std)
                                {
                                    try
                                    {
                                        ...
                                        frm_ReportViewer_New rpt = new frm_ReportViewer_New();
                                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CurrentFinYear", clsMethods_GL.getFinancialYear_ID(dtpFrom.Value), true);
                                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("PreviusFinYear", clsMethods_GL.getFinancialYear_ID(dtpFrom.Value.AddYears(-1)), true);
                                        rpt.print(sReportPath, glb_dtsGeneralLedger, glb_dtsReportExport.dt_rptParameter, clsAutocode.getReportID(Report));
                                    }
                                    catch (Exception ex)
                                    {
                                ...
                                else if (Report == enum_ReportName.ST_AccountOpeningBalance)
                                {
                                    ...
                                    //                clsHelpMethods_Local.startProgressBar(0, glMasters.Count + 2, 1, ProgressBar);
                                ...
                                else if (Report == enum_ReportName.ST_BankBook)
                                {
                                    try
                                    {
                                        ...
                                        foreach (tbl_accGLMaster oGLMaster in oGLMasters)
                                        {
                                            ...
                                            clsHelpMethods_Local.startProgressBar(0, oGLMasters.Count + 2, 1, ProgressBar);
                                        }
                                    ...
                                    try
                                    {
                                        ...
                                        if (true)
                                        {
                                            ...
                                            foreach (tbl_accGLMaster oAccount in oAccMasters)
                                            {
                                                ...
                                                clsHelpMethods_Local.startProgressBar(0, oAccMasters.Count + 2, 1, ProgressBar);
                                            }
                                    ...
                                    try
                                    {
                                        ...
                                        else
                                        {
                                            if (txtAcctCode.Tag != null)
                                            {
                                                ...
                                                //glb_dts_BankBook.dt_Company.Adddt_CompanyRow(clsSecurity.DigiteqName, clsSecurity.DigiteqEmail, clsSecurity.decryptPassword(clsSecurity.CompanyName), clsSecurity.decryptPassword(clsSecurity.CompanyAddress1), clsSecurity.CompanyAddress2, clsCommon.getCompanyImage(), "Bank Book", "", "", clsSecurity.UserNameLoged, "Filter : Date From " + dtpFrom.Value.ToString("yyyy-MMM-dd") + " - Date To " + dtpTo.Value.ToString("yyyy-MMM-dd") + " | Account : " + txtAcctCode.Text + "");
                                                ...
                                                //"\\Reports\\ACC\\rpt_accBankBook.rpt"
                                ...
                                else if (Report == enum_ReportName.ST_BankReconcilation)
                                {
                                    try
                                    {
                                        ...
                                        if (txtAcctCode.Tag != null && txtAcctCode.Tag.ToString().Trim().Length > 0)
                                        {
                                            ...
                                            string sQuary = "SELECT dbo.[func_AccountOPBL2]('" + dtpTo.Value.ToString("yyyy-MM-dd") + "','" + txtAcctCode.Tag.ToString() + "')";
                                         ...
                                         //   string sQuary = "select opbl from dbo.func_AccountOPBL('1988-8-23','" + dtpTo.Value.ToString("yyyy-MM-dd") + "','" + clsMethods_GL.getFinancialYear_ID(dtpTo.Value.Date) + "','%') where gl_id='" + txtAcctCode.Tag.ToString() + "'";
                                            ...
                                            if (iCompanyAccount_ID != -1)
                                            {
                                              ...
                                              && p.CompanyAccount_ID == iCompanyAccount_ID && !p.IsDeleted && (p.ChequeStatus_ID == clsAutocode.getChequeStatusID(ChequeStatus.Deposited) || p.ChequeStatus_ID == clsAutocode.getChequeStatusID(ChequeStatus.Realized) || p.ChequeStatus_ID == clsAutocode.getChequeStatusID(ChequeStatus.ReDeposited)
                                                ...
                                                {
                                                    foreach (tbl_accChequeRegister accChequeRegister in tbl_accChequeRegister.SelectAll().Where(
                                                        p => p.DateCheque.Date <= dtpTo.Value.Date && p.ChequeAmount > 0 && p.CompanyAccount_ID == iCompanyAccount_ID && !p.IsDeleted
                                                        && (p.ChequeStatus_ID == clsAutocode.getChequeStatusID(ChequeStatus.New)
                                                        || p.ChequeStatus_ID == clsAutocode.getChequeStatusID(ChequeStatus.Realized)
                                                        || p.ChequeStatus_ID == clsAutocode.getChequeStatusID(ChequeStatus.Returned_R))
                                                        ))
                                                    {
                                                        if ((accChequeRegister.ChequeStatus_ID == "3") || (accChequeRegister.ChequeStatus_ID == "4"))
                                                        {
                                                ...
                                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CurrentFiscalYearDate", clsGenaralName.getName_FinancialYearName(clsMethods_GL.getFinancialYear_ID_Current()), true);
                                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("AccountName", txtAcctCode.Text.ToString(), true);
                                    ...
                                    try
                                    {
                                        ...
                                        if (txtAcctCode.Tag != null && txtAcctCode.Tag.ToString().Trim().Length > 0)
                                        {
                                            ...
                                            if (iBankAccountNumber != -1)
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    foreach (tbl_accPaymentVoucher accPV in tbl_accPaymentVoucher.SelectAll().Where(p => p.PaymentVoucherDate.Date <= dtpTo.Value.Date && p.ChequeAmount > 0))
                                                    {
                                                        ...
                                                        foreach (tbl_accChequeRegister accChequeRegister in tbl_accChequeRegister.SelectAllByPaymentVoucher_ID(accPV.PaymentVoucher_ID).Where(p => p.CompanyAccount_ID == iBankAccountNumber && (p.ChequeStatus_ID == clsAutocode.getChequeStatusID(ChequeStatus.New) || p.ChequeStatus_ID == clsAutocode.getChequeStatusID(ChequeStatus.Realized) || p.ChequeStatus_ID == clsAutocode.getChequeStatusID(ChequeStatus.Returned_R) || p.ChequeStatus_ID == clsAutocode.getChequeStatusID(ChequeStatus.Deleted))))
                                                        {
                                                            ...
                                                            if (accChequeRegister.ChequeStatus_ID == "4" && accChequeRegister.IsDeleted)
                                                                if (accChequeRegister.ReconcilationDate.Date <= dtpTo.Value.Date)
                                                                    continue;
                                                ...
                                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("UserName", clsSecurity.UserNameLoged, true);
                                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("CurrentFiscalYearDate", clsGenaralName.getName_FinancialYearName(clsMethods_GL.getFinancialYear_ID_Current()), true);
                                                //  glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("LastFiscalYearDate", clsGenaralName.getName_FinancialYearName(clsMethods_Fin.getLastFinanceYearID()), true);
                                                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("AccountName", txtAcctCode.Text.ToString(), true);
                                ...
                                else if (Report == enum_ReportName.ST_GLCodeWise_SubAccounts)
                                {
                                    ...
                                    if (txtAcctCode.Tag != null && txtAcctCode.Tag.ToString().Trim().Length > 0)
                                    {
                                        ...
                                        foreach (tbl_accGLPosting_Detail oGLPostingDetail in oGLPostingDetails)
                                        {
                                            ...
                                            clsHelpMethods_Local.startProgressBar(0, oGLPostingDetails.Count + 2, 1, ProgressBar);
                                        }
        ...
        private void clearField()
        {
            ...
            clsCommon.SetVisibility_Panel(pnlHideOpbl, false);

            chkUseChequeDate.Checked = true;
            chk_HideZeroAmount.Checked = true;
            chkHideopbl.Checked = true;
        ...
        private void setEnableDisableConctrol(int iReportID)
        {
            ...
            if (iReportID == (int)enum_ReportName.ST_ACC_Trail_Balance)
            {
                ...
                clsCommon.SetVisibility_Panel(pnlHideOpbl, true);
            }
