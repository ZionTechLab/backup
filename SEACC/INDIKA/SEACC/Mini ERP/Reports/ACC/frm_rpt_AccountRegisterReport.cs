    ...
    public partial class frm_rpt_AccountRegisterReport : MettroForm
    {
                            ...
                            if (clsHelpMethods_Local.GetReportPath(clsAutocode.getReportID(Report), ref sReportTitle_Main, ref sReportTitle_Sub, ref sReportPath))
                            {
                                        ...
                                        if (oDetails != null)
                                        {
                                            ...
                                            foreach (tbl_accPaymentVoucher oDetail in oDetails)
                                            {
                                                ...
                                                glb_dts_accPaymentVoucher.dt_accPaymentVoucher.Adddt_accPaymentVoucherRow(oDetail.PaymentVoucher_ID, oDetail.PaymentVoucherDate, oDetail.Payee, "", "", "",
                                                    oDetail.TotalAmount, sCreditorName, "", "", clsGenaralName.getName_AccCostCenter1(oDetail.CostCenter1_ID), clsGenaralName.getName_AccCostCenter2(oDetail.CostCenter2_ID),
                                                    oDetail.DateCreate, clsGenaralName.getName_Employee(oDetail.Employee_ID), sBankName, oDetail.BankAcc_No, sChequeRefNo, sChequeNo, dtmDateCheque, dChequeAmount, 0, oDetail.IsDeleted, oDetail.Narration, oDetail.Remark, false, "", "", "", "");
                                                #endregion
                                ...
                                if (Report == enum_ReportName.RG_Account_Payable_Note_Summary_Report || Report == enum_ReportName.RG_Account_Payable_Note_Detail_Report)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        {
                                            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("DynamicalColoumn", chkUseDateAsBillDate.Checked ? "Bill Date" : "APN Date", true);
        ...
        private void printDataSet(string path, string sReportTitle, string sReportTitle2, DataSet ojbDataSet, string sFilter, string sReportID)
        {
            try
            {
                ...
                {
                    if (chkUseDateAsBillDate.Checked)
                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("DynamicalColoumn", clsCommon.fncsetstring("Bill Date"), true);
                    else
                        glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("DynamicalColoumn", clsCommon.fncsetstring("APN Date"), true);
                }
