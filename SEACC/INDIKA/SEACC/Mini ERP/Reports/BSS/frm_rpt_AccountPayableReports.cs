    ...
    public partial class frm_rpt_AccountPayableReports : MettroForm
    {
                            ...
                            if (clsHelpMethods_Local.GetReportPath(clsAutocode.getReportID(Report), ref sReportTitle_Main, ref sReportTitle_Sub, ref sReportPath))
                            {
                                ...
                                if (Report == enum_ReportName.RG_Supplier_wise_Outstanding_Summary || Report == enum_ReportName.RG_Supplier_wise_Outstanding_Detail)
                                {
                                    try
                                    {
                                        ...
                                        if (chkAPNDate.Checked)
                                            glbDtsSales.dt_bssSupplierOutstanding.Merge(DBHandling.ExecQuery("Exec srh_bssSupplierOutstandingSelectAllBySupplierID_BillDate '" + sSupplier + "','" + dtpDateTo.Value.Date + "','" + iHideDebitNote + "', '" + txtBranch.Tag.ToString() + "'").Tables[0]);
                                        else
                                            glbDtsSales.dt_bssSupplierOutstanding.Merge(DBHandling.ExecQuery("Exec srh_bssSupplierOutstandingSelectAllBySupplierID '" + sSupplier + "','" + dtpDateTo.Value.Date + "','" + iHideDebitNote + "', '" + txtBranch.Tag.ToString() + "'").Tables[0]);
                                ...
                                else if (Report == enum_ReportName.AP_Creditors_Age_anlysis_Detail || Report == enum_ReportName.AP_Creditors_Age_anlysis_Summary)
                                {
                                    try
                                    {
                                        ...
                                        {
                                            oAPNa = tbl_accAccountPayableNote.SelectAllBySupplier_ID(txtSupplier.Tag.ToString().Trim()).Where(p => !p.IsDeleted && !p.IsSeattled).ToList();
                                            sFilter = "Single Supplier -" + txtSupplier.Text;
                                        }
                                        else
                                        {
                                            if (cmbCreditorType.SelectedIndex == 0)
                                                oAPNa = tbl_accAccountPayableNote.SelectAll().Where(p => !p.IsDeleted && !p.IsSeattled).ToList();
                                            else if (cmbCreditorType.SelectedIndex == 1)
                                                oAPNa = tbl_accAccountPayableNote.SelectAll().Where(p => !p.IsDeleted && !p.IsSeattled && p.Supplier_ID != "default").ToList();
                                        ...
                                        foreach (tbl_accAccountPayableNote oAPN in oAPNa)
                                        {
                                            ...
                                            clsHelpMethods_Local.startProgressBar(0, oAPNa.Count + 2, 1, prog_ProgressBar);
                                        }
                                        print(sReportPath, sReportTitle_Main, glb_dts_Accounts.dt_accCreditorsAgeAnalysis, clsAutocode.getReportID(Report));
                                    }
                                    catch (Exception ex)
                                    {
                                ...
                                else if (Report == enum_ReportName.AP_Tax)
                                {
                                    try
                                    {
                                        ...
                                        foreach (tbl_accAccountPayableNote oNotes in oAccountPayble)
                                        {
                                            ...
                                            clsHelpMethods_Local.startProgressBar(0, oAccountPayble.Count + 2, 1, prog_ProgressBar);
                                        }
                                ...
                                if (Report == enum_ReportName.SettledTransactions_Creditor)
                                {
                                    string sQry = "exec [dbo].[sp_GetRpt_SetteledTransactions_Creditor]";
        ...
        private void print(string path, string sReportTitle, DataSet objDataTable, string sReportID)
        {
            try
            {
                ...
                glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("DateRange", "As At : " + dtpDateTo.Value.ToString("dd MMM yyyy"), true);
                ...
                if (Report == enum_ReportName.RG_Supplier_wise_Outstanding_Summary)
                    glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("isSumaryReport", "1", true);
        ...
        private void setEnableDisableConctrol(int iReportID)
        {
            ...
            //else if (rdoCreaditorsAge.Checked)
            //    cmbCreditorType.Show();
            //else if (rdoCreaditorsAgeSummary.Checked)
            //    cmbCreditorType.Show();
        }
        #endregion
...
//if (Report == enum_ReportName.AP_Supplier_Outstanding_GRN || Report == enum_ReportName.AP_Supplier_Outstanding_PO)
//{
//    try
//    {
//        Cursor = Cursors.WaitCursor;
...
//            glb_dtsReportExport.dt_rptParameter.Adddt_rptParameterRow("isPO", sRetortType_Formular, true);
