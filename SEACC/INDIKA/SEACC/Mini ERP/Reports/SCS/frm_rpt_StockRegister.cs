...
namespace Digiteq
{
    public partial class frm_rpt_StockRegister : MettroForm
    {
                    ...
                    try
                    {
                        ...
                        if (clsSecurity.PermissionToPrint_WithMessage(clsAutocode.getReportID(Report)))
                        {
                            ...
                            if (clsHelpMethods_Local.GetReportPath(clsAutocode.getReportID(Report), ref sReportTitle_Main, ref sReportTitle_Sub, ref sReportPath))
                            {
                                ...
                                bool bItemcategorySelected = false;
                                ...
                                if (txtItemCategory.Tag != null && txtItemCategory.Tag.ToString().Length > 0)
                                ...
                                if (bItemcategorySelected)
                                    sFilter += (sFilter != "" ? " | " : "") + " " + lblItemCategory.Text + " : " + txtItemCategory.Text.Trim();
                                    ...
                                    else
                                    {
                                        try
                                        {
                                            ...
                                            foreach (tbl_scsPurchaseOrder detail in oPo)
                                            {
                                                ...
                                                glb_dts_scsPurchaseOrder.dt_scsPurchaseOrder.Adddt_scsPurchaseOrderRow(detail.PurchaseOrder_ID, detail.PurchaseOrderDate, "", "", "", detail.SubTotal, detail.DiscountTotal, detail.VatTotal, detail.GrandTotal, detail.Supplier_ID, clsGenaralName.getName_Supplier(detail.Supplier_ID), clsGenaralName.getSupplierAddressRegister(detail.Supplier_ID), "", "", "", clsGenaralName.getName_CurrencyCode(detail.Currency_ID), detail.DeliveryTerms, detail.DeliveryAddress, detail.Remark, detail.ForexRate, clsGenaralName.getName_StockNoteType(detail.StockNoteType_ID), detail.DateCreate, detail.CostCenter, detail.OrderdBy, detail.DueDate, detail.QuotaionNo, "", detail.IssuedRefNo_ID, detail.IsDeleted, detail.CreateUser_ID, detail.ApprovedUser_ID, detail.DateCreate, detail.DateApproved, 0, detail.NbtTotal, detail.OtherTaxTotal);
                                    ...
                                    else
                                    {
                                        try
                                        {
                                            ...
                                            foreach (tbl_scsPurchaseOrder detail in oPo)
                                            {
                                                ...
                                                glb_dts_scsPurchaseOrder.dt_scsPurchaseOrder.Adddt_scsPurchaseOrderRow(detail.PurchaseOrder_ID, detail.PurchaseOrderDate, "", "", "", detail.SubTotal, detail.DiscountTotal, detail.VatTotal, detail.GrandTotal, detail.Supplier_ID, clsGenaralName.getName_Supplier(detail.Supplier_ID), clsGenaralName.getSupplierAddressRegister(detail.Supplier_ID), "", "", "", clsGenaralName.getName_CurrencyCode(detail.Currency_ID), detail.DeliveryTerms, detail.DeliveryAddress, detail.Remark, detail.ForexRate, clsGenaralName.getName_StockNoteType(detail.StockNoteType_ID), detail.DateCreate, detail.CostCenter, detail.OrderdBy, detail.DueDate, detail.QuotaionNo, "", detail.IssuedRefNo_ID, detail.IsDeleted, detail.CreateUser_ID, detail.ApprovedUser_ID, detail.DateCreate, detail.DateApproved, iItemCount, detail.NbtTotal, detail.OtherTaxTotal);
                                            }
                                            #endregion
                                ...
                                else if (Report == enum_ReportName.RG_Finished_Goods_Transfer_Note_Summary) // rdoFGTNSummary.Checked
                                {
                                    try
                                    {
                                        ...
                                        foreach (tbl_scsStoreProduction oFGTN in oFGTNs)
                                        {
                                            ...
                                            clsHelpMethods_Local.startProgressBar(0, oFGTNs.Count + 2, 1, ProgressBar);
                                        }
                                        #endregion
                                ...
                                else if (Report == enum_ReportName.RG_Finished_Goods_Transfer_Note_Details) //rdoFGTNDetails.Checked
                                {
                                    try
                                    {
                                        ...
                                        foreach (tbl_scsStoreProduction oFGTN in oFGNs)
                                        {
                                            ...
                                            clsHelpMethods_Local.startProgressBar(0, oFGNs.Count + 2, 1, ProgressBar);
                                        }
                                        #endregion
                                    ...
                                    else
                                    {
                                        try
                                        {
                                            ...
                                            foreach (tbl_scsStoreGoodIssueNote detail in tbl_scsStoreGoodIssueNote.SelectAll().Where(p => p.StoreGoodIssueNote_ID != "default" && p.StoreGoodIssueNoteDate.Date >= dtpFrom.Value.Date && p.StoreGoodIssueNoteDate.Date <= dtpTo.Value.Date && p.CompanyBranch_ID == txtBranch.Tag.ToString()))
                                            {
                                                ...
                                                {
                                                    decimal dFloorStockQty = clsHelpMethods_Local.GetFlowStock_Qty(detail.StoreGoodIssueNoteDate, idetail.Item_ID, idetail.FromStore_ID);
                                ...
                                else if (Report == enum_ReportName.RG_GIN_Summary || Report == enum_ReportName.RG_GIN_Detail)
                                {
                                    try
                                    {
                                        ...
                                        frm_ReportViewer_New rpt = new frm_ReportViewer_New();
                                        rpt.print(sReportPath, glb_dtsExternalGoodIssueNote, glb_dtsReportExport.dt_rptParameter, clsHelpMethods_Local.GetReportPath(clsAutocode.getReportID(Report)));
                                    }
                                    catch (Exception ex)
                                    {
                                        clsValidate.WriteErrorLog("", iFormID,ex);
                                ...
                                else if (Report == enum_ReportName.RG_DIN_Summary || Report == enum_ReportName.RG_DIN_Detail)
                                {
                                    try
                                    {
                                        ...
                                        foreach (tbl_scsDiscardedGoodNote oDis in oDisList)
                                        {
                                            ...
                                            clsHelpMethods_Local.startProgressBar(0, oDisList.Count + 2, 1, ProgressBar);
                                        }
                                        #endregion
                                ...
                                else if (Report == enum_ReportName.RG_PurchaseRequisitionSummary || Report == enum_ReportName.RG_PurchaseRequisitionDetail)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        foreach (tbl_scsPurchaseRequisition header in tbl_scsPurchaseRequisition.SelectAll().Where(p => p.PurchaseRequisitionNoteDate.Date >= dtpFrom.Value.Date && p.PurchaseRequisitionNoteDate.Date <= dtpTo.Value.Date))
                                        {
                                            ...
                                            glb_dtsPurchaseRequisitionNote.dt_PurchaseRequisitionNote.Adddt_PurchaseRequisitionNoteRow(header.PurchaseRequisitionNote_ID, header.PurchaseRequisitionNoteDate, clsGenaralName.getName_Department(header.FromDepartment_ID), clsGenaralName.getName_Section(header.FromSection_ID), clsGenaralName.getName_Store(header.FromStore_ID), clsGenaralName.getName_Area(header.FromSelectArea_ID), header.FromSelectArea_ID, header.FromDepartment_ID, header.FromStore_ID, header.Remark, header.Job_ID, header.RequestedBy, header.DateCreate, header.IsDeleted, 1);
                                            ...
                                            if (Report == enum_ReportName.RG_PurchaseRequisitionDetail)
                                            {
                                                ...
                                                {
                                                    glb_dtsPurchaseRequisitionNote.dt_PurchaseRequisitionNoteDetail.Adddt_PurchaseRequisitionNoteDetailRow(detail.PurchaseRequisitionNote_ID, detail.Item_ID, detail.ItemSubCategory_ID, detail.ItemSerialNo, detail.ItemSubCategory2_ID, detail.ItemSerialNo2, detail.FromSelectArea_ID, detail.FromDepartment_ID, detail.FromSection_ID, detail.FromStore_ID, detail.Qty, detail.QtySettle, detail.Weight, clsGenaralName.getName_UomAndCode(detail.Uom_ID), clsGenaralName.getName_Item(detail.Item_ID), clsGenaralName.getName_Uom(detail.Uom_ID), detail.Remark);
                                                }
                                            }
                                        }
                                        #endregion
        ...
        private void clearField()
        {
            ...
            txtItemCategory.Text = "<All Category>";
            ...
            clsCommon.SetEnableDisable_NormalLabel(lblItemID, true);
        ...
        private void setEnableDisableConctrol(int iReportID)
        {
            ...
            //if (iReportID == (int)enum_ReportName.RG_LoanIN || iReportID == (int)enum_ReportName.RG_LoanOut)
            //{
...
//private void rdoPRSummary_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdoPurchaseRequisition_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdoPOSummery_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdoDINSummery_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
...
//    setEnableDisableConctrol();
//}
//private void rdoGrnSummary_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdoGrnDetail_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdbInternalGRNDetails_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdoItemSplitSummary_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdoItemSplitDetail_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdoExternalGINSummerys_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdoExternalGINDetail_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdbInternalGIN_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdoDGNSummery_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdoDGNDetail_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdoDINDetail_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdbInternalStoreSRSummary_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdbInternalStoreSRDetail_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdbInternalStoreGIN_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdbInternalStoreGINDetails_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdbInternalStoreGRN_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdbInternalStoreGRNDetails_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdbInternalSectionSRSummary_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdbInternalSectionSRDetail_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdbInternalSectionGINSummary_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdbInternalSecrtionGINDetail_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdbInternalSecrtionGRNSummary_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdbInternalSectionGRNDetail_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void CkeckBox_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdoPOSummery_CheckedChanged_1(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
//private void rdoPRNSummary_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}

//private void rdoPRNDetail_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
...
//    setEnableDisableConctrol();
//}
//private void rdoStockAdjustmentSummery_CheckedChanged(object sender, EventArgs e)
//{
//    setEnableDisableConctrol();
//}
#endregion
...
#region DIN Summary
//else if (rdoDINSummery.Checked)
//{
//    if (clsSecurity.PermissionToPrint_WithMessage(clsAutocode.getReportID(enum_ReportName.RG_DIN_Summary)))
//    {
//        sFormula = ""; sFilter = "";
//        sFormula += " {vw_rpt_scsDiscardedGoodNote.p_Date} >= '" + dtpFrom.Value.Year.ToString() + dtpFrom.Value.Month.ToString("00") + dtpFrom.Value.Day.ToString("00") + "'" + " and {vw_rpt_scsDiscardedGoodNote.p_Date} <= '" + dtpTo.Value.Year.ToString() + dtpTo.Value.Month.ToString("00") + dtpTo.Value.Day.ToString("00") + "'";

//        if (rdoDeleted.Checked)
//            sFormula += " and {vw_rpt_scsDiscardedGoodNote.isDeleted} =True";
//        if (rdoActual.Checked)
//            sFormula += " and {vw_rpt_scsDiscardedGoodNote.isDeleted} = False";

//        if (bStoreSelected)
//        {
//            sFormula += " and {vw_rpt_scsDiscardedGoodNote.storeName}= '" + txtStore.Text.ToString() + "'";
//            sFilter += " Store Name : " + txtStore.Text.Trim();
//        }

//        iReportNo = (int)enum_ReportName.RG_DIN_Summary;
//        print("\\reports\\SCS\\Registry\\rpt_scs_Ex_DIN_Summary.rpt", " Discarded Items Register ", "[DIN Summary]", "", sFormula.ToString(), sFilter);
//    }
//}
#endregion
...
//else
//{
//if (clsSecurity.PermissionToPrint_WithMessage(clsAutocode.getReportID(enum_ReportName.RG_DIN_Detail)))
//{
//    sFormula = ""; sFilter = "";
//    sFormula += " {vw_rpt_scsDiscardedGoodNote.p_Date} >= '" + dtpFrom.Value.Year.ToString() + dtpFrom.Value.Month.ToString("00") + dtpFrom.Value.Day.ToString("00") + "'" + " and {vw_rpt_scsDiscardedGoodNote.p_Date} <= '" + dtpTo.Value.Year.ToString() + dtpTo.Value.Month.ToString("00") + dtpTo.Value.Day.ToString("00") + "'";
...
//    iReportNo = (int)enum_ReportName.RG_DIN_Detail;
//    print("\\reports\\SCS\\Registry\\rpt_scs_Ex_DIN_Register_Detail.rpt", " Discarded Items Register ", "[DIN Detailed]", "", sFormula.ToString(), sFilter);
//}
//}
#endregion

#region Split Note Detail
//#region ItemSplitDetail
//else if (rdoItemSplitDetail.Checked)
//{
//    if (clsSecurity.PermissionToPrint_WithMessage(clsAutocode.getReportID(enum_ReportName.RG_ItemSplitDetail)))
//    {
//        #region Old Report
//        if (!clsConfig.bDataSetActive_SplitNote)
//        {
//            sFormula = ""; sFilter = "";
//            sFormula += "{vw_rpt_scsItemSplitNote.p_Date} >= '" + dtpFrom.Value.Year.ToString() + dtpFrom.Value.Month.ToString("00") + dtpFrom.Value.Day.ToString("00") + "'" + " and {vw_rpt_scsItemSplitNote.p_Date} <= '" + dtpTo.Value.Year.ToString() + dtpTo.Value.Month.ToString("00") + dtpTo.Value.Day.ToString("00") + "'";

//            if (rdoDeleted.Checked)
//                sFormula += " and {vw_rpt_scsItemSplitNote.isDeleted} = True";
//            if (rdoActual.Checked)
//                sFormula += " and {vw_rpt_scsItemSplitNote.isDeleted} = False";

//            if (bStoreSelected)
//            {
//                sFormula += " and {vw_rpt_scsItemSplitNote.fromStore_ID} = '" + txtStore.Tag.ToString() + "'";
//                sFilter += " Store ID : " + txtStore.Text.Trim();
//            }
...
//            //"\\Reports\\SCS\\Registry\\rpt_scs_In_ItemSplitNote_Detail_Dataset.rpt"
//            iReportNo = (int)enum_ReportName.RG_ItemSplitDetail;
//            print("\\reports\\SCS\\Registry\\rpt_scs_In_ItemSplitNote_Detail.rpt", " Item Splits Register ", "[ISN Detailed]", "", sFormula.ToString(), sFilter);
//        }
//        #endregion
...
//            try
//            {
//                string sReportTitle_Main = "", sReportTitle_Sub = "", sRptPath = "", sItemId = "", sISubCategory = "", sISubCategory2 = "", sIsInOut = "", sRemarks = "";
//                decimal dQty = 0, dWeight = 0;
//                if (clsHelpMethods_Local.GetReportPath(clsAutocode.getReportID(enum_ReportName.RG_ItemSplitDetail), ref sReportTitle_Main, ref sReportTitle_Sub, ref sRptPath))
//                {
//                    if (sRptPath != "")
//                    {
//                        glb_dts_SplitNote.Clear();
...
//                        frm_ReportViewer_New rpt = new frm_ReportViewer_New();
//                        rpt.print(sRptPath, glb_dts_SplitNote, glb_dtsReportExport.dt_rptParameter, clsAutocode.getReportID(enum_ReportName.RG_ItemSplitDetail));
//                    }
//                }
//            }
//            catch (Exception ex)
...
#region ItemSplitSummery New Dataset Edition
//else if (rdoItemSplitSummary.Checked)
//{


//        bool bPermission = false;
//        if (rdoItemSplitSummary.Checked)
//        {
//            if (clsSecurity.PermissionToPrint_WithMessage(clsAutocode.getReportID(enum_ReportName.RG_ItemSplitSummary)))
//                bPermission = true;
...
//            try
//            {
//                enum_ReportName enmReport = enum_ReportName.RG_ItemSplitSummary;
//                if (rdoItemSplitSummary.Checked)
//                    enmReport = enum_ReportName.RG_ItemSplitSummary;
...
//                if (bSupplierSelected)
//                    sFilter += " Salesman Name : " + txtSupplier.Text.Trim();
//                #endregion


//                foreach (tbl_scsItemSpred detail in tbl_scsItemSpred.SelectAll().Where(p => p.CompanyBranch_ID == txtBranch.Tag.ToString() && p.ItemSpredDate.Date >=dtpFrom.Value.Date && p.ItemSpredDate.Date <=dtpTo.Value.Date))
//                    {
...
//                    glb_dtsSplitNote.dt_SplitNote.Adddt_SplitNoteRow(detail.ItemSpred_ID,detail.ItemSpredDate,detail.Remark,"","",detail.QtyInputTotal,detail.QtyOutputTotal,detail.WeightInputTotal,detail.WeightOutputTotal,detail.IsChecked,detail.IsApproved,detail.IsFinished,detail.IsDeleted,detail.IsLocked,detail.IsSeattled,detail.CreateUser_ID,clsGenaralName.getName_User(detail.CreateUser_ID));



//                    }

//                  glb_dtsSplitNote.dt_Company.Adddt_CompanyRow(clsSecurity.DigiteqName, clsSecurity.DigiteqEmail, clsSecurity.CompanyName, clsSecurity.CompanyAddress1, clsSecurity.CompanyAddress2, clsCommon.getCompanyImage(), sReportTitle_Main, sReportTitle_Sub, sDaterange, clsSecurity.UserNameLoged, sFilter);


//                    frm_ReportViewer_New rpt = new frm_ReportViewer_New();
//                    rpt.print(sReportPath, glb_dtsSplitNote, glb_dtsReportExport.dt_rptParameter, clsAutocode.getReportID(enmReport));
//                }
...
//        {
//            //sFormula += " and {vw_rpt_scsItemSplitNote.fromStore_ID}= '" + txtStore.Tag.ToString() + "'";
//            sFormula += " and {vw_rpt_scsItemSplitNote.store_ID}= '" + txtStore.Tag.ToString() + "'";
//            sFilter += " Store : " + txtStore.Text.Trim();
//        }

//        iReportNo = (int)enum_ReportName.RG_ItemSplitSummary;
//        print("\\reports\\SCS\\Registry\\rpt_scs_In_ItemSplitNote_Summary.rpt", " Item Splits Register ", "[ISN Summary]", "", sFormula.ToString(), sFilter);
//    }
//}
#endregion

#region Purchase Requisition Detail Summary
//else if (rdoPurchaseRequisitionDetail.Checked)
//{
//    if (clsSecurity.PermissionToPrint_WithMessage(clsAutocode.getReportID(enum_ReportName.RG_PurchaseRequisitionDetail)))
//    {
//        sFormula = ""; sFilter = "";
//        sFormula += " {vw_rpt_scsPurchaseRequisition.p_Date} >= '" + dtpFrom.Value.Year.ToString() + dtpFrom.Value.Month.ToString("00") + dtpFrom.Value.Day.ToString("00") + "'" + " and {vw_rpt_scsPurchaseRequisition.p_Date} <= '" + dtpTo.Value.Year.ToString() + dtpTo.Value.Month.ToString("00") + dtpTo.Value.Day.ToString("00") + "'";

//        if (rdoDeleted.Checked)
//        {
//            sFormula += " and {vw_rpt_scsPurchaseRequisition.isDeleted} =True";
//            sFilter += (sFilter != "" ? " | " : "") + "Cancelled Records Only ";
//        }
//        if (rdoActual.Checked)
//        {
//            sFormula += " and {vw_rpt_scsPurchaseRequisition.isDeleted} = False";
//            sFilter += (sFilter != "" ? " | " : "") + "Active Records Only ";
//        }
//        if (rdoAll.Checked)
//            sFilter += (sFilter != "" ? " | " : "") + "All Records ";
...
//        if (bStoreSelected)
//        {
//            sFormula += " and {vw_rpt_scsPurchaseRequisition.fromStore_ID}= '" + txtStore.Tag.ToString() + "'";
//            sFilter += "Store ID : " + txtStore.Text.Trim();
//        }
//        if (bSectionSelected)
//        {
//            sFormula += " and {vw_rpt_scsPurchaseRequisition.fromSection_ID}= '" + txtSection.Tag.ToString() + "'";
//            sFilter += "Section ID : " + txtSection.Text.Trim();
//        }
//        if (bDepartmetSelected)
//        {
//            sFormula += " and {vw_rpt_scsPurchaseRequisition.fromDepartment_ID}= '" + txtDepartment.Tag.ToString() + "'";
//            sFilter += "Department ID : " + txtDepartment.Text.Trim();
//        }


//        iReportNo = (int)enum_ReportName.RG_PurchaseRequisitionDetail;
//        print("\\reports\\SCS\\Registry\\rpt_scs_Ex_PurchaseRequisition_Detail.rpt", " Purchase Requisition Register ", "[PR Detailed]", "", sFormula.ToString(), sFilter);
//    }
//}
#endregion

#region PR Summery
//else if (rdoPRSummary.Checked)
//{
//    if (clsSecurity.PermissionToPrint_WithMessage(clsAutocode.getReportID(enum_ReportName.RG_PurchaseRequisitionSummary)))
//    {
//        sFormula = "";
//        sFilter = "";
//        sFormula += "{vw_rpt_scsPurchaseRequisition.p_Date} >= '" + dtpFrom.Value.Year.ToString() + dtpFrom.Value.Month.ToString("00") + dtpFrom.Value.Day.ToString("00") + "'" + " and {vw_rpt_scsPurchaseRequisition.p_Date} <= '" + dtpTo.Value.Year.ToString() + dtpTo.Value.Month.ToString("00") + dtpTo.Value.Day.ToString("00") + "'";
...
//        iReportNo = (int)enum_ReportName.RG_PurchaseRequisitionSummary;
//        print("\\reports\\SCS\\Registry\\rpt_scs_Ex_PurchaseRequisition_Summary.rpt", " Purchase Requisition Register ", "[PR Summary]", "", sFormula.ToString(), sFilter);
//    }
//}
#endregion

#region DGN Detailed Report
//else if (rdoDGNDetail.Checked)
//{
//    if (clsSecurity.PermissionToPrint_WithMessage(clsAutocode.getReportID(enum_ReportName.RG_DGN_Detail)))
//    {
//        sFormula = ""; sFilter = "";
//        sFormula += " {vw_rpt_scsDamagedGoodNote.p_Date} >= '" + dtpFrom.Value.Year.ToString() + dtpFrom.Value.Month.ToString("00") + dtpFrom.Value.Day.ToString("00") + "'" + " and {vw_rpt_scsDamagedGoodNote.p_Date} <= '" + dtpTo.Value.Year.ToString() + dtpTo.Value.Month.ToString("00") + dtpTo.Value.Day.ToString("00") + "'";

//        if (rdoDeleted.Checked)
//        {
//            sFormula += " and {vw_rpt_scsDamagedGoodNote.isDeleted} =True";
//            sFilter += (sFilter != "" ? " | " : "") + "Cancelled Records Only ";
//        }
//        if (rdoActual.Checked)
//        {
//            sFormula += " and {vw_rpt_scsDamagedGoodNote.isDeleted} = False";
//            sFilter += (sFilter != "" ? " | " : "") + "Active Records Only ";
//        }
//        if (rdoAll.Checked)
//            sFilter += (sFilter != "" ? " | " : "") + "All Records ";
...
//        if (bStoreSelected)
//        {
//            sFormula += " and {vw_rpt_scsDamagedGoodNote.storeName}= '" + txtStore.Text.ToString() + "'";
//            sFilter += " Store Name : " + txtStore.Text.Trim();
//        }

//        iReportNo = (int)enum_ReportName.RG_DGN_Detail;
//        print("\\reports\\SCS\\Registry\\rpt_scs_Ex_DGN_Registry_Detail.rpt", "Damaged Goods Register", "[DGN Detailed]", " ", sFormula.ToString(), sFilter);
//    }
//}
#endregion

#region  DGN Summary
//else if (rdoDGNSummery.Checked)
//{
//    if (clsSecurity.PermissionToPrint_WithMessage(clsAutocode.getReportID(enum_ReportName.RG_DGN_Summary)))
//    {
//        sFormula = ""; sFilter = "";
//        sFormula += " {vw_rpt_scsDamagedGoodNote.p_Date} >= '" + dtpFrom.Value.Year.ToString() + dtpFrom.Value.Month.ToString("00") + dtpFrom.Value.Day.ToString("00") + "'" + " and {vw_rpt_scsDamagedGoodNote.p_Date} <= '" + dtpTo.Value.Year.ToString() + dtpTo.Value.Month.ToString("00") + dtpTo.Value.Day.ToString("00") + "'";
...
//        iReportNo = (int)enum_ReportName.RG_DGN_Summary;
//        print("\\reports\\SCS\\Registry\\rpt_scs_Ex_DGN_Summary.rpt", " Damaged Goods Register ", "[DGN Summary]", "", sFormula.ToString(), sFilter);
//    }
//}
#endregion

#region Store Internal-SR Summary
//else if (rdbInternalStoreSRSummary.Checked)
//{
//    if (clsSecurity.PermissionToPrint_WithMessage(clsAutocode.getReportID(enum_ReportName.RG_Internal_Store_ISRSummary)))
//    {
//        sFormula = ""; sFilter = "";
//        sFormula += " {vw_rpt_scsStoreRequisitionNote.p_Date} >= '" + dtpFrom.Value.Year.ToString() + dtpFrom.Value.Month.ToString("00") + dtpFrom.Value.Day.ToString("00") + "'" + " and {vw_rpt_scsStoreRequisitionNote.p_Date} <= '" + dtpTo.Value.Year.ToString() + dtpTo.Value.Month.ToString("00") + dtpTo.Value.Day.ToString("00") + "'";
//        if (rdoDeleted.Checked)
//        {
//            sFormula += " and {vw_rpt_scsStoreRequisitionNote.isDeleted} =True";
//            sFilter += (sFilter != "" ? " | " : "") + "Cancelled Records Only ";
//        }
//        if (rdoActual.Checked)
//        {
//            sFormula += " and {vw_rpt_scsStoreRequisitionNote.isDeleted} = False";
//            sFilter += (sFilter != "" ? " | " : "") + "Active Records Only ";
//        }
//        if (rdoAll.Checked)
//            sFilter += (sFilter != "" ? " | " : "") + "All Records ";
...
//        if (bStoreSelected)
//        {
//            sFormula += " and {vw_rpt_scsStoreRequisitionNote.toStore_ID}= '" + txtStore.Tag.ToString() + "'";
//            sFilter += " Store ID : " + txtStore.Text.Trim();
//        }
//        if (bSectionSelected)
//        {
//            sFormula += " and {vw_rpt_scsStoreRequisitionNote.toSection_ID}= '" + txtSection.Tag.ToString() + "'";
//            sFilter += " Section ID : " + txtSection.Text.Trim();
//        }
//        if (bDepartmetSelected)
//        {
//            sFormula += " and {vw_rpt_scsStoreRequisitionNote.toDepartment_ID}= '" + txtDepartment.Tag.ToString() + "'";
//            sFilter += " Department ID : " + txtDepartment.Text.Trim();
//        }

//        iReportNo = (int)enum_ReportName.RG_Internal_Store_ISRSummary;
//        print("\\reports\\SCS\\Registry\\rpt_scs_Store_iSR_Summary.rpt", "Inter-Stores Goods Trf.", "[IST-Stores Summary]", " ", sFormula.ToString(), sFilter);
//    }
//}
#endregion

#region  Store Internal-SR Detail Summary
//else if (rdbInternalStoreSRDetail.Checked)
//{
//    if (clsSecurity.PermissionToPrint_WithMessage(clsAutocode.getReportID(enum_ReportName.RG_Internal_Store_ISR_Detail)))
//    {
//        sFormula = "";
//        sFormula += " {vw_rpt_scsStoreRequisitionNote.p_Date} >= '" + dtpFrom.Value.Year.ToString() + dtpFrom.Value.Month.ToString("00") + dtpFrom.Value.Day.ToString("00") + "'" + " and {vw_rpt_scsStoreRequisitionNote.p_Date} <= '" + dtpTo.Value.Year.ToString() + dtpTo.Value.Month.ToString("00") + dtpTo.Value.Day.ToString("00") + "'";
...
//        //if (bSuplierSelected)
//        //    sFormula.Append(" and {vw_rpt_scsPurchaseOrder.supplier_ID}" + " = '" + txtSupplier.Tag.ToString() + "' ");

//        iReportNo = (int)enum_ReportName.RG_Internal_Store_ISR_Detail;
//        print("\\reports\\SCS\\Registry\\rpt_scs_Store_iSR_Register_Detail.rpt", " Inter-Stores Goods Trf. ", "[IST-Stores Detailed]", "", sFormula.ToString(), sFilter);
//    }
//}
#endregion

#region GIN Detail Report
//else if (rdoExternalGINDetail.Checked)
//{
//    if (clsSecurity.PermissionToPrint_WithMessage(clsAutocode.getReportID(enum_ReportName.RG_GIN_Detail)))
//    {
//        sFormula = ""; sFilter = "";
//        sFormula += " {vw_rpt_scsExternalGoodIssuedNote.p_Date} >= '" + dtpFrom.Value.Year.ToString() + dtpFrom.Value.Month.ToString("00") + dtpFrom.Value.Day.ToString("00") + "'" + " and {vw_rpt_scsExternalGoodIssuedNote.p_Date} <= '" + dtpTo.Value.Year.ToString() + dtpTo.Value.Month.ToString("00") + dtpTo.Value.Day.ToString("00") + "'";

//        if (rdoDeleted.Checked)
//        {
//            sFormula += " and {vw_rpt_scsExternalGoodIssuedNote.isDeleted} =True";
//            sFilter += (sFilter != "" ? " | " : "") + "Cancelled Records Only ";
//        }
//        if (rdoActual.Checked)
//        {
//            sFormula += " and {vw_rpt_scsExternalGoodIssuedNote.isDeleted} = False";
//            sFilter += (sFilter != "" ? " | " : "") + "Active records Only ";
//        }
//        if (rdoAll.Checked)
//            sFilter += (sFilter != "" ? " | " : "") + "All Records ";
...
//        if (bStoreSelected)
//        {
//            sFormula += " and {vw_rpt_scsExternalGoodIssuedNote.store_ID}" + " = '" + txtStore.Tag.ToString() + "' ";
//            sFilter += " Store ID : " + txtStore.Text.Trim();
//        }
//        if (bDepartmetSelected)
//        {
//            sFormula += " and {vw_rpt_scsExternalGoodIssuedNote.department_ID}" + " = '" + txtDepartment.Tag.ToString() + "' ";
//            sFilter += " Department ID : " + txtDepartment.Text.Trim();
//        }
//        if (bCustomerSelected)
//        {
//            sFormula += " and {vw_rpt_scsExternalGoodIssuedNote.customer_ID}" + " = '" + txtCustomer.Tag.ToString() + "' ";
//            sFilter += " Customer ID : " + txtCustomer.Text.Trim();
//        }
//        if (bSupplierSelected)
//        {
//            sFormula += " and {vw_rpt_scsExternalGoodIssuedNote.supplier_ID}" + " = '" + txtSupplier.Tag.ToString() + "' ";
//            sFilter += " Supplier ID : " + txtSupplier.Text.Trim();
//        }
//        if (bItemIDSelected)
//        {
//            sFormula += " and {vw_rpt_scsExternalGoodIssuedNote_Detail.item_ID}" + " = '" + txtItemID.Tag.ToString() + "' ";
//            sFilter += " Item ID : " + txtItemID.Text.Trim();
//        }

//        iReportNo = (int)enum_ReportName.RG_GIN_Detail;
//        print("\\reports\\SCS\\Registry\\rpt_scs_Ex_GoodIssueNote_Detail.rpt", "Goods Issued Register", "[GIN Detailed]", "", sFormula.ToString(), sFilter);
//    }
//}
#endregion

#region  GIN Summary
//else if (rdoExternalGINSummery.Checked)
//{
//    if (clsSecurity.PermissionToPrint_WithMessage(clsAutocode.getReportID(enum_ReportName.RG_GIN_Summary)))
//    {
//        sFormula = ""; sFilter = "";
//        sFormula += " {vw_rpt_scsExternalGoodIssuedNote.p_Date} >= '" + dtpFrom.Value.Year.ToString() + dtpFrom.Value.Month.ToString("00") + dtpFrom.Value.Day.ToString("00") + "'" + " and {vw_rpt_scsExternalGoodIssuedNote.p_Date} <= '" + dtpTo.Value.Year.ToString() + dtpTo.Value.Month.ToString("00") + dtpTo.Value.Day.ToString("00") + "'";
...
//            sFilter += " Supplier ID : " + txtSupplier.Text.Trim();
//        }
//        iReportNo = (int)enum_ReportName.RG_GIN_Summary;
//        print("\\reports\\SCS\\Registry\\rpt_scs_Ex_GoodIssueNote_Summary.rpt", "Goods Issued Register", "[GIN Summary]", "", sFormula.ToString(), sFilter);
//    }
//}
#endregion
...
#region GRN Detail
#region Old Method
//  #region Commented  area - views
//  //-----------------------Commented - views-------------------------------------------------------------------
////  sFilter = "";//sFormula = "";
//  sFormula += " {vw_rpt_scsExternalGoodReceivedNote.p_Date} >= '" + dtpFrom.Value.Year.ToString() + dtpFrom.Value.Month.ToString("00") + dtpFrom.Value.Day.ToString("00") + "'" + " and {vw_rpt_scsExternalGoodReceivedNote.p_Date} <= '" + dtpTo.Value.Year.ToString() + dtpTo.Value.Month.ToString("00") + dtpTo.Value.Day.ToString("00") + "'";
//  if (bStoreSelected)
//  {
//      sFormula += " and {vw_rpt_scsExternalGoodReceivedNote.store_ID}= '" + txtStore.Tag.ToString() + "'";
//      sFilter += " Store ID : " + txtStore.Text.Trim();
//  }
//  if (bSuplierSelected)
//  {
//      sFormula += " and {vw_rpt_scsExternalGoodReceivedNote.supplier_ID} = '" + txtSupplier.Tag.ToString() + "'";
//      sFilter += " Supplier ID : " + txtSupplier.Text.Trim();
//  }
//  if (bItemIDSelected)
//  {
//      sFormula += " and {vw_rpt_scsExternalGoodReceivedNote_Detail.item_ID}= '" + txtItemID.Tag.ToString() + "'";
//      sFilter += " Item ID : " + txtItemID.Text.Trim();
//  }

//  if (rdoDeleted.Checked)
//      sFormula += " and {vw_rpt_scsExternalGoodReceivedNote.isDeleted} =True";

//  if (rdoActual.Checked)
//      sFormula += " and {vw_rpt_scsExternalGoodReceivedNote.isDeleted} = False";

//  if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.idealWheels.ToString())
//      print("\\reports\\SCS\\Registry\\rpt_scs_Ex_GRN_Registry_Detail_APL.rpt", " Goods Received Register ", "[GRN Detailed]", "", sFormula.ToString(), sFilter);
//  else
//      print("\\reports\\SCS\\Registry\\rpt_scs_Ex_GRN_Registry_Detail.rpt", "Goods Received Register", "[GRN Detailed]", "", sFormula.ToString(), sFilter);
//  //------------------------------------------------------End Commented - views----------------------------------------------------------------
//  // }
//  #endregion 
#endregion
...
//string sReportTitle_Main = "", sReportTitle_Sub = "", sReportPath = "";
//if (clsHelpMethods_Local.GetReportPath(clsAutocode.getReportID(), ref sReportTitle_Main, ref sReportTitle_Sub, ref sReportPath))
//{
//if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.gem.ToString())
//{
//    //fill data table
//    #region GRN_Registry_Detail_Gem
//    foreach (tbl_scsExternalGoodReceivedNote oGRN in tbl_scsExternalGoodReceivedNote.SelectAll().Where(p => p.ExternalGoodReceivedNote_ID != "default" && p.ExternalGoodReceivedNoteDate.Date >= dtpFrom.Value.Date && p.ExternalGoodReceivedNoteDate.Date <= dtpTo.Value.Date && p.CompanyBranch_ID == clsSecurity.BranchID))
//    {
//        decimal dGemWeight = 0;
//        bool bStoreOK = true, bSupplierOK = true, bItemOK = true;
//        if (bStoreSelected)
...
//            string sSupplierName = clsGenaralName.getName_Supplier(oGRN.Supplier_ID);
//            string sStockNoteType = clsGenaralName.getName_StockNoteType(oGRN.StockNoteType_ID);
//            //foreach (tbl_scsExternalGoodReceivedNote_Detail_Gem oDetail in tbl_scsExternalGoodReceivedNote_Detail_Gem.SelectAllByExternalGoodReceivedNote_ID(oGRN.ExternalGoodReceivedNote_ID))
//            //{
//            //    dGemWeight += oDetail.MetalWeight;
//            //    string sOldItem = "", sItemType = "";
//            //    tbl_genItemMaster_Gem oItem = tbl_genItemMaster_Gem.Select(oDetail.Item_ID);
//            //    if (oItem != null)
//            //    {
//            //        sOldItem = oItem.RefNo;
//            //        sItemType = clsGenaralName.getName_ItemType(oItem.ItemType_ID);
//            //    }
//            //    if (bItemIDSelected)
//            //        bItemOK = txtItemID.Tag.ToString().Trim() == oDetail.Item_ID ? true : false;
//            //    if (bItemOK)
//            //    {
//            //        string sItemName = clsGenaralName.getName_Item(oDetail.Item_ID);
//            //        glb_dtsScsGoodReceivedNote_Gem.dt_scsGoodReceivedNote_Gem.Adddt_scsGoodReceivedNote_GemRow(oDetail.ExternalGoodReceivedNote_ID, oDetail.Item_ID, clsGenaralName.getName_Item(oDetail.Item_ID), oDetail.ItemSubCategory_ID, oDetail.ItemSubCategory2_ID, oDetail.ItemSerialNo, oDetail.ItemSerialNo2, oDetail.GemDetail, oDetail.MetalDetail, oDetail.MetalWeight, oDetail.GemWeight, oDetail.GemQty, oDetail.SellingPrice, sOldItem, sItemType);
//            //    }
//            //}
//            glb_dtsScsGoodReceivedNote_Gem.dt_scsGoodReceivedNote.Adddt_scsGoodReceivedNoteRow(oGRN.ExternalGoodReceivedNote_ID, oGRN.ExternalGoodReceivedNoteDate, oGRN.Remark,
//               oGRN.Supplier_ID, sSupplierName, (oGRN.PurchaseOrder_ID) == "default" ? "" : (oGRN.PurchaseOrder_ID), oGRN.Store_ID, oGRN.Currency_ID, oGRN.CurrencyRate, oGRN.PaymentTerms, oGRN.PaymentMode, oGRN.CreditPeriod, oGRN.PaymentDueDate, (oGRN.DeliveryOrderNumber) == "default" ? "" : (oGRN.DeliveryOrderNumber), oGRN.InvoiceNo, oGRN.DiscountPercentage, oGRN.NbtPercentage, oGRN.VatPercentage, oGRN.OtherTaxPercentage, oGRN.SubTotal, oGRN.DiscountTotal, oGRN.NbtTotal, oGRN.VatTotal, oGRN.OtherTaxTotal, oGRN.GrandTotal, (oGRN.CostCenter) == "default" ? "" : (oGRN.CostCenter)
//               , sStoreName, sStockNoteType, clsGenaralName.getName_User(oGRN.CreateUser_ID), dGemWeight, oGRN.IsWeightCalculation, oGRN.IsDeleted);
//        }
//    }
//    #endregion
//    iReportNo = (int)enum_ReportName.RG_GRNDetail;
//    print("\\reports\\SCS\\Registry\\rpt_scs_GRN_Registry_Detail_gem.rpt", " Goods Received Register ", "[GRN Detailed]", "", glb_dtsScsGoodReceivedNote_Gem);
//}
//else
//{
//    //fill data table
//    //change the report display format for new version
//    //print("\\reports\\SCS\\Registry\\rpt_scs_GRN_Registry_Detail.rpt", " Goods Received Register ", "[GRN Detailed]", "", glb_dtsScsGoodReceivedNote_Gem);
//    if (rdoDeleted.Checked)
//        sFilter += (sFilter != "" ? " | " : "") + "Cancelled Records Only ";
//    if (rdoActual.Checked)
//        sFilter += (sFilter != "" ? " | " : "") + "Active records Only ";
...
#region Damage Good Note
//    bool bIsDetail = false, bIsShow = true;
//    if (rdbGTND.Checked)
//        bIsDetail = true;

////    if (clsSecurity.PermissionToPrint_WithMessage(clsAutocode.getReportID(enum_ReportName.NP_DamagedGoodsNote)))
//    {
//        try
//        {
//            Cursor = Cursors.WaitCursor;
...
//            foreach (tbl_scsStoreGoodIssueNote oGTN in tbl_scsStoreGoodIssueNote.SelectAll().Where(p => p.StoreGoodIssueNote_ID != "default" && p.StoreGoodIssueNoteDate.Date >= dtpFrom.Value.Date && p.StoreGoodIssueNoteDate.Date <= dtpFrom.Value.Date))
//            {
...
//                if (bIsShow)
//                {
//                    glb_dtsScsGoodTransferNote.dt_scsGoodTransferNote.Adddt_scsGoodTransferNoteRow(oGTN.StoreGoodIssueNote_ID, oGTN.StoreGoodIssueNoteDate, oGTN.FromStore_ID, oGTN.ToStore_ID, oGTN.Job_ID, oGTN.IsDeleted ? "Deleted" : "Active", oGTN.Remark);
//                    if (bIsDetail)
//                    {
//                        foreach (tbl_scsStoreGoodIssueNote_Detail detail in tbl_scsStoreGoodIssueNote_Detail.SelectAllByStoreGoodIssueNote_ID(oGTN.StoreGoodIssueNote_ID))
//                        {
//                            string sItemTypeID = "", sItemName = "";
//                            tbl_genItemMaster oItem = tbl_genItemMaster.Select(detail.Item_ID);
//                            if (oItem != null)
//                            {
//                                sItemTypeID = oItem.ItemType_ID;
//                                sItemName = oItem.ItemName;
//                            }
//                            glb_dtsScsGoodTransferNote.dt_scsGoodTransferNote_Detail.Adddt_scsGoodTransferNote_DetailRow(detail.StoreGoodIssueNote_ID, detail.Item_ID, detail.ItemSerialNo, sItemTypeID, clsGenaralName.getName_ItemType(sItemTypeID), sItemName, detail.Uom_ID, detail.Qty, detail.Weight, detail.TatalCost_FIFO, "");
//                        }
//                    }
//                }
//            }
...
//            {
//                if (clsConfig.sSoftwareModel.Trim() == SoftwareModel_Sales.gem.ToString())
//                    print("\\Reports\\SCS\\Registry\\rpt_scs_GoodTransferNote_Detail_Gem.rpt", "Goods Transfer Register", "[GTN Detailed]", "", glb_dtsScsGoodTransferNote);
//                else
//                    print("\\Reports\\SCS\\Registry\\rpt_scs_GoodTransferNote_Detail.rpt", "Goods Transfer Register", "[GTN Detailed]", "", glb_dtsScsGoodTransferNote);
//            }
//            else
//                print("\\Reports\\SCS\\Registry\\rpt_scs_GoodTransferNote_Summary.rpt", "Goods Transfer Register", "[GTN Summary]", "", glb_dtsScsGoodTransferNote);
//        }
//        catch (Exception ex)
//        {
//            SEACCException.Show(ex);
