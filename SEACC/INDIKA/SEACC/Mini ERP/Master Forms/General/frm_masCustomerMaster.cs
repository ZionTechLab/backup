...
namespace Digiteq
{
    public partial class frm_masCustomerMaster : SEACC_Form
    {
        ...
        private void frmSupplierMaster_Load(object sender, EventArgs e)
        {
            ...
            if (clsSecurity.PermissionToSave(clsSecurity.UserIDLoged, 2000, true, false))
                chkBlacklisted.Enabled = true;
            else
                chkBlacklisted.Enabled = false;

            if (clsSecurity.PermissionToSave(clsSecurity.UserIDLoged, 2001, true, false))
                panel3.Enabled = true;
            else
                panel3.Enabled = false;

            if (clsSecurity.PermissionToSave(clsSecurity.UserIDLoged, 2003, true, false))
                panel4.Enabled = true;
            else
                panel4.Enabled = false;

            if (clsSecurity.PermissionToSave(clsSecurity.UserIDLoged, 2002, true, false))
                pnlOrderPermishionOveride.Enabled = true;
            else
                pnlOrderPermishionOveride.Enabled = false;

            if (clsSecurity.PermissionToApproved(clsSecurity.UserIDLoged, 2))
        ...
        private void Update_Image(string Customer_ID)
        {
            Byte[] img = new byte[0];
            Byte[] img2 = new byte[0];

            img = ImageToByte(pbxImage.Image);
            img2 = ImageToByte(pbxImage2.Image);

            tbl_genCustomerMaster_Image cImage = tbl_genCustomerMaster_Image.Select(Customer_ID);
            if (cImage != null)
            {
                tbl_genCustomerMaster_Image imageRecord = new tbl_genCustomerMaster_Image(Customer_ID, img, img2);
                imageRecord.Update();
            }
            else
            {
                tbl_genCustomerMaster_Image imageRecord = new tbl_genCustomerMaster_Image(txtCustomerID.Tag.ToString(), img, img2);
                imageRecord.Insert();
            }
        }
                                ...
                                if (IsUpdate)  //update records
                                {
                                    if (txtCustomerID.Tag != null && txtCustomerID.Tag.ToString().Trim().Length > 0)
                                    {
                                        ...
                                        if (oldRecord != null)
                                        {
                                            ...
                                            Update_Image(oldRecord.Customer_ID);
                                            ...
                                            tbl_genCustomerMaster detail = new tbl_genCustomerMaster(txtCustomerID.Tag.ToString().Trim(), txtCustomerID.Tag.ToString().Trim(), txtCustomerName.Text.Trim(),
                                               ...
                                               "", chkVATEnable.Checked, chkSVATEnable.Checked, chkNBTEnable.Checked, false, chkCuswiseItemCodeEnable.Checked, txtTitle.Text.Trim(), txtNICNo.Text.Trim(),
                                               dtDateOfBirth.Value, txtAccountType.Tag.ToString(), chkIsPostingEnable_Vat.Checked, chkIsPostingEnable_NBT.Checked, txtSalesReturnGLID.Tag.ToString(),
                                               oldRecord.IsCashCustomer, clsSecurity.CompanyID, clsSecurity.BranchID, cmbPriceMode.SelectedIndex, (cmbItemPrice.SelectedIndex != -1) ? ((ComboBoxItem)cmbItemPrice.SelectedItem).Value : "default",
                                               oldRecord.CreateUser_ID, clsSecurity.UserIDLoged, oldRecord.DeletedUser_ID, oldRecord.CreateTerminal_ID, clsSecurity.TerminalID, oldRecord.DeletedTerminal_ID, oldRecord.DateCreate,
                                               clsSecurity.getServerDateTime(), oldRecord.DateDeleted, txtSalesAcctCode.Tag.ToString(), chkPosCustomer.Checked, false, 0,chkLockCrPeriod.Checked,chkLockCrLimit.Checked,chkLockRtnChq.Checked, txtWhatsApp.Text.ToString());
                                            detail.Update();
                                            ...
                                            if (finance != null)
                                            {
                                                ...
                                                finance.CreditPeriod = decimal.Parse(cmbCreditPeriod.Text.Trim());
                                            ...
                                            else
                                            {
                                                tbl_genCustomerFinance newFinance = new tbl_genCustomerFinance(txtCustomerID.Tag.ToString().Trim(), decimal.Parse(txtDepositAmount.Text.Trim()),
                                                    decimal.Parse(cmbCreditPeriod.Text.Trim()), decimal.Parse(txtCreditLimit.Text.Trim()), decimal.Parse(txtSalesDues.Text.Trim()),
                                                    decimal.Parse(txtCreditBalance.Text.Trim()), decimal.Parse(txtTotalSales.Text.Trim())
                                                    , 0,                             0, 0, 0,                                                    0, 0, 0, 
                                                    clsSecurity.getServerDateTime(), 0,
                                ...
                                else  //insert records
                                {
                                    ...
                                    if (txtCustomerID.Text.Trim().Length > 0)
                                    {
                                        ...
                                        tbl_genCustomerMaster detail = new tbl_genCustomerMaster(txtCustomerID.Tag.ToString().Trim(), txtCustomerID.Tag.ToString().Trim(), txtCustomerName.Text.Trim(),
                                              ...
                                              txtCustomerTypeID.Tag.ToString(), txtCategoryID.Tag.ToString(), txtCustomerClassID.Tag.ToString(), txtCurrencyID.Tag.ToString(), txtSalesManagerID.Tag.ToString(),
                                              txtAreaManagerID.Tag.ToString(), txtSalesRepID.Tag.ToString(), txtSalesManagerID.Tag.ToString(),
                                              "", chkVATEnable.Checked, chkSVATEnable.Checked, chkNBTEnable.Checked, false, chkCuswiseItemCodeEnable.Checked, txtTitle.Text.Trim(), txtNICNo.Text.Trim(), dtDateOfBirth.Value, txtAccountType.Tag.ToString(), chkIsPostingEnable_Vat.Checked, chkIsPostingEnable_NBT.Checked, txtSalesReturnGLID.Tag.ToString(), false, clsSecurity.CompanyID, clsSecurity.BranchID, cmbPriceMode.SelectedIndex, (cmbItemPrice.SelectedIndex != -1) ? ((ComboBoxItem)cmbItemPrice.SelectedItem).Value : "default", clsSecurity.UserIDLoged, "default", "default", clsSecurity.TerminalID, "default", "default", clsSecurity.getServerDateTime(), clsSecurity.getServerDateTime(), clsSecurity.getServerDateTime(), txtSalesAcctCode.Tag.ToString(), chkPosCustomer.Checked, false, 0, chkLockCrPeriod.Checked, chkLockCrLimit.Checked, chkLockRtnChq.Checked
                                              , txtWhatsApp.Text.ToString());
                                        detail.Insert();
                                        ...
                                        tbl_genCustomerFinance finance = new tbl_genCustomerFinance(txtCustomerID.Tag.ToString().Trim(), decimal.Parse(txtDepositAmount.Text.Trim()),
                                            decimal.Parse(cmbCreditPeriod.Text.Trim()), decimal.Parse(txtCreditLimit.Text.Trim()), decimal.Parse(txtSalesDues.Text.Trim()),
                                            decimal.Parse(txtCreditBalance.Text.Trim()), decimal.Parse(txtTotalSales.Text.Trim()), 0,
                                            0, 0, 0,
                                            0, 0, 0, clsSecurity.getServerDateTime(), 0,
                                            decimal.Parse(txtLoyaltyAmount.Text.Trim()), btLoyalityStartDate, txtLoyalityCardNo.Text, 0, 0, decimal.Parse(txtCreditPeriod_Commisstion.Text.Trim())
                                          );
                                        finance.Insert();

                                        Update_Image(detail.Customer_ID);
                                        UpdateContactDetails();
        ...
        private void btnLoadImage_Click(object sender, EventArgs e)
        {
         ...
         //   s_FileName = filedialog.FileName;
            pbxImage.ImageLocation = filedialog.FileName;
        }
        #endregion
        ...
        private void btnAddBranches_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (sBranchName != "")
                    {
                        if (clsConfig.bDisableMultipleCustomerBranch && dgvBranches.RowCount > 0)
                        {
                            MessageBox.Show("Couldn't Create Multiple Customer Branches.", clsFormatter.GetMessageCaption(), MessageBoxButtons.OK, MessageBoxIcon.Information);
                            btnClearBranches_Click(null, null);
                            return;
                        }
        ...
        private void frm_masCustomerMaster_SF_printButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (clsSecurity.PermissionToPrint_WithMessage(clsAutocode.getReportID(enum_ReportName.Mas_Customer)))
                {
                    ...
                    if (clsHelpMethods_Local.GetReportPath(clsAutocode.getReportID(enum_ReportName.Mas_Customer), ref sReportTitle_Main, ref sReportTitle_Sub, ref sReportPath))
                    {
        ...
        private void ClearFields()
        {
           ...
           // txtDebtorCode.Clear();
            ...
            cmbCreditPeriod.Text = "0";
         //   txtCreditPeriod2.Text = "0";
          //  txtCreditPeriod3.Text = "0";
        ...
        //    txtChequeDeposittedAmount.Text = "0.00";
         ...
         //   txtChequeRealizedCount.Text = "0";
          ...
          //  txtChequeReturnedCount.Text = "0";
            ...
            chkLockCrPeriod.Checked = false;
            chkLockCrLimit.Checked = false;
            chkLockRtnChq.Checked = false;
        ...
        private void ClearFieldContact()
        {
            ...
            //txtRowNo.Clear();
            //int iRowCount = 0;
            //foreach(DataGridViewRow oRow in dgvAddressBook.Rows)
            //{
            //    iRowCount++;
            //}
            //txtRowNo.Text = iRowCount.ToString();
        ...
        {
            dt.Rows.Clear();
            //List<vw_searchGenCustomerMaster> details = vw_searchGenCustomerMaster.SelectAll();
            //foreach (vw_searchGenCustomerMaster detail in details)
            //{
            //    dt.Rows.Add(detail.Customer_ID, detail.CustomerName);
            //}
        ...
        //private void RefreshGrid_Route()
        //{
        //    int iRow;
        //    dgvRoute.Rows.Clear();
        //    if (txtCustomerID.Tag != null && txtCustomerID.Tag.ToString().Length > 0)
        //    {
        //        List<tbl_genCustomerMaster_Route> details = tbl_genCustomerMaster_Route.SelectAllByCustomer_ID(txtCustomerID.Tag.ToString().Trim());
        //        foreach (tbl_genCustomerMaster_Route detail in details)
        //        {
        //            dgvRoute.Rows.Add();
        //            iRow = dgvRoute.Rows.Count - 1;

        //            dgvRoute["RouteName", iRow].Value = detail.Route_ID;
        //            dgvRoute["RouteName", iRow].Tag = clsGenaralName.getName_Route(detail.Route_ID);
        //            dgvRoute["TownID", iRow].Value = txtTownID.Text.Trim();
        //            dgvRoute["TownID", iRow].Tag = txtTownID.Tag.ToString().Trim();
        //            dgvRoute["CityID", iRow].Value = txtCityID.Text.Trim();
        //            dgvRoute["CityID", iRow].Value = txtCityID.Tag.ToString().Trim();
        //        }
        //        ClearFieldRoute(); //this is need
        //    }
        //}
        ...
        {
            //sourceCustomerMaster.Filter = " CustomerName LIKE '%" + sCustomerName.Trim() + "%'";
        ...
        private void FillDetails(string sID)
        {
            if (sID.Length > 0)
            {
               ...
               //     txtCustomerID.Enabled = false;
                    ...
                    dtDateOfBirth.Text = Convert.ToDateTime(detail.DateOfBirth).ToString();
                    ...
                    txtWhatsApp.Text = detail.whatsApp;
                    ...
                    chkLockRtnChq.Checked = detail.isOverideReturnChequeLock;
                    chkLockCrLimit.Checked = detail.isOverideCredidLimitLock;
                    chkLockCrPeriod.Checked = detail.isOveridecredidPeriodLock;
                    if (detail.IsDeleted && !chkDeleted.Enabled)
                    ...
                    if (finance != null)
                    {
                        ...
                        cmbCreditPeriod.Text = finance.CreditPeriod.ToString();
                ...
                if (detail != null && detail.Customer_ID.Length > 0)
                {
                    ...
                    if (cImage != null && cImage.Image != null)
                    {
                        ...
                        if (cImage.image2.Length > 0)
                        {
                            MemoryStream ms = new MemoryStream(cImage.image2);
                            pbxImage2.Image = Image.FromStream(ms);
                        }
                        else
                            pbxImage2.Image = pbxImage.InitialImage;
                    }
                    else
                    {
                        pbxImage.Image = pbxImage.InitialImage;
        ...
        private void FillDetailsAddressBook(int iRow)
        {
            try
            {
                ...
                // clsValidate.ValidateGridValue(dgvTDetail, "Element", row.Index, 0);
                // txtcon_Email.Text = dgvAddressBook.Rows[iRow].Cells["Email"].Value.ToString();
                //dgvTDetail.Rows[e.RowIndex].Cells["value"].Value
                //txtcon_Email.Text = dgvAddressBook["Email", iRow].Value.ToString();//Email txtcon_Email
        ...
        private bool CheckValidity()
        {
            ...
            if (txtAddressRegister.TextLength == 0 )
            {
                strMessage += "\n" + "Registerd Address ";
                bStatus = false;
            }
if (txtTelephone.TextLength == 0)
        ...
        private bool CheckNumberValidity()
        {
            ...
            try
            {
                ...
                if (!clsCommon.isCurrency(cmbCreditPeriod.Text.Trim()))
                {
                    strMessage += "\n Credit Period";
                    bStatus = false;
                }
        ...
        {
            //dgvAccounts["Depositted", e.RowIndex].Value = 0;
            //dgvAccounts["Realized", e.RowIndex].Value = 0;
            //dgvAccounts["Returned", e.RowIndex].Value = 0; 
        }
        ...
        private void btnLoadImage2_Click(object sender, EventArgs e)
        {
            ...
            filedialog.ShowDialog();
         //   s_FileName2 = ;
            pbxImage2.ImageLocation = filedialog.FileName;
        }
    }
}
