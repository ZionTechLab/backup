...
namespace Digiteq
{
    public partial class frm_mtrCustomerClass : MettroForm
    {


        #region Form Load
        public frm_mtrCustomerClass()
        {
            sFormConfigCode = clsAutocode.getFormConfigCode(FormName.ZCustomerClass);
            iFormID = clsSecurity.getFormID(FormName.ZCustomerClass);
                ...
                if (txtClassID.TextLength > 0)
                {
                    if (CheckValidity())
                    {
                        if (clsSecurity.PermissionToDelete(clsSecurity.UserIDLoged, iFormID))
                        {
                            ...
                            tbl_zCustomerClass detail = tbl_zCustomerClass.Select(txtClassID.Text.Trim());
                            if (detail != null)
                            {
                                detail.Delete();
                   ...
                   if (clsSecurity.PermissionToSave(clsSecurity.UserIDLoged, iFormID, IsUpdate))
                    {
                        try
                        {
                            ...
                            if (txtClassID.TextLength > 0)
                            {
                                ...
                                {
                                    tbl_zCustomerClass oldRecord = tbl_zCustomerClass.Select(txtClassID.Text.Trim());
                                    if (oldRecord != null)
                                    {
                                        //Country Header
                                        tbl_zCustomerClass detail = new tbl_zCustomerClass(txtClassID.Text.Trim(), txtClassName.Text.Trim());
                                        detail.Update();
        ...
        private void RefreshGrid()
        {
            try
            {
                ...
                List<tbl_zCustomerClass> details = tbl_zCustomerClass.SelectAll();
                foreach (tbl_zCustomerClass detail in details)
                {
                    if (detail.CustomerClass_ID.Trim() != "default")
                    {
                        dgvDetail.Rows.Add();
                        iRow = dgvDetail.Rows.Count - 1;
                        dgvDetail["CategoryID", iRow].Value = detail.CustomerClass_ID;
        ...
        private void FillDetails(string sID)
        {
            try
            {
                if (sID.Length > 0)
                {
                    tbl_zCustomerClass detail = tbl_zCustomerClass.Select(sID);
                    if (detail != null)
                    {
                        ...
                        //asign values
                        txtClassID.Text = detail.CustomerClass_ID;
