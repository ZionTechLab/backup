...
namespace Digiteq
{
    public partial class frm_mtrSupplierType : MettroForm
    {
        

        #region Form Load
        public frm_mtrSupplierType()
        {
            sFormConfigCode = clsAutocode.getFormConfigCode(FormName.ZSupplierType);
            iFormID = clsSecurity.getFormID(FormName.ZSupplierType);
                ...
                if (txtCategoryID.TextLength > 0)
                {
                    if (CheckValidity())
                    {
                        if (clsSecurity.PermissionToDelete(clsSecurity.UserIDLoged, iFormID))
                        {
                            ...
                            tbl_zSupplierType detail = tbl_zSupplierType.Select(txtCategoryID.Text.Trim());
                            if (detail != null)
                            {
                                detail.Delete();
                   ...
                   if (clsSecurity.PermissionToSave(clsSecurity.UserIDLoged, iFormID, IsUpdate))
                    {
                        try
                        {
                            ...
                            if (txtCategoryID.TextLength > 0)
                            {
                                ...
                                {
                                    tbl_zSupplierType oldRecord = tbl_zSupplierType.Select(txtCategoryID.Text.Trim());
                                    if (oldRecord != null)
                                    {
                                        //Country Header
                                        tbl_zSupplierType detail = new tbl_zSupplierType(txtCategoryID.Text.Trim(), txtCategoryName.Text.Trim());
                                        detail.Update();
                            ...
                            {
                                MessageBox.Show("Supplier Type " + clsFormatter.GetMessageFrom(MessageType.IDIsEmpty), clsFormatter.GetMessageCaption(), MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                        catch (Exception ex)
                        {
        ...
        private void RefreshGrid()
        {
            try
            {
                ...
                List<tbl_zSupplierType> details = tbl_zSupplierType.SelectAll();
                foreach (tbl_zSupplierType detail in details)
                {
                    if (detail.SupplierType_ID.Trim() != "default")
                    {
                        dgvDetail.Rows.Add();
                        iRow = dgvDetail.Rows.Count - 1;
                        dgvDetail["CategoryID", iRow].Value = detail.SupplierType_ID;
        ...
        private void FillDetails(string sID)
        {
            try
            {
                if (sID.Length > 0)
                {
                    tbl_zSupplierType detail = tbl_zSupplierType.Select(sID);
                    if (detail != null)
                    {
                        ...
                        //asign values
                        txtCategoryID.Text = detail.SupplierType_ID;
        ...
        private void frm_mtrSupplierType_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void txtSupplierType_DoubleClick(object sender, EventArgs e)
        {
            Search_SupplierTypeID();
        }
        #endregion
