...
namespace Digiteq
{
    public partial class frm_mtrMachineClass : MettroForm
    {


        #region Form Load
        public frm_mtrMachineClass()
        {
            sFormConfigCode = clsAutocode.getFormConfigCode(FormName.ZMachineClass);
            iFormID = clsSecurity.getFormID(FormName.ZMachineClass);
                ...
                if (txtClassID.TextLength > 0)
                {
                    if (CheckValidity())
                    {
                        if (clsSecurity.PermissionToDelete(clsSecurity.UserIDLoged, iFormID))
                        {
                            ...
                            tbl_zMachineClass detail = tbl_zMachineClass.Select(txtClassID.Text.Trim());
                            if (detail != null)
                            {
                                detail.Delete();
                   ...
                   if (clsSecurity.PermissionToSave(clsSecurity.UserIDLoged, iFormID, IsUpdate))
                    {
                        try
                        {
                            ...
                            if (txtClassID.TextLength > 0)
                            {
                                ...
                                {
                                    tbl_zMachineClass oldRecord = tbl_zMachineClass.Select(txtClassID.Text.Trim());
                                    if (oldRecord != null)
                                    {
                                        //Country Header
                                        tbl_zMachineClass detail = new tbl_zMachineClass(txtClassID.Text.Trim(), txtClassName.Text.Trim(), txtPrifix.Text.Trim().ToUpper());
                                        detail.Update();
        ...
        private void RefreshGrid()
        {
            try
            {
                ...
                List<tbl_zMachineClass> details = tbl_zMachineClass.SelectAll();
                foreach (tbl_zMachineClass detail in details)
                {
                    if (detail.MachineClass_ID.Trim() != "default")
                    {
                        dgvDetail.Rows.Add();
                        iRow = dgvDetail.Rows.Count - 1;
                        dgvDetail["CategoryID", iRow].Value = detail.MachineClass_ID;
        ...
        private void FillDetails(string sID)
        {
            try
            {
                if (sID.Length > 0)
                {
                    tbl_zMachineClass detail = tbl_zMachineClass.Select(sID);
                    if (detail != null)
                    {
                        ...
                        //asign values
                        txtClassID.Text = detail.MachineClass_ID;
