...
namespace Digiteq_Logic
{
    ...
    {
        H1, H2, H3, H4, H5, H6, Footer1, Footer2, Line1, Line2, Space, Detail1, Detail2, DataTable, DIV, TableColomn1, TableColomn2, TableColomn3, TableColomn4
    }
    ...
    public class EmailLineformating
    {
        ...
        public Elament H4 = new Elament(ElementAlign.Left, null, 11, "#333333", fontStyle.NA);
        public Elament H5 = new Elament(ElementAlign.Center, null, 10, "#666666", fontStyle.NA);
    ...
    public class clsEmailConfig
    {
        ...
        {
            EmailLineformating formating=new EmailLineformating();
            StringBuilder sb = new StringBuilder();
            sb.Append("<div style=\"font-size:10px; color:#666666; font-family: Arial, Helvetica, sans-serif;\">");
            foreach (emailLine l1 in list1)
            {
                switch (l1.LineType)
                {
                    case LineType.H1:
                        {
                            sb.Append(CreateEmailLine(l1, formating.H1));
                            break;
                        }
                    case LineType.H2:
                        {
                            sb.Append(CreateEmailLine(l1, formating.H2));
                            break;
                        }
                    case LineType.H3:
                        {
                    ...
                    case LineType.DataTable:
                        {
                            ...
                            foreach (DataRow dr in l1.Table.Rows)
                            {
                                ...
                                foreach (DataColumn column in l1.Table.Columns)
                                {
                                    ...
                                    {
                                        String Valiue = dr[column].ToString();
                                        decimal dValue=decimal.Parse(Valiue==""?"0":Valiue);
                                        sb.Append("<td style=\"border-top:hidden; border-right:hidden; padding:4px; text-align: right;\">" + clsFormatter.FormatDecimalPlaces_Price(dValue) + "</td>");
                                    }
                                    else if (column.DataType == typeof(System.Int32))
                                    {
                                        String Valiue = dr[column].ToString();
                                        int dValue = int.Parse(Valiue == "" ? "0" : Valiue);
                                        sb.Append("<td style=\"border-top:hidden; border-right:hidden; padding:4px; text-align: right;\">" + clsFormatter.FormatToNumberNoDecimal(dValue) + "</td>");
                                    }
                                    else
