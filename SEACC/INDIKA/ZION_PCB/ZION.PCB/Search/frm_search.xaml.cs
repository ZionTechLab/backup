...
namespace ZION.PCB.Search
{
    ...
    public partial class frm_search : Window
    {
        ...
        public frm_search(bool bShowAll)
        {
            InitializeComponent();
            if (!bShowAll)
        ...
        public frm_search(bool bShowAll, List<string> lstParameeters)
        {
            InitializeComponent();
            if(!bShowAll)
                btn_Settings.Visibility = Visibility.Collapsed;
        ...
        private void FilterChanged(Key k)
        {
            try
            {
                ...
                {
                    if (sSearch_ID != "5253" && sSearch_ID != "5025" && sSearch_ID != "6505" && sSearch_ID != "6506" && sSearch_ID != "5006" && sSearch_ID != "6500" && sSearch_ID != "5116") //GL Name, Users, Income Type, PCB Account, Currency, Exp. Category
                    {
                        sFilter += " AND isCanceled = 0";
                        if (sSearch_ID == "6504") // Expenditure
                            sFilter += " AND isReimburst = 0";
                        if (sSearch_ID == "6501" || sSearch_ID == "6502" || sSearch_ID == "6507") //IOU Request , IOU, IOU Refund
                            sFilter += " AND isSettled = 0";
                        //if (sSearch_ID == "6503") //Reimbursment Request
                        //    sFilter += " AND isApproved = 0";
                    }
        ...
        private void btn_Settings_Click(object sender, RoutedEventArgs e)
        {
            ...
            else if (Grd_Settings.Height == 127)
