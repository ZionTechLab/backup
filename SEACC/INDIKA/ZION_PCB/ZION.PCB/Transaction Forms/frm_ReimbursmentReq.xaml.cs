...
namespace SEACC_PCB
{
    ...
    public partial class frm_ReimbursmentReq : Window
    {
        ...
        string sPCAccCode = "";
        bool bPostSummarizedApn = false;
        #endregion
        decimal FloatAmount=0, BookBalance=0, IOUAmount=0, AvailableBalance=0;
        #region Form Load
        public frm_ReimbursmentReq(decimal _FloatAmount, decimal _BookBalance, decimal _IOUAmount, decimal _AvailableBalance)
        {
            FloatAmount = _FloatAmount;
            BookBalance = _BookBalance;
            IOUAmount = _IOUAmount;
            ...
            bPostSummarizedApn = DBHandling.ExecQuery_ReturnBool("select [configValue] from [dbo].[tbl_securityConfigStatus] where [valueID]=704");
                    ...
                    if (SEACC_Form.CheckPermission_ToPrint())
                    {
                        ...
                        if (oReports != null)
                        {
                            ...
                            if (details != null)
                            {
                                ...
                                {
                                    foreach (tbl_pcbTxExpenditure_Detail oExD in tbl_pcbTxExpenditure_Detail.SelectAllByExpenditure_ID(oExpenditure.Expenditure_ID))
                                    {
                                        dts_pettyCash.dt_Expenditure.Adddt_ExpenditureRow(oExpenditure.Expenditure_ID, oExpenditure.ExpenditureDate, details.PcbAccount_ID, clsGenaralName.getName_PCAccount(details.PcbAccount_ID),oExD.PcbExpenditureCategory_ID,clsGenaralName.getName_ExpCategory(oExD.PcbExpenditureCategory_ID),
                                         oExpenditure.SpentUser_ID, clsGenaralName.getName_User(oExpenditure.SpentUser_ID), oExpenditure.Cost_Center_ID, "", oExD.Amount, oExpenditure.Remarks+" - "+ oExD.Remarks, "");
                                    }
                                }
                                ...
                                glb_dts_ExportReport.dt_rptParameter.Adddt_rptParameterRow("FloatAmount", clsFormatter.FormatDecimalPlaces_Price(FloatAmount),true);
                                glb_dts_ExportReport.dt_rptParameter.Adddt_rptParameterRow("BookBalance", clsFormatter.FormatDecimalPlaces_Price(BookBalance), true);
                                glb_dts_ExportReport.dt_rptParameter.Adddt_rptParameterRow("IOUAmount", clsFormatter.FormatDecimalPlaces_Price(IOUAmount), true);
                                glb_dts_ExportReport.dt_rptParameter.Adddt_rptParameterRow("AvailableBalance", clsFormatter.FormatDecimalPlaces_Price(AvailableBalance), true);
                            ...
                            if (oOldReim != null)
                            {
                                if (!oOldReim.IsCanceled && !oOldReim.IsApproved)
                                {
                                    ...
                                    if (iNoOfReimbursements > 0)
                                    {
                                        ...
                                        int iRow;
                                        string sGLCode = "", sCategoryID = "", sRemarks="";
                                        bool bIsCredit;
                                        ...
                                        foreach (DataRow row in dtGL.Rows)
                                        {
                                            ...
                                            dDebitAmount = clsValidate.ValidateRowValue(row, "DebitAmount", 0m);
                                            dCreditAmount = clsValidate.ValidateRowValue(row, "CreditAmount", 0m);
                                            sRemarks = clsValidate.ValidateRowValue(row, "Remarks", "");
                                            ...
                                            tbl_accAccountPayableNote_SubTotal_EX Insdetail = new tbl_accAccountPayableNote_SubTotal_EX(iRow, txtRemReq.Tag.ToString(), sCategoryID,
                                                sGLCode, "default", "default", "default", "default", "default", "default", dAmount, bIsCredit, sRemarks);
                                            Insdetail.InsertEX();
                                        }
        ...
        private void fillDoubleEntry()
        {
                ...
                if (bSelect)
                {
                    ...
                    foreach (tbl_pcbTxExpenditure_Detail detail in tbl_pcbTxExpenditure_Detail.SelectAllByExpenditure_ID(sExpID))
                    {
                        ...
                        if (oCategory != null)
                        {
                            ...
                            if (oExpType != null)
                            {
                                Amoint += detail.Amount;
                                iRowNo++;
                                dtGL.Rows.Add(iRowNo, oExpType.Gl_ID, clsGenaralName.getName_AccountName(oExpType.Gl_ID), detail.Amount, clsFormatter.FormatDecimalPlaces_Price(0), detail.Remarks);
                            }
                        }
                    }
                }
            ...
            if (oAccount != null)
                dtGL.Rows.Add(++iRowNo, oAccount.Gl_ID, clsGenaralName.getName_AccountName(oAccount.Gl_ID), 0, Amoint,"");
            #endregion

            #region Group By GL Code
            int iLineNo = 1;
            if (bPostSummarizedApn)
            {
