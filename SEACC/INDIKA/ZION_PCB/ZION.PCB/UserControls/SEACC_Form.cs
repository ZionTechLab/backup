...
namespace SEACC_PCB.UserControls
{
    class SEACC_Form : Grid
    {
        ...
        public void Initialize()
        {
            ...
            if (oFunction != null)
            {
                ...
                tbl_securityFunctionMaster_Permission detail = tbl_securityFunctionMaster_Permission.Select(clsSecurity.BranchID ,Digiteq_Logic.clsSecurity.UserIDLoged, (int)enmFormName);
                if (detail != null)
                {
    ...
    public static class clsCommon
    {
        ...
        public static string getAutoGeneratedCodePCBExpenditure(string sAccCode)
        {
            string sCode = "";

            tbl_pcbMasAccount oAcc = tbl_pcbMasAccount.Select(sAccCode);
            if (oAcc != null)
            {
                sCode = oAcc.Prefix;
                ...
                if (sCode != "")
                {
                    tbl_securityFunctionMaster_Form detail = tbl_securityFunctionMaster_Form.Select((int)FormName.PCB_AddExpenditure);
                    if (detail != null)
                    {
                        sCode += oAcc.Counter.ToString(getWidthFormat(detail.Length));
                        oAcc.Counter++;
                        oAcc.Update();
                    }
                }                                    
