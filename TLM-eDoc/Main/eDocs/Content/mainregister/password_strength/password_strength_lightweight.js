...
;(function ( $, window, document, undefined ) {
    ...
    Plugin.prototype._create = function () {
        ...
        this.element.append('<input type="password" class="form-control"/>');
        ...
        this.element.append(
            ...
            <ul> \
            <li data-criterion="length" class="valid">4-20 <strong>Characters</strong></li> \<li data-criterion="symbol" class="valid">At least one symbol</li>\
    ...
    Plugin.prototype._bind_input_events = function($s) {
        ...
        $s.bind('keyup', function () {
            ...
            var symbolone = password.match(symbol) ? 1 : 0;
            var capitalletters = password.match(upperCase) ? 1 : 0;
            var loweletters = password.match(lowerCase) ? 1 : 0;
            var number = password.match(numbers) ? 1 : 0;
            var containWhiteSpace = password.indexOf(' ') >= 0 ? 1 : 0;

            var total = characters + capitalletters + loweletters + number + symbolone;
            that._update_indicator(total);

            that._update_info('length', password.length >= 4 && password.length <= 20);
            that._update_info('capital', capitalletters);
            that._update_info('number', number);
            that._update_info('symbol', symbolone);
    ...
    Plugin.prototype._update_indicator = function(total) {
        ...
        if (total === 0) {
            ...
            document.getElementById("btn_savePass").disabled = true;
        } else if (total === 2) {
            ...
            meter.addClass('medium').html('<p>strong</p>');
           
            document.getElementById("btn_savePass").disabled = false;
        }
        else {
            meter.addClass('strong').html('<p>medium</p>');
            document.getElementById("btn_savePass").disabled = true;
