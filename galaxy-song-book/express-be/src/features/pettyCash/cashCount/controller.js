const { denominationSchema, updateSchema, idSchema } = require('./validation');
...
exports.sign = asyncHandler(async (req, res) => {
  ...
  const row = await repo.sign({ ...data, userId: req.userId });
  res.status(201).json(row);
});

exports.countersign = asyncHandler(async (req, res) => {
  const data = await idSchema.validate(req.body, VALIDATE_OPTS);
  const row = await repo.countersign({ ...data, userId: req.userId });
  res.status(201).json(row);
});

exports.audit = asyncHandler(async (req, res) => {
  const data = await idSchema.validate(req.body, VALIDATE_OPTS);
  const row = await repo.audit({ ...data, userId: req.userId });
  res.status(201).json(row);
});
