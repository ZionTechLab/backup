const { updateSchema, idSchema, actSchema } = require('./validation');
const repo = require('./repository');
const asyncHandler = require('../../../middleware/asyncHandler');
const { AppError } = require('../../../middleware/errorHandler');
const { VALIDATE_OPTS } = require('../../../middleware/validation');
const { getEffectivePermissions } = require('../../../repository/permissions');

exports.getUi = asyncHandler(async (req, res) => {
  const data = await repo.getUi({ tenantId: req.tenantId, companyId: req.companyId });
  res.json(data);
});

exports.getAll = asyncHandler(async (req, res) => {
  // Approvers may see the whole company. Everyone else sees only their own.
  const codes = await getEffectivePermissions(req.userId, { companyId: req.companyId });
  const canSeeAll = codes.includes('iou-request-certify') || codes.includes('iou-request-approve');
  const wantsMine = req.query.mine === 'true' || req.query.mine === '1';
  const mine = canSeeAll ? wantsMine : true;
  const rows = await repo.getAll({ tenantId: req.tenantId, companyId: req.companyId, mine, userId: req.userId });
  res.json(rows);
});

exports.get = asyncHandler(async (req, res) => {
  const filters = Object.keys(req.query || {}).length ? req.query : (req.body || {});
  const row = await repo.get(filters);
  if (!row) throw new AppError('Not found', 404);
  res.json(row);
});

exports.getApproved = asyncHandler(async (req, res) => {
  const rows = await repo.getApproved({ tenantId: req.tenantId, companyId: req.companyId });
  res.json(rows);
});

exports.update = asyncHandler(async (req, res) => {
  const data = await updateSchema.validate(req.body, VALIDATE_OPTS);
  const row = await repo.update({ ...data, userId: req.userId, tenantId: req.tenantId, companyId: req.companyId });
  res.status(201).json(row);
});

exports.act = asyncHandler(async (req, res) => {
  const data = await actSchema.validate(req.body, VALIDATE_OPTS);
  const row = await repo.act({ ...data, userId: req.userId });
  res.status(200).json(row);
});

exports.close = asyncHandler(async (req, res) => {
  const data = await idSchema.validate(req.body, VALIDATE_OPTS);
  const row = await repo.close({ ...data, userId: req.userId });
  res.status(200).json(row);
});

exports.cancel = asyncHandler(async (req, res) => {
  const data = await idSchema.validate(req.body, VALIDATE_OPTS);
  const result = await repo.cancel({ ...data, userId: req.userId });
  res.status(200).json(result);
});
