...
export default function ImageMultiInputField({ name, formik, className, placeholder, max = 20 }) {
  ...
  const onFilesSelected = (files) => {
    ...
    const list = imageFiles.slice(0, available);
  ...
  return (
    <div className={`image-multi-input position-relative w-100 ${className || 'mb-3'}`}>
      ...
      <div
        className={`image-multi-dropzone d-flex align-items-center gap-2 ${dragOver ? 'dragover' : ''}`}
      ...
      {displayedItems.length > 0 && (
        <div className="image-multi-strip mt-2">
          ...
          <div className="image-multi-scroll d-flex gap-2 overflow-auto p-1 w-100" ref={containerRef}>
            {displayedItems.map((it, idx) => (
              <div className="image-thumb flex-shrink-0 rounded overflow-hidden" key={it.id} onClick={() => setViewer({ open: true, index: idx })}>
